...
class CloudApiBase(ABC):
    ...
    async def get_contents_async(self, base_url, request_query):
        """
        This asynchronous function makes the API request using the baseURL and assembled endpoint with parameters (requestQuery).
        """
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{base_url}{request_query}") as response:
    ...
    async def send_post_request_async(self, post_request):
        """
        This asynchronous function sends a batch request to the API.
        """
        try:
            headers = {'Content-Type': 'application/json'}
            json_batch = json.dumps(post_request)
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.base_url}{self.endpoint}", headers=headers, data=json_batch) as response:
    ...
    async def get_async(self, response_type):
        """
        This function calls ProcessRequestAsync() to send the CloudAPI request and get the response.
        This function coordinates a string or deserialized response object return.
        """
        response = await self.process_request_async(self.parameters)
        return self.handle_response(response, response_type)
    ...
    async def send_version_request_async(self):
        """
        ...
        try:
            ...
            version_endpoint = "/".join(segments)
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}{version_endpoint}") as response:
