...
def gui_tkinter():
    ...
    def perform_selection_check(values):
        try:
            ...
            raise_if_false(MAX_CORES >= int(values["cores"]) > MIN,
    ...
    num_cores_label = ttk.Label(master=num_cores_frame,
    ...
    memory_label = ttk.Label(master=memory_frame,
                             text=f"How many GB of RAM does the tool require? (integer, max. {MAX_RAM})")
    ...
    image_name_label = ttk.Label(master=image_name_frame, text=f"Docker image name")
    ...
    docker_url_label = ttk.Label(master=docker_url_frame, text=f"Docker registry URL")
    ...
    docker_user_label = ttk.Label(master=docker_user_frame, text=f"Docker registry user")
...
def main():
    ...
    try:
        ...
        messagebox.showinfo("Message", str(needs2faerror) +
