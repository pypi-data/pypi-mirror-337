...
def smoothn(
	...
	for i in axis:
		...
		Lambda = Lambda + (cos(pi * (arange(1, sizy[i] + 1) - 1.0) / sizy[i]).reshape(siz0.astype(int)))
		# else:
		#  Lambda = Lambda + siz0
	...
	# (a**2 -1)/16
	try:
		sMinBnd = np.sqrt((((1 + sqrt(1 + 8 * hMax ** (2.0 / N))) / 4.0 / hMax ** (2.0 / N)) ** 2 - 1) / 16.0)
		sMaxBnd = np.sqrt((((1 + sqrt(1 + 8 * hMin ** (2.0 / N))) / 4.0 / hMin ** (2.0 / N)) ** 2 - 1) / 16.0)
	except:
		sMinBnd = None
		sMaxBnd = None
	...
	while RobustIterativeProcess:
		...
		while tol > TolZ and nit < MaxIter:
			...
			if isauto and not remainder(log2(nit), 1):
				...
				if not s0:
					ss = np.arange(nS0) * (1.0 / (nS0 - 1.0)) * (log10(sMaxBnd) - log10(sMinBnd)) + log10(sMinBnd)
					g = np.zeros_like(ss)
...
def dctND(data, f=dct):
	...
	if nd == 1:
		...
		return f(f(f(data, norm="ortho", type=2, axis=0), norm="ortho", type=2, axis=1), norm="ortho", type=2, axis=2)
