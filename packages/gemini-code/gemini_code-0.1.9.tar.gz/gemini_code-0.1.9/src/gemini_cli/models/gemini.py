...
class GeminiModel:
    ...
    def __init__(self, api_key, model_name="gemini-2.0-pro"):
    ...
    def generate(self, prompt, conversation=None):
        ...
        # Convert conversation to the new Gemini format
        message_history = self._format_conversation_for_gemini(conversation)
        ...
        if message_history:
            ...
            contents.extend(message_history)
            # Add the current prompt
            contents.append({"role": "user", "parts": [{"text": prompt}]})
        else:
            # If no history, just add system prompt and current prompt
            contents = [
...
"""
    ...
    def _format_conversation_for_gemini(self, conversation):
        """Convert our conversation format to Gemini Client API format."""
        gemini_messages = []
        ...
        for message in conversation:
            ...
            # Map our roles to Gemini roles (system becomes user)
            if role == "system":
                gemini_role = "user"
            else:
                gemini_role = role
