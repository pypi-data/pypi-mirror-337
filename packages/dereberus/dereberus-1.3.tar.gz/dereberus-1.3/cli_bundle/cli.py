...
def get_valid_resource(resource=None, service=None, client=None):
    try:
        while True:
            ...
            valid_response = DereberusApi.post(os.getenv("DEREBERUS_TOKEN"), endpoint, data=data)
...
def resource(realm):
    ...
    if user_api_token is None:
        click.echo('API token not found')
        return
    list_response = DereberusApi.get(os.getenv("DEREBERUS_TOKEN"), '/resources/list', data={'user_api_token': user_api_token})
