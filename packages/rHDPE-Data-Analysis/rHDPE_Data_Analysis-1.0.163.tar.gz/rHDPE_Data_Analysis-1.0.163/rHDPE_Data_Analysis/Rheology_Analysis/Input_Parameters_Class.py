...
class Input_Parameters():
    ...
    def __init__( self ):
        ...
        self.shiny_cv_vs_af = False

        self.shiny_pa_vs_cv = False

        self.shiny_vgp = False
...
def read_parameters_from_numbers_file( filename, ip ):
    ...
    ip.read_and_analyse_features = bool( rows[14][1].value )

    ip.plot_data = bool( rows[15][1].value )

    ip.sandbox = bool( rows[16][1].value )

    ip.sample_mask = [int( i.value ) for i in rows[17][1:] if i.value != None]

    ip.feature_selection = [int( i.value ) for i in rows[18][1:] if i.value != None]
