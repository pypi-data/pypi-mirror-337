...
def summarize_results(results: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for sample, df in results.items():
        ...
        try:
            ...
            for ref in df['reference'].unique():
                ...
                unique_single_snp_count = len(single_snp_haplotypes)
                
                # Add warning if we exceed theoretical maximum
                if unique_single_snp_count > theoretical_max_snps:
                    logger.warning(f"Sample {sample} reference {ref} has more unique single SNP haplotypes "
    ...
    if all_data:
        ...
        for ref in combined_df['reference'].unique():
            ...
            # Add warning if we exceed theoretical maximum
            if unique_single_snp_count > theoretical_max_snps:
                logger.warning(f"Overall results for reference {ref} have more unique single SNP haplotypes "
                             f"than theoretically possible: found={unique_single_snp_count}, "
                             f"max={theoretical_max_snps}")
