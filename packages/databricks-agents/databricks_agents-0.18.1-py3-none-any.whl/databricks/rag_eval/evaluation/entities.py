...
class MetricResult:
    ...
    def to_mlflow_assessment_v3(
        ...
        source_id = context.get_context().get_user_name() or self.metric_full_name
        if isinstance(self.metric_value, mlflow_eval.Assessment):
            if self.metric_value.source is not None:
                ...
                source = mlflow_entities.AssessmentSource(
                    source_type=source_type,
                    source_id=self.metric_value.source.source_id or source_id,
                )
            else:
                source = mlflow_entities.AssessmentSource(
                    source_type=mlflow_entities.AssessmentSourceType.LLM_JUDGE,
...
class EvalResult:
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        for metric in self.metric_results:
            if metric.metric_full_name in _EXCLUDED_METRICS_FROM_LOGGING:
                continue
