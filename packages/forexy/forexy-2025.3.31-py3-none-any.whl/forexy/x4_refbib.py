...
#   print(xref_out.encoding)    # encoding type (json, text, ...)
    ...
    if format=="piped" or format=="xml":
      ...
      data=xref_out.json()
      print_bibtex(data,doi,jour,volume,year)

  return


def print_exfor(data):
...
def print_bibtex(data,doi,jour,volume,year):
...
def get_args(ver):
  ...
  parser=argparse.ArgumentParser(\
   ...
   epilog="example: x4_refbib.py -i J,NDS,120,272,2014 -d dict_9131.json -o x4_refbib_out.txt -r doi -e email@address.com")
  parser.add_argument("-v", "--version",\
   action="version", version=ver)
  parser.add_argument("-i", "--ref_inp",\
   help="EXFOR reference code or DOI")
  parser.add_argument("-a", "--fauthor",\
   help="family name of the first author")
