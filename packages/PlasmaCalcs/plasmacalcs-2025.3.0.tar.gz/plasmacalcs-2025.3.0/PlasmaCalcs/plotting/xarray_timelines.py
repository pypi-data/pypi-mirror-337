...
class XarrayTimelines(PlotSettingsMixin):
    ...
    '''
    def __init__(self, array, t='t', dims=None, *, werr=False,
        ...
        if werr:
            ...
            elif 'mean' not in array or 'std' not in array:
                raise InputConflictError('werr=True is only valid for Dataset with "mean" and "std" data_vars.')
            if werr == 'fill':
                raise NotImplementedError('werr="fill" not yet implemented.')
    ...
    def get_effective_data_interval(self, *, robust=UNSET):
        ...
        for dimsel, _style in self.iter_dimpoints():
            ...
            if consider.ndim != 1:
                raise DimensionError(f'expected 1D array after indexing; got {consider.ndim}D array')
            if self.werr:
                yerr = self.werr_std.isel(dimsel)
                consider = xr.Dataset({'mean+std': consider+yerr, 'mean-std': consider-yerr})
    ...
    def _plot_lines(self, **kw_plot):
        ...
        if self.werr == False:
            ...
            raise NotImplementedError('[TODO] werr="fill" not yet implemented.')
        else:
        ...
        for dimsel, style in self.iter_dimpoints():
            ...
            if self.werr == False:
                ...
                raise NotImplementedError(f'werr={werr!r}')
