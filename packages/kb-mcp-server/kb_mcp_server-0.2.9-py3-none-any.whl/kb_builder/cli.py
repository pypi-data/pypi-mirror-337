...
def build_command(args):
    ...
    try:
        ...
        if hasattr(args, 'update') and args.update:
            ...
            # Use app.embeddings.index directly instead of app.index()
            # This ensures that the embeddings are properly created and indexed
            app.embeddings.index([(i, doc.get("text", ""), doc.get("metadata")) for i, doc in enumerate(documents)])
