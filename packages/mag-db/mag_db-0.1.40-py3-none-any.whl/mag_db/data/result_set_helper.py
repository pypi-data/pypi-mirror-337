...
class ResultSetHelper:
    ...
    def to_maps(result_set: list[Dict[str, Any]], db_output: DbOutput) -> List[Dict[str, Any]]:
        return [
            {
                db_output.__column_name_mapping.get_target_name(column_name) or column_name:
                    row_map[column_name] for column_name in db_output.__column_names
            }
    ...
    def to_beans(result_set: list[Dict[str, Any]], db_output: DbOutput) -> List[Any]:
        ...
        for row_map in result_set:
            kwargs = {}
            for column_name in db_output.__column_names:
                target_name = db_output.get_target_name(column_name)
                target_type = ClassUtils.get_field_type_of_class(db_output.__result_class, target_name)
            ...
            result_list.append(db_output.__result_class(**kwargs))
