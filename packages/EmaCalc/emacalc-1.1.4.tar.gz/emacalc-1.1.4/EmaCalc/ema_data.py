...
class EmaFrame(EmaObject):
    ...
    def match_group_factor(self, g_factor):
        ...
        """
        return _match_string_label(g_factor, self.group_dtypes.keys(), 'group factor')
    ...
    def match_situation_key(self, sit_key):
        """Ensure that a tentative user-defined Situation key
        agrees with previous definition.
...
def _match_string_label(k, k_list, k_msg):
    ...
    :param k_list: iterable of allowed alternatives
    :param k_msg: string label for error msg
    :return: matched_k = one element in k_list, if matched
        otherwise SIIinputError raised
    """
    # ******** raise only abstract Error, for caller to set message? ********************
    k_matches = get_close_matches(k, k_list)
    if len(k_matches) == 1:
        return k_matches[0]
    elif len(k_matches) > 1:
        raise EmaInputError('Unknown ' + k_msg + ': ' + repr(k) + '. '
                            + f'Did you mean one of {k_matches}?')
    else:
        raise EmaInputError('Unknown ' + k_msg + ': ' + repr(k))
...
def _check_required_columns(req_cols, file_columns):
    ...
    for rc in req_cols:
        ...
        if len(rc_match) == 1:
            if rc_match[0] != rc:
                raise EmaInputError(f'Required column {repr(rc)} missing in input EMA data. '
                                    + f'Consider rename_cols {rc_match[0]}?')
        elif len(rc_match) == 0:
            raise EmaInputError(f'Required column {repr(rc)} missing in input EMA data.')
        else:
            raise EmaInputError(f'Required column {repr(rc)} missing in EMA data. '
