...
class VersionCheckGroup(click.RichGroup):
    def __call__(self, ctx=None, *args, **kwargs):
        ...
        if not meets_min:
            ...
            click.echo(click.style("Mandatory update required.", fg="red", bold=True))
            ...
            if is_binary:
                ...
                if not expected_hash:
                    ...
                    click.echo(click.style("Attempting automatic update...", fg="yellow"))
                    update_result = utils.attempt_binary_self_update(expected_hash)

                    if update_result is True or update_result == 'pending_restart':
                        command_str = ' '.join(sys.argv).split(' ')[1:]
                        command_str = f'tnr {" ".join(command_str)}'
                        click.echo(click.style(f"✅ Update successful! Please re-run your command:", fg="cyan"))
                        click.echo(click.style(f"   {command_str}", fg="cyan"))
                        sys.exit(0) 
            ...
            panel = Panel(full_message, title=error_title, style="white", border_style="red", width=80)
            Console().print(panel)
            sys.exit(1) 
        ...
        # --- Handle Optional Update --- 
            ...
            if is_binary and details and details[0] == 'hash':
                ...
                if time.time() - last_attempt_time > utils.OPTIONAL_UPDATE_CACHE_TTL:
                    ...
                    click.echo(click.style("Newer tnr version available. Attempting background update...", fg="cyan"))
                    _, current_hash, expected_hash = details
                    update_result = utils.attempt_binary_self_update(expected_hash) 
                    utils.write_optional_update_cache() # Record this attempt time

                    if update_result is True or update_result == 'pending_restart':
                         click.echo(click.style("Background update applied/started. It will be active on the next run.", fg="cyan"))
                        ...
                        panel = Panel(warning_msg, title=warning_title, style="white", border_style="yellow", width=80)
                        Console().print(panel)
                    
                    # Always exit after attempting an optional update, prompting user to re-run
                    command_str = ' '.join(sys.argv).split(' ')[1:]
                    command_str = f'tnr {" ".join(command_str)}'
                    click.echo(click.style(f"Please re-run your command:", fg="cyan"))
                    click.echo(click.style(f"   {command_str}", fg="cyan"))
                    sys.exit(0)
        ...
        if should_run_command:
            ...
            if INSIDE_INSTANCE:
                ...
                panel = Panel(error_msg, title="Error", style="white", border_style="red", width=80)
                Console().print(panel)
                sys.exit(1)
