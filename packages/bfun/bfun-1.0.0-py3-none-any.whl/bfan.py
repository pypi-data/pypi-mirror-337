...
Usage:
  bfan.py run  [--image=<path or name>] [--view=(compact | summary | jsonSummary)] [--env=<env1=val1,env2=val2>] [--clearPassed --stdout --genXml --outputName=<outputName> --arch=<binArch> --tags=tag1,tag2,tagx (--prefix <paths>)] [<path>...]
  bfan.py clean  [<path>...]
  bfan.py diff   [<path>...]
  bfan.py update [<path>...]
...
def LoadSuitConfig(suite, pathToConfig):
   
    path = pathToConfig + os.sep +"suites"+os.sep + suite + os.sep + "config" + ".yaml"       
    if suite not in CONFIG_CASCHE:
        try:
            with open(path) as f:
                config = yaml.load(f, Loader=yaml.FullLoader)
                CONFIG_CASCHE[suite] = config
        except IOError:
...
class JunitGenerator():
    ...
    def startTest(self, testName, suite):
        ...
        if (not testName in self.data[suite]['tests']):
            self.data[suite]['tests'][testName]={'passed':True,'msg':"","steps":{}}
        ...
        for suite in self.data:
            ...
            for test in self.data[suite]['tests']:
                for step in self.data[suite]['tests'][test]['steps']:
                    time = self.data[suite]['tests'][test]["steps"][step]['time']
                    customTestname = test
                    if self.reportName!='report':
                        customTestname = test +"-"+self.reportName 
                    f.write('<testcase name="{}" classname="{}" time="{}">\n'.format(step,customTestname,time))
...
class TestExecutor():
    def __init__(self,path,tel,ydef,config):
        ...
        self.config= z = {**config, **LoadSuitConfig(self.suite, self.configPath)}
        self.stepName = "??"
    ...
    def do_run_Action_diff(self, tel, action, spec):      
        ...
        fs,fr = loadFilterChain(srcf, reff, filters, self.suite,  self.configPath);
        if not fs or not fr:
            tel.stopDiff()
...
def loadFilterChain(srcf, reff, filters, suite, configPath):
    fs = srcf
    fr = reff
...
def FilterOutBaseOnTag(tags,ydef):
    if len(tags) == 0:
        return True;
    ttags=[];
