...
def create_mean_projection_plot(tensor_np, ax=None, add_colorbar=True):
    """
    Create a heatmap of the mean projection across all dimensions for n-dimensional data.
    ...
    """
    logger.debug("Creating mean projection plot")
    ...
    try:
        ...
        if len(tensor_shape) <= 2:
            ...
            # For higher dimensions, compute mean along all but first two dimensions
            # This creates a 2D projection that preserves the first two dimensions
            axes_to_mean = tuple(range(2, len(tensor_shape)))
            projection = np.nanmean(tensor_np, axis=axes_to_mean)
            title = f"Mean Projection ({len(tensor_shape)}D → 2D)"
        ...
        logger.error(f"Failed to create mean projection: {e}")
        ax.text(
            0.5,
            0.5,
            f"Mean Projection Error: {str(e)}",
            ha="center",
            va="center",
            transform=ax.transAxes,
            fontsize=8,
        )
        ax.set_title("Mean Projection (Error)")
        im = None
