...
if in_folder is None:
  file_count = len([f for f in os.listdir(BASE_DIR) if os.path.isdir(os.path.join(BASE_DIR, f))])
else:
...
def update_prompt(prompt, suffix=""):
    """Updates the input line dynamically with a suffix displayed to the right."""
    user_input = ""
    ...
    while True:
        user_input = input()  # Get input normally
        
        # Reprint the line with suffix
        sys.stdout.write(f"\r{prompt}{user_input} {suffix} ")
        sys.stdout.flush()
        
        return user_input  # Return the input after Enter is pressed
...
def run():
  ...
  while True:
    choice = update_prompt("\033[1;36mcmd: \033[0m", f"\033[1;36m{file_count} files\033[0m")
