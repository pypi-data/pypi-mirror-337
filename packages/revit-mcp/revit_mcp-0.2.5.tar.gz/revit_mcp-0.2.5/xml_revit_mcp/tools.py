...
def call_func(ctx: Context, method: str = "CallFunc", params: List[str] = None) -> str:
    """
    调用Revit函数服务，支持清理重复元素和零面积房间，遵循JSON-RPC 2.0规范。

    特性:
    - 支持批量函数调用（ClearDuplicates, DeleteZeroRooms）
    - 自动去重处理相同函数调用
    - 事务化操作确保数据一致性
    - 严格遵循JSON-RPC 2.0规范
    ...
    参数:
        ...
        params (List[Dict]): 函数调用参数列表，每个字典包含:
            - func (List[str]): 要调用的函数列表，支持:
                ...
                - "DeleteZeroRooms": 清理面积=0(不是未放置的房间)
    ...
    示例:
        ...
        # 清理零面积房间
        response = call_func(ctx, params=[{"func": ["DeleteZeroRooms"]}])

        # 同时执行两种清理
        response = call_func(ctx, params=[{"func": ["ClearDuplicates", "DeleteZeroRooms"]}])
    ...
    try:
        ...
        validated_params = []
        supported_functions = ["ClearDuplicates", "DeleteZeroRooms"]
...
def create_rooms(ctx: Context, method: str = "CreateRooms", params: List[dict[str, str]] = None) -> dict:
    ...
    参数:
        ...
        params (List[Dict]): 标高参数列表，每个字典包含:
            - elementId (Union[int, str]): 标高元素ID,或楼层平面视图ID
...
def create_room_tags(ctx: Context, method: str = "CreateRoomTags", params: List[dict[str, str]] = None) -> dict:
    """
    ...
    示例:
        ...
        {
            "jsonrpc": "2.0",
            "result": [212801, 212802, 212803],  # 创建的房间标签ID列表
            "id": 1
        }
