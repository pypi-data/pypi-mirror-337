...
class PatternsTestCase(unittest.TestCase):
    ...
    def test_no_introducing_spaces(self):
        forms = (segments.Form("d b"),
                 segments.Form("d a b")
                 )
        p = patterns.Pattern._from_str("ab", " ⇌ a / {p,b,d,t}_b <2>")
        f3 = p.apply(forms[1], ("b", "a"))
        self.assertEqual(f3, forms[0], f"Compare: generated {[x for x in f3]} vs original {[x for x in forms[0]]}")
