...
from at_common_workflow.utils.mappings import ArgumentMapping, ResultMapping, NullResultMapping
...
class TaskBuilder:
    ...
    def output(self, context_key: Optional[str] = None, result_path: Optional[str] = None) -> 'WorkflowBuilder':
        ...
        if context_key is None:
            ...
            # We need to validate other aspects like input_model, output_model, processor_function
            if not self._task_def.input_model:
                raise TaskConfigurationError(format_error("Input model must be defined", task_name=self._task_def.name))
            
            if not self._task_def.output_model:
                raise TaskConfigurationError(format_error("Output model must be defined", task_name=self._task_def.name))
