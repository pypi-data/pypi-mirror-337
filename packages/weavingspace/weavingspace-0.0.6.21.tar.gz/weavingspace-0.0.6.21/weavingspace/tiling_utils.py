...
def safe_union(gs:gpd.GeoSeries, n:int=10,
  ...
  r = RESOLUTION * n
  polys = [p for p in gs]
  polys_b = [p.buffer(r, join_style = 2, cap_style = 3) for p in polys]
  union = polys_b[0]
  for p in polys_b[1:]:
    union = union.union(p)
  union = union.buffer(-r, join_style = 2, cap_style = 3)
  # union = gpd.GeoSeries(
  #   [p.buffer(RESOLUTION * 1, join_style = 2, cap_style = 3) 
  #    for p in gs], crs = gs.crs) \
  #     .union_all().buffer(-RESOLUTION * 1, join_style = 2, cap_style = 3)
  if as_polygon:
