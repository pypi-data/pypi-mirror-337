...
def batchsteps_to_tasks(
    ...
    jobs: dict[uuid.UUID, meta.DB2FS | meta.FS2DB | meta.ExecSQL],
    ...
    filecatalog_connections_id = [
        conn.id for conn in connections.connections if conn.is_file_catalog_connection
    ][0]
    ...
    for batchstep in batchsteps:
        ...
        if isinstance(job, meta.ExecSQL):
...
def execsql_to_task(
    task_id: uuid.UUID, task_dependencies: list[uuid.UUID], job: meta.ExecSQL
) -> tasks.SnowflakeExecutorTask:
