...
class V0041OpenapiSlurmdbdJobsRespMeta(BaseModel):
    ...
    command: Optional[List[StrictStr]] = Field(
        default=None, description="CLI command (if applicable)"
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "plugin": V0042OpenapiMetaPlugin.from_dict(obj["plugin"])
                if obj.get("plugin") is not None
                else None,
                "client": V0042OpenapiMetaClient.from_dict(obj["client"])
                if obj.get("client") is not None
                else None,
                "command": obj.get("command"),
                "slurm": V0042OpenapiMetaSlurm.from_dict(obj["slurm"])
                if obj.get("slurm") is not None
                else None,
            }
        )
        return _obj
