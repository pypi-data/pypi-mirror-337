...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, cur_=1):
    ...
    try:
        ...
        if message_id:
            ...
            is_sended = False
            try:
                tmp_media = json.loads(MSG_MEDIA)
                print(f"{tmp_media=}")
                ext_ = 'webp'
                if tmp_media[0]['file_name'].endswith('webm'):
                    ext_ = 'webm'
                elif tmp_media[0]['file_name'].endswith('tgs'):
                    ext_ = 'tgs'

                POST_FNAME = datetime.now(timezone.utc).strftime(f'%d-%m-%Y_%H-%M-%S-%f.{ext_}')
                print(f"{tmp_media=}, {POST_FNAME=}")
                file = await bot.get_file(tmp_media[0]['file_id'])
