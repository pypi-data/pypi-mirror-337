...
class PhoneValidator:
    def __init__(self, country_code: str = '+7', min_length: int = 10, max_length: int = 15):
        self._rules = []
...
def input_validation(
    ...
    def decorator(handler_func: Callable):
        ...
        def wrapper(message: Message, *args, **kwargs):
            # Получаем текст или контакт
            if hasattr(message, 'content_type') and message.content_type == 'contact':
            ...
            if error:
                ...
                return bot.register_next_step_handler(msg, wrapper)

            # Если валидация успешна
            bot.send_message(message.chat.id, success_message)

            # Вызываем исходный обработчик с валидными данными
