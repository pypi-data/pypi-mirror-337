...
class conv_data_gui():
    ...
    def build_gui(self):
        ...
        self.hs['ConvData acc'].titles = ['Select Data to be Converted', 'Define the Format to be Converted to']
            ...
            with h5py.File(self.in_dat_fn, 'r') as f:
                ...
                if self.in_dat_type == 'Tomo raw':
                    ...
                    if ('processed_XANES2D' in keys):
                        ...
                        self.in_dat_items = list(
                            f["/processed_XANES/proc_spectrum"].keys()
                        )
                        if "gen_masks" in f["/processed_XANES"].keys():
                            self.in_dat_items += list(
                                f["/processed_XANES/gen_masks"].keys()
                            )
    ...
    def ConvDatInItms_sel_chg(self, a):
        ...
        if (self.in_dat_type == 'Tomo raw') and self.in_dat_slcd_item:
            with h5py.File(self.in_dat_fn, 'r') as f:
                self.in_dat_item_sz = f[dat_dict.TOMO_FLY_ITEM_DICT[
                    self.in_dat_slcd_item]['path']].shape
            self.hs['ConvDatInItmInfo txt'].value = dat_dict.TOMO_FLY_ITEM_DICT[
                self.in_dat_slcd_item][
                    'description'] + f"\nshape: {self.in_dat_item_sz}" + f"\ndtype: {dat_dict.TOMO_FLY_ITEM_DICT[self.in_dat_slcd_item]['dtype']}"
            self.in_dat_path_in_h5 = dat_dict.TOMO_FLY_ITEM_DICT[
    ...
    def ConvDatInItmCmf_btn_clk(self, a):
        ...
        self.out_fn_set = False
        self.hs['ConvData acc'].selected_index = 1
        self.boxes_logic()
