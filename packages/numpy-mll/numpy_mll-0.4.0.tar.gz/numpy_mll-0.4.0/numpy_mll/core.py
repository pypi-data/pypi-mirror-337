...
def showall():
    ...
    print(
        '''
#EXP-1-3-IMPLEMENTATION OF WORD ANALYSIS
...
# chunked.draw() 
# Instead, print the chunked structure:
# print(chunked)
...
def showtxt():
    """Print the contents of data.txt"""
    file_path = os.path.join(os.path.dirname(__file__), "sam.txt")
    
    if not os.path.exists(file_path):
        print("Error: sam.txt not found.")
        return
    
    with open(file_path, "r", encoding="utf-8") as file:
        print(file.read())

# Define exp1 to exp10 functions with indented multiline prints
def exp1():
    print('''
        IMPLEMENTATION OF WORD ANALYSIS
        from collections import Counter
        import string
