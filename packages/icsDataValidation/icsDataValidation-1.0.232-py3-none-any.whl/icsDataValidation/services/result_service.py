    ...
    if in_src and in_trgt :
        ...
        if result_params.src_columns_aggregate != {}:
            ...
            if column in result_params.src_columns_aggregate:
                aggregation_type = result_params.src_columns_aggregate[column][0]
            ...
            if column in result_params.aggregation_differences_trgt_minus_src and not result_params.aggregation_differences_trgt_minus_src[column] == '0_0':
                aggregation_equal = False
                aggregation_difference_trgt_minus_src = result_params.aggregation_differences_trgt_minus_src[column]
            elif aggregation_result_src is not None and aggregation_result_trgt is not None:
...
def prepare_object_level_result(
    ...
    object_level_comparison_result = {
        ...
        "TRGT_OBJECT_TYPE": src_object.type,
...
class ResultService(TestingToolParams):
    ...
    def __init__(self, prep_result: dict, remaining_mapping_objects: dict, start_time_utc: str):
        super().__init__()
        self.results = prep_result
