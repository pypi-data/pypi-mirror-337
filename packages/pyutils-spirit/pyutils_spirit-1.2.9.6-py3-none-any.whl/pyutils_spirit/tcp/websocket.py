...
class WebSocket:
    ...
    def WebSocketServer(cls, host: str, port: int):
        ...
        def decorator_handler(other_cls) -> type:
            nonlocal param_cls  # 用nonlocal来修改外部变量
            param_cls = other_cls
        ...
        try:
            # 这个是装饰器的返回值，实际会传入 WebSocketServer 中调用
            return decorator_handler
        finally:
            # 在finally中调用banner和服务器启动
            draw_websocket_banner()
            # 确保 param_cls 在启动服务器时有效
            if param_cls is not None:
                cls.__start_server(host, port, param_cls)
