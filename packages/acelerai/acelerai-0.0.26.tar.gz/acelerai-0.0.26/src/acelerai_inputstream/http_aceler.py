...
class AcelerAIHttpClient():
    ...
    def upload_asset(self, path, pkey):
        """
        ...
        try:
            ...
            files = {'file': (f'{path.split("/")[-1]}', file_stream)}

            # Thesse parameters are going to be on the backend
            file_stream = pd.DataFrame()

            # Request api
            url_base = "https://localhost:1000/Project"
            endpoint = f"{url_base}/{pkey}/UploadAsset"

            response = requests.post(endpoint, files = files, headers=headers, verify=False)
            if response.status_code == 200:
    ...
    def get_asset(self, file_name, pkey, id):
        try:
            ...
            response = requests.post(endpoint, json = {"pathCloud": file_name, "id": id}, headers=headers, verify=False)
