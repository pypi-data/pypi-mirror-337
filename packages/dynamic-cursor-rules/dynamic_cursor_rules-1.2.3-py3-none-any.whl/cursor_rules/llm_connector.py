...
class LLMConnector:
    ...
    def request(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            if request.provider == LLMProvider.OPENAI:
                ...
                response = self._request_anthropic(request, model, request_id)
    ...
    def _request_anthropic(self, request: LLMRequest, model: str, request_id: str) -> LLMResponse:
        ...
        # Ensure system message and prompt are not empty
        system = request.system_message
        prompt = request.prompt
        
        # Handle empty messages (Anthropic requires non-empty content)
        if not system:
            system = "You are a helpful AI assistant."
        
        if not prompt:
            prompt = "Please provide a useful response."
        
        print(f"\n--- Anthropic API Request Details ---")
        print(f"System message preview: {system[:100]}...")
        print(f"Prompt preview: {prompt[:100]}...")
        print(f"Prompt content valid: {len(prompt) > 0}")
        print(f"Prompt content empty: {prompt.strip() == ''}")
        ...
        try:
            ...
            print(f"Request details: model={model}, max_tokens={request.max_tokens}, temperature={request.temperature}")
            raise
