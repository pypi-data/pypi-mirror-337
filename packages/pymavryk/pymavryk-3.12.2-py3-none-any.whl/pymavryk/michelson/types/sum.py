...
from pymavryk.michelson.types.adt import ADTMixin
from pymavryk.michelson.types.adt import Nested
...
class OrType(MichelsonType, ADTMixin, prim='or', args_len=2):
    ...
    def generate_pydoc(cls, definitions: list, inferred_name=None, comparable=False):
        ...
        assert isinstance(flat_args, dict), f'sum type has to be named (in the scope of PyMavryk)'
        if cls.is_enum:
    ...
    def from_python_object(cls, py_obj) -> 'OrType':
        ...
        if isinstance(py_obj, dict):
            ...
            assert key_to_path, f'sum type has to be named (in the scope of PyMavryk)'
    ...
    def to_python_object(self, try_unpack=False, lazy_diff=False, comparable=False) -> Union[tuple, dict]:
        ...
        ), f'sum type has to be named (in the scope of PyMavryk)'
