...
async def mock_db():
    ...
    class MockCursor:
        ...
        def skip(self, count):
            self._skip = count
            return self  # Permitir el encadenamiento
        ...
        async def to_list(self, length=None):
            # Eliminar los primeros `_skip` documentos y limitar el resultado a `_limit` o `length`
...
async def test_insert(mock_db):
    dao = AsyncMongoDAO(mock_db, "contacts")
    data = {"name": "John Doe", "email": "johndoe@example.com"}
    inserted_id = await dao.insert(data)

    assert inserted_id == "123456789", "El ID insertado debe coincidir con el mock"
...
async def test_find_by_id(mock_db):
    ...
    assert result is not None, "Debe retornar un documento"
    assert result["_id"] == ObjectId("656ad08c4f1a2c48b0e598b5"), "El ID del documento debe coincidir"
    assert result["name"] == "John Doe", "El nombre debe ser 'John Doe'"
