...
class ShardDownloader:
    ...
    def download_file(self, url_path: str, output_path: str, retries: int = RETRY_ATTEMPTS) -> bool:
        ...
        for retry in range(retries):
            for server in best_servers:
                try:
                    ...
                    traceback.print_exc()
                    logger.error(f"Failed to download {url_path} from {server}: {str(e)}")
            ...
            if retry < retries - 1:
                ...
                if retry < FAST_RETRY_ATTEMPTS:
                    ...
                    logger.info(f"Fast retrying download of {url_path} (attempt {retry + 1}/{retries}, wait: {wait_time}s)")
                else:
                    wait_time = SLOW_RETRY_INTERVAL
                    logger.info(f"Slow retrying download of {url_path} (attempt {retry + 1}/{retries}, wait: {wait_time}s)")
