...
class SpecialKeys(Enum):
    ...
    # Use the same path as the source file
    SAME = auto()
    ...
    def __init__(
        ...
        if group_path is not None:
            try:
                ...
                group_scope = json.loads(group_scope, cls=JSONCDecoder)
        ...
        try:
            ...
            system_scope = json.loads(system_scope, cls=JSONCDecoder)
...
class SystemItem:
    ...
    def _init_target(self, data: dict[str, Any]) -> Path:
        ...
        match target_dir:
            ...
            case SpecialKeys.SAME:
                # Use the same path as the source file
                if target_stem is None:
                    target = self.relative_source_path
                else:
                    source_name = self.relative_source_path.name
                    source_ext = self.relative_source_path.name.split('.', 1)[-1]
                    
                    if source_name == source_ext:
                        # Source has no extension
                        target_name = target_stem
                    else:
                        # Source has an extension and we need to append it to
                        # the target stem to get target name
                        target_name = target_stem + '.' + source_ext
                    target = Path(self.relative_source_path.parent) / target_name
            case _:
...
def load_plugin(plugin_path: Path, wd_path: Path) -> Dict[str, Any]:
    ...
    for reserved_variable in [
            ...
            'AUTO_FLAT_SUBFOLDER', 'AUTO_FLAT', 'SAME']:
...
def main_regolith():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['pack', 'unpack']:
            ...
            try:
                config = json.loads(sys.argv[1], cls=JSONCDecoder)
            except Exception:
    ...
    try:
        if mode == 'eval':
            ...
            with open(undo_path, 'r', encoding='utf8') as f:
                old_op_stack = json.load(f, cls=JSONCDecoder)
...
def main_app_run(args: argparse.Namespace, systems_path: Path):
    ...
    if args.scope is not None:
        try:
            config_scope = json.loads(args.scope, cls=JSONCDecoder)
        except Exception:
    ...
    if args.replacements is not None:
        try:
            global_replacements: dict[str, str] | None = json.loads(
                args.replacements, cls=JSONCDecoder)
        except Exception:
