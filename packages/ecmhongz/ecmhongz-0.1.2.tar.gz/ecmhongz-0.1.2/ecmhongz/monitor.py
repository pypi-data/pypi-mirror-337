...
def save_to_csv(task_name, cpu_usage, gpu_data_list, other_metrics, timestamp, time_stamp_insert):
    ...
    try:
        ...
        # 定义保存数据的目录（调用者目录下的 monitor_data 文件夹）
        save_dir = os.path.join(caller_dir, 'monitor_data')
...
def draw(table_path, format):
    ...
    if format == "mysql":
        try:
            ...
            if not data:
                logging.warning(f"No data found in table {table_path}.")
                return
        ...
        try:
            ...
            if df.empty:
                logging.warning(f"No data found in file {table_path}.")
                return
    ...
    output_file = os.path.join(output_dir, f"{table_path}_metrics.html")
    ...
    while _monitor_running:
        try:
            ...
            if metrics["gpu_info"] is None or metrics["cpu_usage"] is None:
                logging.warning("未能采集到部分指标，跳过本次采样。")
            ...
            if _output_format == "csv":
                ...
                logging.error(f"未知的输出格式：{_output_format}")
                break
...
def start(task_name: str, sampling_interval: float = 1, output_format: str = "csv"):
    ...
    # 控制台输出
    print(f"ecm监控工具已启动，正在监控任务 '{_task_name}' ，数据将保存至:monitor_data/{_task_name}_{_timestamp}.csv。")
...
def stop():
    ...
    if not _monitor_running:
        logging.warning("ecm监控工具没有在运行。")
        return
    _monitor_running = False
    _monitor_thread.join()
    print(f"任务 '{_task_name}' 已结束，ecm监控工具停止，共采集{_inserted_count}个样本，详细数据将保存至:monitor_data/{_task_name}_{_timestamp}.csv，简略数据如下：。")

    # 使用调用者的目录，而不是当前包的 __file__ 目录
    caller_dir = get_caller_directory()
    csv_file_path = os.path.join(caller_dir, 'monitor_data', f"{_task_name}_{_timestamp}.csv")
