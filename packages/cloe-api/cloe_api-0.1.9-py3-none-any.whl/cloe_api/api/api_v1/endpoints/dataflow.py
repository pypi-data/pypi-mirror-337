...
def auto_column_mapping(
    ...
    source_tables: list[base.Table | None] = [
        databases.id_to_tables.get(source_table_id, None)
    ...
    if sink_table is None or not all(source_tables):
        raise HTTPException(
            status_code=400,
            detail=(
                "Sink_table_id or source_table_ids were either empty or not found in"
                " databases."
            ),
        )
    ...
    sink_column_names_alpha = {
        only_alpha.sub("", column_name): column
        for column_name, column in sink_columns.items()
    }
        ...
        if column_name in sink_column_names_alpha:
            ...
            if column.data_type != sink_datatype:
                if sink_datatype in simplified_name_conversion:
                    datatype_conversion = simplified_name_conversion[
                        sink_datatype
                    ].output_type
    ...
    return [
        ...
        for column_mapping in sorted(
            column_mappings, key=lambda p: p.source_column_name or ""
        )
    ]
