...
class WhisperTimestampedASR(ASRBase):
    """使用whisper_timestamped库作为后端的ASR实现
    
    这是最初测试的后端实现，虽然比faster-whisper慢，但GPU安装更简单。
    主要用于生成带时间戳的转录结果。
    """
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        Args:
            modelsize: 模型大小，如'tiny'、'base'、'small'、'medium'、'large'
            cache_dir: 模型缓存目录
            model_dir: 自定义模型目录（当前未实现）
        Returns:
            加载好的Whisper模型
        """
        import whisper
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        Args:
            audio: 音频数据，numpy数组格式
            init_prompt: 初始提示文本，用于改善转录质量
        Returns:
            包含转录结果的字典，包括时间戳和文本
        """
...
class FasterWhisperASR(ASRBase):
    """使用faster-whisper库作为后端的ASR实现
    
    相比原始Whisper实现速度提升约4倍（离线模式下）。
    GPU版本需要特定CUDNN版本支持。
    """

    sep = ""  # FasterWhisper会自动添加必要的空格
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        Returns:
            加载好的FasterWhisper模型
        Raises:
            ValueError: 当modelsize和model_dir都未指定时
        """
        ...
        # 使用CUDA FP16模式，在NVIDIA GPU上性能最佳
        model = WhisperModel(model_size_or_path, device="cuda", compute_type="float16", download_root=cache_dir)

        # GPU INT8模式（可选）
        # 注：相比FP16，转录质量略差且速度慢约20%
        #model = WhisperModel(model_size, device="cuda", compute_type="int8_float16")

        # CPU INT8模式（可选）
        # 注：可用但速度比CUDA FP16慢约10倍
    ...
    def ts_words(self, segments):
        ...
        pause_markers = {'，', ',', '、'}  # 轻度停顿标点
        
        for segment in segments:
            # 使用更严格的非语音判断标准
            if segment.no_speech_prob > 0.85 and segment.avg_logprob < -1.0:
                if current_sentence:
            ...
            # 调整时间间隔阈值，使其更符合自然语言停顿
            if (time_gap > 0.6 and segment.no_speech_prob > 0.7) or time_gap > 1.0:
                if current_sentence:
                    o.extend(current_sentence)
                    current_sentence = []
            
            for word in segment.words:
                w = word.word.strip()  # 去除多余空格
                if not w:  # 跳过空白词
                    continue
                ...
                # 根据标点符号和语音特征进行分句
                if any(marker in w for marker in sentence_end_markers):
                    # 对于轻度停顿标点，仅在时间间隔较大时分句
                    if any(marker in w for marker in pause_markers) and len(current_sentence) < 5:
                        continue
                    o.extend(current_sentence)
                    current_sentence = []
...
class OpenAIWhisperASR(ASRBase):
    """
    使用OpenAI官方API作为后端的ASR实现
    
    优点：
    - 无需本地GPU，使用OpenAI云端服务
    - 支持最新的Whisper模型
    - 可扩展性好，随OpenAI API更新自动获得改进
    ...
    def __init__(self, lan, modelsize=None, cache_dir=None, model_dir=None, logfile=sys.stderr):
        ...
        self.audio_cache = AudioCache()  # 初始化音频缓存管理器
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        # 设置默认模型版本
        self.model_name = modelsize if modelsize else "whisper-1"
    ...
    def ts_words(self, response):
        """
        提取带时间戳的词语列表
        
        Args:
            response: OpenAI API的响应结果
        Returns:
            列表，每个元素为(开始时间,结束时间,词语)的元组
        """
        words = []
...
def add_shared_args(parser):
    ...
    parser.add_argument('--buffer_trimming', type=str, default="segment", choices=["sentence", "segment"],help='缓冲区修剪策略 -- 修剪标有标点符号并被句子分割器检测到的完整句子，或 Whisper 返回的完整段落。必须为 “sentence” 选项安装句子分句器。')
