...
class AshDisperseResult:
    ...
    def __init__(self, params, C0_FT, Cz_FT):
        ...
        for igrain in range(Ng):
            C0[:, :, igrain] = np.real(irfft2(C0_FT[:, :, igrain], s=(Ny,Nx))) / (res_x * res_y)
            for k in range(Nz):
                Cz[:, :, k, igrain] = np.real(irfft2(Cz_FT[:, :, k, igrain], s=(Ny,Nx))) / (res_x * res_y)
    ...
    def from_netcdf(r, filename):
        ...
        C0_FT = da.C0_FT_r.values + 1j*da.CO_FT_i.values
    ...
    def to_netcdf(self, filename="AshDisperse.nc", compress=True, complevel=5):
        ...
        da = xa.Dataset(
            ...
            coords=dict(
                ...
                kx=self.kx[:self.params.solver.Nx//2+1],
                ky=self.ky,
