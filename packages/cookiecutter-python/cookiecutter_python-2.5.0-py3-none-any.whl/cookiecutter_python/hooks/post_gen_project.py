...
# mapping of Doc Builders ID to docs template folder path
...
# TODO: read from cookiecutter['_template'] / cookiecutter.json
# TODO newer: compute this data inside get_docs_gen_internal_config
# delete mkdocs.yml if not using mkdocs
# delete sphinx files if not using sphinx

builder_id_2_files: t.Dict[str, t.Iterable[str]] = {
    # some Docs Builder systems keep all files in one dir (ie docs)
    # some others, like mkdocs, keep files in multiple dirs
    'mkdocs': [
        'mkdocs.yml',
        'scripts/gen_api_refs_pages.py'
    ],
}
"""Files outside of docs folder, per Docs Builder ID"""
...
def post_file_removal(request):
    ...
    files_to_remove = [
        ## REMOVE Files based on 'cli', 'module' or 'pytest-plugin' Project Type
    ...
    #### Keep only generated docs files corresponding to the Docs Builder (ie mkdocs) selected

    # 1. Delete all docs folders, except the one corresponding to the selected Docs Builder
    ...
    # 2. Delete all docs-related files outside of docs folder, except the ones
    #    corresponding to the selected Docs Builder (ie if sphinx, we delete mkdocs.yml, etc)
