#!/usr/bin/env python3
"""
Test script for DynamoDB Database operations.
This script tests the functionality of the unified Database interface with DynamoDB.
"""
...
# Import the Database and Logger classes
...
def setup_database(auth, logger, region: str, profile: Optional[str] = None):
    """
    Set up a DynamoDB database connection using the authenticated manager.
    ...
    Returns:
        Database instance
    """
    logger.info(f"Setting up DynamoDB database connection in region {region}")
    ...
    try:
        ...
        db = auth.get_database(
            ...
            namespace="db_test",
            instance_id=f"dynamo-test-{uuid.uuid4()}",
            opt=opt
        )
        logger.info("DynamoDB database connection established successfully")
        return db
    except Exception as e:
        logger.error(f"Failed to set up DynamoDB connection: {e}")
        raise
...
def test_crud_operations(db, logger) -> Dict[str, Any]:
    ...
    try:
        ...
        db.update_document(
            ...
            "users", 
            {
                "is_active": "false",  # Using string instead of boolean
                "metadata": {
                    "login_count": 2,
...
def test_query_operations(db, logger, item_ids: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Query active users
        # Note: DynamoDB requires a hash key for queries, so we need to query by a field that is a hash key
        ...
        # Query in-stock electronics (using multiple query calls due to DynamoDB limitations)
        # First query by category
        electronics = db.query(TestProduct, {"category": "electronics"})
        # Then filter the results
        in_stock_electronics = [item for item in electronics if item['in_stock'] is True]
...
def main():
    ...
    parser.add_argument("--log-file", default="logs/dynamo_test.log", help="Log file path")
    ...
    logger.info("=" * 50)
    logger.info("Starting DynamoDB Database test")
    ...
    try:
        # Setup database
        db = setup_database(auth, logger, args.region, args.profile)
