...
import { getInitialImageValueForStudio, parseSpecName } from '../widgets/CreateNotebookJobForm/Studio/studioHelpers';
...
describe('Image value tests', () => {
  ...
  it('Parse 1P Image spec', () => {
    const imageName =
      'python__SAGEMAKER_INTERNAL__arn:aws:sagemaker:us-west-2:236514542706:image/tensorflow-1.15-cpu-py36';
    const spec = parseSpecName(imageName);
    expect(spec).toEqual({
      arnEnvironment:
        'arn:aws:sagemaker:us-west-2:236514542706:image/tensorflow-1.15-cpu-py36',
      kernel: 'python',
  ...
  it('Parse Custom Image Version spec', () => {
    const imageName =
      'glue_pyspark__SAGEMAKER_INTERNAL__arn:arn:aws:sagemaker:us-west-2:236514542706:image-version/my-custom-image/2';
    const spec = parseSpecName(imageName);
    expect(spec).toEqual({
      arnEnvironment:
        'arn:arn:aws:sagemaker:us-west-2:236514542706:image-version/my-custom-image/2',
      kernel: 'glue_pyspark',
      version: '2',
      imageVersionAlias: null
    });
  ...
  it('Parse SMD Version spec', () => {
    const imageName =
      'glue_pyspark__SAGEMAKER_INTERNAL__arn:aws:sagemaker:us-west-2:123458284337:image/sagemaker-distribution-cpu-v0__SAGEMAKER_INTERNAL__0.13';
    const spec = parseSpecName(imageName);
    expect(spec).toEqual({
      arnEnvironment:
        'arn:aws:sagemaker:us-west-2:123458284337:image/sagemaker-distribution-cpu-v0__SAGEMAKER_INTERNAL__0.13',
      kernel: 'glue_pyspark',
      version: null,
      imageVersionAlias: '0.13'
    });
  });
