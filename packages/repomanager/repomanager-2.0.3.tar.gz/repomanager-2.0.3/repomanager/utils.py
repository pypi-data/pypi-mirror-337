...
def repomanager_cmdline_args():
    ...
    parser.add_argument('--threads','-t',
                        type=int,
                        default=8,
                        help='Number of parallel threads for cloning repositories',
                        metavar='N')
    parser.add_argument('--depth',
                        type=int,
                        default=1,
                        help='Clone depth. Use 0 for full clone',
                        metavar='N')
    return parser
...
class Command():
    ...
    def run(self, logging=True, **kwargs):
        ...
        out = "Max retries reached. Could not resolve host."
        xret_code = -1
        max_retries = 5
        retry_delay = 5
        for attempt in range(max_retries):
            try:
                x = subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE, encoding="utf-8", **kwargs)
                # x = subprocess.Popen(cmd, stdout=subprocess.PIPE,stderr=subprocess.PIPE,  **kwargs)
                xret_code = x.returncode
                out = x.stdout
                err = x.stderr
            except Exception as e:
                logger.warning(e)
                logger.error('Failed to clone "{0}" at directory "{1}". Reason : {2}'.format(cmd, cwd, str(e)))
                break
            else:
                if xret_code != 0 and logging:
                    if out:
                        logger.warning(out)
                    if "could not resolve host" in err.lower():
                        logger.warning("Retrying connection to host . . .")
            ...
            logger.error("Max retries reached. Could not resolve host.")
            raise SystemExit(1)
        return out, xret_code
