...
try: from pyimager.main import new_img
except: from main import new_img
import unicodedata
...
if __name__ == "__main__":
    ...
    drawn = 0
    for i in CHARS:
        c = Text(i).text.chain[0]
        r = draw_char(img, c, [[0,0], [1,0], [0,1], [1,1]], help=True)
        print(c, r)
        if r: drawn += 1
    print(f"{used:0>4}/{total} => {used/total:.2%} used\n{drawn:0>4}/{used} => {drawn/used:.2%} drawn")