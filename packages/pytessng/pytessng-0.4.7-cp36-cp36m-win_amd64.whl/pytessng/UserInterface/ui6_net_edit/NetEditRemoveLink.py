...
from pytessng.UserInterface.public import BaseUIVirtual, BaseMouseSelector
...
class NetEditRemoveLink(BaseUIVirtual):
    ...
    def __init__(self):
        super().__init__()
        # 按钮
        self.action: QAction = GlobalVar.actions_related_to_mouse_event["remove_link"]
        # 将按钮与状态改变函数关联
    ...
    def load_ui(self):
        if self.action.isChecked():
            ...
            # 显示提示信息
            self.utils.show_message_box("请使用鼠标进行框选来删除路段：\n  -  从左向右框选时，需要框选整个路段；\n  -  从右向左框选时，只需框选路段的一部分即可！")
    ...
    def apply_remove_links(self, params: dict):
        MyOperation().apply_net_edit_operation(self.mode, params, self)
