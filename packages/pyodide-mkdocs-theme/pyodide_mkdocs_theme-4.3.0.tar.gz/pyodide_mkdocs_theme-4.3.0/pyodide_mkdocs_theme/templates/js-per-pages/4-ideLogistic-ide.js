...
class IdeStorageManager extends TerminalRunner {
  ...
  getStorage(editorId=null){
    ...
    if(!upToDate) this.updateGenericStorageData(storage)
    return storage
  }
...
`
  ...
  dropArchiveFactory(asyncLockedZipDropper){
    return async ev=>{
      ...
      const zipFiles = [
        ...ev.dataTransfer[ useItems?'items':'files' ]
      ].filter(
        itemOrFile => itemOrFile.kind === "file" && itemOrFile.type === "application/zip"
      ).map(
        itemOrFile => useItems ? itemOrFile.getAsFile() : itemOrFile
      )
      await asyncLockedZipDropper(ev, zipFiles)
    }
  }


  async dropArchive(_e, zipFiles){
    if(zipFiles.length!=1){
      this.giveFeedback("Cannot Import multiple archives.")
    }else{
      this.getZipContentAsBytes(zipFiles[0])
    }
  }
