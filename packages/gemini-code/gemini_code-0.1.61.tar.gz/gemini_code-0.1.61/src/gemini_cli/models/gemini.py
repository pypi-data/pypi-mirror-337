...
Includes Test Runner tool & Quality Tools & TaskComplete Tool.
IMPROVED ARG PARSING. Prompt v12. Syntax FIXED.
"""
...
# Updated Regex to find tool calls more reliably
...
class GeminiModel:
    ...
    def generate(self, prompt: str, conversation=None) -> str | None:
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                # === Parse and Execute Tool Calls (Uses updated method) ===
    ...
    # --- UPDATED _parse_and_execute_tool_calls with robust string parsing ---
            ...
            try:
                ...
                for kw_match in kw_arg_pattern.finditer(args_str):
                    ...
                    if kw_match.group(3) is not None: # '''
                        ...
                        try: args[arg_name] = bytes(kw_match.group(5), "utf-8").decode("unicode_escape")
                        except Exception: args[arg_name] = kw_match.group(5) # Fallback
                        logging.debug(f"Parsed arg '{arg_name}' as single-quoted string")
                    elif kw_match.group(6) is not None: # "
                        try: args[arg_name] = bytes(kw_match.group(6), "utf-8").decode("unicode_escape")
                        except Exception: args[arg_name] = kw_match.group(6) # Fallback
                ...
                # Check if there's unparsed content (might indicate parsing error or complex structure)
...
4.  **Reflect & Generate Next Action OR Finish:** Analyze results.
    *   If more steps needed, generate code for the *next single action* (usually `edit`, `bash`, or `test_runner`).
...
5.  **Repeat:** The CLI will continue executing your generated code and providing results until `task_complete` is called.
...
*   **`edit` Tool Usage:** Use `content='...'` to create/overwrite files. Use `old_string='...'` and `new_string='...'` arguments to replace specific text. Do not use `content` and `old_string`/`new_string` in the same call.
