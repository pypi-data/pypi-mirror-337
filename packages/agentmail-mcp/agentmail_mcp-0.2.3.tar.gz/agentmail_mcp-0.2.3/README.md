...
1. Navigate to cursor settings and click on the features tab, and scroll down to the MCP section.
2. Open the terminal and run the following command to find out where the MCP server is located. It should be in some .venv/bin/ directory. Copy this path.

```bash
which agentmail-mcp
```

2. Click on the "+" button to add a new MCP server.
3. Enter the following information:
   - Name: agentmail
   - type: command
   - command: {path_to_agentmail_mcp} --api-key {AGENT_MAIL_API_KEY}
4. Click on the "Save" button.

Make sure to change Cursor to Agent Mode.

Then ask it: "What mcp tools do you have access to?"
