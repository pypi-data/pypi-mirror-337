...
import image_article_comprehension.model.chat as chat
from image_article_comprehension.model.google_genai import GenaiConversationMessage, GenaiMessagePart, MessageType
import image_article_comprehension.utils as utils
from image_article_comprehension.create.prompts.create_xuanti_search_prompt import (
    EXTRACT_SEARCH_KEYWORDS_PROMPT,
    NOTE_PROVIDER_PROMPT,
    SAME_TOPIC_FIND_PROMPT,
    EXTRACT_MATERIALS_PROMPT,
    MERGE_SEARCH_NOTE_SCRIPT_AND_RENSHE_SCRIPT,
...
import image_article_comprehension.comprehension.script0221.script_compehension as script_comprehension
import image_article_comprehension.comprehension.script0221.script_prompt as script_prompt
import image_article_comprehension.create.script_pipeline as script_pipeline
...
def _find_material_image(note_list, note_id, image_index):
    ...
    real_index = image_index if (type(image_index) is int) else int(image_index) - 1
...
def _generate_script_by_history_note(process_result, renshe_info, final_xuanti):
    ...
    for same_topic_note in same_topic_notes[:1]:
        ...
        for index , h_note in enumerate(history_notes):
            # 历史参考帖子
            history_note_prompt = NOTE_PROMPT.format(note_description=f"【历史创作帖子{index+1}】", title=h_note.get("title"), body_text=h_note.get("body_text"))
            history_note_conversation_message = google_genai.GenaiConversationMessage.text_and_images(history_note_prompt, h_note.get("images"))
            history_message.append(history_note_conversation_message)
        # 搜索同题帖子
        same_topic_note_prompt = NOTE_PROMPT.format(note_description="【参考的同选题帖子】", title=same_topic_note.get("title"), body_text=same_topic_note.get("body_text"))
        same_topic_note_conversation_message = google_genai.GenaiConversationMessage.text_and_images(same_topic_note_prompt, same_topic_note.get("images"))
        history_message.append(same_topic_note_conversation_message)

        history_note_and_search_generate_prompt = HISTORY_NOTE_AND_SEARCH_GENERATE_PROMPT.format(final_xuanti=final_xuanti)
        # 生成脚本
        ...
        result = {
            ...
            "history_note": history_notes
        }

        image_description_list = script_ans.get("创作的脚本",[]).get("图集描述",[])

        for i in image_description_list:
            # 图片生成
...
if __name__ == "__main__":
    ...
    filter_xuanti_path = "/Users/nieqi/Documents/workspace/python/image_article_comprehension/create/result/xuanti_result_0218/account_陸清禾_5657ba2703eb846a34fcc55b/filter_xuanti.json"
    renshe_path = "/Users/nieqi/Documents/workspace/python/image_article_comprehension/aigc_data/renshe_0305/account_陸清禾_5657ba2703eb846a34fcc55b.json"
    # 替换 end

    filter_xuanti_result = json.load(open(filter_xuanti_path, "r"))

    result_save_base_dir_path = "/Users/nieqi/Documents/workspace/python/image_article_comprehension/create/result/xuant_search"
    ...
    for result in filter_xuanti_result:
        ...
        if result.get("xuanti_creation").get("最终的选题") == "日落时分城市建筑光影与紫藤花交织":
