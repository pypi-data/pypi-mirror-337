...
class TestRuntime(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.driver = Driver([DependencyCollectionPhase(sources), TypingPhase(SimpleTypeSystem()),
    ...
    def execute(self, program: ast.Module):
        ...
        with redirect_stdout(io.StringIO()) as f:
            target_module, deps_asts = self.driver.run(program)
            ...
            exec(compile(ast.fix_missing_locations(target_module), '<ast>', 'exec'), scope)

        return TestResult(scope, f.getvalue())

        parsed_ast = ast.parse(program)
        print(ast.dump(parsed_ast, indent=2))

        env, b, mods, known = type_check(parsed_ast, program, parent_dir=self.sources)

        # We compile the necessary modules for execution
        new_modules = {_mod: compile(ast.fix_missing_locations(_ast[0]), "<gpython>", "exec")
                       for _mod, _ast in mods.items()}

        #print(ast.dump(ast.fix_missing_locations(b), indent=2))
        scope = globals()
        print(ast.unparse(ast.fix_missing_locations(b)))

        # We will partially initialize modules
        for name, code in new_modules.items():
            module_obj = builtin_types.ModuleType(name)
            sys.modules[name] = module_obj

        # Then update namespaces where necessary
        uninitialized = [name for name in mods if mods[name][1]]
        for name in uninitialized:
            name_tokens = name.split('.')
            for i, token in enumerate(name_tokens[:-1]):
                module_name = '.'.join(name_tokens[:i+1])
                to_add_name = name_tokens[i+1]
                to_add_module = sys.modules[f"{module_name}.{to_add_name}"]
                if not hasattr(sys.modules[module_name], to_add_name):
                    setattr(sys.modules[module_name], to_add_name, to_add_module)

        # Now finally, execute all dependency modules
        for name, code in new_modules.items():
            module_obj = sys.modules[name]
    ...
    def test_tuple_assign_with_previous_typing_information_static_error(self):
        program = self.read_source("tuple_assign_with_previous_typing_information_static_error.py")
    ...
    def test_tuple_assign_with_previous_typing_information_runtime_error(self):
        program = self.read_source("tuple_assign_with_previous_typing_information_runtime_error.py")
        self.assertRaises(RuntimeException, lambda: self.execute(program))
