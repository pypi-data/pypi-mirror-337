...
class Model(wrappers.ModelData):
    ...
    def start_and_await_inference(self, inputs: Dict[str, Any], params: Dict[str, Any], timeout_ms: int | None = None, polling_interval_ms: int = 500) -> Any:
...
class Task(wrappers.TaskData):
    ...
    def __init__(self, task_data: wrappers.TaskData, model: Model) -> None:
        ...
        if model.id != task_data._data.model_id:
            raise ValueError(f"The provided model '{model.id}' does not match the one associated with the task: '{task_data._data.model_id}'.")
        self._model = model
