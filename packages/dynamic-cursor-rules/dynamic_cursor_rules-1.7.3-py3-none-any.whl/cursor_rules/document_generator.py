...
class DocumentGenerator:
    ...
    def generate_all_documents(self) -> Dict[str, str]:
        ...
        try:
            ...
            if cursorrules_paths: # Check if list is not empty
                if len(cursorrules_paths) == 1:
                 ...
                 # Handle case where _generate_cursorrules returns empty list without exception
                 print_warning(f"Cursor Rules generation returned no files ({elapsed:.1f}s).")
        except Exception as e:
            elapsed = time.time() - start_time
        ...
        start_time = time.time()
        try:
            # This call now uses the new LLM-based method
        ...
        try:
            start_time = time.time()
            # Assume _create_development_log exists and is robust or raises its own errors
            ...
            print_success(f"Created Development Log ({elapsed:.1f}s)")
             ...
             elapsed = time.time() - start_time
             # Log error but allow completion if dev log fails?
    ...
    def _generate_cursorrules(self, cursor_dir: str) -> List[str]:
        ...
        for category in categories_to_generate:
            ...
            try:
                ...
                # Base system prompt for single file generation
                # Define as f-string for consistency and potentially cleaner parsing
                system_message_template = f"""
                You are an expert at creating rules for Cursor IDE. Your task is to generate ONE SINGLE rule file (.mdc format) for the specified category.
                
                The project is currently in its FOUNDATION phase, focusing on setting up core structures and standards.
                
                The rule file MUST follow this exact format:
                # <Rule Title matching the category>
                pattern: <Specific glob pattern for files this rule applies to>
                description: <Semantic description of when/why this rule applies, mentioning the FOUNDATION phase context>
                ...
                @file: <path/to/relevant/example/file1.ext>
                @file: <path/to/relevant/example/file2.ext>
                
                Focus on providing actionable, project-specific guidance relevant to the category '{{category}}' and the FOUNDATION phase. Be precise with the 'pattern:'. Use the provided project context.
                Output ONLY the content of the .mdc file, starting with the # Title line.
                """ # Note: using f-string requires escaping literal braces with {{}}

                # Specific user prompt including context (already an f-string)
                user_prompt = f"""
                Generate the .mdc rule file content for the category: {category}
                ...
                Remember to output ONLY the raw .mdc file content, starting with # Title.
                """

                # --- Execute Single LLM Call --- 
                request = LLMRequest(
                    prompt=user_prompt,
                    # Use .format() on the template string, placeholder is now {{category}}
                    system_message=system_message_template.format(category=category), 
                    provider=self.llm_provider,
                    temperature=0.2,
                ...
                if not rule_content or not rule_content.startswith('#'):
                    raise ValueError("LLM response is empty or doesn't start with a title (#).")
                
                # Extract title from the first line
                first_line = rule_content.split('\n', 1)[0]
                if first_line.startswith('# '):
                     rule_title = first_line[2:].strip()
                else:
                     rule_title = category # Fallback to category name if title format is off
                
                # Basic validation for required markers
                if 'pattern:' not in rule_content or 'description:' not in rule_content:
                     logger.warning(f"Rule content for '{category}' might be missing pattern: or description: markers.")
                     # Decide whether to save anyway or raise error - let's save for now

                # --- Save Rule File --- 
                # Use category for filename to ensure predictability, sanitize it
                ...
                try:
                    ...
                    logger.info(f"Successfully generated rule file: {rule_path}")
                except IOError as e:
    ...
    def _generate_action_items(self, output_dir: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        1.  Logical phases (e.g., Setup, Backend Core, Frontend Basics, Testing Setup) appropriate for the FOUNDATION stage. Use level 2 Markdown headings (`## Phase Title`) for phases.
        2.  Specific, actionable tasks within each phase. Use Markdown bullet points (`- Task Title`) for tasks.
        3.  Break down complex tasks into smaller sub-tasks where appropriate. Use indented bullet points (`  - Sub-task Title`) for sub-tasks.
        ...
        user_prompt = f"""
        Analyze the following documents and generate the Markdown action plan for the FOUNDATION phase.
        ...
        Remember to output ONLY the structured Markdown action plan.
        """
        ...
        try:
            ...
            # --- Future/Optional: Parse MD to ActionPlan and save JSON ---
            # try:
            ...
            #          tasks_json_path = os.path.join(output_dir, "action_items.json")
            #          # Ensure ActionPlan has save_to_json
            ...
            # Return paths (JSON path is None for now)
            return tasks_md_path, tasks_json_path
    ...
    def _create_development_log(self, output_dir: str, generated_docs: Dict[str, str]) -> str:
        """
        Create a development log summarizing generated documents and next steps.
        """
        # Implementation of _create_development_log method
        # This method should be implemented to create a development log based on the generated documents
        # and return the path to the created log file
        # This is a placeholder and should be replaced with the actual implementation
        return "path_to_development_log.md"