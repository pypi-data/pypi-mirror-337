...
def generate_technical_roles(
    ...
    m_writer.write_string_to_disk(tech_roles_script, output_path / "tech_roles.sql")
...
def generate_functional_roles(
    ...
    func_model_filepath: Annotated[
        pathlib.Path,
        typer.Option(
            help="Relative path to functional roles file (relative to git-root-path).",
        ),
    ],
    git_tag_regex: Annotated[
        Optional[str],
    ...
    """
    func_model = utils.read_yaml_from_disk(git_root_path / func_model_filepath) or {}
    func_roles = [
    ...
    if git_tag_regex is not None:
        ...
        structure_functional_old_raw: dict[str, dict | list] = (
            git_client.get_yaml_from_tag(func_model_filepath) or {}
        )
        func_model_old = {
            k: v
