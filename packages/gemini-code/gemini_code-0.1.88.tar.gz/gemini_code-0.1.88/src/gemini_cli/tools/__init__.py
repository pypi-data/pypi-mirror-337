...
from .tree_tool import TreeTool # <-- Add import

# AVAILABLE_TOOLS maps tool names (strings) to the actual tool classes.
AVAILABLE_TOOLS = {
    "view": ViewTool, # ViewTool is updated to handle size checks internally
    "edit": EditTool,
    "ls": LsTool,
    "grep": GrepTool,
    "glob": GlobTool,
...
if summarizer_available: AVAILABLE_TOOLS["summarize_code"] = SummarizeCodeTool # Add summarizer
if test_runner_available: AVAILABLE_TOOLS["test_runner"] = TestRunnerTool
if "tree" not in AVAILABLE_TOOLS: AVAILABLE_TOOLS["tree"] = TreeTool # <-- Register instance
