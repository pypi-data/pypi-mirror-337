...
class Logger:
    ...
    def __init__(self, foldername: str = None, level: str = 'DEBUG'):
        le = {'CRITICAL': 50, 'ERROR': 40, 'WARNING': 30, 'INFO': 20, 'DEBUG': 10}
        if foldername is None:
            self.__path = str(Path('Logs').absolute())
        else:
            self.__path = foldername
        if not os.path.exists(self.__path):  # 判断是否为文件

            try:
                os.mkdir(self.__path)
                print(f"目录 {self.__path} 创建成功")
            except FileExistsError:
                print(f"目录 {self.__path} 已经存在")
            except Exception as e:
                print(f"创建目录时出错: {e}")
            self.__path = self.__path + os.sep + f'{str(time.strftime("%Y-%m-%d"))}.log'
        else:
            self.__path = self.__path + os.sep + f'{str(time.strftime("%Y-%m-%d"))}.log'
        print(self.__path)
