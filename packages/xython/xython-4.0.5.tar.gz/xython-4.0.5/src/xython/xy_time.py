...
class xy_time():
	...
	def change_any_text_time_to_dt_obj(self, input_text_time):
		...
		# Tuesday
		for one_week in self.xvar["week_vs_enum"].keys():
			if one_week in dt_string:
				result["week"] = self.xvar["week_vs_enum"][one_week]
		...
		if resql_result:
			result["mon"] = self.xvar["month_vs_no"][resql_result[0][3][0]]
		...
		if resql_result:
			...
			for num in self.xvar["no_vs_month"].keys():
				if bbb[0][0] in self.xvar["no_vs_month"][num]:
					result["mon"] = num
	...
	def change_ymd_list_to_sec(self, input_list):
		...
		:return:
		"""
		total_sec = int(input_list[0]) * self.xvar["year_sec"] + int(input_list[1]) * self.xvar["month_sec"] + int(
			input_list[2]) * self.xvar["day_sec"]
		return total_sec
	...
	def data_national_holiday_in_year(self, input_ymd_list1, input_ymd_list2):
		...
		holiday_list2d = self.xvar["holiday_list"]
	...
	def get_all_holiday_for_lunar_year(self, input_year):
		...
		"""
		result = []
		for one_list in self.xvar["lunar_holiday_list"]:
	...
	def get_all_holiday_for_solar_year(self, input_year):
		...
		result = []
		for one_list in self.xvar["solar_holiday_list"]:
			if one_list[0][0] <= input_year and one_list[0][3] >= input_year:
	...
	def get_holiday_list_between_day1_and_day2(self, input_anytime1, input_anytime2):
		...
		result = []
		# self.xvar["holiday_list"] = [[[20060101, 20061231], [9, 6, "양", 1, "", "임시공휴일"]],[[19880101, 19881231], [9, 17, "양", 1, "", "임시공휴일"]],....]
		for list1d in self.xvar["holiday_list"]:
	...
	def get_weekno_set_for_dt_obj(self, input_dt_obj=""):
		...
		result = [weekno, self.xvar["week"][weekno], self.xvar["요일"][weekno]]
		return result
	...
	def get_all_data_for_dt_obj(self, input_dt_obj=""):
		...
		result["utc_timestamp"] = input_dt_obj.timestamp() - 1640995200
		result["weekno"] = input_dt_obj.strftime('%w')
		result["요일"] = self.xvar["요일"][int(result["weekno"])]
		result["ymd"] = input_dt_obj.strftime("%Y-%m-%d")
		result["ymdhms"] = input_dt_obj.strftime("%Y-%m-%d-%H-%M-%S")
		result["sec_total"] = int(result["timestamp"])
		result["day_total"] = int(result["timestamp"] / 86400)
