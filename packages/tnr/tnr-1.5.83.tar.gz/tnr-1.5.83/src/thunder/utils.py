...
def write_optional_update_cache():
    ...
    try:
        ...
        click.echo(click.style(f"Warning: Could not write optional update cache: {e}", fg="yellow"))
...
def calculate_sha256(filepath):
    ...
    try:
        ...
        click.echo(click.style(f"Warning: Could not calculate hash of {filepath}: {e}", fg="yellow"))
        return None
...
def check_binary_hash():
    ...
    if current_platform == 'darwin': current_platform = 'darwin' 
    ...
    if current_arch in ['amd64', 'x86_64']:
        ...
        click.echo(click.style(f"Warning: Unsupported architecture '{current_arch}' for binary check.", fg="yellow"))
    ...
    try:
        ...
        click.echo(click.style(f"Warning: Could not read binary hash cache: {e}", fg="yellow"))
    ...
    try:
        ...
        if not expected_hash:
            click.echo(click.style(f"Warning: Could not find hash for {current_platform}/{arch} in manifest.", fg="yellow"))
        ...
        try:
            with open(cache_file, 'w') as f:
             ...
             click.echo(click.style(f"Warning: Could not write binary hash cache: {e}", fg="yellow"))
        ...
        click.echo(click.style(f"Warning: Failed to fetch manifest file: {e}", fg="yellow"))
        # Fallback to assuming it's okay if we can't check
        return True, None
    except json.JSONDecodeError as e:
        click.echo(click.style(f"Warning: Failed to parse manifest file: {e}", fg="yellow"))
        return True, None # Assume OK if manifest is invalid
    except Exception as e:
        click.echo(click.style(f"Warning: An unexpected error occurred during binary hash check: {e}", fg="yellow"))
        return True, None
...
def check_meets_min_version(): # Renamed from check_pip_version
            ...
            with open(cache_file) as f:
                ...
                if ('timestamp' in cached and 'min_version' in cached and 
                    ...
                    try:
                        ...
                        meets = version_parser.parse(current_version) >= version_parser.parse(min_version)
                        details = ('version', current_version, min_version) if not meets else None
    ...
    try:
        ...
        if not min_version:
             click.echo(click.style("Warning: Could not retrieve minimum version from API.", fg="yellow"))
        ...
        try:
            with open(cache_file, 'w') as f:
                ...
                json.dump({'timestamp': time.time(), 'min_version': min_version}, f)
        except Exception as e:
            click.echo(click.style(f"Warning: Could not write min version cache: {e}", fg="yellow"))
        ...
        click.echo(click.style(f"Warning: Failed to fetch minimum required version: {e}", fg="yellow"))
        return True, None # Assume OK if API fails
    except Exception as e:
        click.echo(click.style(f"Warning: Unexpected error during min version check: {e}", fg="yellow"))
        return True, None
...
def attempt_binary_self_update(expected_hash):
    ...
    if current_arch in ['amd64', 'x86_64']:
        ...
        click.echo(click.style(f"Cannot auto-update: Unsupported architecture '{current_arch}'", fg="red"))
        return False
    ...
    click.echo(click.style(f"Attempting to download update...", fg="cyan"))
...
exit /b 0
"""
            updater_bat_path = os.path.join(tempfile.gettempdir(), f"tnr_updater_{current_pid}.bat") # Still use PID for unique name
            with open(updater_bat_path, "w") as bf: bf.write(updater_bat_content)
            ...
            try:
                ...
                subprocess.Popen(
                 ...
                 click.echo(click.style(f"Error launching background updater: {e}", fg="red"))
                 # Clean up temporary file if Popen failed
                 if new_exe_temp_file and os.path.exists(new_exe_temp_file):
                      try: os.remove(new_exe_temp_file)
            ...
            try: # Attempt direct replacement
                ...
                try:
                    ...
                    try: os.rename(old_exe_path, current_exe_path) 
                    except OSError: pass 
            ...
            if direct_replace_failed_permission:
                if click.confirm(click.style("Update requires admin privileges. Retry using sudo?", fg="yellow"), default=False):
                    ...
                    if not new_exe_temp_file or not os.path.exists(new_exe_temp_file):
                         click.echo(click.style("Error: Temporary update file missing. Cannot retry with sudo.", fg="red"))
                         return False
                    ...
                    try:
                        ...
                        result = subprocess.run(sudo_cmd, check=True, capture_output=True, text=True)
                        ...
                        click.echo(click.style(f"Sudo command failed (Exit Code {sudo_err.returncode}):", fg="red"))
                        if sudo_err.stderr:
                            click.echo(sudo_err.stderr)
                        if sudo_err.stdout:
                             click.echo(sudo_err.stdout) # Show output even on error
                        # Attempt to restore original if possible
                        if os.path.exists(old_exe_path) and not os.path.exists(current_exe_path):
                            try: os.system(f"sudo mv \"{old_exe_path}\" \"{current_exe_path}\"") # Use sudo to restore too
                            except: pass
                        return False # Sudo failed
                    except FileNotFoundError: # Handle case where sudo is not installed
                         click.echo(click.style("Error: 'sudo' command not found. Cannot elevate privileges.", fg="red"))
                         return False
                    except Exception as sudo_exec_err:
                        click.echo(click.style(f"Failed to execute sudo command: {sudo_exec_err}", fg="red"))
                        return False # Sudo failed
                else:
        ...
        click.echo(click.style(f"Error: Failed to download update: {e}", fg="red"))
        return False
    except Exception as e:
        return False
    finally:
        ...
        if new_exe_temp_file and os.path.exists(new_exe_temp_file):
            try:
                ...
                click.echo(click.style(f"Warning: Failed cleanup {new_exe_temp_file}: {e}", fg="yellow"))