...
from pysqream.utils import NotSupportedError, ProgrammingError, ArraysAreDisabled, mark_dbapi_attribute, dbapi_method
...
class Cursor:
    ...
    def fetchmany(self, size: int = None, data_as: str = 'rows', fetchone: bool = False):
        ...
        if not self.__more_to_fetch:
            if len(self.__parsed_rows) == 0:
        ...
        self.__logger.debug(f"Fetched {len(res) if res else 0} rows", connection_id=self.__connection_id, statement_id=self.__stmt_id)
    ...
    def __execute_sqream_statement(self, statement: str, params: list[Any] | tuple[Any] | None = None,
        ...
        self.__lb_params = self.__client.prepare_statement(statement)

        if self.__lb_params.get("reconnect", False):
            ...
            self.__client.reconnect(self.__stmt_id, self.__conn_params.database, self.__conn_params.service,
                                    self.__conn_params.username, self.__conn_params.password, self.__lb_params["listener_id"], self.__lb_params['ip'],
                                    self.__lb_params['port_ssl'] if self.__conn_params.use_ssl else self.__lb_params['port'])
        ...
        self.__column_list = self.__client.get_query_type_out()
