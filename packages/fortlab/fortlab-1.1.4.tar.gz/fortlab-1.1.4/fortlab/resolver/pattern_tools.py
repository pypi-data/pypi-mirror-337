...
class Pattern(object):
    ...
    def rsplit(self, string, is_add=False):
        ...
        assert abs(self).match(pattern_match),str(self,string,t,pattern_match)
        lhs = (''.join(t[:-2])).strip()
    ...
    def lsplit(self, string):
        ...
        rhs = (''.join(t[2:])).strip()
        assert abs(self).match(pattern_match),str(pattern_match)
        return lhs, pattern_match, rhs
    ...
    def __and__(self, other):
        if isinstance(other, Pattern):
            ...
            assert isinstance(other,str),str(other)
    ...
    def named(self, name = None):
        if name is None:
            label = self.label
            assert label[0]+label[-1]=='<>' and ' ' not in label,str(label)
        else:
            label = '<%s>' % (name)
