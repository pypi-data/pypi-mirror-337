...
def _get_default_custom_metric_assessment_source():
    try:
...
def _get_default_custom_metric_assessment_v3_source():
    try:
        source_id = context.get_context().get_user_name()
    except:
        source_id = ""
...
def _normalize_custom_assessment(
    assessment: Union[mlflow_eval.Assessment, mlflow_entities.Assessment],
    metric_name: str,
) -> Union[mlflow_eval.Assessment, mlflow_entities.Assessment]:
    """
    Normalize the custom assessment, in either v2 or v3 format:
    ...
    if isinstance(assessment, mlflow_entities.Assessment):
        if assessment.source is None:
            assessment.source = _get_default_custom_metric_assessment_v3_source()

    # TODO(ML-51637): Deprecate usage of MLflow v2 assessments
    elif isinstance(assessment, mlflow_eval.Assessment):
        if assessment.source is None:
            assessment = mlflow_eval.Assessment(
                name=assessment.name,
                source=_get_default_custom_metric_assessment_source(),
...
def _convert_custom_metric_value(
...
) -> List[Union[mlflow_eval.Assessment, mlflow_entities.Assessment]]:
    """
    Convert the custom metric value to a list of MLflow Assessments (either v2 or v3).
    ...
    Supported metric values:
        ...
        - string
        - Assessment object
        - List[Assessment] (either assessment v2 or v3)
    """
    ...
    if isinstance(metric_value, (int, float, bool, str)):
        # TODO(ML-51637): Convert usage of MLflow v2 assessments to v3
        return [
    ...
    if isinstance(metric_value, (mlflow_eval.Assessment, mlflow_entities.Assessment)):
    ...
    if isinstance(metric_value, Collection):
        ...
        for item in metric_value:
            if not isinstance(
                item, (mlflow_eval.Assessment, mlflow_entities.Assessment)
            ):
                raise error_utils.ValidationError(
                    f"Got unsupported result from custom metric '{metric_name}'. "
