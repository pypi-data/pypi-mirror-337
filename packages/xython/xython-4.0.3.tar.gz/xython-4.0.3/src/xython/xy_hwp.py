...
class han:
	...
	def __init__(self, file_name=""):
		...
		self.var_action_name_vs_parameter_set_id = self.xvar["han"]["action_name_vs_parameter_set_id"]
		self.var_action_name_vs_manual = self.xvar["han"]["action_name_vs_manual"]
		self.current = {}
	...
	def apply_font_style_for_selection(self, my_range):
		...
		:return:
		"""
		if self.xvar["han"]["basic_underline"]:
			my_range.Font.Underline = 1
		if self.xvar["han"]["basic_size"]:
			my_range.Font.Size = self.xvar["han"]["basic_size"]
		if self.xvar["han"]["basic_bold"]:
			my_range.Font.Bold = 1
		if self.xvar["han"]["rgb_int"]:
			my_range.Font.TextColor.RGB = self.xvar["han"]["rgb_int"]
	...
	def check_action_name_as_dic(self, input_action_name):
		...
		parameter_name = self.xvar["han"]["action_name_vs_parameter_set_id"][input_action_name]
		...
		for one in self.xvar["han"]["parameter_set_id_vs_parameters"][parameter_name]:
	...
	def check_options(self, input_parameter):
		...
		try:
			result = self.xvar["han"]["parameter_vs_parameter_option"][input_parameter]
		except:
			result = "없음"
		if not result:
			result = "없음"
	...
	def check_parameter_set_id(self, input_action):
		...
		result = "없음"
		try:
			result = self.xvar["han"]['action_name_vs_parameter_set_id'][input_action]
		except:
			pass
		return result
	...
	def check_parameters(self, input_parameter_set_id):
		...
		# all_data = basic_data_for_han.basic_data().xvar
		try:
			result = self.xvar["han"]["parameter_set_id_vs_parameters"][input_parameter_set_id]
		except:
			result = "없음"
		if not result:
			result = "없음"
	...
	def filter_all_action_name_by_input_text(self, input_text):
		...
		"""
		result = []
		for one_action_name in list(self.xvar["han"]['action_name_vs_manual'].keys()):
	...
	def free_basic_font_style(self):
		...
		:return:
		"""
		self.xvar["han"]["apply_basic_font"] = False
		self.xvar["han"]["basic_underline"] = False
		self.xvar["han"]["basic_size"] = False
		self.xvar["han"]["basic_bold"] = False
		self.xvar["han"]["rgb_int"] = False
	...
	def get_all_action_name(self):
		...
		:return:
		"""
		result = list(self.xvar["han"]['action_name_vs_manual'].keys())
		return result
	...
	def set_basic_font_style(self, **input_dic):
		...
		:return:
		"""
		self.xvar["han"]["apply_basic_font"] = True

		if "size" in input_dic.keys():
			self.xvar["han"]["basic_size"] = input_dic["size"]
		if "color" in input_dic.keys():
			self.xvar["han"]["rgb_int"] = self.xcolor.change_scolor_to_rgbint(input_dic["color"])
		if "bold" in input_dic.keys():
			self.xvar["han"]["basic_bold"] = True
		if "underline" in input_dic.keys():
			self.xvar["han"]["basic_underline"] = True
		if "italic" in input_dic.keys():
			self.xvar["han"]["basic_italic"] = True
