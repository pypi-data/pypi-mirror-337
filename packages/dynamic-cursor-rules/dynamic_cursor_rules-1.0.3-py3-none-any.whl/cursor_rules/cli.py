...
DEFAULT_CLAUDE_MODEL = "claude-3-opus-20240229"  # Latest Claude model
...
def main():
    ...
    # LLM command
    llm_parser = subparsers.add_parser("llm", help="Work with LLM providers")
    ...
    # LLM config command
    llm_config_parser = llm_subparsers.add_parser("config", help="Configure LLM provider API keys")
    llm_config_parser.add_argument("-p", "--provider", required=True, 
                                 help="Provider name (openai, anthropic)")
    llm_config_parser.add_argument("-k", "--api-key", required=True, help="API key")
    
    # LLM test command
    llm_test_parser = llm_subparsers.add_parser("test", help="Test LLM providers")
    llm_test_parser.add_argument("-p", "--provider", default="all", 
                               help="Provider name (openai, anthropic, all)")
    llm_test_parser.add_argument("-q", "--query", default="Generate 3 rules for a Python project", 
                               help="Test query")
    ...
    if args.command == "list":
        ...
        if llm_command == "list":
            # LLM list command
            handle_llm_list()
        elif llm_command == "config":
            # LLM config command
            handle_llm_config(args.provider, args.api_key)
        elif llm_command == "test":
            # LLM test command
            handle_llm_test(args.provider, args.query)
        else:
            print(f"Unknown LLM command: {llm_command}")
