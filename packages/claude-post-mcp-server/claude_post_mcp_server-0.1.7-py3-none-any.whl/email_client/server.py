...
async def search_emails_async(mail: imaplib.IMAP4_SSL, search_criteria: str) -> list[dict]:
    ...
    try:
        ...
        for num in messages[0].split()[:MAX_EMAILS]:  # Limit to MAX_EMAILS
            _, msg_data = await loop.run_in_executor(None, lambda: mail.fetch(str(num), '(RFC822)'))
...
async def get_email_content_async(mail: imaplib.IMAP4_SSL, email_id: str) -> dict:
    ...
    loop = asyncio.get_event_loop()
    try:
        _, msg_data = await loop.run_in_executor(None, lambda: mail.fetch(str(email_id), '(RFC822)'))
    ...
    try:
        ...
        def send_sync():
            with smtplib.SMTP_SSL(EMAIL_CONFIG["smtp_server"]) as server:
                server.connect(host=EMAIL_CONFIG["smtp_server"], port=EMAIL_CONFIG["smtp_port"])
