...
class FrameRouter:
    ...
    def __init__(self, *, station: "Station"):
        ...
        self.known_stations: dict[Address, set[Interface[Any]]] = {}
    ...
    def interface_for(self, address: Address) -> "Interface":
        """
        Given an address, return the best interface to use for contacting that address.
        """
        backup_candidates = []
        for interface in self.station.interfaces.values():
            if interface.is_preferred_address(address):
                return interface
            if interface.gateway:
                backup_candidates.append(interface)
        if not backup_candidates:
            raise ConfigurationError("No valid interfaces can be used to connect.")
