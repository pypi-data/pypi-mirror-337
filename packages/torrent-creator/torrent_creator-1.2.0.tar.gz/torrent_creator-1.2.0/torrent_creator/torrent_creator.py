...
def print_help():
    logging.info("""Usage:
    - torrent-creator -p : Show current path for .torrent file output.
    - torrent-creator -a : Show current announce URL.
    - torrent-creator -t : Show trackers from an existing torrent file.
    - torrent-creator -l : List files in the torrent directory.
    - torrent-creator -o [path] : Override output directory.
    - torrent-creator -a [url] : Override announce URL.
    - torrent-creator --ps [size] : Override piece size (bytes).
    - torrent-creator --reset-env : Reconfigure the .env file.
    - torrent-creator -set [-p | -a | --ps | --platform | --magnet | --tracker-type]
    - torrent-creator <file_or_folder> : Create a torrent using .env or overrides.

Example:
    python torrent-creator --output "/path/to/output" --announce "http://tracker.example.com/announce" --ps 1048576 </path/to/video>/<Directory/Folder>
    """)
...
def main():
    ...
    if args.trackers:
        if not args.filename:
            logging.error('Please provide the path to an existing torrent file using: torrent-creator -t <file.torrent>')
            sys.exit(1)
        try:
