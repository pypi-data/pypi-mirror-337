...
class Migrator(Migrator):
    ...
    def migrate_from_0_0_1(self, globals_dict):
        ...
        def create_products_product(id, ref, name, description, category_id):
            ...
            if description is not None:
                kw.update(bv2kw('description', description))
            kw.update(topic_group_id=category_id)
            return products_Product(**kw)

        @override(globals_dict)
        def create_tickets_ticket(id, modified, created, closed, private,
                                  planned_time, project_id, site_id,
                                  product_id, nickname, summary, description,
                                  upgrade_notes, ticket_type_id,
                                  duplicate_of_id, reported_for_id,
                                  fixed_for_id, assigned_to_id, reporter_id,
                                  state, waiting_for, deadline, priority,
                                  feedback, standby, faculty_id):
            if state:
                state = settings.SITE.models.tickets.TicketStates.get_by_value(
                    state)
            kw = dict()
            kw.update(id=id)
        ...
        @override(globals_dict)
        def create_faculties_competence(id, seqno, user_id, faculty_id,
                                        affinity, product_id):
            kw = dict()
            kw.update(id=id)
        ...
        @override(globals_dict)
        def create_faculties_faculty(id, ref, parent_id, name, affinity,
                                     product_cat_id):
            kw = dict()
            kw.update(id=id)
            kw.update(ref=ref)
    ...
    def migrate_from_0_0_2(self, globals_dict):
        ...
        @override(globals_dict)
        def create_faculties_faculty(id, ref, seqno, parent_id, name, affinity,
                                     product_cat_id):
            kw = dict()
            kw.update(id=id)
            # kw.update(ref=ref)
    ...
    def migrate_from_1_0_1(self, globals_dict):
        ...
        if settings.SITE.is_installed('deploy'):
            globals_dict.update(tickets_Deployment=rt.models.deploy.Deployment)
            globals_dict.update(tickets_Milestone=rt.models.deploy.Milestone)
        else:
    ...
    def unused_migrate_from_2016_12_0(self, globals_dict):
        ...
        @override(globals_dict)
        def create_tickets_ticket(
                id, modified, created, assigned_to_id, closed, private,
                planned_time, project_id, site_id, topic_id, nickname, summary,
                description, upgrade_notes, ticket_type_id, duplicate_of_id,
                reported_for_id, fixed_for_id, reporter_id, state, waiting_for,
    ...
    def migrate_from_2016_12_1(self, globals_dict):
        ...
        @override(globals_dict)
        def create_tickets_project(id, ref, parent_id, start_date, end_date,
                                   company_id, contact_person_id,
                                   contact_role_id, private, closed,
                                   planned_time, name, assign_to_id, type_id,
                                   description, srcref_url_template,
            ...
            if contact_person_id:
                contact_person_id = str(
                    int(contact_person_id) + PARTNER_OFFSET)
            if company_id:
        ...
        @override(globals_dict)
        def create_tickets_ticket(id, modified, created, user_id, private,
                                  closed, planned_time, project_id, site_id,
                                  topic_id, nickname, summary, description,
                                  upgrade_notes, ticket_type_id,
                                  duplicate_of_id, reported_for_id,
                                  fixed_for_id, reporter_id, end_user_id,
                                  state, waiting_for, deadline, priority,
                                  feedback, standby, faculty_id):
            if state:
                state = settings.SITE.models.tickets.TicketStates.get_by_value(
