...
class Ai:
    ...
    def generate_chat_otvet(self, messages: list = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False, tools: list = None, private: bool = True, reasoning_effort: str = 'high') -> str:
        ...
        try:
            if messages is None:
                ...
                seed = random.randint(0, 1000)
            payload = {"messages": messages, "model": model, "jsonMode": jsonMode, "tools": tools, "private": private, "reasoning_effort": reasoning_effort, "seed": seed}
            response = requests.post(f"https://text.pollinations.ai/openai", headers={"Content-Type": "application/json"}, json=payload)
            response.raise_for_status()
            api_response = respomse.json()['choices'][0]['message']['content']
            return api_response
        except Exception as e:
            raise Exception(f"Error during API call: {e}")

    def generate_voice_otvet(self, prompt: str = None, filename: str = "ozv", voice: str = 'nova') -> Union[bool, None]:
    ...
    def transcribe_audio(self, audio_path: str = None, question: str = "Напиши что ты слышишь") -> Union[str, None]:
        '''Транскрипция аудио, возвращает текст'''
        try:
            if audio_path is None:
                raise TypeError("Забыли указать audio_path")
            else:
                ...
                if not base64_audio:
                    ...
                    payload = {"model": "openai-audio", "messages": [{"role": "user", "content": [{"type": "text", "text": question}, {"type": "input_audio", "input_audio": {"data": base64_audio, "format": audio_format}}]}]}
                    response = requests.post("https://text.pollinations.ai/openai", headers={"Content-Type": "application/json"}, json=payload)
                    response.raise_for_status()
    ...
    def generate_image(self, prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = True, model: str = 'flux', save: bool = False, filename: str = 'gen_img') -> Union[str, bool]:
