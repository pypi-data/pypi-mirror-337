...
class Maverick_mes(torch.nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.use_attn = kwargs.get("use_attn", False)
        self.teacher_forcing = kwargs.get("teacher_forcing", True)
        self.use_reps = kwargs.get("use_reps", False)
        ...
        elif self.span_representation == "concat_start_end":
            self.mention_hidden_size = self.token_hidden_size * 2
        else:
            raise ValueError()
        self.coref_hidden_size = self.token_hidden_size * 2

        self.num_cats = len(CATEGORIES) + 1  # +1 for ALL
        self.all_cats_size = self.token_hidden_size * self.num_cats

        if self.use_reps:
            self.coref_all_mlps = RepresentationLayer(
                type=self.representation_layer_type,  # fullyconnected
                input_dim=self.mention_hidden_size,
                output_dim=self.mention_hidden_size * self.num_cats,
                hidden_dim=self.mention_hidden_size*2,
            )
            self.antecedent_all_weights = nn.Parameter(
                torch.empty((self.num_cats, self.mention_hidden_size, self.mention_hidden_size))
            )
            self.antecedent_all_biases = nn.Parameter(torch.empty((self.num_cats, self.mention_hidden_size)))
            ...
            self.coref_start_all_mlps = RepresentationLayer(
                ...
                hidden_dim=self.coref_hidden_size,
            )
            ...
            if self.use_attn:
                self.coref_attn_all_mlps = RepresentationLayer(
                    type=self.representation_layer_type,  # fullyconnected
                    input_dim=self.token_hidden_size,
                    output_dim=self.all_cats_size,
                    hidden_dim=self.coref_hidden_size,
                )
                self.antecedent_a2a_all_weights = nn.Parameter(
                    torch.empty((self.num_cats, self.token_hidden_size, self.token_hidden_size))
                )
                self.antecedent_a2a_all_biases = nn.Parameter(torch.empty((self.num_cats, self.token_hidden_size)))
        if self.use_attn:
            self.attn_classifier = RepresentationLayer(
                type=self.representation_layer_type,
                input_dim=self.token_hidden_size,
                output_dim=1,
                hidden_dim=self.token_hidden_size,
            )
            self.attn_token_representation = RepresentationLayer(
                type=self.representation_layer_type,
                input_dim=self.token_hidden_size,
    ...
    def reset_parameters(self) -> None:
        if self.use_reps:
            W = [self.antecedent_all_weights]
            B = [self.antecedent_all_biases]
        else:
            W = [
            ...
            if self.use_attn:
                W.append(self.antecedent_a2a_all_weights)
                B.append(self.antecedent_a2a_all_biases)
    ...
    def eos_mention_extraction(self, lhs, eos_mask, gold_mentions, gold_starts, stage):
        ...
        for bidx in range(0, lhs.shape[0]):
            ...
            if self.teacher_forcing and stage == "train":
                start_idxs_batch = (
                    ((torch.sigmoid(gold_starts[bidx]) > 0.5)).nonzero(as_tuple=False).squeeze(-1)
                )  # NUM_GOLD_STARTS
            elif start_idxs_batch.shape[0] * 2 > lhs_batch.shape[0]:
                # allow at most half of the tokens to be possible start
                start_idxs_batch = start_idxs_batch[:lhs_batch.shape[0]//2]
            ...
            s2e_representations = self._get_mention_reps(lhs_batch, possible_start_idxs, possible_end_idxs)

            # classification of mentions
            s2e_logits = self.start_end_classifier(s2e_representations).squeeze(-1)

            mask = torch.sigmoid(s2e_logits) > 0.5

            # mention_start_idxs and mention_end_idxs
            mention_idxs.append(possibles_start_end_idxs[mask].detach().clone())
            ...
            if self.use_reps:
                mention_reps.append(s2e_representations[mask])

        return (start_idxs, mention_idxs, start_loss, mention_loss, mention_reps)

    def _get_mention_reps(self, lhs_batch, possible_start_idxs, possible_end_idxs):
        starts_hidden_states = lhs_batch[possible_start_idxs]  # start
        ends_hidden_states = lhs_batch[possible_end_idxs]  # end
        attn_hidden_states = self._compute_attn_emb(lhs_batch, possible_start_idxs, possible_end_idxs)
        if self.use_attn:
            s2e_representations = torch.cat(
                (
                    self.start_token_representation(starts_hidden_states),
                    self.attn_token_representation(attn_hidden_states),
        ...
        return s2e_representations

    def _compute_attn_emb(self, token_emb, candidate_starts, candidate_ends):
        if self.use_attn is False:
            return None
        num_tokens = token_emb.shape[0]
        candidate_tokens=torch.arange(0, num_tokens, device=token_emb.device).unsqueeze(0).expand(candidate_starts.shape[0], -1)
        ...
        token_attn=torch.squeeze(self.attn_classifier(token_emb), 1)
    ...
    def mes_span_clustering(
        self, mention_start_reps, mentions_attn_reps, mention_end_reps, mention_reps, mention_start_idxs, mention_end_idxs, gold, stage, mask, add, sing
    ):
        if mention_start_idxs.shape[0] == 0:
            return torch.tensor([0.0], requires_grad=True, device=self.encoder.device), []
        coref_logits = (
            self._calc_coref_logits_from_mentions(mention_reps)
            if self.use_reps
            else self._calc_coref_logits(mention_start_reps, mentions_attn_reps, mention_end_reps)
        )
    ...
    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_cats, self.mention_hidden_size if self.use_reps else self.token_hidden_size)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3)  # bnkf/bnlg

    def _calc_coref_logits(self, top_k_start_coref_reps, top_k_attn_coref_reps, top_k_end_coref_reps):
        ...
        if self.use_attn:
            all_attns = self.transpose_for_scores(self.coref_attn_all_mlps(top_k_attn_coref_reps.unsqueeze(0)))
            logits = logits + (
                torch.einsum("bnkf, nfg, bnlg -> bnkl", all_attns, self.antecedent_a2a_all_weights, all_attns) 
            )
            biases = biases + (
                torch.einsum("bnkf, nf -> bnk", all_attns, self.antecedent_a2a_all_biases).unsqueeze(-2)
            )

        return logits + biases
    
    def _calc_coref_logits_from_mentions(self, mention_reps):
        all_reps = self.transpose_for_scores(self.coref_all_mlps(mention_reps))

        logits = torch.einsum("bnkf, nfg, bnlg -> bnkl", all_reps, self.antecedent_all_weights, all_reps)

        biases = torch.einsum("bnkf, nf -> bnk", all_reps, self.antecedent_all_biases).unsqueeze(-2)
    ...
    def forward(
        ...
        if longdoc == None:
            ...
            if self.teacher_forcing and stage == "train":
                ...
                if mention_idxs.shape[0] * 2 > lhs.shape[1]:
                    # limit number of mentions to at most half of the input tokens
                    mention_idxs = mention_idxs[:lhs.shape[1] // 2]
            ...
            if self.use_reps:
                if self.teacher_forcing and stage == "train":
                    mention_reps = self._get_mention_reps(lhs[0], mention_start_idxs, mention_end_idxs)
                elif mention_idxs.shape[0] * 2 > lhs.shape[1]:
                    mention_reps = mention_reps[0][:lhs.shape[1] // 2, :]
                else:
                    mention_reps = mention_reps[0]
                mention_reps = mention_reps.unsqueeze(0)
                mentions_start_hidden_states = None
                mentions_end_hidden_states = None
                mentions_attn_hidden_states = None
            else:
                mention_reps = None
                ...
                mentions_attn_hidden_states = self._compute_attn_emb(lhs[0], mention_start_idxs, mention_end_idxs)
