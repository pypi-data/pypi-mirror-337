...
class SimpleDao(BaseDao):
    ...
    def delete(self, where: Where) -> int:
        ...
        if where.has_fields():
            delete.set_fields_of_where(where.__fields)
    ...
    def update(self, param_bean: T, where: Where):
        ...
        if where.has_fields():
            update.set_fields_of_where(where.__fields)
    ...
    def query(self, bean_class: type, where: Where) -> T:
        ...
        if where.has_fields():
            query.set_fields_of_where(where.__fields)
    ...
    def list(self, bean_class: type, where: Where) -> DbList[T]:
        ...
        if where.__is_page_query:
            return query.execute_beans_by_page(bean_class, self._table_info.get_column_names(True), self._table_info.column_name_map, where.__page)
        else:
    ...
    def list_maps(self, where: Where) -> DbList[Dict[str, Any]]:
        ...
        if where.__is_page_query:
            return query.execute_maps_by_page(self._table_info.get_column_names(True), where.__page)
        else:
