...
class CTA(StrEnum):
    AGENT_FORCED_SEND_NOW = "agent_forced_send_now" # Agent force sends the message to the user
    RELEASE_HOLD = "release_hold" # Release the hold on the message
...
class ScheduledMessages(ChattyAssetModel):
    ...
    CTAs: Optional[List[CTA]] = Field(default_factory=list)
    ...
    def set_status(self, status: ScheduledMessageStatus):
        if status == ScheduledMessageStatus.CANCELLED or status == ScheduledMessageStatus.SENT:
            self.active = False
            self.CTAs = []
        elif status == ScheduledMessageStatus.EXPIRED_ON_HOLD:
            self.CTAs = [CTA.AGENT_FORCED_SEND_NOW]
        elif status == ScheduledMessageStatus.ON_HOLD:
            self.CTAs = [CTA.RELEASE_HOLD]
