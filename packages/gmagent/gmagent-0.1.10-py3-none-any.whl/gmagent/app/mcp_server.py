...
from .api.core import gm_api_core
...
async def run_strategy(code: str) -> Dict[str, Any]:
    """Run a trading strategy by calling FastAPI endpoint.
    ...
    # workaround to bypass MCP's bug. foward calling to fastapi
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"http://localhost:8000{settings.API_V1_STR}/strategy/run",
                json={"code": code},
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPError as e:
            raise Exception(f"Failed to execute strategy: {str(e)}")
        except Exception as e:
            raise Exception(f"Unexpected error during strategy execution: {str(e)}")
