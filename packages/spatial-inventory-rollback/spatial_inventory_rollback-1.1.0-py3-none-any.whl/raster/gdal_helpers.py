...
@contextmanager
def __open_band(band_num: int, *args):
...
def get_raster_dimension(path: str) -> RasterBound:
...
def get_raster_no_data(path: str, band_num: int = 1) -> Union[int, float]:
...
def write_output(
    path: str, data: np.ndarray, x_off: int, y_off: int, band_num: int = 1
):
...
def read_dataset(
    path: str, bounds: RasterBound = None, raster_band: int = 1
) -> GDALHelperDataset:
    ...
    Returns:
        GDALHelperDataset: an object with fields:
...
def get_default_geotiff_creation_options() -> list[str]:
