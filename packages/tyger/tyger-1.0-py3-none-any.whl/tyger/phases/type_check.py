...
from tyger.discipline.base.types import unk, TypeName  # TODO: remove this dependency
from tyger.discipline.types import TypeSystem, TypeException, Context
...
class TypingPhase(Phase):
        ...
        for node in nodes:
            match node:
                ...
                case ast.ClassDef(name, bases, keywords, body, decorator_list):
                    ...
                    for b in body:
                        match b:
                            case ast.FunctionDef(name, args, body, returns) if name.startswith("__"):
    ...
    def type_check_expr_to_type(self, node: ast.expr, env: Environment, target_type: type, context) -> type:
        typ = self.type_check_expr(node, env, context)
        if not self.type_system.consistent(typ, target_type):
            raise TypeException(f"Typre mismatch: {typ} </: {target_type}")
        return typ

    def get_expected_type(self, node: ast.expr, env: Environment, context: Context) -> tuple | type | None:
        ...
        match node:
                        ...
                        if value_id in env:
                            ...
                            match value_type.__qualname__:
                                case list.__qualname__:
                                    ...
                                    self.type_system.check_slice_type(value_type, slice_typ)
                                case dict.__qualname__:
                                    dom, cod = get_args(value_type)
                                    self.check_consistency(dom, slice_typ)
                                    exp_type = cod
                        node.__target_type__ = exp_type
                    case _:
            ...
            case ast.List(elts) | ast.Tuple(elts):
                exp_type = tuple(self.get_expected_type(el, env, context) for el in elts)
            case ast.Starred(value):
                raise Exception("Unsupported expression Starred")
        return exp_type

    def type_check_expr(self, node: ast.expr, env: Environment = None, context: Context = Context.initial()) \
            -> type | dict[str, Any]:
        env = env or {}
        node.__tags__ = {}
        match node:
            ...
            case ast.Lambda(args, body):
                ...
                self.type_check_expr_to_type(body, {**env, **local}, cod, context.taint_context(node, cod))
            ...
            case ast.List(elts, ctx):
                inner_ty = functools.reduce(self.type_system.gjoin,
                                            (self.type_check_expr(elt, env, context.no_expected()) for elt in elts))
                t = list[inner_ty]
                ...
                for g in generators:
                    match g:
                        case ast.comprehension(target, _iter, ifs, is_async):
                            ...
                            for i in ifs:
                                self.type_check_expr(i, {**env, **menv}, context.no_expected())
                        case _:
            ...
            case ast.Dict(keys, values):
                key_type = functools.reduce(self.type_system.gjoin,
                                            (self.type_check_expr(key, env, context) for key in keys))
                value_type = functools.reduce(self.type_system.gjoin,
    ...
    def type_check_stmt(self, node: ast.stmt, linear: Environment = None,
                        context: Context = None, package_name: str = "") -> Environment:
        ...
        match node:
                    ...
                    if arg.annotation:
                        ...
                        match arg.annotation:
                            case ast.Attribute(value, attr):
                                ...
                                dom.append(self.type_system.parse_type(ast.Name(attr, ast.Load())))
                            case _:
                    ...
                    match returns:
                        case ast.Attribute(value, attr):
                            cod = self.type_system.get_cod_type_from_annotation(
                                self.get_type_from_annotation(ast.Name(attr, ast.Load())))
                        case _:
                            cod = self.type_system.get_cod_type_from_annotation(
            ...
            case ast.Assign(targets, value):
                ...
                for t in targets:
                    ...
                    self.check_consistency(target_type, vty)
            ...
            case ast.AnnAssign(target, annotation, value, simple):
                match target:
                    case ast.Name(idx):
                        if idx in linear:
                            raise TypeException(f"Cannot redefine variable {idx}")
                        ...
                        target.__target_type__ = typ
                        target.__prev_val__ = ast.Name(idx, ast.Load()) if idx in linear else ast.Constant(None)
                        if value:
                            self.type_check_expr_to_type(value, linear, typ, context.taint_context(node, typ))
                        # TODO: check this redef
                        linear[idx] = typ

            case ast.Return(value):
                vty = self.type_check_expr(value, linear, context)
                self.check_consistency(context.expected, vty)
                self.check_consistency(context.expected, context.taint_type(node, vty))
            ...
            case ast.For(target, iter, body, or_else):
                ...
                contextp = context.taint_context(context.taint_context(node, ty),
                                                 inner_ty)  #should it be tainted by ty or inner_ty?, better to taint both
                l = self.type_check_stmts(body, linear, contextp)
                l = self.type_check_stmts(or_else, l, contextp)
                ret_env = l
            case ast.AugAssign(target, op, value):
                ...
                op_type = self.type_system.type_of_bin_primitive(op, target_type, value_type)
                ...
                for name in name:
                    ...
                    if name.name in module_env:
                        ...
                        try:
                            ...
                            raise AttributeError(f"Module {module_name} does not have attribute {name.name}")
            case ast.Import(name):
                for name in name:
                    # Update namespaces
                    module_name: str = name.name
                    if module_name.startswith("tyger.discipline") and module_name.endswith("types"):
                        ...
                        if imported_discipline == running_discipline:
                            ...
                            linear[name.asname or module_name] = self.type_system.get_types()
    ...
    def get_module_env(self, module_name: str) -> Environment:
        ...
        if module_name not in self.namespaces:
            if module_name.startswith("tyger"):
                ...
                module_env: Environment = self.type_check_stmts(module.body, package_name=module_name)
    ...
    def type_check_stmts(self, stmts: list[ast.stmt],
                         linear: Environment = None,
                         context: Context = None, package_name="") -> Environment:
        l = linear or {}
        context = context or self.type_system.get_initial_context()
        """
        ...
        for func in func_defs:
            ...
            self.type_check_stmts(body, func_body_env, func_context.taint_context(func, cod), package_name)
    ...
    def function_signature(self, node: ast.FunctionDef, linear: Environment, context: Context) -> tuple[
        type, Environment]:
        env: Environment = {}
        tys: list[type] = []
        default_type = self.type_system.get_default_type()
        for a in node.args.args:
            ...
            env[a.arg] = ty
            tys.append(ty)
        cod = self.type_system.get_cod_type_from_annotation(self.get_type_from_annotation(node.returns)
                                                            if node.returns else default_type)
    ...
    def check_consistency(self, expected_type: type | tuple | None, actual_type):
        if expected_type is None:
            return
        match expected_type:
            case tuple():
                # In this case, we need to check each element consistency
                a_types = self.type_system.unwrap_value_type(actual_type, len(expected_type))

                if len(a_types) != len(expected_type):
                    raise Exception("Arity mismatch")
                for exp, act in zip(expected_type, a_types):
                    self.check_consistency(exp, act)
            case _:
                if not self.type_system.consistent(actual_type, expected_type):
                    raise TypeException(f"Consistency error: {actual_type} </: {expected_type}")
