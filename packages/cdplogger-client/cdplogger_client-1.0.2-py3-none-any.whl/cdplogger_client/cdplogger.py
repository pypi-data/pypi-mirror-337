    ...
    def _create_value(self, signal_names, min_values, max_values, last_values):
        ...
        if len(min_values) == 0 or len(max_values) == 0:
            ...
            for i in range(len(signal_names)):
                value[signal_names[i]] = Value(self._value_from_variant(last_values[i]),
                                               self._value_from_variant(last_values[i]),
                                               self._value_from_variant(last_values[i]))
        else:
            for i in range(len(signal_names)):
                value[signal_names[i]] = Value(self._value_from_variant(min_values[i]),
                                               self._value_from_variant(max_values[i]),
                                               self._value_from_variant(last_values[i]))
        return value
