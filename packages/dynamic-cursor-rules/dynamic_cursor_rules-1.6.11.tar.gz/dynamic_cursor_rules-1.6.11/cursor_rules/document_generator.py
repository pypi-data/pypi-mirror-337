        ...
        if use_llm:
            try:
                ...
                if not rules_content:
                    raise ValueError("LLM returned an empty response for cursor rules.")
                ...
                if json_match:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse rules JSON: {e}. Trying manual extraction.")
                
                # If JSON parsing fails or didn't happen, try manual extraction
                if not generated_files:
                    logger.info("Attempting manual rule extraction from LLM response.")
                ...
                # If we reach here, neither JSON nor manual extraction worked
                raise ValueError("LLM response received but failed to extract valid rule data.")

            except Exception as e:
                # Log the error and re-raise
                logger.error(f"Failed to generate cursor rules with LLM: {e}")
                raise RuntimeError(f"LLM failed to generate cursor rules: {e}") from e

        # If no LLM is available, raise an error as we removed the fallback
        else:
             logger.warning("LLM provider not available, and fallback generation is disabled for cursor rules.")
             raise RuntimeError("LLM provider not available, fallback disabled for cursor rules.")
