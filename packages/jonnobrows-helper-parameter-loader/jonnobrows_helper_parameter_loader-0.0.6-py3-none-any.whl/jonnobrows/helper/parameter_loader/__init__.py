...
class ParameterLoader:
    _required: list[str]
    _required_file: str
    _required_from_file: list[str]
    _allow_other: bool = False
    _all_required: list[str]

    def __init__(self, required=None, required_file=None, allow_other=False) -> None:
        self._required = required or []
        self._required_file = required_file or "REQUIRED_PARAMS"
        self._required_from_file = []
        self._allow_other = allow_other
        self._all_required = []

    @property
    def _required_parameters(self) -> list[str]:
        if self._required_file and not self._required_from_file:
            logger.info(
                "Trying to load required environment variables from file: %s",
                self._required_file,
            )
            try:
                if os.path.exists(self._required_file):
                    with open(self._required_file, "r") as f:
                        self._required_from_file = f.read().splitlines()
            except Exception as exc:
                logger.warning(
                    "Exception when trying to load required parameters file: %s",
                    exc_info=exc,
                )
        if not self._all_required:
            self._all_required.extend(self._required)
            self._all_required.extend(self._required_from_file)
            self._all_required = sorted(set([r for r in self._all_required if r]))
            logger.info(
                f"Resolved required environment variables: {', '.join(self._all_required)}"
            )
        return self._all_required

    @property
    def _allow_list(self) -> list[str]:
        return self._required_parameters if self._allow_other else []
