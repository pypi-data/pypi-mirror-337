...
class OneBotAdapter(IMAdapter, UserProfileAdapter, BotProfileAdapter):
    ...
    def __init__(self, config: OneBotConfig):
        ...
        self.hypercorn_config._log.access_logger = HypercornLoggerWrapper(
            self.logger)
        self.hypercorn_config._log.error_logger = HypercornLoggerWrapper(
            self.logger)
    ...
    async def _inject_websocket_service(self):
        ...
        self.web_server.app.mount(register_base_url, app)
        self.logger.info(f"OneBot adapter started")
        ...
        try:
            ...
            if self.config.host and self.config.port:
                ...
                for route in self.web_server.app.routes:
                    if self.config.websocket_url in route.path:
