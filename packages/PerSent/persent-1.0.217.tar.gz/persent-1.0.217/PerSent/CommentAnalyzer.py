...
if __name__ == "__main__":
    ...
    print("Persian Weighted Sentiment Analyzer")
    print("Available commands:")
    print("1. Load weighted lexicon from CSV")
    print("2. Train model from CSV")
    print("3. Load saved model")
    print("4. Analyze text")
    print("5. Analyze CSV file")
    print("6. Exit")
    ...
    while True:
        ...
        if choice == '1':
            csv_file = input("Path to weighted lexicon CSV: ").strip()
            try:
                analyzer.load_weighted_lexicon(csv_file)
                print("Weighted lexicon loaded successfully")
            except Exception as e:
                print(f"Error: {str(e)}")
                
        elif choice == '2':
            csv_file = input("Path to training CSV: ").strip()
            text_col = input("Text column [text]: ").strip() or 'text'
            emotion_col = input("Emotion column [emotion]: ").strip() or 'emotion'
            weight_col = input("Weight column [weight]: ").strip() or 'weight'
            
            try:
                result = analyzer.train_from_csv(csv_file, text_col, emotion_col, weight_col)
                print(result['message'])
            except Exception as e:
                print(f"Error: {str(e)}")
                
        elif choice == '3':
            model_name = input("Model name [weighted_sentiment_model]: ").strip() or 'weighted_sentiment_model'
            try:
            ...
            try:
                ...
                print("\nSentiment Analysis Results (Weighted):")
                for emotion, percent in sorted(results.items(), key=lambda x: x[1], reverse=True):
            ...
            text_col = input("Text column [text]: ").strip() or 'text'
            output_col = input("Output column [sentiment_analysis]: ").strip() or 'sentiment_analysis'
            
            if analyzer.analyze_csv(input_file, output_file, text_col, output_col):
