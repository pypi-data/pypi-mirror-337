...
def test_run_with_lock(tmp_path):
    original_file = tmp_path / 'cur' / '1685154999.M900454P14950.fully,S=13592,W=13909:2,FSa'
    compressed_file = tmp_path / 'cur' / '1685154999.M900454P14950.fully,S=13592,W=13909:2,FSaZ'
    original_file.parent.mkdir(parents=True)
    Path.mkdir(tmp_path / 'tmp', parents=True, exist_ok=True)
    original_file.write_text('Test file')
    compressor = MailCompressor(
        maildir=str(tmp_path / 'cur'),
