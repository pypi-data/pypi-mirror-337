...
def get_executable_path():
    ...
    2. PyInstaller's _MEIPASS directory (if running as a bundled executable)
    3. Package directory's bin folder
    ...
    # 2. Check PyInstaller temp folder (_MEIPASS)
    if getattr(sys, 'frozen', False):  # This means the script is running from a PyInstaller bundle
        pyinstaller_bin = Path(sys._MEIPASS) / 'pyvfp/bin/vfp_to_json.exe'
        if pyinstaller_bin.exists() and pyinstaller_bin.is_file():
            return str(pyinstaller_bin)

    # 3. Check package's bin folder (relative to this file)
