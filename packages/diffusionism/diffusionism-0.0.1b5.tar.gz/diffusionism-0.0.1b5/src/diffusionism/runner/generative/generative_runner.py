...
class GenerativeRunner(ModelRunner):
    ...
    def __evaluation_step(self, batch, batch_idx, log_prefix = None, return_images: bool = False) -> Mapping[str, torch.Tensor]:
        ...
        if len(collection_dict) > 0:
            ...
            self.log_dict({key : value.mean() for key, value in collection_dict.items()}, prog_bar=True, logger=True, on_step=False, on_epoch=True, sync_dist=True)
        if return_images:
    ...
    def end_for_validation_epoch(self, outputs: dict[str, torch.Tensor]):
        log = {f'Val-{key}_epoch' : value.mean() for key, value in outputs.items()}
        if len(self.__validation_generation_features) > 0 and len(self.__validation_target_features) > 0:
            ...
            log.update({f'Val-{type(metric).__name__}_epoch' : metric(generation_features.detach_(), target_features.detach_()).mean() for metric in self.feature_metrics})
    ...
    def end_for_test_epoch(self, outputs: dict[str, torch.Tensor]):
        log = {f'Test-{key}_epoch' : value.mean() for key, value in outputs.items()}
        if len(self.__test_generation_features) > 0 and len(self.__test_target_features) > 0:
            ...
            log.update({f'Test-{type(metric).__name__}_epoch' : metric(generation_features.detach_(), target_features.detach_()).mean() for metric in self.feature_metrics})
        if len(self.global_metrics) > 0:
