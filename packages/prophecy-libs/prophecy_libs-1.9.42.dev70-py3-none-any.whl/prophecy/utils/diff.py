...
class DataFrameDiff:
    ...
    def add_row_matches_column(cls, joined_df):
        ...
        # comparision: comapres row using left_struct = right_struct
        # This correctly handled nulls
    ...
    def add_column_comparison_results(cls, joined_df):
        ...
        Nulls are only considered equal when the row exists in both dataframes (presence_in_left=1 AND presence_in_right=1).
        If a row only exists in one dataframe, all column comparisons will result in "not equal".
        """
        left_struct = "left_values"
        right_struct = "right_values"
        ...
        # Only consider values equal (including null = null) when both rows exist
        comparison_expressions = [
            (
                F.when(
                    both_rows_exist,
                    F.expr(f"{left_struct}.`{field}` <=> {right_struct}.`{field}`"),
