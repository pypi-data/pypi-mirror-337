...
class ComparisonService(TestingToolParams):
    ...
    """
    def __init__(self, src_object: DatabaseObject, trgt_object: DatabaseObject, db_service_src, db_service_trgt, global_filter: list, exclude_columns: list, comp_id: int):
        super().__init__()
    ...
    def row_count_comparison(self):
        ...
        self.result_params.src_row_count, self.result_params.error_list_rows_src = self.db_service_src.get_row_count_from_object(self.src_object)
        self.result_params.trgt_row_count, self.result_params.error_list_rows_trgt = self.db_service_trgt.get_row_count_from_object(self.trgt_object)
    ...
    def aggregation_comparison(self):
        ...
        src_columns_aggregate = self.db_service_src.create_checksums(self.src_object, self.result_params.src_columns, self.global_filter, self.exclude_columns, self.numeric_scale)

        trgt_column_datatypes = self.db_service_trgt.get_data_types_from_object(self.trgt_object, self.result_params.trgt_columns)
        trgt_columns_aggregate = self.db_service_trgt.create_checksums(self.trgt_object, self.result_params.trgt_columns, self.global_filter, self.exclude_columns, self.numeric_scale)
        ...
        if self.result_params.src_row_count != 0 and self.result_params.trgt_row_count != 0:
            ...
            aggregation_differences_trgt_minus_src = {
                                                        ...
                                                        for k in src_columns_aggregate.keys() 
                                                            if k in trgt_columns_aggregate 
                                                            and str(src_columns_aggregate[k][1]) != str(trgt_columns_aggregate[k][1]) 
                                                            and not (src_columns_aggregate[k][0].upper() == 'AGGREGATEBOOLEAN' or trgt_columns_aggregate[k][0].upper() == 'AGGREGATEBOOLEAN')
                                                    }
            aggregation_differences_trgt_minus_src_boolean = {
                                                    k:  str(
                                                            - int(src_columns_aggregate[k][1].split('_',1)[0])
            ...
            aggregation_differences_trgt_minus_src = {
                                                        k: trgt_columns_aggregate[k][1] for k in src_columns_aggregate.keys() 
                                                        if k in trgt_columns_aggregate 
                                                        and str(src_columns_aggregate[k][1]) != str(trgt_columns_aggregate[k][1])
                                                    }
        else:
    ...
    def group_by_comparison(self):
        ...
        # group-by option 2 - group_by_columns defined as one list for all tables
        elif self.use_group_by_columns and src_column_count_distincts and trgt_column_count_distincts and self.migration_config["GROUP_BY_AGGREGATION"]["GROUP_BY_COLUMNS"]:
            ...
            if global_group_by_columns_in_object:
                object_group_by_column=self._get_group_by_column_by_validation(global_group_by_columns_in_object, src_column_count_distincts, trgt_column_count_distincts)
        ...
        elif (not self.use_group_by_columns or not object_group_by_columns) and src_column_count_distincts and trgt_column_count_distincts:
    ...
    def pandas_dataframe_comparison(self):
        ...
        if (
            ...
            src_pdf = self.db_service_src.create_pandas_df(self.src_object, self.result_params.intersection_columns_trgt_src, self.global_filter, self.exclude_columns)
            trgt_pdf = self.db_service_trgt.create_pandas_df(self.trgt_object, self.result_params.intersection_columns_trgt_src, self.global_filter, self.exclude_columns)
        ...
        if "SAMPLE_KEYS" in self.migration_config.keys():
            ...
            if f"{self.src_object.database}.{self.src_object.schema}.{self.src_object.name}" in  sample_comparison_config.keys():
                ...
                trgt_sample_pdf, trgt_key_filters, trgt_used_columns, trgt_sample_query = self.db_service_trgt.create_pandas_df_from_sample(
                                                                                                                                ...
                                                                                                                                where_clause=self.global_filter, 
