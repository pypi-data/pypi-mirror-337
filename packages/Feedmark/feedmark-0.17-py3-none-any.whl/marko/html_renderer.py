...
class HTMLRenderer(Renderer):
    ...
    def render_asterisk_emphasis(self, element: inline.AsteriskEmphasis) -> str:
        return self.render_emphasis(element)

    def render_underscore_emphasis(self, element: inline.UnderscoreEmphasis) -> str:
        return self.render_emphasis(element)
    ...
    def render_link_ref(self, element: inline.Link) -> str:
        template = '<a href="{}"{}>{}</a>'
        body = self.render_children(element)

        link_ref_defs = self.root_node.link_ref_defs
        result = link_ref_defs.get(element.key) or ("", None)
        url = result[0]
        url = Literal.strip_backslash(url) if url else url
        if url and url[0] == "<" and url[-1] == ">":
            url = url[1:-1]
        url = self.escape_url(url)
        title = result[1][1:-1] if result[1] else None  # drop outer quotes
        title = Literal.strip_backslash(title) if title else title

        title = f' title="{self.escape_html(title)}"' if title else ""
        return template.format(url, title, body)
    ...
    def render_image_ref(self, element: inline.Image) -> str:
        template = '<img src="{}" alt="{}"{} />'
