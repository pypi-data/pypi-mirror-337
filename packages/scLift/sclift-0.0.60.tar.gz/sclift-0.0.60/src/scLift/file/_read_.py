...
def barcodes_add_anno(annotation_file: path, cell_anno: DataFrame, clusters: str = None) -> DataFrame:
    ...
    if clusters is not None:
        ...
        if cell_anno[cell_anno[clusters].isna()].shape[0] > 0:
            ul.log(__name__).warn(f"Due to the presence of `NA` in the `{clusters}`, it is forcibly assigned as `unknown`.")
...
def read_barcodes_file(
    ...
    if barcode_split_character is None or content_split_size == 1:
        ...
        if len(__cell_annot__.columns) == 1:
            ul.log(__name__).warn("Parameter `barcode_split_character` is `-`, but it is not working, ignore.")
            __is_with__ = False
...
def read_variants(
    ...
    for variant_file in tqdm(new_files):
        ...
        if variant.shape[0] == 0:
            ul.log(__name__).warn(f"The content of file {variant_file} is empty")
            continue
        ...
        if key not in key_list:
            ...
            ul.log(__name__).warn(f"The `{old_key}` already exists in the previous file, set this key to `{key}`")
            variant["id"] = key
