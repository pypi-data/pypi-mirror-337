...
class STTModel:
    ...
    async def check_connectivity(self, test_wav: str=None) -> bool:
        """
        测试与远程STT服务的连接是否正常
        
        Args:
            test_wav: 测试音频文件路径，如果为None则使用默认测试文件
        ...
        try:
            if not test_wav:
                test_wav = r'C:\Project\AgentEngine\example_docs\test.wav'
            
            result = await self.process_audio_file(test_wav)
            # 检查返回结果是否为字典类型且非空
