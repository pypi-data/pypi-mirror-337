...
def generate_mask_file(action, fname, action_labels, mask_dir):
...
class UCF101(Dataset):
    ...
    def __generate_person_masks__(self):
        ...
        processes = []
        times = []
        for action, fname in prog:
            start = time.time()
            if self.max_workers == 0:
                generate_mask_file(action, fname, self.action_labels, self.mask_dir)
            else:
                process = multiprocessing.Process(target=generate_mask_file, args=(action, fname, self.action_labels, self.mask_dir))
