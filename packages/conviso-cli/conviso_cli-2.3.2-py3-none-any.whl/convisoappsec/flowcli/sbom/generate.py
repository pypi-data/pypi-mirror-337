...
def generate(context, flow_context, project_code, asset_id, company_id, repository_dir, send_to_flow, custom_sca_tags,
    ...
    try:
        ...
        if command_exists('syft'):
            command = [f"syft scan {repository_dir} -o cyclonedx-json={file_name} --select-catalogers '-github-actions,-python-installed-package-cataloger'"]
        else:
            subprocess.run(
            ...
            command = [f"./conviso/syft scan {repository_dir} -o cyclonedx-json={file_name} --select-catalogers '-github-actions,-python-installed-package-cataloger'"]
