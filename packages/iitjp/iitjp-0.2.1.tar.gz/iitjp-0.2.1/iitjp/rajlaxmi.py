...
# Notify user about ffmpeg
if not check_ffmpeg():
    warnings.warn(
        "FFmpeg not found. Audio playback may not work correctly.\n"
...
def play_audio(filename):
    ...
    try:
        # Method 1: Cross-platform pydub playback with custom temp files
        try:
            if not check_ffmpeg():
                print("[⚠️ FFmpeg not available. Audio playback may not work properly.]")
            ...
            if ensure_temp_dir():
                ...
                if PLATFORM == 'windows' or sys.platform == 'win32':
                    ...
                    try:
                        # Try using winsound first (built into Python)
                        import winsound
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"[⚠️ open command failed: {e2}]")
                
                else:
                    # Linux and other platforms
                    players = ["aplay", "paplay", "ffplay -nodisp -autoexit", "mplayer"]
                ...
                print("[ℹ️ Using direct pydub playback]")
                play(audio)
