...
class SQClient:
    ...
    def generate_message_header(self, data_length: int, is_text_msg: bool = True, protocol_version: int = PROTOCOL_VERSION):
        """
    ...
    def send_data(self, capacity: int, packed_cols: [], byte_count: int):
        ...
        for packed_col in packed_cols:
            self.logger.debug(f"Packed data sent: {packed_col}", connection_id=self.connection_id, statement_id=self.statement_id)
