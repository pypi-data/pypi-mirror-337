...
2025/03/27 1.3.1 Installer_sync バグ修正
```
...
class DebugHelper:
    ...
    def installer_sync(self, command, shell=False, back=0):
        self.log_step(f"command: {command}", success=None, back=0)
        ...
        def log_stdout(pipe):
            for line in iter(pipe.readline, ''):
                self.log_step(line.rstrip(), success=None, back=back + 1 )
            pipe.close()

        # stderrも同様にログ出力する（必要に応じて）
        def log_stderr(pipe):
            for line in iter(pipe.readline, ''):
                self.log_step(f"ERR: {line.rstrip()}", success=None, back=back + 1)
            pipe.close()
