    ...
    def S_submit(self):
        ...
        if self.UI.Cluster_Combobox.get() == "hust":
            ...
            if self.para["Version"]=="22.1":
                ...
                messagebox.showerror("提交错误", "CPT集群暂仅支持2023R1版本")
        else:
            return
...
echo fluent $FLUENT_SOLVER_VER -g -t\$SLURM_NTASKS -i  $JOU_FILE -mpi=openmpi -cnf=\${FL_SCHEDULER_HOST_FILE}
fluent $FLUENT_SOLVER_VER -g -t\$SLURM_NTASKS -i  $JOU_FILE -mpi=openmpi -cnf=\${FL_SCHEDULER_HOST_FILE}
