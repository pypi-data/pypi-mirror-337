...
# --- set this to True to correct the time stamp shift of monthly output due to mvstream
#     (only for monthly timeseries, i.e. if ave_freq=0 !)
...
path_of_pyicon = os.path.dirname(pyic.__file__)
git_revision_number = subprocess.check_output(f'git -C {path_of_pyicon} rev-parse --verify HEAD', shell=True).decode("utf-8")[:-1]
git_branch = subprocess.check_output(f'git -C {path_of_pyicon} rev-parse --abbrev-ref HEAD', shell=True).decode("utf-8")[:-1]
git_files_modified = subprocess.check_output(f'git -C {path_of_pyicon} diff --name-only', shell=True).decode("utf-8")
python_version = subprocess.check_output('which python', shell=True).decode("utf-8")[:-1]
git_files_modified = git_files_modified.replace('\n', '\n  ')

print('--------------------------------------------------------------------------------')
print('pyicon Quickplots')
print('--------------------------------------------------------------------------------')
print('')
if iopts.batch or iopts.slurm:
  print('--- Using batch mode ---')
print(f'Current directory: {os.getcwd()}')
print(f'Path of pyicon: {path_of_pyicon}')
print(f'Branch of pyicon in use: {git_branch}')
print(f'Git revision number of pyicon: {git_revision_number}')
print(f'The following files are modified from last commit:\n  {git_files_modified}')
...
do_plot_atm_ref_ts  = {do_plot_atm_ref_ts}
...
if do_ocean_plots:
  ...
  fig_names += ['ssh', 'ssh_variance', 'sst', 'sss', 'mlotst_mar', 'mlotst_sep', 'mld_mar', 'mld_sep'] 
  fig_names += ['sec:Ice']
...
if do_atmosphere_plots:
  ...
  fig_names += ['atm_sfc_sob', 'atm_sfc_sob_bias', 'atm_sfc_thb', 'atm_sfc_thb_bias']
...
for tave_int in tave_ints:
  ...
  if not iopts.no_plots:
    ...
    #if mask_int.sum()==0 or mask_int_atm.sum()==0: # this line break ocean-only plots; solution needs to be found
    if mask_int.sum()==0:
    ...
    if fig_name in fig_names:
      try:
        ...
        try:
          FigInf = pyicqp.qp_hplot(fpath=path_data+fname, var='ssh', depth=0, it=0,
                                   t1=t1, t2=t2,
                                   clim=2, cincr=0.2, cmap='RdBu_r',
                                   IcD=DIcD['monthly'],
                                   save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
    ...
    if fig_name in fig_names:
      try:
        ...
        try:
          zos, it_ave          = pyic.time_average(DIcD['monthly'], 'ssh',        t1=t1, t2=t2)
          zos_square, it_ave   = pyic.time_average(DIcD['monthly'], 'ssh', t1=t1, t2=t2)
          zos_var = np.sqrt(zos_square-zos**2)
          IaV = pyic.IconVariable('zos_var', 'm', 'ssh variance')
          IaV.data = zos_var
          IaV.interp_to_rectgrid(fpath_ckdtree)
    ...
    if do_ocean_plots:
      ...
      if np.any(np.isin(fig_names, tmp_list)) or calc_bias:
    ...
    if do_ocean_plots:
      ...
      if np.any(np.isin(fig_names, tmp_plist)):
        try:
...
#      mass_flux_vint = mass_flux.sum(axis=0)
    ...
    if fig_name in fig_names:
      FigInf, Dhandles = pyicqp.qp_timeseries(IcD_atm_mon, fname_atm_mon, ['tas_gmean'],
        t1=t1, t2=t2, ave_freq=ave_freq, shift_timeseries = shift_timeseries, omit_last_file=omit_last_file,
        var_add=-273.15, units='$^o$C', use_tave_int_for_ts=use_tave_int_for_ts,
        fpath_ref_data_atm=fpath_ref_data_atm, do_plot_atm_ref_ts=do_plot_atm_ref_ts, do_djf=do_djf, do_jja=do_jja,
        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
      )
      save_fig(fig_name, path_pics, fig_name)
    ...
    if fig_name in fig_names:
      FigInf, Dhandles = pyicqp.qp_timeseries(IcD_atm_mon, fname_atm_mon, ['radtop_gmean'], 
        t1=t1, t2=t2, ave_freq=ave_freq, shift_timeseries = shift_timeseries, omit_last_file=omit_last_file, use_tave_int_for_ts=use_tave_int_for_ts,
        fpath_ref_data_atm=fpath_ref_data_atm_rad, do_plot_atm_ref_ts=do_plot_atm_ref_ts, do_djf=do_djf, do_jja=do_jja,
        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
      )
      save_fig(fig_name, path_pics, fig_name)
    ...
    if fig_name in fig_names:
      ...
      FigInf, Dhandles = pyicqp.qp_timeseries(IcD_atm_mon, fname_atm_mon, ['rlut_gmean'], 
        t1=t1, t2=t2, ave_freq=ave_freq, shift_timeseries = shift_timeseries, omit_last_file=omit_last_file, 
        var_fac=vfc, use_tave_int_for_ts=use_tave_int_for_ts,
        fpath_ref_data_atm=fpath_ref_data_atm_rad, do_plot_atm_ref_ts=do_plot_atm_ref_ts, do_djf=do_djf, do_jja=do_jja,
        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
      )
      save_fig(fig_name, path_pics, fig_name)
    ...
    if fig_name in fig_names:
      ...
      FigInf, Dhandles = pyicqp.qp_timeseries(IcD_atm_mon, fname_atm_mon, ['prec_gmean'], 
        t1=t1, t2=t2, ave_freq=ave_freq, shift_timeseries = shift_timeseries, omit_last_file=omit_last_file, 
        var_fac=vfc, use_tave_int_for_ts=use_tave_int_for_ts,
        fpath_ref_data_atm=fpath_ref_data_atm_prec, do_plot_atm_ref_ts=do_plot_atm_ref_ts, do_djf=do_djf, do_jja=do_jja, units='mm/day',
        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
      )
      save_fig(fig_name, path_pics, fig_name)
    ...
    if fig_name in fig_names:
      ...
      FigInf, Dhandles = pyicqp.qp_timeseries(IcD_atm_mon, fname_atm_mon, ['evap_gmean'], 
        t1=t1, t2=t2, ave_freq=ave_freq, shift_timeseries = shift_timeseries, omit_last_file=omit_last_file, 
        var_fac=vfc, use_tave_int_for_ts=use_tave_int_for_ts,
        fpath_ref_data_atm=fpath_ref_data_atm, do_plot_atm_ref_ts=do_plot_atm_ref_ts, do_djf=do_djf, do_jja=do_jja, units='mm/day',
        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
      )
      save_fig(fig_name, path_pics, fig_name)
...
#Dd['prec']     = '.1f'
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if var_ref in f.variables:
        ...
        pyic.hplot_base(IcD_atm2d, IaV, clim=100, clevs=[-100,-80,-60,-40,-20,0,20,40,60,80,100], cmap='RdBu_r', 
                        use_tgrid=False,
                        projection=projection, xlim=xlim, ylim=ylim,  do_write_data_range=True,
                        #land_facecolor='none',
                        asp=0.5,
                        save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
                       )
        ...
        print(f"Variable '{var_ref}' not found in the reference data.")
      f.close()  
    ...
    if fig_name in fig_names and do_conf_dwd and exist_ref:
      ...
      if var_ref1 in f.variables and var_ref2 in f.variables:
        ...
        print(f"Variable(s) '{var_ref1}' and '{var_ref2}' not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and do_conf_dwd and exist_ref:
      ...
      # --- reference
      f = Dataset(fpath_ref, 'r')
      if var_ref in f.variables and var_ref2 in f.variables:
    ...
    # ---
    fig_name = 'atm_sfc_sob'
    if fig_name in fig_names and do_conf_dwd:
      FigInf = pyicqp.qp_hplot(fpath=path_data+fname, var=vsob_s, it=0,
                               it_ave=it_ave_atm,
                               t1=t1, t2=t2,
                               var_fac=1.,
                               units='$W m^{-2}$',
                               clim=[0.,180.], cincr=10.0, cmap='RdYlBu_r',
                               land_facecolor='none',
                               IcD=IcD_atm2d,
                               save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
                               **Ddict_global)
      save_fig('sfc short wave net flux', path_pics, fig_name, FigInf)

    # ---
    fig_name = 'atm_sfc_sob_bias'
    if fig_name in fig_names:
      var = vsob_s
      var_ref = 'ssr'
      # --- reference
      ...
      if var_ref in f.variables:
        ...
        # --- calculate bias
        data_bias = datai-data_ref
        IaV = pyic.IconVariable('data_bias', '$W m^{-2}$', 'sfc short wave net flux bias')
        IaV.data = data_bias
    ...
    # ---
    fig_name = 'atm_sfc_thb'
    if fig_name in fig_names and do_conf_dwd:
      FigInf = pyicqp.qp_hplot(fpath=path_data+fname, var=vthb_s, it=0,
                               it_ave=it_ave_atm,
                               t1=t1, t2=t2,
                               var_fac=1.,
                               units='$W m^{-2}$',
                               clim=[-80.,0.], cincr=5.0, cmap='RdYlBu_r',
                               land_facecolor='none',
                               IcD=IcD_atm2d,
                               save_data=save_data, fpath_nc=path_nc+fig_name+'.nc',
                               **Ddict_global)
      save_fig('sfc long wave net flux', path_pics, fig_name, FigInf)

    # ---
    fig_name = 'atm_sfc_thb_bias'
    if fig_name in fig_names:
      var = vthb_s
      var_ref = 'str'
      # --- reference
      ...
      if var_ref in f.variables:
        ...
        # --- calculate bias
        data_bias = datai-data_ref
        IaV = pyic.IconVariable('data_bias', '$W m^{-2}$', 'sfc long wave net flux bias')
        IaV.data = data_bias
        pyic.hplot_base(IcD_atm2d, IaV, clim=40., cincr=5., cmap='RdBu_r', 
                        use_tgrid=False,
                        projection=projection, xlim=xlim, ylim=ylim,
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if var_ref1 in f.variables and var_ref2 in f.variables:
        ...
        print(f"Variables '{var_ref1}' and '{var_ref2}' not found in the reference data.")
      f.close()      
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 't_zm' in f.variables:
        ...
        print(f"Variable t_zm not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'u_zm' in f.variables:
        ...
        print(f"Variable u_zm not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'v_zm' in f.variables:
        ...
        print(f"Variable v_zm not found in the reference data.")        
      f.close()  
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'q_zm' in f.variables:
        ...
        print(f"Variable q_zm not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'r_zm' in f.variables:
        ...
        print(f"Variable r_zm not found in the reference data.")
      f.close()  
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'cc_zm' in f.variables:
        ...
        print(f"Variable cc_zm not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'clwc_zm' in f.variables:
        ...
        print(f"Variable clwc_zm not found in the reference data.")
      f.close()
    ...
    if fig_name in fig_names and exist_ref:
      ...
      if 'ciwc_zm' in f.variables:
        ...
        print(f"Variable ciwc_zm not found in the reference data.")
      f.close()
    ...
    if do_hamocc_plots:
      ...
      if np.any(np.isin(fig_names, tmp_list)):
