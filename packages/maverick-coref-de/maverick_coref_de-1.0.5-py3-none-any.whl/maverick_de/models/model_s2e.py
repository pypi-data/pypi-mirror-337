...
class Maverick_s2e(torch.nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.coref_rep_size = self.token_hidden_size * 2

        self.antecedent_start_rep = RepresentationLayer(
            type=self.representation_layer_type,  # fullyconnected
            input_dim=self.token_hidden_size,
            output_dim=self.coref_rep_size,
            hidden_dim=self.coref_hidden_size,
        )
        self.antecedent_end_rep = RepresentationLayer(
            type=self.representation_layer_type,  # fullyconnected
            input_dim=self.token_hidden_size,
            output_dim=self.coref_rep_size,
            hidden_dim=self.coref_hidden_size,
        ...
        # self.antecedent_s2e_rep = RepresentationLayer(
        #     type=self.representation_layer_type,  # fullyconnected
        #     input_dim=self.token_hidden_size,
        #     output_dim=self.coref_rep_size,
        #     hidden_dim=self.coref_hidden_size,
        # )
        # self.antecedent_e2s_rep = RepresentationLayer(
        #     type=self.representation_layer_type,  # fullyconnected
        #     input_dim=self.token_hidden_size,
        #     output_dim=self.coref_rep_size,
        #     hidden_dim=self.coref_hidden_size,
        # )
        self.antecedent_s2s_classifier=torch.nn.Linear(self.coref_rep_size, self.coref_rep_size)
        self.antecedent_s2e_classifier=torch.nn.Linear(self.coref_rep_size, self.coref_rep_size)
        self.antecedent_e2s_classifier=torch.nn.Linear(self.coref_rep_size, self.coref_rep_size)
        self.antecedent_e2e_classifier=torch.nn.Linear(self.coref_rep_size, self.coref_rep_size)
        ...
        if self.use_attn:
            self.antecedent_attn_rep = RepresentationLayer(
                type=self.representation_layer_type,  # fullyconnected
                input_dim=self.token_hidden_size,
                output_dim=self.coref_rep_size,
                hidden_dim=self.coref_hidden_size,
            ...
            self.antecedent_a2a_classifier=torch.nn.Linear(self.coref_rep_size, self.coref_rep_size)
    ...
    def _compute_attn_emb(self, token_emb, candidate_starts, candidate_ends):
        ...
        candidate_tokens_attn=torch.nn.functional.softmax(candidate_tokens_attn_raw, dim=1) #candidate_tokens_attn(num_candidate_spans,num_tokens), assigns attention weights for tokens in every candidate span
    ...
    def s2e_mention_cluster(self, mention_start_reps, mention_attn_reps, mention_end_reps, mention_start_idxs, mention_end_idxs, gold, stage):
        coref_logits = self._calc_coref_logits(mention_start_reps, mention_attn_reps, mention_end_reps)
    ...
    def _calc_coref_logits(self, top_k_start_coref_reps, top_k_attn_coref_reps, top_k_end_coref_reps):
        start_reps = self.antecedent_start_rep(top_k_start_coref_reps)
        end_reps = self.antecedent_end_rep(top_k_end_coref_reps)
        
        # s2s
        temp = self.antecedent_s2s_classifier(start_reps)  # [batch_size, max_k, dim]
        top_k_s2s_coref_logits = torch.matmul(temp, start_reps.permute([0, 2, 1]))  # [batch_size, max_k, max_k]

        # e2e
        temp = self.antecedent_e2e_classifier(end_reps)  # [batch_size, max_k, dim]
        top_k_e2e_coref_logits = torch.matmul(temp, end_reps.permute([0, 2, 1]))  # [batch_size, max_k, max_k]

        # s2e
        temp = self.antecedent_s2e_classifier(start_reps)  # [batch_size, max_k, dim]
        top_k_s2e_coref_logits = torch.matmul(temp, end_reps.permute([0, 2, 1]))  # [batch_size, max_k, max_k]

        # e2s
        temp = self.antecedent_e2s_classifier(end_reps)  # [batch_size, max_k, dim]
        top_k_e2s_coref_logits = torch.matmul(temp, start_reps.permute([0, 2, 1]))  # [batch_size, max_k, max_k]
        ...
        if self.use_attn:
            attn_reps = self.antecedent_attn_rep(top_k_attn_coref_reps)
            temp = self.antecedent_a2a_classifier(attn_reps)  # [batch_size, max_k, dim]
            top_k_a2a_coref_logits = torch.matmul(temp, attn_reps.permute([0, 2, 1]))  # [batch_size, max_k, max_k]
            coref_logits = coref_logits + top_k_a2a_coref_logits
    ...
    def forward(
        ...
        mentions_attn_hidden_states = self._compute_attn_emb(lhs[0], mention_start_idxs, mention_end_idxs).unsqueeze(0)
