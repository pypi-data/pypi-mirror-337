...
class GeminiModel:
    ...
    def __init__(self, api_key: str, console: Console, model_name: str ="gemini-2.5-pro-exp-03-25"):
        ...
        self.chat_history = [
            ...
            {'role': 'model', 'parts': [genai.Part(text="Okay, I'm ready. Provide the directory context and your request.")]}
        ]
    ...
    def generate(self, prompt: str) -> str | None:
        ...
        try:
            ...
            if ls_tool:
                ...
                log.info(f"Orientation ls result: {ls_result}")
        ...
        self.chat_history.append({'role': 'user', 'parts': [genai.Part(text=turn_input_prompt)]})
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                # --- Start Fallback Logic (Simplified) ---
                ...
                # === Process LLM Response: Check for Function Call ===
                if hasattr(response_part, 'function_call') and response_part.function_call:
                    ...
                    self.chat_history.append({'role': 'model', 'parts': [response_part]})
                    ...
                    with self.console.status(f"[yellow]{status_msg}..."):
                         try:
                             ...
                             if tool_instance:
                                 ...
                                 log.debug(f"Tool '{tool_name}' result: {str(tool_result)[:500]}...") # Log truncated result
                             else:
                    ...
                    # Display result snippet to user? Maybe just errors.
                    if tool_error:
                        self.console.print(f"[red] -> Error executing {tool_name}: {str(tool_result)[:100]}...[/red]")
                    else:
                         # Simple non-error status update
                    ...
                    if tool_name == "task_complete":
                        ...
                        task_completed = True
                        # The result of task_complete IS the summary
                        final_summary = tool_result 
                        # We break *after* adding the function response below
                        
                    # === Add Function Response to History ===
                    # This is crucial for the LLM's next turn
                    function_response_part = genai.Part.from_function_response(
                        name=tool_name,
                        response={"result": tool_result} # Gemini expects the result in a dict
                    )
                    # History expects a list of parts, even if just one
                    ...
                    if task_completed: 
                     ...
                     # === LLM Returned Text (No Function Call) ===
                     llm_text = response_part.text
                     log.info(f"LLM returned text response (Iter {iteration_count}): {llm_text[:100]}...")
                     # Add the text response to history
                     self.chat_history.append({'role': 'model', 'parts': [response_part]})
                     self._manage_context_window()
                     
                     # Assume text response means the LLM thinks it's done or is explaining plan
                     # We can treat this as the final answer unless logic changes
                     last_text_response = llm_text
                     # We could potentially check here if the LLM explicitly asks for confirmation
                     # or states the task is done without calling the tool. For now, assume text = final.
                     task_completed = True # Treat text response as completion for now
                     final_summary = llm_text # Use the text as the summary
                     break # Exit the loop
                
                else:
                     # === Unexpected Response Part ===
                     log.warning(f"LLM returned unexpected response part (Iter {iteration_count}): {response_part}")
                     # Add it to history anyway? Maybe safer to stop.
                     self.chat_history.append({'role': 'model', 'parts': [response_part]})
                     self._manage_context_window()
                     last_text_response = f"(Agent received an unexpected response part: {response_part})"
            ...
            for func_decl in self.function_declarations:
                 ...
                 if func_decl.parameters and func_decl.parameters.properties:
                      ...
                      for prop, details in func_decl.parameters.properties.items():
                            ...
                            args_list.append(f"{prop}: {prop_type}{suffix}")
                      args_str = ", ".join(args_list)
                 
                 desc = func_decl.description or "(No description provided)"
...
5.  **Complete:** Once the *entire* task is finished, **you MUST call the `task_complete` function**, providing a concise summary of what was done in the `summary` argument. Do not just stop outputting; the `task_complete` call is essential.
...
*   **Task Completion Signal:** ALWAYS finish by calling `task_complete(summary=...)`. The summary should accurately reflect the outcome (success, partial success, error, or what was done).
