...
import requests
from crudclient import ClientConfig
...
class TripletexConfig(ClientConfig):
    ...
    def prepare(self) -> None:
        """
        Run once at client startup: initialize session if missing.
        """
        logger.info(f"Preparing Tripletex client configuration for {self.hostname}")
        if not self.session_token:
            logger.info("No session token found, initializing a new session")
            self.refresh_token()
        else:
            logger.debug("Using existing session token")
