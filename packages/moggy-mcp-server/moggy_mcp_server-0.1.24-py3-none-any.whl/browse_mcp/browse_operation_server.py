...
from browse_manager import BrowserManager
...
class BrowserNavigationServer(FastMCP):
    ...
    def register_tools(self):
        ...
        @self.mcp.tool(description="mark elements on current web page")       
        async def content_elements_mark():
            page: Page = await self.browser_manager.ensure_browser()
            playwright_controller = PlaywrightController()
            mark_data: Dict[str, InteractiveRegion] = await playwright_controller.get_interactive_rects(page=page)
            return json.dumps(mark_data)
