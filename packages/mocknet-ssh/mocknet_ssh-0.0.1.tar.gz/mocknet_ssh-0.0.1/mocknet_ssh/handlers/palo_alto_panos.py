...
COMMANDDIR = "mocknet_ssh.handlers.command_output.paloalto_panos"
OS = "paloalto_panos"


class PaloAltoPANOSHandler(asyncssh.SSHServerSession):
    ...
    def _load_dynamic_commands(self, command_dir: MultiplexedPath) -> None:
        ...
        logger.info(f"Commandir type: {type(command_dir)}")
    ...
    def connection_made(self, chan: asyncssh.SSHServerChannel) -> None:
        self._chan = chan
        logger.info("📡 Palo Alto SSH session opened")

    def shell_requested(self) -> bool:
        return True

    def session_started(self) -> None:
        logger.info("🚪 Palo Alto session started")
        if self._chan:
            self._chan.write("Welcome to PAN-OS\n")
    ...
    def _prompt(self) -> str:
        return "PA-VM# " if self.mode == "config" else "PA-VM> "


def main() -> None:
    users: Dict[str, str] = {"testadmin": "1234"}
    asyncio.run(run_ssh_server(users, handler_factory=PaloAltoPANOSHandler))
