...
@jm_download.handle()
async def handle_jm(args: Message | QQMessage = CommandArg()):
    logger.info(f"收到命令: {args}")
    # 获取参数
    if isinstance(args, QQMessage):
        ...
        try:
            ...
            if pdf_path.exists():
                ...
                try:
                    ...
                    try:
                        if isinstance(args, QQMessage):
                            await jm_download.send(build_qq_file(file_uri), timeout=600)
                        else:
...
# 构造QQ平台的文件消息
def build_qq_file(file_path: str) -> QQMessage:
    """
    根据QQ适配器的消息格式构造文件消息
    Args:
        file_path: 文件路径
    Returns:
        QQMessage: QQ适配器的文件消息
    """
    return QQMessage(QQMessageSegment.file(file_path))