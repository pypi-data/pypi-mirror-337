...
def create_master_pipeline(
    ...
    for pipeline in pipelines:
        ...
        exec_pipe = factory_objects.ExecutePipelineActivity(
            name=f"Execute {pipeline.name}", type_properties=type_properties
        )
        pipe_activities.append(exec_pipe)
    resource_properties = factory_objects.PipelineResourceProperties(
        activities=pipe_activities, folder={"name": folder_name}
    )
    return factory_objects.PipelineResource(
        name=master_name, properties=resource_properties
    )
    ...
    for batch in batches:
        ...
        for pipeline_id, pipeline in pipeline_build_plan.items():
            ...
            pipeline_property = factory_objects.PipelineResourceProperties(
                activities=pipe_activities, folder={"name": f"{folder_root}"}
            )
            full_pipeline = factory_objects.PipelineResource(
                name=pipeline_names[pipeline_id], properties=pipeline_property
            )
        ...
        recurrence = (
            factory_objects.ScheduleTriggerTypeProperties.transform_cron_to_recurrence(
                batch.cron, batch.timezone
            )
        )
        ...
        used_trigger.append(
            factory_objects.Trigger(
                name=f"{master.name}_trigger", properties=trigger_properties
            )
        )

        logger.info("Pipeline trigger created")
        pipeline_resources += [i for i in pipelines.values()]
