...
from typing import Any, Dict, Union, Optional, Tuple, List, Set
...
def save_binary_model(data: Dict[str, Any], file_path: Union[str, Path], 
                      compression_method: str = 'zlib', level: int = 6) -> None:
    """
    ...
    if compression_method == 'zlib':
        binary_data = zlib.compress(binary_data, level=level)
        compression_code = 1
    elif compression_method == 'lzma':
        binary_data = lzma.compress(binary_data, preset=level)
        compression_code = 2
    elif compression_method == 'gzip':
        binary_data = gzip.compress(binary_data, compresslevel=level)
        compression_code = 3
...
def load_binary_model(file_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    with open(file_path, 'rb') as f:
        ...
        if len(binary_data) != uncompressed_size:
            raise ValueError(f"Decompression error: expected {uncompressed_size} bytes, got {len(binary_data)}")
        
        # Parse binary data
        result = {"abbrev_types": [], "collocations": [], "sent_starters": [], "ortho_context": {}}
        offset = 0
