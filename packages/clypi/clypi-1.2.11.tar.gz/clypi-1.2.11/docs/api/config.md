...
- `disable_colors`: whether we should disable all colors and text styles
- `fallback_term_width`: if we cannot get the current terminal width (e.g.: subprocesses, non-tty devices, etc.), what should the fallback terminal width be (mostly used for displaying errors)
