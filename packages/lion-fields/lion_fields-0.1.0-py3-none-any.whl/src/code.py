...
class Import(HashableModel):
    ...
    alias: str | None = Field(
        default=None, description="Alias name if used ('import X as Y'), else None."
    )
    level: int = Field(
        default=0,
        description=(
...
class Attribute(HashableModel):
    ...
    type: str | None = Field(
        default=None, description="String type annotation if declared. None if untyped."
    )
...
class Class(HashableModel):
    ...
    docstring: str | None = Field(
        default=None, description="Documentation for the class/interface, if any."
    )
    decorators: list[Decorator] = Field(
        default_factory=list,
...
class Module(HashableModel):
    ...
    path: str | None = Field(
        default=None, description="Filesystem path if known (e.g., 'src/utils.py')."
    )
    docstring: str | None = Field(
        default=None, description="File-level docstring or comments if any."
    )
    imports: str | None = Field(
        default_factory=list,
