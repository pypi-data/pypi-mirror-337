...
class IRGradOriNet(nn.Module):
    def __init__(self, in_dim = 3, dim=32, num_classes=1):
        ...
        # self.downlayer5 = Downlayer(in_channels=dim * 8, out_channels=dim * 16)
        ...
        # self.uplayer4 = Uplayer(in_channels=dim * 16, out_channels=dim * 8)
    ...
    def forward(self, x, flag = None):
        ...
        if torch.isnan(f3).any():
            print(f"[{self.__class__.__name__}] f3")


        f4 = self.downlayer4(self.downsample(f3))
        if torch.isnan(f4).any():
            print(f"[{self.__class__.__name__}] f4")


        # f5 = self.downlayer5(self.downsample(f4))

        fskip1 = self.skip1(f1)
        if torch.isnan(fskip1).any():
            print(f"[{self.__class__.__name__}] fskip1")

        fskip2 = self.skip2(f2)
        if torch.isnan(fskip2).any():
            print(f"[{self.__class__.__name__}] fskip2")

        fskip3 = self.skip3(f3)
        if torch.isnan(fskip3).any():
            print(f"[{self.__class__.__name__}] fskip3")


        # fskip4 = self.skip4(f4)
        # fskip5 = self.skip5(f5)

        # d4 = self.uplayer4(self.upsample(f5), fskip4)
        d3 = self.uplayer3(self.upsample(f4), fskip3)
        if torch.isnan(d3).any():
            print(f"[{self.__class__.__name__}] d3")

        # print(d3.shape)

        d2 = self.uplayer2(self.upsample(d3), fskip2)
        if torch.isnan(d2).any():
            print(f"[{self.__class__.__name__}] d2")
        # print(d2.shape)

        d1 = self.uplayer1(self.upsample(d2), fskip1)
        if torch.isnan(d1).any():
            print(f"[{self.__class__.__name__}] d1")
        ...
        outall = torch.cat([out1, out2, out], 1)
        outall = self.allout(outall)
