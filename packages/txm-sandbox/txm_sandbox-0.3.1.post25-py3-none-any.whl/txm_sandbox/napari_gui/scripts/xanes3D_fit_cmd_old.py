...
try:
    ...
    with h5py.File(xanes3d_data_fn, "a") as f:
        ...
        #     alias = sorted(list(f[f"/{proc_xanes_path}/proc_parameters/LCF/ref_alias"].keys()))
        #     xana_lcf_ref = []
        #     for key in alias:
        #         lcf_ref = f[f"/{proc_xanes_path}/proc_parameters/LCF/ref/{key}/data/chi_ref"][:]
        #         lcf_ref_eng = f[f"/{proc_xanes_path}/proc_parameters/LCF/ref/{key}/data/Mono Energy"][:]
        #         xana_lcf_ref.append(np.interp(xana_eng_list, lcf_ref_eng, lcf_ref))            
        #     xana_lcf_ref_spec = np.array(xana_lcf_ref)
        ...
        if xana_type in ["wl", "raw spectra - wl"]:
            ...
            wl_fit_params = {
                ...
                "model": tem["method"],
                "on": tem["params"]["spec"],
                "order": int(tem["params"]["order"]),
                "eoff": float(tem["params"]["eng_offset"]),
            }
            _g12 = {}
            for jj in sav_items:
                ...
                for ii in range(xana_data_shape[1]):
                    ...
                    for jj in sav_items:
                        ...
                        if jj == "weighted_attenuation":
                            _g12[jj][ii] = np.float32(xana.weighted_atten)[:]
                        if jj == "lcf_fit":
                            _g12[jj][ii] = np.float32(xana.lcf_model["rlt"][:, :-1].reshape(
