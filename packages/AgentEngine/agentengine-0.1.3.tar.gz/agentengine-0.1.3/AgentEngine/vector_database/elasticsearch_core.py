...
class ElasticSearchCore:
    ...
    def get_user_indices(self, index_pattern: str = "*") -> List[str]:
        ...
        return [index_name for index_name in indices if not index_name.startswith('.')]
    ...
    def accurate_search(self, index_name: str, query_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def semantic_search(self, index_name: str, query_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
                ...
                "unique_sources": {
                    ...
                    "aggs": {
                        "file_sample": {
                            "top_hits": {
                                "size": 1,
                                "_source": ["path_or_url", "file_size", "create_time", "file"]
                            }
                        }
                    }
                }
    ...
    def get_index_mapping(self, index_name: str) -> List[str]:
    ...
    def get_index_stats(self, index_name: str) -> Dict[str, Dict[str, Any]]:
        """Get formatted statistics for a specific index"""
    ...
    def print_all_indices_info(self, index_pattern: str = "*") -> None:
        ...
        for index_name in user_indices:
            ...
            print_index_info(stats, index_name)
    ...
    def create_test_knowledge_base(
        ...
        """
        print(f"\nCreating test knowledge base sample_articles'...")
        ...
        try:
            ...
            success = self.create_vector_index("sample_articles", embedding_dim)
            if not success:
                print(f"Failed to create index sample_articles")
            ...
            for article in articles:
                ...
                article["file"] = url.split("/")[-1]  # Add filename
            ...
            num_indexed = self.index_documents("sample_articles", articles)
            
            print(f"Test knowledge base created with {num_indexed} documents")
            return "sample_articles", num_indexed
...
def main():
    ...
    if index_name in user_indices:
        ...
        cleanse_source = es_core.get_cleanse_source_info(index_name)
        
        print(f"\nBase Info:")
        for metric_name, value in stats["base_info"].items():
            print(f"  {metric_name}: {value}")
            
        print(f"\nSearch Performance:")
        for metric_name, value in stats["search_performance"].items():
            print(f"  {metric_name}: {value}")
            
        print(f"\nFields: {', '.join(fields)}")
        print(f"Unique Sources Count: {unique_sources_count}")
        print(f"Cleanse Source: {cleanse_source}")
        
        # # Delete documents by URL
        # print("\nDeleting documents by URL...")
        # url = "https://raw.githubusercontent.com/elastic/elasticsearch-labs/main/notebooks/search/articles.json"
        # deleted_count = es_core.delete_documents_by_path_or_url(index_name, url)
