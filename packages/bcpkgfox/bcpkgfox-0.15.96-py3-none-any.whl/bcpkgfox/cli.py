    ...
    class venv_mangt:
        ...
        def install_imports(self):
            ...
            try:
                ...
                try:
                    for lib in librarys:
                        if "pynput" in librarys:
                            pynput_log = subprocess.run(
                                [pip_path, 'install', "bcpkgfox['dependency1']"],
                                check=True,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE,
                                text=True
                            )
                            print(f"\033[0J{pynput_log.stdout.strip()}\033[0J")

                        elif "screeninfo" in librarys:
                            screeninfo_log = subprocess.run(
                                [pip_path, 'install', "bcpkgfox['dependency2']"],
                                check=True,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE,
                                text=True
                            )
                            print(f"\033[0J{screeninfo_log.stdout.strip()}\033[0J")
