...
class HandlerCore:
    ...
    def muc_ping_result(self, result: dict) -> None:
        """The slix 410 plugin got a new result"""
        room_from, _ = result.get('key', (None, None))
        room_from = JID(room_from.bare)
        tab = self.core.tabs.by_name_and_class(room_from, tabs.MucTab)
        if tab:
            prev: PingStatus = result.get('previous', PingStatus.UNTRIED)
            new: PingStatus = result.get('result', PingStatus.UNTRIED)
            if new != prev:
                tab.on_self_ping_result(new)
    ...
    def next_adhoc_step(self, iq, adhoc_session):
        ...
        on_validate = on_cancel = lambda *args, **kwargs: False
        if status == 'executing':
            if not form:
                self.core.information(
