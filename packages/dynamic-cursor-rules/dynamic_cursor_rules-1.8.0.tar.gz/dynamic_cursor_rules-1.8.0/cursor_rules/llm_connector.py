...
class LLMConnector:
    ...
    def _request_anthropic(self, request: LLMRequest) -> LLMResponse:
        ...
        # Format the parameters for Anthropic API
        # Note: Anthropic requires 'system' as a top-level parameter, not in messages array
        params = {
            "model": model,
            "messages": [
        ...
        # Add system message as top-level parameter if provided
        if request.system_message:
