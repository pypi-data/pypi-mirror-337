...
def init_generation(retriever, model, prompt=""):
    log("Initializing generation chain")
    if not prompt:
        """Ответь на поставленный вопрос, используя только данный контекст. В выводе покажи только сам ответ, без объяснений.
...
Вопрос: {input}"""
    ...
    document_chain = create_stuff_documents_chain(model, llm_prompt)
