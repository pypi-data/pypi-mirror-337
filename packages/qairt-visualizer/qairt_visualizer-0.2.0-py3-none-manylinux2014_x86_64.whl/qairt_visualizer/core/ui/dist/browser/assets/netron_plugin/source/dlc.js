...
dlc.Container = class {
    ...
    static open(context) {
        ...
        if (entries instanceof Map) {
            // QAIRT Netron: Check for model.params.bin file which was added in DLC 4.1.0
            if (entries.has('model') || entries.has('model.params') || entries.has('model.params.bin')) {
                return new dlc.Container(context, entries.get('model'), entries.get('model.params'), entries.get('model.params.bin'), entries.get('dlc.metadata'));
            }
        }
    ...
    async read() {
        ...
        if (this._params) {
            ...
            switch (signature) {
                ...
                case '4.NETP':
                case '4.NR64': {
                    dlc.Container._params4(stream, this.graphs, signature, this._paramsBin); // QAIRT Netron: Pass params bin data
                    break;
                }
                default: {
                    const buffer = stream.peek(Math.min(stream.length, 16));
                ...
                for (const attribute of node.attributes || []) { // QAIRT Netron : Added or condition as I/O nodes will not have attributes
                    switch (attribute.kind) {
                        case 0: {
                            ...
                            switch (value.kind) {
                                case 0x7fffffff:
                                case 0x0608: // QAIRT Netron: Handle string datatype as defined in https://mlggrok.qualcomm.com/xref/SNPE_mainline/QAISW/FirstParty/QNN/HTP/src/buffer/schema/QnnHtpContextBlob.fbs
    ...
    static _params4(stream, graphs, signature, paramsBin) {
        ...
        for (const graph of graphs) {
            ...
            for (const tensor of graph.tensors) {
                if (tensor.location === 4) {
                    ...
                    tensor.data = this.getTensorData(tensors, tensor, paramsBin, buffers);
                }
            }
            for (let i = 0; i < graph.nodes.length; i++) {
    ...
    static getTensorData(tensors, tensor, paramsBin, buffers) {
      ...
      if (paramsBin) {
        ...
        return paramsBin.peek().subarray(data_position.offset, data_position.offset + data_position.size);
      } else if (buffers) {
    ...
    static _signature(stream) {
        if (stream) {
            ...
            // QAIRT Netron: Handle DLC 4.1 signature
            if (match([ 0xD5, 0x0A, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00 ]) && buffer.length >= 16) {
