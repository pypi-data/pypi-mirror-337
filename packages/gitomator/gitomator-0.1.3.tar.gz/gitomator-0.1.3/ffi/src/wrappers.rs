...
pub fn clone_repository(url: *const c_char, path: *const c_char) -> Result<*mut GitRepository, GitResultCode> {
    let mut url_str = c_char_to_string(url)?;
    let path_str = c_char_to_string(path)?;
    
    // If the URL starts with "http" or "https" and doesn't end with ".git", append it.
    if (url_str.starts_with("http") || url_str.starts_with("https")) && !url_str.ends_with(".git") {
        url_str.push_str(".git");
    }
