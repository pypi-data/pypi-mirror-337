...
INPUT_EXTENSIONS = {
    ...
    ".docx": InputFormat.WORD_X,
    ".xls": InputFormat.EXCEL,
    ".xlsx": InputFormat.EXCEL_X,
    ".ppt": InputFormat.PPT,
    ".pptx": InputFormat.PPT_X,
...
FORMAT_CONVERSION_MAP = {
    InputFormat.PDF: {
        ...
        OutputFormat.IMAGE,  # PDF转图片
        OutputFormat.HTML,   # PDF转HTML
        OutputFormat.TEXT,   # PDF转文本
    },
    InputFormat.WORD: {OutputFormat.PDF},      # Word转PDF
    InputFormat.WORD_X: {OutputFormat.PDF},    # Word转PDF
    InputFormat.EXCEL: {OutputFormat.PDF},     # Excel转PDF
    InputFormat.EXCEL_X: {OutputFormat.PDF},   # Excel转PDF
    InputFormat.PPT: {OutputFormat.PDF},       # PPT转PDF
    InputFormat.PPT_X: {OutputFormat.PDF},     # PPT转PDF
    InputFormat.IMAGE: {OutputFormat.PDF},     # 图片转PDF
...
class FileHandler:
    ...
    async def get_output_dir(self, file_path: str, output_dir: str = "") -> str:
        """确定输出目录"""
        output_dir_candidates = []
        
        # 1. 用户指定的输出目录（如果有）
        if output_dir:
            output_dir_candidates.append(output_dir)
...
class Converter:
    ...
    async def convert_file(self, file_path: str, format: str, output_dir: str = "") -> ConversionResult:
        """转换单个文件"""
        if not self.api_key:
        ...
        # 获取输出目录
        output_dir = await self.file_handler.get_output_dir(file_path, output_dir)
