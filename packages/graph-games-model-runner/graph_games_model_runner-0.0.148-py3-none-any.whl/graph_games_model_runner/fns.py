...
def gensample(config, static_board_config_map, sample_raw, augmentation=False):
    ...
    static_board_config_uuid = x["static_board_config_uuid"]
    static_board_config = static_board_config_map[static_board_config_uuid]
    ...
    my_legal_actions = private_state.legal_actions

    def is_captured_by_me(point):
        return point in my_points

    def is_captured(point):
        return point in public_state.captured_points

    def is_available_to_me(point):
        for legal_action in my_legal_actions:
            if point in legal_action.points:
                return True
        return False
    ...
    # print("imagined_public_state.captured_points: ", imagined_public_state.captured_points)
    #
    ...
    # captured_by_me: false,
    # captured_by_other: false,
    # available_to_me: false,
    # not_available_to_me: true,
    ...
    adjusted_global_y_binary = qvalue2multihot(int(sample_raw["y_json"]))

    return Data(
        x=x_features,
        global_y=adjusted_global_y_binary,
        edge_index=edge_index,
        edge_attr=torch.ones(num_edges,1),
        global_attr=torch.ones(1),
    )

    return imagined_public_state.captured_points
    ...
    return Data(
        x=x_readings,
        y=json.loads(sample_raw["y_json"]),
        edge_attr=torch.ones(num_edges, 1),
        edge_index=edge_index,
...
def load_dataset(url, api_key, dataset_uuid, model_config):
    ...
    all_static_board_config_uuids = [
        json.loads(a["x_json"])["static_board_config_uuid"] for a in dataset
    ]
