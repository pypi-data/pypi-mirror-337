...
Includes Test Runner tool. Prompt v5 (Few-Shot Example).
"""
...
class GeminiModel:
    ...
    def __init__(self, api_key, model_name="gemini-2.5-pro-exp-03-25"):
        ...
        self.generation_config = genai.types.GenerationConfig(
            temperature=0.5, # Slightly lower temp might help instruction following
            top_p=0.95,
            top_k=40,
        )
        ...
        # Use the v5 prompt with few-shot example
    ...
    def generate(self, prompt: str, conversation=None) -> str | None:
        ...
        try:
            ...
            # Cleanup might still be useful if model generates stray tags
    ...
    def _extract_function_call_from_response(self, response):
        ...
        except (IndexError, AttributeError, KeyError, TypeError): pass # Expected if no call
        return None
    ...
    def _cleanup_internal_tags(self, text: str) -> str:
        # Keep this in case model outputs stray verbose thoughts despite instructions
    ...
    # --- V5 Prompt with Few-Shot Example ---
    def _create_system_prompt(self) -> str:
        """Creates the system instruction prompt using a few-shot example."""
        tools_description = []
        # ... (code to populate tools_description remains the same) ...
        if AVAILABLE_TOOLS:
...
2.  **Plan:** Mentally devise a plan of function calls needed.
3.  **Execute:** Call the necessary functions (`edit`, `bash`, `test_runner`, etc.) to perform the task.
4.  **Verify:** Use tools (`view`, `grep`, `test_runner`, etc.) to confirm your actions succeeded.
5.  **Summarize:** Report concisely to the user what you did and the outcome. Avoid large code blocks unless requested.

**Key Instructions:**
*   **Act Autonomously:** Especially for gathering information. Don't ask, DO.
*   **Use Tools for Action & Verification.**
*   **Be Concise.**

**Example Interaction:**

*   **User:** Can you add a docstring to the `calculate_area` function in `geometry.py`?
*   **Assistant (Internal Thought):** Need to see the current function in `geometry.py` to add the docstring correctly.
*   **Assistant (Action):** [Calls `view(file_path='geometry.py')`]
*   **Assistant (Internal Thought after getting file content):** Okay, I see the function `def calculate_area(length, width): return length * width`. Now I can plan the edit.
*   **Assistant (Action):** [Calls `edit(file_path='geometry.py', old_string='def calculate_area(length, width):', new_string='def calculate_area(length, width):\\n    \"\"\"Calculates the area of a rectangle.\"\"\"')`]
*   **Assistant (Internal Thought):** Edit called. Now verify.
*   **Assistant (Action):** [Calls `view(file_path='geometry.py')` or `grep(pattern='Calculates the area', file_path='geometry.py')`]
*   **Assistant (Internal Thought after verification):** Verification successful. Docstring added.
*   **Assistant (Response to User):** Okay, I added a docstring to the `calculate_area` function in `geometry.py`.

**AVAILABLE FUNCTIONS:**
{chr(10).join(tools_description) if tools_description else "No functions currently available."}

Now, process the user's request following this workflow, especially the autonomous information gathering demonstrated in the example.
"""
        return system_prompt

    # ... (_create_tool_definitions remains the same as previous step) ...
