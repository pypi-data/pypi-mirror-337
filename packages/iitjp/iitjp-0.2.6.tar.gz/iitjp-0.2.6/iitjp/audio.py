...
try:
    from .resource_manager import get_resource_path, get_resource_bytes
except ImportError:
    # Fallback if resource_manager is not available
    def get_resource_path(file):
        """Fallback implementation if resource_manager not available"""
        return file
    
    def get_resource_bytes(file):
        """Fallback implementation if resource_manager not available"""
        return None
...
# Global cache of audio file paths
# This ensures we don't lose track of files between lookups
_AUDIO_PATH_CACHE = {}
...
class AudioPlayer:
    ...
    def resolve_audio_path(audio_file):
        """
        Resolve an audio file path - handles both direct paths and filenames
        
        Args:
            audio_file: Either a filename (e.g., 'greeting.mp3') or a full path
            
        Returns:
            The resolved file path or None if not found
        """
        # If file is already a full path and exists, return it
        if os.path.exists(audio_file):
            return audio_file
            
        # Check cache first
        if audio_file in _AUDIO_PATH_CACHE:
            cached_path = _AUDIO_PATH_CACHE[audio_file]
            if os.path.exists(cached_path):
                return cached_path
        
        # Resolve using resource manager
        resolved_path = get_resource_path(audio_file)
        
        if resolved_path and os.path.exists(resolved_path):
            # Cache the result
            _AUDIO_PATH_CACHE[audio_file] = resolved_path
            return resolved_path
            
        # If resource manager fails, try some common locations
        ...
        for base_dir in [
            ...
            if os.path.exists(test_path):
                # Cache the result
                _AUDIO_PATH_CACHE[audio_file] = test_path
                return test_path
                
        # If still not found, we've tried everything
        return None
    ...
    def play(file_path, block=False):
        ...
        Args:
            file_path: Path to the audio file or just the filename
        ...
        resolved_path = AudioPlayer.resolve_audio_path(file_path)
        ...
        if PLATFORM == 'windows' or sys.platform == 'win32':
            return AudioPlayer._play_windows(resolved_path, block)
        elif PLATFORM == 'darwin':  # macOS
            return AudioPlayer._play_macos(resolved_path, block)
        else:  # Linux and other platforms
            return AudioPlayer._play_linux(resolved_path, block)
