...
class Index:
    ...
    def update_data(service, archivo, hoja, rango, valores):
        while True:
            try:
                start_col, start_row = ''.join([c for c in rango.split(":")[0] if c.isalpha()]), int(''.join([c for c in rango.split(":")[0] if c.isdigit()]))
                end_col, end_row = ''.join([c for c in rango.split(":")[1] if c.isalpha()]), int(''.join([c for c in rango.split(":")[1] if c.isdigit()]))
