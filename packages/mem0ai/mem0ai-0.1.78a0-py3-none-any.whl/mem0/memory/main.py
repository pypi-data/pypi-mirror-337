...
class Memory(MemoryBase):
    ...
    def add(
        ...
        if memory_type is not None and memory_type != MemoryType.PROCEDURAL.value:
            raise ValueError(f"Invalid 'memory_type'. Please pass {MemoryType.PROCEDURAL.value} to create procedural memories.")
    ...
    def _create_procedural_memory(self, messages, metadata, prompt=None):
        """
        Create a procedural memory from a conversation.
        """
        ...
        embeddings = self.embedding_model.embed(summary, memory_action="add")
        # Create the memory
        memory_id = self._create_memory(summary, {summary: embeddings}, metadata=metadata)
        capture_event("mem0._create_procedural_memory", self, {"memory_id": memory_id})

        # Return results in the same format as add()
        result = {
            "results": [{"id": memory_id, "memory": summary, "event": "ADD"}]
        }
