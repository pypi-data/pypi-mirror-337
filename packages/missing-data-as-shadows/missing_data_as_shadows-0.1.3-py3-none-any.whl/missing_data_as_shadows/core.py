...
try:
    ...
    # Nastavenie backend-u len ak sme v PyCharm venv
    if 'PyCharm' in sys.version:
        matplotlib.use('Agg')  # Používa non-GUI backend
except:
    pass
...
def create_plot(df, value_name, missing_count_name, name=None, save=False, ax=None, show=True):
    if ax is None:
...
def create_plot_bar(data, attribute_one, attribute_two, name=None, save=False, ax=None, show=True):
    result = prepare_data(data, attribute_one, attribute_two)
    create_plot(result[0], result[1], result[2], name, save, ax, show)
...
def create_subplot_with_shadows(data, atr1, atr2, name=None, save=False, ):
    ...
    scatter_with_shadows_rect_binned(
        ...
        ax=ax_top,  # kreslíme do veľkého horného subplotu
        show=False
    )
