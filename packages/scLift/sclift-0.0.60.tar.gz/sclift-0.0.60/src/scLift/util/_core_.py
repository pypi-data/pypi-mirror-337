...
def to_dense(sm: matrix_data, is_array: bool = False) -> dense_data:
    ...
    if sm is None:
        log(__name__).warn("The input matrix (sm parameter) is not feasible")
        return np.asmatrix([])
...
def to_sparse(dm: dense_data, way_callback=sparse.csr_matrix, is_matrix: bool = True) -> sparse_matrix:
    ...
    if dm is None:
        log(__name__).warn("The input matrix (dm parameter) is not feasible")
        ...
        if data[i] not in new_data:
            ...
            while True:
                ...
                if not isinstance(data[i], str) and not is_warn:
                    log(__name__).warn("Convert non string types to string types.")
                    is_warn = True
...
def get_sub_data(data: collection, size: int) -> collection:
    ...
    if size >= old_size:
        log(__name__).warn("The given size is greater than the original data size")
        return data
...
def check_adata_get(adata: AnnData, layer: str = None, is_dense: bool = True, is_matrix: bool = False) -> AnnData:
    # judge input data
    if adata.shape[0] == 0:
        log(__name__).warn("Input data is empty")
