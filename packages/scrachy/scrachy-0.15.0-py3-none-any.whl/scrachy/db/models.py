...
class Response(Base, kw_only=True):
    ...
    # It's not clear why pylance complains about this use of 'binary', but
    # not the others.
    fingerprint: Mapped[binary] = mapped_column(primary_key=True)  # type: ignore
    ...
    scrape_history: Mapped[list[ScrapeHistory]] = relationship(
        cascade="all, delete-orphan", default_factory=list, repr=False
    )
    """The scrape history"""
...
class ScrapeHistory(Base):
    ...
    fingerprint: Mapped[binary] = mapped_column(
        ForeignKey("response.fingerprint"), primary_key=True
    )
