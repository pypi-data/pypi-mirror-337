...
class S3Manager:
    ...
    def write_file(self, bucket: str, s3_path: str, content: Union[str, bytes],
                  extra_args: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        try:
            ...
            args = {'Body': content, 'Bucket': bucket, 'Key': s3_path}
            if extra_args:
    ...
    def delete_file(self, bucket: str, s3_path: str) -> bool:
        """
    ...
    def get_file_metadata(self, bucket: str, s3_path: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            response = self.client.head_object(Bucket=bucket, Key=s3_path)
            # Convert boto3 response to a regular dict
            metadata = {
    ...
    def copy_file(self, source_bucket: str, source_path: str, 
                 dest_bucket: str, dest_path: str) -> bool:
        """
        ...
        try:
            ...
            self.client.copy(copy_source, dest_bucket, dest_path)
            return True
        except Exception as e:
            print(f"Error copying file in S3: {e}")
            return False
            
    def generate_signed_url(self, bucket: str, s3_path: str, 
                           expiration: int = 3600, method: str = 'GET') -> Optional[str]:
        """
        Generate a signed URL for accessing an S3 file.
    ...
    def close(self):
        ...
        This is a placeholder method to maintain compatibility with the Storage interface.
        S3 connections are managed by boto3 and typically don't need explicit closing.
        """
        # No action needed - boto3 handles connection pooling internally
        pass