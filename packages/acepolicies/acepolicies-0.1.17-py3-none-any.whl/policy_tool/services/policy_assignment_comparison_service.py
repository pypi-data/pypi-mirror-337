...
class PolicyAssignmentComparisonService(object):
        ...
        for desired_cmp, desired_cmp_assignments in self._desired_policy_assignments["column_masking_policies"].items():
            for desired_cmp_assignment, desired_arg_columns in desired_cmp_assignments['table_columns'].items():
                if (
                    ...
                    or sorted([desired_arg_column.upper() for desired_arg_column in desired_arg_columns])!= sorted([current_arg_column.upper() for current_arg_column in self._current_policy_assignments["column_masking_policies"][desired_cmp]["table_columns"][desired_cmp_assignment]])
                ):  
            ...
            for desired_cmp_assignment, desired_arg_columns in desired_cmp_assignments['view_columns'].items():
                if (
                    ...
                    or sorted([desired_arg_column.upper() for desired_arg_column in desired_arg_columns])!= sorted([current_arg_column.upper() for current_arg_column in self._current_policy_assignments["column_masking_policies"][desired_cmp]["view_columns"][desired_cmp_assignment]])
                ):                    
        ...
        for current_cmp, current_cmp_assignments in self._current_policy_assignments["column_masking_policies"].items():
            for current_cmp_assignment in current_cmp_assignments['table_columns']:
                if (
                    ...
                    and current_cmp_assignment.upper() not in [desired_cmp_assignment.upper() for desired_cmp_assignment in self._cmps_unset_already_covered_by_set_actions["table_columns"]]
                ):  
        ...
        for current_cmp, current_cmp_assignments in self._current_policy_assignments["column_masking_policies"].items():
            for current_cmp_assignment in current_cmp_assignments['view_columns']:
                if (
                    ...
                    and current_cmp_assignment.upper() not in [desired_cmp_assignment.upper() for desired_cmp_assignment in self._cmps_unset_already_covered_by_set_actions["view_columns"].keys()]
                ):  
        ...
        for desired_rap, desired_rap_assignments in self._desired_policy_assignments["row_access_policies"].items():
            for desired_rap_assignment, desired_arg_columns in desired_rap_assignments['tables'].items():
                if (
                    ...
                    if current_rap:
                        set_rap_statement = f"ALTER TABLE {table_reference} DROP ROW ACCESS POLICY {current_rap}, ADD ROW ACCESS POLICY {self._database}.{self._policy_schema}.{desired_rap} {columns_string};"
                    else:
            ...
            for desired_rap_assignment, desired_arg_columns in desired_rap_assignments['views'].items():
                if (
                    ...
                    if current_rap:
                        set_rap_statement = f"ALTER VIEW {view_reference} DROP ROW ACCESS POLICY {current_rap}, ADD ROW ACCESS POLICY {self._database}.{self._policy_schema}.{desired_rap} {columns_string};"
                    else:
        ...
        for current_rap, current_rap_assignments in self._current_policy_assignments["row_access_policies"].items():
            for current_rap_assignment in current_rap_assignments['tables']:
                if (
                    ...
                    and current_rap_assignment.upper() not in [desired_rap_assignment.upper() for desired_rap_assignment in self._raps_drop_already_covered_by_add_actions["tables"].keys()]
                ):  
        ...
        for current_rap, current_rap_assignments in self._current_policy_assignments["row_access_policies"].items():
            for current_rap_assignment in current_rap_assignments['views']:
                if (
                    ...
                    and current_rap_assignment.upper() not in [desired_rap_assignment.upper() for desired_rap_assignment in self._raps_drop_already_covered_by_add_actions["views"].keys()]
                ):  
    ...
    def _check_column_already_attached_to_a_cmp(self, desired_cmp_assignment_column: str, desired_cmp_assignment_object_domain: str) -> None:
        ...
        if desired_cmp_assignment_column in self._current_policy_assignments_transposed["column_masking_policies"][desired_cmp_assignment_object_domain]:
            current_cmp = self._current_policy_assignments_transposed["column_masking_policies"][desired_cmp_assignment_object_domain][desired_cmp_assignment_column]
    ...
    def _check_object_already_attached_to_a_rap(self, desired_rap_assignment_object: str, desired_rap_assignment_object_domain: str) -> str:
        ...
        if desired_rap_assignment_object in self._current_policy_assignments_transposed["row_access_policies"][desired_rap_assignment_object_domain]:
            current_rap = self._current_policy_assignments_transposed["row_access_policies"][desired_rap_assignment_object_domain][desired_rap_assignment_object]
