...
from mkdocstrings_handlers import shell
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mkdocstrings_handlers.shell")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.shell._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.shell"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocstrings_handlers.shell`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in shell.__all__ or not hasattr(shell, obj.name)
    ]
