...
def deploy(
    ...
    pipelines, triggers, sql_procs = azure_res.build_factory(
        batches, jobs_c, transaction_based_exec_sql
    )
    for pipeline in pipelines:
        writer.write_string_to_disk(
            pipeline.json(indent=4, by_alias=True, exclude_none=True),
            output_arm_path / "pipeline" / f"{pipeline.name}.json",
        )
    for trigger in triggers:
        writer.write_string_to_disk(
            trigger.json(indent=4, by_alias=True, exclude_none=True),
