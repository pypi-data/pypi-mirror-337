...
class PyttmanApp(PrettyReprMixin):
    ...
    def execute_plugins_before_app_start(self):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.before_app_start):
                plugin.before_app_start(self)

    def execute_plugins_after_app_stop(self):
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.after_app_stops):
                plugin.after_app_stops(self)

    def execute_plugins_no_intent_match(self, message: MessageMixin):
        reply = None
        for plugin in self.plugins:
            if plugin.allowed_to_intercept_at(PyttmanPlugin.PluginInterceptPoint.no_intent_match):
