...
import sys
from .Devices.LoupedeckLive import LoupedeckLive
...
class DeviceManager:
    ...
    def _get_loupedeck_ports(self):
        """
        Lists available serial ports
        :returns: A list of the serial ports available on the system
        """
        logger.debug("list: listing ports..")
        ports = list(serial.tools.list_ports.comports())
        
        # Filter for your specific device if needed
        # For example, if you know the vendor/product ID or description pattern:
        # result = [port.device for port in ports if "YourDeviceIdentifier" in port.description]
        
        print(ports)
        return []
        result = [port.device for port in ports]
        logger.debug(f"list: found {len(result)} ports")
        return result
    
    def get_loupedecks(self) -> list[LoupedeckLive]:
        loupedecks = list()

        paths = self._get_loupedeck_ports()
        for path in paths:
    ...
    def enumerate(self) -> list[StreamDeck]:
        ...
        try:
            loupedecks = self.get_loupedecks()
            streamdecks.extend(loupedecks)
        except Exception as e:
            logger.error(f"enumerate: failed to enumerate Loupedecks: {e}")
            raise e
