...
namespace deform_interpolate {

// Bilinear sampling (aka 'im2col') for input feature map.
template <typename scalar_t, typename transfer_t, int64_t c_per_thread>
__device__ void im2col_bilinear(opmath_t out_reg_array[],
                                const scalar_t *&p_value, const int64_t &height,
                                const int64_t &width, const opmath_t &h_px,
                                const opmath_t &w_px, const opmath_t &attn,
                                const int64_t &w_stride,
                                const int64_t &base_ptr) {
  const int64_t h_low = floor(h_px);
  const int64_t w_low = floor(w_px);
  ...
  int64_t idx1 = h_low_ptr_offset + w_low_ptr_offset + base_ptr;
  int64_t idx2 = h_low_ptr_offset + w_high_ptr_offset + base_ptr;
  int64_t idx3 = h_high_ptr_offset + w_low_ptr_offset + base_ptr;
  int64_t idx4 = h_high_ptr_offset + w_high_ptr_offset + base_ptr;
...
// Used for gradient computation for input feature map.
template <typename scalar_t, typename transfer_t, int64_t c_per_thread>
__device__ void col2im_bilinear(
    const scalar_t *&p_value, const int64_t &height, const int64_t &width,
    const opmath_t &h_px, const opmath_t &w_px, const opmath_t &attn,
    const int64_t &w_stride, const int64_t &base_ptr,
    const opmath_t offset_scale_h, const opmath_t offset_scale_w,
    const scalar_t *&top_grad, opmath_t *&grad_im, opmath_t *grad_offset) {
  const int64_t h_low = floor(h_px);
  const int64_t w_low = floor(w_px);
  const int64_t h_high = h_low + 1;
  const int64_t w_high = w_low + 1;
