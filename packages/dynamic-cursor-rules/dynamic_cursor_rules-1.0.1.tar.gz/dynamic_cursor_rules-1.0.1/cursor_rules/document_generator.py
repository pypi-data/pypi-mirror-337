...
from .llm_connector import LLMProvider, LLMConnector
...
class DocumentGenerator:
    ...
    def generate_all_documents(self) -> Dict[str, str]:
        ...
        docs_dir = os.path.join(os.getcwd(), "documentation")
        ...
        # Generate the .cursorrules file (always at root)
    ...
    def _generate_cursorrules(self) -> str:
        ...
        """
        # Create cursor directory at the current working directory (root)
        ...
        # Generate and save the .cursorrules file with the selected provider
        ...
        try:
            ...
            if not api_key:
                # Fallback to default behavior if no API key is available
                save_cursorrules_file(ruleset, cursorrules_path, project_info)
            else:
                # Use the selected provider
                content = generate_cursorrules_file(
                    ruleset, 
                    project_info, 
            ...
            print(f"Warning: Could not use {self.llm_provider.value} provider: {e}")
            save_cursorrules_file(ruleset, cursorrules_path, project_info)
