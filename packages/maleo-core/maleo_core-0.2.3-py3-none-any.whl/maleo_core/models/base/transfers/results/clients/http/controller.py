...
class BaseHTTPClientControllerResults(BaseModel):
    ...
    status_code:Optional[int] = Field(None, description="HTTP Client's response status code")
    content:Any = Field(None, description="HTTP Client's response content")
    success:Optional[bool] = Field(None, description="HTTP Client's success status")
