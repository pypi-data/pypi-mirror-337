...
class NotifyItem:
    ...
    def get_timestamp(self) -> int:
        """
        uint64; // Filetime: Contains a 64-bit value representing the number of 
...
def to_datetime(filetime: int) -> datetime.datetime:
    ...
    return datetime.datetime.fromtimestamp(
        (filetime - EPOCH_AS_FILETIME) / HUNDREDS_OF_NANOSECONDS, tz=datetime.timezone.utc)
...
"""
    ...
    It is usually called in the interval given by the publishInterval 
    which has been set by the creation of the subscription.
    The callback may not contain all nodes of the subscription.I.e.when a node did not change.
    The callback may contain a node multiple times.I.e.when the node did 
    change multiple times since the last callback.
