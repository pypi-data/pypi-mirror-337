...
# 返回是不是开发者
def isDeveloper() -> bool:
    if os.path.isdir(get_temp_path() + '/developer'):
        return True
    return False
...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        if   isDeveloper():
            ...
            # 安装PyPi文件
            print('正在安装.pyi文件')
            source_dir = os.path.join(os.path.dirname(__file__), "stubs", "ntsulib")
            target_dir = os.path.join(self.install_lib, "ntsulib")
            print(f'install_lib = {self.install_lib}   source_dir = {source_dir.__str__()} target_dir = {target_dir.__str__()}')
...
setup_params = {
    ...
    "url": "",
    "packages": find_packages(exclude=["测试", "命令","注意","developer"]),
    "package_data": {
