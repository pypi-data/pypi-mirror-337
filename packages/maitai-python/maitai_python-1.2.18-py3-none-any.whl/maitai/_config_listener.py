...
class ConfigListener:
    def __init__(self, config, path, type, key=None):
        ...
        self.ws_thread = None
        self.running = False
        self.logger = logging.getLogger("maitai")
        ...
        if event.get("event_type") == "APPLICATION_CONFIG_CHANGE":
            ...
            if application_json:
                try:
                    ...
                    self.logger.info(
                        "Maitai config listener: received configuration change"
                    )
    ...
    def on_error(self, ws, error):
        if not isinstance(error, (AttributeError, ConnectionError)):
            self.logger.error("Maitai config listener: websocket error", exc_info=error)
    ...
    def on_open(self, ws):
        self.logger.info("Maitai config listener: websocket connection established")
