...
from policy_tool.core.snowflake_connection_setup import \
    load_snowflake_credentials
from policy_tool.services.azure_devops_service import (
    create_pr_comment, parse_comment_policy_assignments_to_be_ignored,
    parse_comment_warning_removal_policy_assignments)
from policy_tool.services.policy_assignment_comparison_service import \
    PolicyAssignmentComparisonService
from policy_tool.services.policy_configuration_service import \
    PolicyConfigService
...
from policy_tool.services.policy_validation_service import \
...
class PolicyToolParams:
    ...
    policy_assignments_sql_folder_name: str = "policy_assignments_sql_statements/"
...
def execute_policy_assignments(
    ...
    """
    try:
        timestamp = datetime.datetime.now(ZoneInfo("Europe/Paris")).strftime(
            "%Y_%m_%d__%H_%M_%S"
        )
            ...
            for environment, database in project_config["ENVIRONMENTS"].items():
                if (
                    ...
                    with SnowClient(snowflake_configuration) as snow_client:
                        ...
                        if output_sql_statements and save_sql_statements_in_history:
                            # Pull latest History changes
                            log.info(log_break)
                            logger.info("PULL LATEST HISTORY CHANGES FROM GIT")

                            subprocess.run(["git", "fetch", "origin", policy_config.branch_name])
                        ...
                        if output_sql_statements:
                            policy_assignments_sql_folder_structure = f"{PolicyToolParams.policy_assignments_sql_folder_name}/{database}"

                            filepath_sql_statements = save_file(
                                output_path,
                                output_folder_name=policy_assignments_sql_folder_structure,
                                output_file_name=f"policy_assignments_sql_statements_{project}__{database}__{timestamp}.sql",
                            ...
                            if save_sql_statements_in_history:
                                ...
                                subprocess.run(
                                    [
                                        "git",
                                        "add",
                                        f"policies/{policy_config.snowflake_account_name}/{project}/policy_assignments_history/{policy_assignments_sql_folder_structure}/",
                                    ]
                                )
                                subprocess.run(
                                    [
                                        ...
                                        "commit",
                                        "-m",
                                        f"Latest Policy Assignments History Update from the {policy_config.pipeline_name} Pipeline with ID {policy_config.pipeline_id}",
                                    ]
                                )
                                subprocess.run(
                                    [
                                        ...
                                        "-u",
                                        "origin",
                                        f"HEAD:{policy_config.branch_name}",
                                    ]
                                )
                                log.info(log_break)
                        ...
                        if output_policy_assignments_to_be_ignored:
                            filepath_policy_assignments_to_be_ignored = save_file(
                                ...
                                output_file_name=f"policy_assignments_to_be_ignored_{project}__{database}__{timestamp}.json",
                            ...
                            if policy_config.build_reason == "PullRequest":
                                ...
                                comment_warning_removal_policy_assignments = parse_comment_warning_removal_policy_assignments(
                                    policy_assignment_comparison_service.warning_removal_of_policy_assignment
                                )
                                comment_markdown = (
            ...
            for project in all_fetched_policy_assignments:
                for database in all_fetched_policy_assignments[project]:
                    ...
                    filepath_policy_assignments = os.path.join(
                        output_folder_path,
                        f"fetched_policy_assignments_{project}__{database}__{timestamp}.json",
                    )
                    logging.info(
