...
class EditTool(BaseTool):
    ...
    def execute(self, file_path, old_string=None, new_string=None):
        ...
        Args:
            ...
            new_string: Text to replace it with (optional when creating new file)
        """
        try:
            path = os.path.expanduser(file_path)
            
            # Handle case where only file_path is provided - create empty file
            ...
            # Handle case where only file_path and new_string are provided - create file with content
            if old_string is None and new_string is not None:
                try:
                    # Create directory if it doesn't exist
                    directory = os.path.dirname(path)
                    if directory and not os.path.exists(directory):
            ...
            if old_string == "" or old_string is None:
                try:
                    ...
                    # Create the file with content or empty if new_string is None
                    with open(path, 'w') as f:
