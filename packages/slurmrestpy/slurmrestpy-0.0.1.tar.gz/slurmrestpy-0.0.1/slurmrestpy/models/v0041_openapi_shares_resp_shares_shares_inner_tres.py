...
class V0041OpenapiSharesRespSharesSharesInnerTres(BaseModel):
    ...
    run_seconds: Optional[
        List[V0041OpenapiSharesRespSharesSharesInnerTresRunSecondsInner]
    ] = Field(
        default=None, description="Currently running tres-secs = grp_used_tres_run_secs"
    )
    group_minutes: Optional[
        List[V0041OpenapiSharesRespSharesSharesInnerTresRunSecondsInner]
    ] = Field(default=None, description="TRES-minute limit")
    usage: Optional[List[V0041OpenapiSharesRespSharesSharesInnerTresUsageInner]] = (
        Field(default=None, description="Measure of each TRES usage")
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "run_seconds": [
                    V0041OpenapiSharesRespSharesSharesInnerTresRunSecondsInner.from_dict(
                        _item
                    )
                    for _item in obj["run_seconds"]
                ]
                if obj.get("run_seconds") is not None
                else None,
                "group_minutes": [
                    V0041OpenapiSharesRespSharesSharesInnerTresRunSecondsInner.from_dict(
                        _item
                    )
                    for _item in obj["group_minutes"]
                ]
                if obj.get("group_minutes") is not None
                else None,
                "usage": [
                    V0041OpenapiSharesRespSharesSharesInnerTresUsageInner.from_dict(
                        _item
                    )
                    for _item in obj["usage"]
                ]
