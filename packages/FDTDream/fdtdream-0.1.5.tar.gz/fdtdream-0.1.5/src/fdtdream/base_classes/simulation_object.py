...
from typing import Any, List, TypeVar, Type, Unpack, Self
...
from ..resources.functions import process_type, convert_length
...
class SimulationObject(SimulationObjectInterface, ABC):
    ...
    def _get_position(self, absolute: bool = False) -> NDArray[np.float64]:
        ...
        if absolute and self._get("use relative coordinates", bool):
            for parent in self._parents:
                position += parent._get_position(absolute=False)
    ...
    def place_next_to(self, other_object: SimulationObject, side: EXTREMITIES,
        ...
        # Fetch the coordinate of the specified side
        coordinate = getattr(other_object, side[2:])(side[0])

        # Fetch the effective span of this object
        span = self.span(side[0])

        # Assign new coordinates
        if side[2:] == "max":
            setattr(self, side[0], coordinate + (span / 2) + offset)
        else:
            setattr(self, side[0], coordinate - (span / 2) + offset)
