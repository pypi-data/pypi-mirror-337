...
class WebSocket:
    __web_sockets_assemble: Assemble[str, object] = None
    ...
    def __start_server(cls, host, port, other_cls):
        cls.__web_sockets_assemble = HashAssemble()
    ...
    def WebSocketServer(cls, host: str, port: int):
        ...
        def decorator_handler(other_cls) -> callable:
            draw_websocket_banner()
            cls.__start_server(host, port, other_cls)

            def get_websocket_instance(*args, **kwargs) -> object:
                instance = cls.__web_sockets_assemble.get(f"{host}:{port}")
                if instance is None:
                    instance = other_cls(*args, **kwargs)
                    cls.__web_sockets_assemble.put(f"{host}:{port}", instance)
                    return instance

            get_websocket_instance.__decorator__ = "WebSocketServer"
            get_websocket_instance.__decorator_params__ = {
                "host": host,
                "port": port
            }
            return get_websocket_instance
