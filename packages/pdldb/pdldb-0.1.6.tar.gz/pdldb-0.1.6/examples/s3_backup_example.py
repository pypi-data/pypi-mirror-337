import os
import shutil
from pdldb import S3BackupManager
from examples.example_utils.stopwatch import stopwatch


@stopwatch
def initialize_backup_manager():
    print("Initializing S3 Backup Manager...")
    # Replace with your actual S3 bucket name or use environment variables
    manager = S3BackupManager(
        bucket_name=os.environ.get("S3_BUCKET_NAME"),
        aws_region=os.environ.get("AWS_REGION"),
        prefix="file_backup_example/",
    )
...
def list_available_backups(manager):
    ...
    for backup in backups:
        if backup["type"] == "mirror":
            print(
                f"  - {backup['name']} (Type: {backup['type']}, Created: {backup['created_at']}, Source Directory: {backup.get('source_directory', 'unknown')})"
            )
        else:
            print(
...
def create_mirror_backup(manager, source_path):
    print("Creating Mirror backup (S3 mirror)...")
...
@stopwatch
def restore_full_backup(manager, backup_name, restore_dir):
...
if __name__ == "__main__":
    print("=== S3 BACKUP MANAGER EXAMPLE WORKFLOW ===")
    print("This example demonstrates the full lifecycle of backups using BackupManager")

    base_dir = "examples/example_data/s3_backup_example"
    ...
    try:
        ...
        restore_full_backup(manager, mirror_backup_name, restore_dir_mir_pre_mod)

        print("\n5. Modify Data")
        modify_test_data(data_dir)

        print("\n6. Mirror Backup (S3 Mirror)")
        mirror_backup_name = create_mirror_backup(manager, data_dir)
        list_available_backups(manager)

        print("\n7. Restore Full Backup")
        restore_full_backup(manager, full_backup_name, restore_dir_full)

        print("\n8. Restore Mirror Backup")
        restore_full_backup(manager, mirror_backup_name, restore_dir_mir_post_mod)
