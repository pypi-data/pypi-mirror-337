...
class DocumentGenerator:
    ...
    def _generate_action_items(self, output_dir: str) -> Tuple[str, str]:
        """Generate action items document from the PRD content."""
        tasks_json_path = os.path.join(output_dir, "action_items.json")
        tasks_md_path = os.path.join(output_dir, "action_items.md")
        
        try:
            # Get the PRD content (or fallback to init doc if PRD failed, though 
            # generate_all_docs should prevent this now)
            content_to_parse = self._get_content_for_generation()
            
            # Parse the document to get the action plan
            # Relies on task_parser module
            action_plan, _ = parse_initialization_doc(content_to_parse)
            
            # Check if action_plan generation was successful
            if not action_plan or not action_plan.phases:
                 # Log an error and raise, indicating failure to extract tasks
                 logger.error("Failed to parse a valid action plan from the document content.")
                 raise ValueError("Failed to parse action items from the document content.")
            
            # Save the action plan to JSON
            action_plan.save_to_json(tasks_json_path)
            
            # Save the action plan to Markdown
            action_plan.save_to_markdown(tasks_md_path)
            ...
            # Raise a more specific error if possible, otherwise re-raise
            raise RuntimeError(f"Failed to generate action items: {e}") from e
            
    # No explicit template fallback was present here, but the check ensures success.