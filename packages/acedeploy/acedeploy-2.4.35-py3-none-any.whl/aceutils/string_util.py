...
def add_copy_grants(view_ddl: str) -> str:
    ...
    regex_copy_grants = r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:SECURE\s+)?(?:RECURSIVE\s+)?VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\"\.)|(\w+\.))?\s*(?P<schema>(\"\w+\")|(\w+\.))?\s*(?P<name>(\"\w+\")|(\w+)\b)(?!\.)\s+(?:COPY\s+GRANTS\s+)"
    ...
    if match_copy_grants:
        ...
        # add COPY GRANTS
        regex_view = r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:SECURE\s+)?(?:RECURSIVE\s+)?VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\"\.)|(\w+\.))?\s*(?P<schema>(\"\w+\")|(\w+\.))?\s*(?P<name>(\"\w+\")|(\w+)\b)(?!\.)"
