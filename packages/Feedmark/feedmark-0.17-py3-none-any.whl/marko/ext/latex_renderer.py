...
class LatexRendererMixin:
    ...
    def render_asterisk_emphasis(self, element):
        return self.render_emphasis(element)

    def render_underscore_emphasis(self, element):
        return self.render_emphasis(element)
    ...
    def render_link_ref(self, element):
        if element.title:
            _logger.warning("Setting a title for links is not supported!")
        (element.dest, _) = self.root_node.link_ref_defs.get(element.key)
        body = self.render_children(element)
        return f"\\href{{{element.dest}}}{{{body}}}"
    ...
    def render_image_ref(self, element):
        self._packages.add("graphicx")
        # TODO: check how alt (element.children) and element.title could be used!
        (element.dest, _) = self.root_node.link_ref_defs.get(element.key)
        return f"\\includegraphics{{{element.dest}}}"
