...
def test(filename: str) -> None:
    ...
    print("\n:hammer: Fixing paths\n")
    apibase_fixed = False
    if os.path.exists(os.path.join("test", "ema", "ema-app", "libraries", "PhotoService.ts")):
        with open(os.path.join("test", "ema", "ema-app", "libraries", "PhotoService.ts")) as in_f:
            lines = in_f.readlines()
        with open(
            os.path.join("test", "ema", "ema-app", "libraries", "PhotoService.ts"),
            "w",
        ) as out_f:
            for line in lines:
