...
class xy_db:
	...
	def make_2d_sample_data_in_excel(self):
		...
		self.xexcel.new_sheet()
		for index in range(1, y_len + 1):
			y_title_list.append("title_" + str(index))

		self.xexcel.write_list_1d_from_cell("", [1, 2], y_title_list)
		for no1 in range(1, x_len + 1):
			for no2 in range(1, y_len + 1):
				self.xexcel.write_value_in_cell("", [no1 + 1, no2 + 1], no1 * 10 + no2)

		x_title_list = []
		for index in range(1, x_len + 1):
			x_title_list.append("줄_" + str(index))
		self.xexcel.write_list_1d_from_cell_as_yline("", [2, 1], x_title_list)
	...
	def write_df_to_excel(self, input_df, xy=[1, 1]):
		...
		col_list = input_df.columns.values.tolist()
		value_list = input_df.values.tolist()
		self.xexcel.write_list_1d_from_cell_as_yline("", xy, col_list)
		self.xexcel.write_value_in_range_as_speedy("", [xy[0] + 1, xy[1]], value_list)
