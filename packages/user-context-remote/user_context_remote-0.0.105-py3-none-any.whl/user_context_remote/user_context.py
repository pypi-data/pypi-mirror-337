...
class UserContext(metaclass=GetterSetterGenerator):
    ...
    lang_code_str: str or None = None  # type: ignore
    ...
    def __authenticate_by_user_jwt(self, user_jwt: str) -> requests.Response:
        ...
        if authentication_login_validate_user_jwt_response.status_code != HTTPStatus.OK:
            logger.error(
                "user-context-remote-python-package _authenticate_by_user_jwt() authentication_login_validate_user_jwt_response.status_code != HTTPStatus.OK " + authentication_login_validate_user_jwt_response.text)  # noqa
            # logger.end()
    ...
    def __retrieve_and_populate_user_data(self, authenticate_product_user_response: requests.Response) -> None:
        ...
        if not validate_user_jwt_data_dict["isSuccess"]:
            raise Exception(
                "Can't get data from authenticate_product_user_response: " + validate_user_jwt_data_dict["messageInternalEnglish"] +" the external message: " + validate_user_jwt_data_dict["messageExternalMl"]["en"])  # noqa
