...
class Database:
    ...
    def create_document(self, doc_id: Optional[str], collection_name: str, data: Dict[str, Any]) -> str:
        """
        Create a document in the database.
        
        Args:
            doc_id: Optional document ID. If None, a new ID will be generated
        ...
        """
        self.logger.info(f"Creating document in collection: {collection_name}")
        ...
        try:
            ...
            if self.db_type == DatabaseType.DYNAMODB:
                # For DynamoDB, find model class for the collection
                model_class = None
                ...
                if model_class is None:
                    # Create a temporary model for this collection
                    from pydantic import create_model
                    model_class = create_model(
                        f"Temp{collection_name.capitalize()}Model",
                        id=(str, ...),
                        __base__=BaseModel
                    )
                    self.register_model(model_class, collection_name)
                
                # Set ID if provided
                if doc_id:
                    document_data['id'] = doc_id
                
                # Handle indexed fields for boolean values
                if hasattr(self, '_indexed_fields'):
                    indexed_fields = self._indexed_fields.get(collection_name, [])
                    for field in indexed_fields:
                        if field in document_data and isinstance(document_data[field], bool):
                            document_data[field] = str(document_data[field]).lower()
                
                # Create model instance
                model_instance = model_class(**document_data)
                
                # Store in DynamoDB
                result_id = self._db_manager.put_item(model_instance)
                self.logger.info(f"Successfully created document with ID {result_id} in collection {collection_name}")
                return result_id
                
            elif self.db_type == DatabaseType.FIRESTORE:
                # For Firestore, use the create_document method
                return self._db_manager.create_document(doc_id, collection_name, document_data)
                
            elif self.db_type == DatabaseType.MONGODB:
                # For MongoDB, use the create_document method
                return self._db_manager.create_document(doc_id, collection_name, document_data)
            ...
            self.logger.error(f"Error creating document in collection {collection_name}: {e}")
            raise
