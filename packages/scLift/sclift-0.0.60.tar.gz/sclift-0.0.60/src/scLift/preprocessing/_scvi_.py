...
def poisson_vi(
    ...
    if "clusters" in adata.obs.columns:
        ul.log(__name__).warn("Due to the original inclusion of the `clusters` column, the original `cluster` column name has been changed to `clusters_x`.")
    ...
    try:
        ...
        ul.log(__name__).warn(f"UMAP error, your system does not support it, but it does not affect the process. Continue with execution: {e}")
    ...
    try:
        ...
        ul.log(__name__).warn(f"TSNE error, your system does not support it, but it does not affect the process. Continue with execution: {e}")
