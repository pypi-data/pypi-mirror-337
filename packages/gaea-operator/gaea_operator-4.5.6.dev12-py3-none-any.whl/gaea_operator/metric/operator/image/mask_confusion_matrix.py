...
@METRIC.register_module('mask_confusion_matrix')
class MaskConfusionMatrix(MetricOperator):
    """
    Confusion matrix of the evaluation.
    """
    metric_name = 'mask_confusion_matrix'

    def __init__(self, labels, conf_threshold: float = 0, iou_threshold: float = 0.5, **kwargs):
        super(MaskConfusionMatrix, self).__init__(num_classes=kwargs.get('num_classes', 2))
    ...
    def update(self, predictions: Dict[str, List[Dict]], references: Dict[str, List[Dict]]) -> None:
        ...
        img_ids = self._parse_dt_gt(dts['mask'], gts['mask'])
        ...
        for img_id in img_ids:
            ...
            if len(gt) == 0 and len(dt) == 0:
                ...
                gt_mask = [g['mask'] for g in gt]
                dt_mask = [d['mask'] for d in dt]
                ious = mask_utils.iou(dt_mask, gt_mask, iscrowd)
