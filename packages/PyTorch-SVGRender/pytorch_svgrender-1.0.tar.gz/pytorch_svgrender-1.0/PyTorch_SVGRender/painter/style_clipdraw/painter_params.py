...
class Painter(DiffVGState):
    ...
    @torch.no_grad()
    def render_scaled(self, scale_factor=4):
        """
        Scale the size of the rendered image
        """
        _shapes = self.shapes
        _shape_groups = self.shape_groups

        shapes_resized = copy.deepcopy(_shapes)
        for i in range(len(_shapes)):
            shapes_resized[i].stroke_width = _shapes[i].stroke_width * scale_factor
            for j in range(len(_shapes[i].points)):
                shapes_resized[i].points[j] = _shapes[i].points[j] * scale_factor

        # rescale the rendered image
        self.shapes = shapes_resized
        self.canvas_height = self.canvas_height * scale_factor
        self.canvas_width = self.canvas_width * scale_factor
...
class PainterOptimizer:
    ...
    def init_optimizers(self):
        points_vars, stroke_width_vars, color_vars = self.renderer.set_parameters()
        self.points_optimizer = torch.optim.Adam(points_vars, lr=self.points_lr)
        self.width_optimizer = torch.optim.Adam(stroke_width_vars, lr=self.width_lr)
