...
def main():
    ...
    def generate_code(table_name, columns, primary_key_column):
        ...
        with open(f"generated/domain/entity/{class_name}.java", "w", encoding="utf-8") as f:
            f.write(entity_code)

        # 写入 DTO 类 Java 文件，文件名是表名的驼峰格式
        with open(f"generated/domain/dto/{class_name}DTO.java", "w", encoding="utf-8") as f:
            f.write(dto_code)

        # 写入 VO 类 Java 文件，文件名是表名的驼峰格式
        with open(f"generated/domain/vo/{class_name}VO.java", "w", encoding="utf-8") as f:
            f.write(vo_code)

        # 写入 Controller 类 Java 文件，文件名是表名的驼峰格式
        with open(f"generated/controller/{class_name}Controller.java", "w", encoding="utf-8") as f:
            f.write(controller_code)

        # 写入 Service 类 Java 文件，文件名是表名的驼峰格式
        with open(f"generated/service/{class_name}Service.java", "w", encoding="utf-8") as f:
            f.write(service_code)

        # 写入 Mapper 类 Java 文件
        with open(f"generated/mapper/{class_name}Mapper.java", "w", encoding="utf-8") as f:
            f.write(mapper_code)
