...
class xanes_analysis_gui:
    ...
    def __init__(self, parent_h, form_sz=[650, 740]):
        ...
        self.ana_reg_data_path_in_h5 = None
        self.ana_proc_data_list = [""]

        self.ana_spec_list = []
        self.extra_viewer = "napari"
        self.viewer = None

        self.ana_spec_name = None
        self.ana_data = {}
        self.ana_spec = {}
        self.ana_mask = {}
        self.ana_mask_sngl_op = {}
        ...
        self.ana_mask_comb_op = {}
        self.ana_mask_comb = {}
        self.ana_sngl_mask_for_comb_mask = set()
    ...
    def restart(self):
        ...
        self.hs["AnaDataFn txt"].value = ""
        self.hs["AnaAvaiDatasets sel"].options = []
        self.hs["AnaDatasetInfo txt"].value = "shape: \nmin: \nmax: \nmean: "
        self.hs["AnaSelVars sel"].options = []
        self.hs["AnaVarsEq sel"].options = []
        self.hs["AnaVarName sel"].options = []
        self.hs["AnaSnglMaskVarName drpn"].options = []
        self.hs["AnaSnglMaskSpecName drpn"].options = []
        self.hs["AnaSnglMaskOpSeq sel"].options = []
        self.hs["AnaSnglMaskOpSeqZSli txt"].value = 0
        self.hs["AnaSnglMaskName sel"].options = []
        self.hs["AnaCombMaskVar drpn"].options = []
        self.hs["AnaCombMaskVarInfo txt"].value = ""
        self.hs["AnaCombMaskZSli txt"].value = 0
        self.hs["AnaCombMaskOpStps sel"].options = []
        self.hs["AnaCombMaskName sel"].options = []
        self.hs["AnaAnaSpec drpdn"].options = []
        self.hs["AnaAnaMask drpdn"].options = []
        self.hs["AnaAnaMaskInfo txt"].value = ""

        self.hs["AnaSnglMaskOp drpn"].value = "Threshold"
        self.hs["AnaSnglMaskOp drpn"].tooltip = xanes_analysis_gui.MASK_OP_PAR_DICT[
            "Threshold"
        ]["description"]
        for ii in range(4):
            for jj in range(2):
    ...
    def build_gui(self):
        ...
        self.hs['Ana form'].set_title(1, 'Make Mask')
        ...
        data_setup_GridSpecLayout[12, 82:98] = widgets.HTML(
            value='<b><p style="font-weight: bold; font-size: 125%; color: green; background-color: lightblue; text-align:center">Extra Viewer</p></b>',
            layout={"height": "90%", "width": "100%"},
        )

        data_setup_GridSpecLayout[13:15, 82:98] = widgets.Checkbox(
            description="napari",
            value=True,
        ...
        self.hs["AnaSelDatasetPrevExtOpt chb"] = data_setup_GridSpecLayout[13:15, 82:98]
        ...
        self.hs["AnaDataPrev box"].children = [
            ...
            self.hs["AnaSelDatasetPrevExtOpt chb"],
        ]
        ...
        self.hs["AnaSelDatasetPrevExtOpt chb"].observe(
            self.AnaSelDatasetPrevExtOpt_chb_chg, names="value"
        )
        ...
        #     },
        #     disabled=True)
        self.hs["AnaCombMasksClear btn"] = widgets.Button(
            description="Clear",
            layout={"width": "60%", "height": "6%", "left": "20%", "top": "2%"},
            disabled=True,
        )
        self.hs["AnaCombMasksClear btn"].style.button_color = "darkviolet"
        ...
        self.hs["AnaMask acc"].titles = ["Make A Single Mask", "Combine Masks"]
        ...
        self.AnaConfAnaData_GridSpecLayout[3, 49:51] = widgets.Text(
            value="x", disabled=True, layout={"width": "100%", "height": "100%"}
        )
        self.hs["AnaAnaMaskOp txt"] = self.AnaConfAnaData_GridSpecLayout[3, 49:51]
        ...
        self.hs["AnaAna acc"].titles = ["Config Data for Analysis", "Config Analysis"]
    ...
    def boxes_logic(self):
        def component_logic():
            if self.ana_data_cnfged and not self.ana_mask_sngl_op_seled:
                boxes = [
                ...
                enable_disable_boxes(self.hs, boxes, disabled=True, level=-1)
            elif self.ana_data_cnfged and self.ana_mask_sngl_op_seled:
                boxes = [
                    "AnaSnglMaskOpSeq box",
                    "AnaSnglMaskAddName btn",
                    "AnaSnglMaskRmName btn",
            ...
            if (
                self.ana_data_cnfged
                and self.ana_mask_sngl_op_seled
                and len(self.hs["AnaSnglMaskName sel"].options) == 0
            ):
                ...
                enable_disable_boxes(self.hs, boxes, disabled=False, level=-1)
            if (
                self.ana_mask_sngl_cnfged
                and len(self.hs["AnaCombMaskOpStps sel"].options) == 0
            ):
                boxes = [
        ...
        if (not self.ana_fn_seled) or (not self.ana_data_existed):
            ...
            boxes = ["AnaDataFn btn"]
            enable_disable_boxes(self.hs, boxes, disabled=False, level=-1)
        elif self.ana_fn_seled and self.ana_data_existed and not self.ana_data_cnfged:
            ...
            self.ana_fn_seled
            and self.ana_data_existed
            and self.ana_data_cnfged
            and not self.ana_mask_sngl_cnfged
        ):
            boxes = ["AnaMask tab", "AnaDisp tab"]
            enable_disable_boxes(self.hs, boxes, disabled=True, level=-1)
            boxes = [
            ...
            and self.ana_data_existed
            and self.ana_data_cnfged
            and self.ana_mask_sngl_cnfged
            and not self.ana_mask_cnfged
        ):
            boxes = ["AnaDisp tab"]
            enable_disable_boxes(self.hs, boxes, disabled=True, level=-1)
            ...
            and self.ana_data_existed
            and self.ana_data_cnfged
            and self.ana_mask_cnfged
            and not self.ana_proc_done
        ):
            boxes = ["AnaDisp tab"]
            enable_disable_boxes(self.hs, boxes, disabled=True, level=-1)
            boxes = ["AnaData tab", "AnaMask tab", "AnaConf tab"]
    ...
    def AnaDataFn_btn_clk(self, a):
        ...
        if len(a.files[0]) != 0:
            ...
            with h5py.File(self.ana_fn, "r") as f:
                ...
                for key in f.keys():
                    if "processed_XANES" in key:
                        self.ana_proc_spec_path_in_h5 = f"/{key}/proc_spectrum"
                        self.ana_proc_data_list = list(
                            f[self.ana_proc_spec_path_in_h5].keys()
                        )
                    if "registration_results" in key:
                        if "registered_xanes3D" in f[f"/{key}/reg_results/"].keys():
                            self.ana_reg_data_path_in_h5 = (
                                f"/{key}/reg_results/registered_xanes3D"
                            )
                            self.ana_reg_data_anchor_id = list(
                                f["/registration_results/reg_parameters/scan_ids"][:]
                            ).index(
                                f["/registration_results/reg_parameters/fixed_scan_id"][
                                    ()
                                ]
                            )
                        else:
                            self.ana_reg_data_path_in_h5 = (
                                f"/{key}/reg_results/registered_xanes2D"
                            )
                            self.ana_reg_data_anchor_id = int(
                                f["/registration_results/reg_parameters/fixed_scan_id"][
                                    ()
                                ]
                            )
                if self.ana_reg_data_path_in_h5 is not None:
                    self.ana_proc_data_list.append("registered_xanes")
                if len(self.ana_proc_data_list) != 0:
            ...
            if self.ana_data_existed:
                ...
                for key in self.ana_proc_data_list:
                    if key == "registered_xanes":
                        self.ana_data[key] = h5_lazy_reader(
                            self.ana_fn,
                            self.ana_reg_data_path_in_h5,
                            np.s_[self.ana_reg_data_anchor_id, :],
                        )
                    else:
                        self.ana_data[key] = h5_lazy_reader(
                            self.ana_fn,
            ...
            if len(self.ana_spec_list) != 0:
                ...
                self.ana_spec_name = self.hs["AnaSnglMaskSpecName drpn"].value
            else:
                self.ana_spec_name = None
    ...
    def AnaSelDatasetPrev_btn_clk(self, a):
        ...
        fiji_viewer_off(
            self.parent_h.global_h, gui_h=self, viewer_name="xanes_pp_data_prev_viewer"
        )
        fiji_viewer_on(
            self.parent_h.global_h, gui_h=self, viewer_name="xanes_pp_data_prev_viewer"
        )
        self.parent_h.global_h.ij.py.run_macro("""run("Threshold...")""")
        self.boxes_logic()

    def AnaSelDatasetPrevExtOpt_chb_chg(self, a):
        self.extra_viewer = a["owner"].value

    def AnaSnglMaskVarName_drpn_chg(self, a):
        if self.ana_data_cnfged:
            self.ana_mask_sngl_op = {}
            self.hs["AnaSnglMaskOpSeq sel"].options = []
    ...
    def AnaSnglMaskRmName_btn_clk(self, a):
        ...
        if mk not in self.ana_sngl_mask_for_comb_mask:
            if mk:
            ...
            print(
                "The selected mask has been used in making combinational masks. You need to remove the combinational mask first before removing this mask."
            )
        self.boxes_logic()
    ...
    def AnaSnglMaskNameCfm_btn_clk(self, a):
        ...
        if len(self.hs["AnaSnglMaskName sel"].options) != 0:
            ...
            self.hs["AnaMask acc"].selected_index = 1
        else:
            self.ana_mask_sngl_cnfged = False
    ...
    def AnaCombMaskPrev_btn_clk(self, a):
        ...
        if self.ana_spec_name is None:
            print(f"There is no spectrum data available.")
        else:
    ...
    def AnaCombMaskVarAdd_btn_clk(self, a):
        ...
        self.ana_sngl_mask_for_comb_mask.add(mk)
    ...
    def AnaCombMaskVarRm_btn_clk(self, a):
        ...
        if (idx is not None) and (idx != 0):
            ...
            op_list.remove(rm_op)
            if "complement" in rm_op:
                op_list[0] = op_list.strip("[")
            else:
                mk_short = rm_op.split(" ")[-1]
                for ii in self.hs["AnaCombMaskVar drpn"].options:
                    if mk_short in ii:
                        mk = ii
                self.ana_sngl_mask_for_comb_mask.remove(mk)
        ...
        elif (len(self.hs["AnaCombMaskOpStps sel"].options) != 1) and (idx == 0):
    ...
    def AnaCombMasksClear_btn_clk(self, a):
        self.hs["AnaCombMaskOpStps sel"].options = []
        self.hs["AnaCombMaskOp drpn"].options = ["", "Complement (I-A)"]
        self.hs["AnaCombMaskOp drpn"].value = ""
        self.ana_mask_comb_op_seled = False
    ...
    def AnaMaskSav_btn_clk(self, a):
            ...
            if self.parent_h.gui_name == "xanes2D":
                if "processed_XANES2D" in f:
                    ...
                    for mask_name in self.ana_mask.keys():
                        ...
                        gm1.create_dataset(
                            mask_name,
                            data=self.ana_mask[mask_name]["mask"]
                            .compute()
                            .astype(np.int8),
                            dtype=np.int8,
                        )
                    ...
                    if "gen_masks" in f["processed_XANES"].keys():
                        del f["processed_XANES/gen_masks"]
                    gm0 = f["processed_XANES"].create_group("gen_masks")
                    for mask_name in self.ana_mask.keys():
                        if mask_name in gm0.keys():
                            del gm0[mask_name]
                        gm1 = gm0.create_group(mask_name)
                        ...
                        dicttoh5(
                            ...
                            mode="a",
                            update_mode="replace",
                            h5path=f"/processed_XANES/gen_masks/{mask_name}/source",
                        )
                ...
                if "processed_XANES3D" in f:
                    ...
                    if "gen_masks" not in f["processed_XANES"].keys():
                        gm0 = f["processed_XANES"].create_group("gen_masks")
                    else:
                        gm0 = f["processed_XANES/gen_masks"]
                    for mask_name in self.ana_mask.keys():
                        if mask_name in gm0.keys():
                            del gm0[mask_name]
                        gm1 = gm0.create_group(mask_name)
        ...
        self.hs["Ana form"].selected_index = 2
        self.hs["AnaAna acc"].selected_index = 0
        self.boxes_logic()
