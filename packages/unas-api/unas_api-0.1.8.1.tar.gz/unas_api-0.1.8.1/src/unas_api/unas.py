...
class Product:
    ...
    class Status:
        def __init__(self, type: StatusType, value: StatusValue, name: Optional[str]):
            self.type = type
            self.value = value
            self.name = name
    ...
    def set_product(self, product: Product):
        payload = f'''<?xml version="1.0" encoding="UTF-8" ?><Products>
            <Product>
                ...
                {f"""
                <Statuses>
                    {''.join(f'<Status><Type><![CDATA[{status.type}]]></Type><Value><![CDATA[{status.value}]]></Value>{f'''<Name>{status.name}</Name>''' if status.name else ''}</Status>' for status in product.statuses)}
                </Statuses>
