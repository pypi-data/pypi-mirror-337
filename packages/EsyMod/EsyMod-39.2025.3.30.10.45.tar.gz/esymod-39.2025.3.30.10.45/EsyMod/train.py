...
class TrainningProcess:
    ...
    def init_dataset(self, record:dict=None):
        ...
        total_datest = Dataset([torch.randn(100, 10), torch.randint(0, 10, (100,))])
        trainset, valset = total_datest.k_split(5)[self.val_index]

        torch.save((total_datest, trainset, valset), self.result_dir.cat('dataset.pth'))
        
        train_loader = trainset.get_loader(64, num_workers=4, shuffle=True)
        val_loader = valset.get_loader(16, num_workers=4, shuffle=True)
        
        if record is not None:
            record['trainset'] = trainset
            record['valset'] = valset
            record['total_datest'] = total_datest
            record['train_loader'] = train_loader
            record['val_loader'] = val_loader
            
        return total_datest, trainset, valset, train_loader, val_loader
    ...
    def init_model(self, record:dict=None):
        ...
        if self.result_dir.cat('model_last.pth').exist():
            model = model.load_state_dict(torch.load(self.result_dir.cat('model_last.pth'), weights_only=True, map_location=self.device))
