...
class ModelConverter:
    ...
    async def to_create_training_job_input(
        ...
        image_arn = runtime_environment_parameters.sm_image
        image_version_alias = None
        # parse image_arn which looks like following: "arn:aws:sagemaker:us-west-2:354058284337:image/sagemaker-distribution-cpu-v0__SAGEMAKER_INTERNAL__0.14" extract the version 0.14 from at the end, __SAGEMAKER_INTERNAL__ is a separator
        if "__SAGEMAKER_INTERNAL__" in runtime_environment_parameters.sm_image:
            image_arn, image_version_alias = image_arn.split("__SAGEMAKER_INTERNAL__")
        else:
            image_arn = runtime_environment_parameters.sm_image
            image_version_alias = None

        training_image_metadata = await get_image_metadata(
            image_arn, get_region_name(), image_version_alias
        )
