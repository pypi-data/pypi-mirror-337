...
class xy_excel:
	...
	def __init__(self, filename="", cache_on_off=""):
		...
		self.xvar["setup"] = {}  # setup용 전용 입니다
		self.xvar["default"] = {}
		self.xvar["use_same_sheet"] = False
		self.xvar["r1c1"] = False

		self.xvar["pen_color"] = ""
		self.xvar["pen_style"] = 4
		self.xvar["pen_thickness"] = 5
		self.xvar["start_point_width"] = 2
		self.xvar["start_point_length"] = 2
		self.xvar["start_point_style"] = 1
		self.xvar["end_point_width"] = 2
		self.xvar["end_point_length"] = 2
		self.xvar["end_point_style"] = 1
	...
	def check_basic_data(self, sheet_name="", xyxy=""):
		...
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		range_obj = sheet_obj.Range(sheet_obj.Cells(x1, y1), sheet_obj.Cells(x2, y2))
		self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x2, y2])
	...
	def check_font_element(self, input_key):
		...
		:return:
		"""
		base_dic = self.xvar["check_font_para"]
		try:
	...
	def check_line_style(self, input_list):
		...
		for one in input_list:
			if type(one) == type([]) or type(one) == type(()):
				result["position"] = one
			elif one in self.xvar["check_line_position"].keys():
				result["position"] = self.xvar["check_line_position"][one]
			elif one in self.xvar["check_color_name"].keys():
				result["color"] = self.xvar["check_color_name"][one]
			elif one in self.xvar["check_line_style"].keys():
				result["line_style"] = self.xvar["check_line_style"][one]
			elif one in self.xvar["check_line_thickness"].keys():
				result["thickness"] = self.xvar["check_line_thickness"][one]
	...
	def check_line_style_as_dic(self, input_list):
		...
		for one in input_list:
			if one in self.xvar["check_line_thickness"].keys():
				result["thickness"] = self.xvar["check_line_thickness"][one]
			elif one in self.xvar["check_line_style"].keys():
				result["style"] = self.xvar["check_line_style"][one]
			elif one in self.xvar["check_line_position"].keys():
				result["area"] = self.xvar["check_line_position"][one]
	...
	def draw_line_in_range_by_auto(self,  sheet_name="", xyxy="", *input_list):
		...
		line_thickness_dic = self.xvar["check_line_thickness"]
		line_style_dic = self.xvar["check_line_style"]
		line_position_dic = self.xvar["check_line_position"]
			...
			for one in one_list:
				if one in list(line_thickness_dic.keys()):
					...
					try:
						...
						if result:
							if result[0][0] in self.xvar["check_color_name"]:
	...
	def draw_line_in_range_each_setup(self, sheet_name, xyxy, position=[7, 8, 9, 10], line_style="-", thickness="t-1", scolor="bla"):
		...
		for one in position:
			if one in list(self.xvar["check_line_position"].keys()):
				checked_position.extend(self.xvar["check_line_position"][one])
				print(one, "있음  ", self.xvar["check_line_position"][one])
			else:
				checked_position.extend([one])


		checked_line_style = self.xvar["check_line_style"][line_style]
		checked_color = self.xcolor.change_scolor_to_rgbint(scolor)
		checked_thickness = self.xvar["check_line_thickness"][thickness]
	...
	def insert_chart_1(self, sheet_name, chart_type, pxywh, source_xyxy):
		...
		self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x2, y2])
		range_object = sheet_obj.Range(self.xvar["r1c1"])
	...
	def new_shape(self, sheet_name, xy="", size=[25, 25], shape_style="circle", input_scolor="", input_value=""):
		...
		if input_value:
			Shp1.TextFrame2.VerticalAnchor = self.xvar["shape_font"]["align_v"]
			Shp1.TextFrame2.HorizontalAnchor = self.xvar["shape_font"]["align_h"]
			Shp1.TextFrame2.TextRange.Font.Bold = self.xvar["shape_font"]["bold"]

			Shp1.TextFrame2.TextRange.Characters.Font.Fill.ForeColor.RGB = self.xvar["shape_font"]["color"]
			Shp1.TextFrame2.TextRange.Characters.Text = input_value
			Shp1.TextFrame2.TextRange.Characters.Font.Size = self.xvar["shape_font"]["size"]
	...
	def new_shape_as_circle_with_number(self, sheet_name, xy="", pwh=25, scolor="red", input_value=1, font_size=""):
		...
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
		if font_size: self.xvar["font"]["size"] = font_size
		...
		Shp1.TextFrame2.TextRange.Font.Bold = self.xvar["font"]["bold"]
		Shp1.TextFrame2.TextRange.Characters.Text = input_value
		Shp1.TextFrame2.TextRange.Characters.Font.Size = self.xvar["font"]["size"]
	...
	def new_shape_at_pxyxy(self, sheet_name, pxyxy, shape_no=1):
		...
		if type(shape_no) == type(123):
			pass
		elif shape_no in list(self.xvar["shape_enum"].keys()):
			shape_no = self.xvar["shape_enum"][shape_no]
	...
	def new_shape_box_for_same_size_with_xyxy(self, sheet_name, xyxy, line_color="bla", line_thickness="thin"):
		...
		try:
			thickness = self.xvar["line"]["check_line_style"][line_thickness]
		except:
	...
	def new_shape_for_number_circle_by_setup(self, sheet_name, xy="", input_no=1):
		...
		rgb_list_1d = self.xcolor.change_scolor_to_rgb(self.xvar["shape"]["color"])
		rgb_int = self.xcolor.change_rgb_to_rgbint(rgb_list_1d)

		Shp1 = sheet_obj.Shapes.AddShape(9, pxyxy[0], pxyxy[1], self.xvar["shape"]["width"],
											self.xvar["shape"]["height"])
		Shp1.Fill.ForeColor.RGB = rgb_int
		Shp1.TextFrame2.VerticalAnchor = self.xvar["shape_font"]["align_v"]
		Shp1.TextFrame2.HorizontalAnchor = self.xvar["shape_font"]["align_h"]

		Shp1.TextFrame2.TextRange.Font.Size = self.xvar["shape_font"]["size"]
		Shp1.TextFrame2.TextRange.Font.Bold = self.xvar["shape_font"]["bold"]
		Shp1.TextFrame2.TextRange.Font.Italic = self.xvar["shape_font"]["italic"]
		Shp1.TextFrame2.TextRange.Font.Name = self.xvar["shape_font"]["name"]

		Shp1.TextFrame2.TextRange.Font.Strikethrough = self.xvar["shape_font"]["strikethrough"]
		Shp1.TextFrame2.TextRange.Font.Subscript = self.xvar["shape_font"]["subscript"]
		Shp1.TextFrame2.TextRange.Font.Superscript = self.xvar["shape_font"]["superscript"]
		Shp1.TextFrame2.TextRange.Font.Alpha = self.xvar["shape_font"]["alpha"]
		Shp1.TextFrame2.TextRange.Font.Underline = self.xvar["shape_font"]["underline"]

		rgb2_list_1d = self.xcolor.change_scolor_to_rgb(self.xvar["shape_font"]["color"])
	...
	def new_shape_line_in_range(self, sheet_name, xyxy, scolor, thickness, line_style, transparency, head_dic="", tail_dic=""):
		...
		enum_line = self.xvar["end_style_vs_enum"]
		base_data = self.xvar["dic_base_cell_data"]
...
9		"""
	...
	def is_empty_yline(self, y_no):
		...
		result = self.xlbook.WorksheetFunction.CountA(self.xvar["sheet"].Columns(y1).EntireColumn)
		return result
	...
	def paint_cell_by_words(self, input_list):
		...
		for x in range(self.xvar["x1"], self.xvar["x2"] + 1):
			for y in range(self.xvar["y1"], self.xvar["y2"] + 1):
				cell_value = self.xvar["sheet"].Cells(x, y).Value
				temp_int = 0
	...
	def read_line_value(self, sheet_name, xyxy, position):
		...
		if type("abc") == type(position):
			position_list = self.xvar["check_line_position"][position]
		elif type([]) == type(position):
			for one in position:
				aa = self.xvar["check_line_position"][one]
	...
	def reset_basic_pen_setup(self):
		...
		self.xvar["pen_color"] = self.xcolor.change_scolor_to_rgbint("bla")
		self.xvar["pen_style"] = 4
		self.xvar["pen_thickness"] = 5
		self.xvar["start_point_width"] = 2
		self.xvar["start_point_length"] = 2
	...
	def set_font_for_shape(self, **input_dic):
		...
		self.xvar["shape_font"].update(input_dic)
		return self.xvar["shape_font"]
	...
	def set_font_in_part_of_cell_value(self, sheet_name, xy, from_to, input_list):
		...
		** 보관용
		...
		try:
			my_range.Font.Size = self.xvar["font"]["size"]
			my_range.Font.Bold = self.xvar["font"]["bold"]
			my_range.Font.Italic = self.xvar["font"]["italic"]
			my_range.Font.Name = self.xvar["font"]["name"]

			my_range.Font.Strikethrough = self.xvar["font"]["strikethrough"]
			my_range.Font.Subscript = self.xvar["font"]["subscript"]
			my_range.Font.Superscript = self.xvar["font"]["superscript"]
			my_range.Font.Underline = self.xvar["font"]["underline"]
			my_range.Font.Color = self.xvar["font"]["rgb_int"]
		except:
			pass
	...
	def set_font_in_range_with_setup(self, sheet_name, xyxy, input_list=[]):
		...
		range_obj.Font.Size = self.xvar["font"]["size"]
		range_obj.Font.Bold = self.xvar["font"]["bold"]
		range_obj.Font.Italic = self.xvar["font"]["italic"]
		range_obj.Font.Name = self.xvar["font"]["name"]

		range_obj.Font.Strikethrough = self.xvar["font"]["strikethrough"]
		range_obj.Font.Subscript = self.xvar["font"]["subscript"]
		range_obj.Font.Superscript = self.xvar["font"]["superscript"]
		range_obj.Font.Underline = self.xvar["font"]["underline"]
		range_obj.Font.Color = self.xvar["font"]["rgb_int"]
	...
	def set_formats_for_target_line(self, target_drawing):
		...
		"""
		target_drawing.DashStyle = self.xvar["pen_style"]
		target_drawing.ForeColor.RGB = self.xvar["pen_color"]
		target_drawing.Weight = self.xvar["pen_thickness"]
		target_drawing.BeginArrowheadLength = self.xvar["start_point_length"]
		target_drawing.BeginArrowheadStyle = self.xvar["start_point_style"]
		target_drawing.BeginArrowheadWidth = self.xvar["start_point_width"]
		target_drawing.EndArrowheadLength = self.xvar["end_point_length"]
		target_drawing.EndArrowheadStyle = self.xvar["end_point_style"]
		target_drawing.EndArrowheadWidth = self.xvar["end_point_width"]
	...
	def set_numberproperty_in_range(self, sheet_name, xyxy, type1):
		...
		self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x2, y2])
		range_obj = sheet_obj.Range(self.xvar["r1c1"])
	...
	def set_pattern_in_range(self, sheet_name, xyxy, input_list=[]):
		...
		if input_list:
			...
			if type(input_list) == type([]):
				...
				# 만약 사전 형식이면, 기존에 저장된 자료로 생각하고 update한다
				self.xvar["pattern"].update(input_list)
		a = 2
		if a == 1:
			range_obj.Interior.Color = 5296274
			range_obj.Interior.Pattern = self.xvar["range_color"]["pattern"]
			range_obj.Interior.PatternColor = self.xvar["range_color"]["pattern"]
			...
			range_obj.Interior.Color = 5296274
			range_obj.Interior.Pattern = self.xvar["range_color"]["pattern"]  # xlSolid
			range_obj.Interior.PatternColor = self.xvar["range_color"][
	...
	def set_pen_color_style_thickness(self, scolor="bla", style="", thickness=5):
		...
		self.xvar["pen_color"] = self.xcolor.change_scolor_to_rgbint(scolor)
		self.xvar["pen_style"] = style
		self.xvar["pen_thickness"] = thickness
	...
	def set_pen_end_style(self, length=2, style=1, width=2):
		...
		:param width:
		"""
		self.xvar["end_point_length"] = length  # 2-default, 3-long, 1-short
		self.xvar["end_point_style"] = style  # 1-없음,2-삼각형,3-얇은화살촉,4-화살촉,5-다이아몬드,6-둥근
		self.xvar["end_point_width"] = width  # 2-default, 3-넓은, 1-좁은
	...
	def set_pen_start_style(self, length=2, style=1, width=2):
		...
		:param width:
		"""
		self.xvar["start_point_length"] = length  # 2-default, 3-long, 1-short
		self.xvar["start_point_style"] = style  # 1-없음,2-삼각형,3-얇은화살촉,4-화살촉,5-다이아몬드,6-둥근
		self.xvar["start_point_width"] = width  # 2-default, 3-넓은, 1-좁은
	...
	def setup_font(self, input_list):
		...
		for one in input_list:
			if one in ["진하게", "굵게", "찐하게", "bold"]: self.xvar["font"]["bold"] = True
			if one in ["italic", "이태리", "이태리체", "기울기"]: self.xvar["font"]["italic"] = True
			if one in ["strikethrough", "취소선", "통과선", "strike"]: self.xvar["font"]["strikethrough"] = True
			if one in ["subscript", "하위첨자", "밑첨자"]: self.xvar["font"]["subscript"] = True
			if one in ["superscript", "위첨자", "웃첨자"]: self.xvar["font"]["superscript"] = True
			if one in ["underline", "밑줄"]: self.xvar["font"]["underline"] = True
			if one in ["vertical", "수직", "가운데"]: self.xvar["font"]["align_v"] = 3
			if one in ["horizental", "수평", "중간"]: self.xvar["font"]["align_h"] = 2

			try:
				self.xvar["font"]["size"] = int(one)
			except:
				pass
			...
			try:
				...
				if result:
					if result[0][0] in self.xvar["check_color_name"]:
						self.xvar["font"]["rgb_int"] = self.xcolor.change_scolor_to_rgbint(one)
			except:
				pass
	...
	def setup_font_basic(self):
		...
		# 폰트 설정의 모든것을 초기화 하는것
		# self.xvar["font"]["background"] = None
		# self.xvar["font"]["colorindex"] = 1
		# self.xvar["font"]["creator"] = None
		# self.xvar["font"]["style"] = None
		# self.xvar["font"]["themecolor"] = None
		# self.xvar["font"]["themefont"] = None
		# self.xvar["font"]["tintandshade"] = None
	...
	def setup_for_shape_line_head(self, head_style, head_h, head_w):
		...
		enum_line = self.xvar["end_style_vs_enum"]
		base_data = self.xvar["dic_base_cell_data"]
		self.xvar["setup"]["line"]["head_style"] = enum_line[base_data["head_style"]]
		self.xvar["setup"]["line"]["head_h"] = enum_line[base_data["head_h"]]
		self.xvar["setup"]["line"]["head_w"] = enum_line[base_data["head_w"]]
	...
	def setup_for_shape_line_tail(self, tail_style, tail_h, tail_w):
		...
		enum_line = self.xvar["end_style_vs_enum"]
		base_data = self.xvar["dic_base_cell_data"]
		self.xvar["setup"]["line"]["tail_style"] = enum_line[base_data["tail_style"]]
		self.xvar["setup"]["line"]["tail_h"] = enum_line[base_data["tail_h"]]
		self.xvar["setup"]["line"]["tail_w"] = enum_line[base_data["tail_w"]]
	...
	def write_value_in_cell_for_speed(self, xy, value):
		...
		if type(value) == type("abc"):
			...
			changed_value = value
		self.xvar["sheet"].Cells(xy[0], xy[1]).Value = changed_value
	...
	def write_value_in_range(self, sheet_name, xyxy, input_any_type):
		...
		for index, list_1d in enumerate(checked_list_2d):
			self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1 + index, y1, x1 + index, y1 + len(list_1d) - 1])
			sheet_obj.Range(self.xvar["r1c1"]).Value = list_1d
		self.set_screen_update_on()
	...
	def write_value_in_range_by_range_priority(self, sheet_name, xyxy, input_value):
		...
		for index, list_1d in enumerate(list_2d):
			if index >= x2 - x1 + 1:
				...
				if len(list_1d) > y2 - y1 + 1:
					self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1 + index, y1, x1 + index, y2])
					sheet_obj.Range(self.xvar["r1c1"]).Value = list_1d[:y2 - y1 + 1]
				else:
					self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1 + index, y1, x1 + index, y1 + len(list_1d) - 1])
					sheet_obj.Range(self.xvar["r1c1"]).Value = list_1d
	...
	def write_value_in_range_for_speed(self, sheet_name, xyxy, input_list_2d):
		...
		if x1 == x2 and y1 == y2:
			...
			self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x1 + len(input_list_2d) - 1, y1 + len(input_list_2d[0]) - 1])
			sheet_obj.Range(self.xvar["r1c1"]).Value = changed_datas
		else:
			# 영역을 선택하면, 두 영역중에 작은 부분을 기준으로 자료를 넣는다
			changed_datas = []
			for x in range(min_x):
				changed_datas.append(input_list_2d[x][:min_y])
				self.xvar["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x1 + min_x - 1, y1 + min_y - 1])
				sheet_obj.Range(self.xvar["r1c1"]).Value = changed_datas
