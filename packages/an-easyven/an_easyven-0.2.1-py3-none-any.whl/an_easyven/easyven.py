# -*- coding: utf-8 -*-
"""EVEN_20250330_00.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1CyMG7SRVqeTFL-w3jHeC1D3ZDD-3r9kY
...
class EasyVen:
    ...
    def check( self ):
        ...
        bin_path,
        lib_path,
        tmp_path ] = self.setup( venv_name = "kohya_env" )
        # self.debug.log_step( f"venv_name : { venv_name }", success = None )
        self.debug.log_step( f"bch_path  : { bch_path }", success = None )
        self.debug.log_step( f"dst_path  : { dst_path }", success = None )
        self.debug.log_step( f"cur_path  : { cur_path }", success = None )
        self.debug.log_step( f"src_path  : { src_path }", success = None )
        self.debug.log_step( f"arc_path  : { arc_path }", success = None )
        self.debug.log_step( f"lcl_path  : { lcl_path }", success = None )
        self.debug.log_step( f"bin_path  : { bin_path }", success = None )
        self.debug.log_step( f"lib_path  : { lib_path }", success = None )
        self.debug.log_step( f"tmp_path  : { tmp_path }", success = None )
