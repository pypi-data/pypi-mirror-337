...
import maverick_de.common.util as util
...
class OntonotesDataset(Dataset):
    def __init__(self, name: str, path: str, batch_size, processed_dataset_path, tokenizer, **kwargs):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer, use_fast=True, add_prefix_space=True, revision=kwargs.get("revision", None))
        self.max_doc_len = kwargs.get("max_doc_len", None)
        self.use_speaker = kwargs.get("use_speaker", False)
        if self.use_speaker:
            special_tokens_dict = {"additional_special_tokens": ["[SPEAKER_START]", "[SPEAKER_END]"]}
            self.tokenizer.add_special_tokens(special_tokens_dict)
        try:
            self.set = load_from_disk(hydra.utils.get_original_cwd() + "/" + processed_dataset_path + "/", keep_in_memory=True)
        except:
    ...
    def _tokenize(self, tokens, clusters, speakers, eos_indices):
        ...
        for idx, (token, speaker) in enumerate(zip(tokens, speakers)):
            if self.use_speaker and last_speaker != speaker:
