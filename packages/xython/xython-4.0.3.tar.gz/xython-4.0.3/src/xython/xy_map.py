...
import webbrowser
import os, math
import xy_color, xy_re, xy_util, xy_common, xy_common_map
...
class xy_map:
	def __init__(self):
		...
		self.main_map = None
		self.all_cxy = xy_common_map.basic_data().xvar
	...
	def draw_marker_with_icon_type(self, input_cxy_list, icon_no=1, tooltip_text=None):
		...
		for index, one_cxy in enumerate(input_cxy_list):
			folium.Marker(location=one_cxy, icon=folium.Icon(color='red', icon=self.xvar["icon_type"][icon_no - 1]),
	...
	def get_test_cxy_list(self, x_count=5, y_count=4, distance=0.1):
		...
		for x in range(x_count):
			for y in range(y_count):
				result.append([self.xvar["basic_cxy"][0] + x * distance, self.xvar["basic_cxy"][1] + y * distance])
		return result
	...
	def make_main_map_object(self, input_cxy, zoom_no):
		...
		:return:
		"""
		input_cxy = self.xvar["basic_cxy"](input_cxy)
	...
	def new_map(self, start_cxy="", zoom_no=8, **input_dic):
		...
		self.xvar.update(input_dic)
		self.xvar["location"] = start_cxy
		self.xvar["zoom_start"] = zoom_no

		self.main_map = folium.Map(
			zoom_control=self.xvar["zoom_control"],
			scrollWheelZoom=self.xvar["scrollWheelZoom"],
			dragging=self.xvar["dragging"],
			location=self.xvar["location"],
			width=self.xvar["width"],
			height=self.xvar["height"],
			min_zoom=self.xvar["min_zoom"],
			max_zoom=self.xvar["max_zoom"],
			zoom_start=self.xvar["zoom_start"],
			tiles=self.xvar["tiles"],
			prefer_canvas=self.xvar["prefer_canvas"],
			control_scale=self.xvar["control_scale"],
			no_touch=self.xvar["no_touch"],
			font_size=self.xvar["font_size"],
			attr=self.xvar["attr"],
			crs=self.xvar["crs"],
			min_lat=None,
			max_lat=None,
	...
	def setup_onoff_for_zoom_by_0or1(self, input_tf=False):
		...
		:return:
		"""
		self.xvar["zoom_control"] = input_tf
	...
	def setup_scroll_wheel_zoom_by_0or1(self, input_tf=False):
		...
		:return:
		"""
		self.xvar["scrollWheelZoom"] = input_tf
