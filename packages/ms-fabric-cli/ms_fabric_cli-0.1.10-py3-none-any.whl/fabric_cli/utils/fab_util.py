...
def sort_items_by_config(ws_items: list) -> list:
    if (
        ...
        return sorted(
            ws_items,
            key=lambda item: (str(item.get_item_type().name),
                              item.get_short_name()),
        )
...
def get_item_with_definition(
    item: Item, args: Namespace, decode: Optional[bool] = True, obtain_definition: bool = True
) -> dict:
    ...
    if not obtain_definition:
        ...
        try:
            ...
            item_def = fs_export._export_item(
                item, args, do_export=False, decode=decode, item_uri=True
            )
        except FabricCLIError as e:
            # Fallback
            if e.status_code == fab_constant.WARNING_UNSUPPORTED_COMMAND:
                # Obtain item
                response = item_api.get_item(args, item_uri=True)
                item_def = json.loads(response.text)
            else:
