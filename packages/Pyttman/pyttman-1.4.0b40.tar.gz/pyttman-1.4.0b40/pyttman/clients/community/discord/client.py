...
class DiscordClient(discord.Client, BaseClient):
    ...
    async def send_message_segmented(self,
        ...
        for segment in outgoing_message.segmented(self.message_length_limit,
                                                  with_format=True):
            await channel.send(segment.as_str())
    ...
    async def on_message(self, message: DiscordMessage) -> None:
        ...
        async with message.channel.typing():
            try:
                ...
                elif reply.file is not None:
                    file = discord.File(reply.file, filename=reply.file_name)
                    await message.channel.send(reply.as_str(), file=file)
                else:
