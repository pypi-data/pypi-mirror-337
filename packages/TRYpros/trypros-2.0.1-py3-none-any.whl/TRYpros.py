...
# PUBLIC FUNCTIONS
__all__ = ['DFColValTransformation', 'FeatureHandler', 
           'get_transformation_lonlat_std', 'get_transformation_get_year',
           'get_transformation_get_value_form', 'map_plot', 
           'save_data', 'is_lon_lat_on_land']
...
# CONSTANTS
MONTHS = [v.lower() for v in list(calendar.month_name) if len(v) > 0]
MONTHS_SHORT = [v[:3] for v in MONTHS]
SEASONS = ["summer", "autumn", "winter", "spring", 'fall']
CONTEXT_COLS = ["DataName", "OriglName", "OrigUnitStr", "Comment"]
VALID_DATA_TYPES = ["trait", "lonlat", "year"]
VALID_STD_TYPES = ["std", "non_std"]
...
class FeatureHandler:
    ...
    def get_utm_data(self, include_wgs=True):
        ...
        """
        df_lonlat = self.combine_lonlat("non_std")
    ...
    def extract_trait_covariate_data(self, verbose=True):
        ...
        for coord in ["longitude", "latitude"]:
            ids_coord = get_covariate_matches(
        ...
        ids_year = get_covariate_matches(
...
def get_transformation_lonlat_std():
    ...
    """
    return DFColValTransformation(f=value_std_latlon_deg, 
                                  col="OrigValueStr")
