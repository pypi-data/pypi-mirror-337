...
class ServiceParameters:
    class GetQuery(
        GeneralParameters.Filters,
        GeneralParameters.Sorts,
        GeneralParameters.Search,
        GeneralParameters.Pagination,
        GeneralParameters.Check
    ):
    ...
    class Get(GetQuery):
        sort_columns:list[GeneralParameters.SortColumn] = Field([GeneralParameters.SortColumn(name="id", order=GeneralParameters.SortOrder.ASC)], description="List of columns to be sorted")
        date_filters:list[GeneralParameters.DateFilter] = Field([], description="Date filters to be applied")
        ...
        def set_sort_columns(self) -> Self:
            ...
            for item in self.sort:
                ...
                if len(parts) == 2 and parts[1].lower() in ["asc", "desc"]:
                    try:
                        sort_columns.append(GeneralParameters.SortColumn(name=parts[0],order=GeneralParameters.SortOrder(parts[1].lower())))
                    except ValueError:
                        continue
            ...
            for filter_item in self.filter:
                ...
                if len(parts) >= 2 and parts[0]:
                    ...
                    if from_date or to_date:
                        date_filters.append(GeneralParameters.DateFilter(name=name, from_date=from_date, to_date=to_date))
