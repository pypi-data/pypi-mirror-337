...
class Provider:
    ...
    def get_registered_type(self, address: str):
        """get_registered_type
        ...
        b_address = address.encode('utf-8')
        data = Variant()
        result = Result(ctrlxdatalayer.clib.libcomm_datalayer.DLR_providerGetRegisteredType(self.__provider, b_address, data.get_handle()))
        return result, data
    ...
    def get_registered_node_paths(self):
        ...
        with Variant() as data:
            result = Result(ctrlxdatalayer.clib.libcomm_datalayer.DLR_providerGetRegisteredNodePaths(self.__provider, data.get_handle()))
            if result != Result.OK:
                return result, []
            return result, data.get_array_string()
    ...
    def get_rejected_node_paths(self):
        ...
        """
        with Variant() as data:
            result = Result(ctrlxdatalayer.clib.libcomm_datalayer.DLR_providerGetRejectedNodePaths(self.__provider, data.get_handle()))
            if result != Result.OK:
                return result, []
            return result, data.get_array_string()

    def publish_event(self, data: Variant, event_type: Variant) -> Result:
        """publish_event
        ...
        @returns <Result>, status of function call
        """
        return Result(ctrlxdatalayer.clib.libcomm_datalayer.DLR_providerPublishEvent(self.__provider, data.get_handle(), event_type.get_handle()))
