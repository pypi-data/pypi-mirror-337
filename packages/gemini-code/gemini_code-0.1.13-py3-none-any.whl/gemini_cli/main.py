...
@click.option('--model', '-m', help='Model to use (default: gemini-1.5-pro)')
@click.pass_context
def cli(ctx, model):
    """Interactive CLI for Gemini and other LLM models."""
    if ctx.invoked_subcommand is None:
        # If no subcommand is specified, start interactive session
        model_name = model or config.get_default_model() or "gemini-1.5-pro"
        print(f"Starting session with model_name: {model_name}")
...
def start_interactive_session(model_name):
    ...
    print(f"In start_interactive_session with model_name: {model_name}")
    ...
    if model_name.startswith("gemini"):
        ...
        if not api_key:
            ...
            return
        print(f"Creating GeminiModel with model_name: {model_name}")
