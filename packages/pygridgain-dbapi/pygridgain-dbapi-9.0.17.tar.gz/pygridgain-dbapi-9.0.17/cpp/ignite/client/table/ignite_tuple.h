 ...
 */
inline std::string parse_column_name(std::string_view name) {
...
public:
    ...
    ignite_tuple(std::initializer_list<std::pair<std::string, primitive>> pairs)
        ...
        for (size_t i = 0; i < m_pairs.size(); ++i)
            m_indices.emplace(std::make_pair(detail::parse_column_name(m_pairs[i].first), i));
    }
    ...
    [[nodiscard]] const primitive &get(std::string_view name) const {
        auto it = m_indices.find(detail::parse_column_name(name));
        if (it == m_indices.end())
    ...
    void set(std::string_view name, T &&value) {
        auto parsed_name = detail::parse_column_name(name);
