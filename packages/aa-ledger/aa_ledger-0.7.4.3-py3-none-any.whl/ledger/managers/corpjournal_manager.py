...
CORPORATION_FILTER = BOUNTY_PRIZES + ESS_TRANSFER + INCURSION + MISSION + DAILY_GOAL_REWARD + CITADEL_INCOME
...
class CorpWalletQuerySet(CorpWalletQueryFilter):
    ...
    def _aggregate_data(self):
        aggregated = {
            ...
            "total": Coalesce(
                Sum(
                    F("amount"),
                    filter=(BOUNTY_FILTER | ESS_FILTER | MISC_FILTER),
                ),
                Value(0),
                output_field=DecimalField(),
            ),
    ...
    def aggregate_ledger(self, page=1, page_size=20):
        accounts = UserProfile.objects.filter(
            main_character__isnull=False,
        ).select_related(
            "user__profile__main_character",
        ...
        logger.debug(CORPORATION_FILTER)
        qs = self.filter(ref_type__in=CORPORATION_FILTER)
        #qs = self
        ...
        for account in accounts:
            ...
            existing_alts = set(alts).intersection(entity_ids)
            if not existing_alts:
                continue
            
            acc_qs = qs.filter(
                Q(first_party_id__in=existing_alts) | Q(second_party_id__in=existing_alts)
            )
            aggregated_data = acc_qs.aggregate(**self._aggregate_data())
            main_id = account.main_character.character_id
            
            data = {
                "main_id": main_id,
                "main_name": account.main_character.character_name,
                "entity_type": "character",
                "alt_names": list(existing_alts),
                "total_amount": aggregated_data["bounty_income"],
                "total_amount_ess": aggregated_data["ess_income"],
                "total_amount_others": aggregated_data["miscellaneous"],
            }
            main_dict[main_id] = data
            proccessed_ids.update(existing_alts)
            proccessed_entry_ids.extend(
                acc_qs.values_list("entry_id", flat=True)
            )
            
        entity_ids = list(entity_ids - proccessed_ids)
        
        qs = (qs.filter(
            Q(second_party_id__in=entity_ids) & ~Q(entry_id__in=proccessed_entry_ids) | Q(first_party_id__in=entity_ids) & ~Q(entry_id__in=proccessed_entry_ids)
            ).select_related(
                "second_party__name",
            )
            .values("second_party__name")
            .annotate_bounty_income()
            .annotate_ess_income()
            .annotate_miscellaneous()
            .annotate(first_party_id=F("first_party__eve_id"))
            .annotate(first_party_name=F("first_party__name"))
            .annotate(entity_type_first=F("first_party__category"))
            .annotate(second_party_id=F("second_party__eve_id"))
            .annotate(second_party_name=F("second_party__name"))
            .annotate(entity_type_second=F("second_party__category"))
        )
        
        #qs = []

        logger.debug(f"Entries: {len(qs)}")

        for entry in qs:
            data = {
                "main_id": entry["second_party_id"],
                "main_name": entry["second_party_name"],
                "entity_type": entry["entity_type_second"],
                "alt_names": None,
                "total_amount": entry["bounty_income"],
                "total_amount_ess": entry["ess_income"],
                "total_amount_others": entry["miscellaneous"],
            }
            # Falls die ID bereits existiert, summiere die Werte
            if entry["second_party_id"] in main_dict:
                main_dict[entry["second_party_id"]]["total_amount"] += data["total_amount"]
                main_dict[entry["second_party_id"]]["total_amount_ess"] += data["total_amount_ess"]
                main_dict[entry["second_party_id"]]["total_amount_others"] += data["total_amount_others"]
            else:
                main_dict[entry["second_party_id"]] = data
        
            
        total_amount = self.aggregate(**self._aggregate_data())

        return {
            "ratting": list(main_dict.values()),
            "total": {
                "total_amount": total_amount["bounty_income"],
                "total_amount_ess": total_amount["ess_income"],
                "total_amount_others": total_amount["miscellaneous"],
                "total_amount_all": total_amount["total"],
            },
        }
