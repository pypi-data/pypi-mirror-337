...
@click.option('--model', '-m', help='Model to use (default: gemini-2.5-pro-exp-03-25)')
@click.pass_context
def cli(ctx, model):
    """Interactive CLI for Gemini and other LLM models."""
    if ctx.invoked_subcommand is None:
        # If no subcommand is specified, start interactive session
        model_name = model or config.get_default_model() or "gemini-2.5-pro-exp-03-25"
    ...
    while True:
        try:
            ...
            if user_input.lower() == '/exit':
                ...
                continue
            # Handle CLI command entered in chat
            elif user_input.lower().startswith('gemini set-default-model'):
                parts = user_input.split()
                if len(parts) >= 3:
                    new_model = parts[2]
                    config.set_default_model(new_model)
                    console.print(f"[green]✓[/green] Default model set to {new_model}. This will take effect next time you start the CLI.")
                    continue
                else:
                    console.print("[red]Error:[/red] Missing model name. Usage: gemini set-default-model MODEL_NAME")
                    continue
