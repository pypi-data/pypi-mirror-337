import numpy as np
from sysidentpy.basis_function import Laguerre


def test_laguerre_init():
    """Test that Laguerre class initializes correctly."""
    h = Laguerre(degree=3, include_bias=False, ensemble=True)
    assert h.degree == 3
...
def test_laguerre_fit():
    """Test that fit correctly generates the Laguerre feature matrix."""
    h = Laguerre(degree=2)
...
def test_laguerre_fit_predefined_regressors():
