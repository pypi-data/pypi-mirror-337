...
from .helpers import find_next, is_paired
...
def look_for_image_or_link(
    ...
    for i, d in list(enumerate(delimiters))[::-1]:
        ...
        result = _expect_inline_link(text, close + 1)
        if result:
            etype = "Image" if d.content == "![" else "Link"
            dest = result[0]
            title = result[1]
            end = result[2]
            rv = MatchObj(etype, text, d.start, end, link_text, dest, title)
        else:
            result = _expect_reference_link(
                text, close + 1, link_text.text, link_ref_defs
            )
            if result:
                etype = "ImageRef" if d.content == "![" else "LinkRef"
                key = result[0]
                label = result[1]
                title = result[2]
                end = result[3]
                rv = MatchObj(etype, text, d.start, end, link_text, label, key, title)
            else:  # not a link
                break
        process_emphasis(text, delimiters, i, matches)
        if etype in ("Link", "LinkRef"):
...
def _expect_reference_link(
    ...
    # Determine the lookup key, from link text if no label present.
    key = link_text
    link_label = _parse_link_label(text, start)
    if link_label is not None:
        assert link_label.text is not None
        key = link_label.text[1:-1] or link_text
    elif text[start : start + 2] == "[]":
        link_label = Group(start, start + 2, "[]")

    result = link_ref_defs.get(key)
    if not result:
        return None

    # link_label might be empty, and difficult to derive later, so capture key too.
    link_key = Group(start, start, key)
    link_title = Group(start, start, result[1])
    return (link_key, link_label, link_title, link_label.end if link_label else start)
...
def process_emphasis(
    ...
    while cur is not None:
        ...
        if opener is not None:
            ...
            match = MatchObj(
                "StrongEmphasis" if n == 2 else (
                    "AsteriskEmphasis" if d_closer.content[0] == "*" else "UnderscoreEmphasis"
                ),
                text,
                d_opener.end - n,
                d_closer.start + n,
...
class MatchObj:
    ...
    def __repr__(self):
        groups_repr = ', '.join([repr(g) for g in self._groups])
        return f"MatchObj({repr(self.etype)}, {repr(self._text)}, {self._start}, {self._end}, {groups_repr})"

    def group(self, n: int = 0) -> str | None:
        if n == 0:
            return self._text[self._start : self._end]
        group = self._groups[n - 1]
        if not group:
            return None
        return group[2]
