...
def convert_ms_since_epoch_to_datetime(milliseconds: int) -> datetime:
    ...
    :rtype: datetime
    """
    # Split the time into seconds and remaining milliseconds.
    time_s = int(milliseconds / 1000)
    time_ms = milliseconds - (time_s * 1000)
    # Create a datetime object with seconds precision.
    ts_s = datetime.fromtimestamp(time_s, tz=UTC)
    # Use the seconds precision object to create a millisecond precision
    # datetime object.
    ts = datetime(ts_s.year, ts_s.month, ts_s.day,
                  ts_s.hour, ts_s.minute, ts_s.second,
                  time_ms * 1000, tzinfo=UTC)
    return ts
