...
def create_mo(domain, lang, folder):
    ...
    catalog.add(
        "help", "hilfe", context="test_module_en", locations=[("testmodule.py", 42)]
    )
    catalog.add(
        "quit",
        "schließen",
...
def test_that_cli_is_unique():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        filename = os.path.join(
            tmpdirname, lang, "LC_MESSAGES", FUNCTION_DOMAIN + ".mo"
        )
        ...
        assert "quit" in CLI.functions.keys()
        ...
        assert "quit" not in CLI.functions.keys()

        assert "naʻa_ku_fehuʻ" in CLI.functions.keys()
        assert "haʻu_ʻo_kai" in CLI.functions.keys()
