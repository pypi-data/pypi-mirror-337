...
class DBModel(Model):
    ...
    @classmethod
    async def exec_sql(cls, sql: str, query=False, for_one=False, db_key='default'):
    ...
    async def exec_query(cls, sql: str, require=False, retry=1, for_one=False, db_key='default'):
        if (not require) or (retry <= 1):
            return await cls.exec_sql(sql, query=True, for_one=for_one, db_key=db_key)
        for i in range(retry):
            sta = await cls.exec_sql(sql, query=True, for_one=for_one, db_key=db_key)
            if sta:
                return sta
