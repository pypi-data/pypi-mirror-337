...
async def get_notes_for_AI(
    ...
    async def core_logic():
        ...
        for note in notes:
            ...
            # Extract knowledge/subject if available - handle both formats
...
async def create_note_for_AI(
    ...
    async def core_logic():
        ...
        # Format the response, with a note that we're returning a potentially incomplete response
        return {
            "note": {
                "id": note_id, # This might be None if we couldn't confirm creation
                "title": response.get("title", title),
                "note": response.get("note", content),
