...
from cloecore.to_datafactory import factory_objects, job_packages

logger = logging.getLogger(__name__)


def transform_metadata_to_activites(
    job: meta.FS2DB | meta.DB2FS | meta.ExecSQLJob, batchstep: meta.Batchstep
...
def prepare_batches_and_jobs(
    ...
    for batchstep in batch.batchsteps:
        activity_packages[batchstep.id] = transform_metadata_to_activites(
            jobs[batchstep_to_job_id[batchstep.id]], batchstep
        )
    pipeline_build_plan = utils.optimize_batch(batch.batchsteps)
    pipeline_names = {
        i: f"{batch.name}_orchestrator_{i}" for i in pipeline_build_plan.keys()
    }
...
def create_pipeline_activities(
    ...
    for batchstep_id in pipeline:
        ...
        batchstep_deps = batch.get_batchstep_by_id(batchstep_id).dependencies
        for batchstep_dep in batchstep_deps:
            dep_execute_pipeline = activity_packages[
                batchstep_dep.dependent_on_batchstep_id
            ]
            condition = (
                "Completed"
                if batchstep_dep.ignore_dependency_failed_state
                else "Succeeded"
            )
