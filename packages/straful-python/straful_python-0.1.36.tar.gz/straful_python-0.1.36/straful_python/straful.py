...
In case the service has been recently started please wait 5 minutes for it to become fully functional."
    ...
    def submit_job(self, *, backend=None, circuit=None, circuits=None, shots=None, comments=""):
        ...
        if circuit is None and circuits is None:
            print("An quantum circuit to be executed or a list of quantum circuits to be executed must be specified.")
            return
        if circuit and circuits:
            print("You can use either 'circuit' or 'circuits' as input arguments but not both at the same time.")
            return
        if circuit and not isinstance(circuit, QuantumCircuit):
            print("The circuit must be an instance of QuantumCircuit.")
            return
        if circuits and not isinstance(circuits, list):
            print("The circuits must be a list of QuantumCircuit instances.")
            return
        if circuits is not None and len(circuits) == 0:
            print("The circuits list is empty.")
            return
        if shots is None:
