...
def iscomplexobj(x):
    ...
    if repr(x).startswith('Quantum object'):
        # qutip.Qobj
        return np.iscomplexobj(x.to('CSR').data.as_scipy())
    else:
...
def triu(matrix):
    ...
    * If `matrix` is a QuTiP operator, the type is *not* preserved:
      the result is equivalent to ``triu(matrix.data.as_ndarray())``.
...
def tril(matrix):
    ...
    * If `matrix` is a QuTiP operator, the type is *not* preserved:
      the result is equivalent to ``tril(matrix.data.as_ndarray())``.
