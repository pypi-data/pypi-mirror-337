...
def MDT_summary_table(mdt_html, rrw_file, esid_file):
    ...
    try:
        ...
        if os.path.exists(esid_file):
            ...
            html_output = "<html><body><table border='1'><tr><th>System</th><th>RRW IOD0 Pass/Total</th><th>RRW IOD1 Pass/Total</th><th>ESID Pass/Total</th><th>Memtest Absent Channel</th></tr>"
            # Check systems that meet the condition of 8 log files and all "Pass"
            for system, esid_count in esid_data.items():
                html_output += f"<tr><td>{system}</td><td>{rrw_count[system][0]['pass']}/{rrw_count[system][0]['total']} Pass</td><td>{rrw_count[system][1]['pass']}/{rrw_count[system][1]['total']} Pass</td><td>{esid_count['pass_count']}/{len(esid_count['log_files'])} Pass</td><td>{memtest_result[system]}</td></tr>"

            html_output += "</table></body></html>"
            # Output the HTML file
            with open(mdt_html, "w") as f:
                f.write(html_output)
    except:
