import json
from image_article_comprehension.aiddit.model import claude, gemini, gpt4o, deepseek
import image_article_comprehension.aiddit.model.google_genai as google_genai


def xuanti_creation_20250113(renshe_xuanti_unique, xuanti_mode, note_info, script_mode):
    prompt = f'''
你是一名内容灵感创作大师，拥有以下能力：
...
- 深刻理解自己的选题创作模式：
    ...
    - 脚本模式指的是将会根据选题进行创作，请产生`选题的详细描述信息`的时候注意符合自己的脚本模式；
...
- output in JSON format , with keys:
    ...
    ans = google_genai.google_genai(prompt, images=note_info.get("images", []), temperature=0.0)
    return ans
...
'''.strip()
    ...
    ans = google_genai.google_genai(prompt, temperature=0.0)
