...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            """CREATE OR REPLACE VIEW "d.s.v" . s.v AS SELECT 1 I;""",
            """CREATE OR REPLACE VIEW "d.s.v" . s.v AS SELECT 1 I;""",
        ),
        (
            """CREATE OR REPLACE VIEW "d.s.v" . "s"."v" AS SELECT 1 I;""",
        ...
        (
            """CREATE OR REPLACE VIEW "d.s.v" 
                . "s"."v" AS SELECT 1 I;""",
            """CREATE OR REPLACE VIEW "d.s.v" 
                . "s"."v" AS SELECT 1 I;""",
        ),
        (
            """CREATE OR REPLACE VIEW "d.s.v"."v" AS SELECT 1 I;""",
            """CREATE OR REPLACE VIEW "d.s.v"."v" AS SELECT 1 I;""",
        ),
        (
            """CREATE OR REPLACE VIEW "d".s.v AS SELECT 1 I;""",
...
def test_fix_view_definition(view_definition, expected):
    result = string_util.fix_view_definition(view_definition)
    assert result == expected

    
@pytest.mark.parametrize(
    "object_type, file_content, old_name, new_name, expected",
    [
        ( # basic case
            "VIEW",
            """CREATE OR REPLACE VIEW S.V_OLD AS SELECT 1 I;""",
            "S.V_OLD",
            "S.V_NEW",
            """CREATE OR REPLACE VIEW S.V_NEW AS SELECT 1 I;""",
        ),
        ( # case insensitive
            "VIEW",
            """CREATE OR REPLACE VIEW s.v_old AS SELECT 1 I;""",
        ...
        ( # old name does not match -> no change
            "VIEW",
            """CREATE OR REPLACE VIEW S.V_OTHER AS SELECT 1 I;""",
            "S.V_OLD",
            "S.V_NEW",
            """CREATE OR REPLACE VIEW S.V_OTHER AS SELECT 1 I;""",
        ),
        ( # special characters in old name
            "VIEW",
            """CREATE OR REPLACE VIEW "S"."V_OLD!?" AS SELECT 1 I;""",
            '"S"."V_OLD!?"',
            "S.V_NEW",
        ...
        ( # special characters in new name
            "VIEW",
            """CREATE OR REPLACE VIEW S.V_OLD AS SELECT 1 I;""",
            "S.V_OLD",
            '"S"."V_NEW$%"',
...
def test_sub_object_name(object_type, file_content, old_name, new_name, expected):
    result = string_util.sub_object_name(object_type, file_content, old_name, new_name)
    assert result == expected
    

@pytest.mark.parametrize("fixed_datetime, format, expected", [
    (datetime(2024, 12, 16, 17, 42, 13, 123456), "%Y%m%d_%H%M%S_%f", "20241216_174213_123456"),
    (datetime(2023, 1, 1, 0, 0, 0, 0), "%Y%m%d_%H%M%S_%f", "20230101_000000_000000"),
    (datetime(2022, 6, 30, 23, 59, 59, 999999), "%Y%m%d_%H%M%S_%f", "20220630_235959_999999"),
    (datetime(2022, 6, 30, 23, 59, 59, 999999), "%Y-%m-%d %H:%M:%S.%f", "2022-06-30 23:59:59.999999"),
])
@patch('aceutils.string_util.datetime')
def test_get_now_string(mock_datetime, fixed_datetime, format, expected):
    mock_datetime.now.return_value = fixed_datetime
    mock_datetime.strftime = datetime.strftime
    
    result = string_util.get_now_string(format)
