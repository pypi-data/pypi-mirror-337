...
class WeightedSentimentAnalyzer:
    ...
    Args:
        model_dir (str, optional): Custom directory path for saving/loading models.
    ...
    """
        ...
        if model_dir is None:
            # Default path - relative to package installation
            try:
                base_path = Path(__file__).parent.parent
                self.model_dir = base_path / "model" / "WeightedSentimentAnalyzer"
            except Exception:
                # Fallback path if __file__ not available
                self.model_dir = Path("PerSent") / "model" / "WeightedSentimentAnalyzer"
        else:
        ...
        default_model = self.model_dir / "weighted_sentiment_model.joblib"
