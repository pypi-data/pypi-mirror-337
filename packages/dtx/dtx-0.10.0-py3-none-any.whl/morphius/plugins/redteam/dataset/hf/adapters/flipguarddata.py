...
class RiskPromptGenerator(BaseRiskPromptGenerator):
        ...
        for record in self.dataset:
            if self._matches_risk_class(record, risk_name):
                ...
                if conversation is not None:
                    logger.debug(f"Record {count} accepted for conversion. Conversation keys: {list(conversation.__dict__.keys())}")
                    yield conversation
                else:
                    logger.debug(f"Record {count} conversion returned None.")
            else:
                logger.debug(f"Record {count} skipped: missing required fields or risk mismatch.")
            count += 1
    ...
    def _convert_to_conversation(self, record: dict, risk_name: str) -> MultiTurnConversation:
        ...
        goal = self.POLICY_TO_GOAL_MAP.get(policy_key, "Bypass AI safeguards using adversarial techniques.")
        ...
        if isinstance(attack_field, str):
            try:
                ...
                logger.debug(f"Could not parse attack field: {attack_field} with error: {e}")
                attack_list = []
        elif isinstance(attack_field, list):
        ...
        user_attacks = [item for item in attack_list if isinstance(item, dict) and item.get("role", "").lower() == "user"]
        if not user_attacks:
            logger.debug("No user attack content found; skipping record.")
            return None
        
        attack_item = user_attacks[-1]
        attack_content = attack_item.get("content", "") if isinstance(attack_item, dict) else str(attack_item)
        ...
        logger.debug(f"Converted record to conversation with base_prompt: {record.get('goal', '')}")
        return conversation
