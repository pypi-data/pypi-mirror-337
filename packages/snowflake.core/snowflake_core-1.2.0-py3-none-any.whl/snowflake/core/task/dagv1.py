...
class DAGTask:
        ...
        if isinstance(tasks, DAGTask):
            ...
            for task in tasks:
                if task._is_finalizer:
                    raise InvalidOperationError(f"Finalizer task {self.name} cannot be predecessor of any task")
                # TODO SNOW-1944797: we should first check all tasks to make sure none of them is a finalizer
                #  before adding any of them to a dag
    ...
    def add_successors(
        ...
        if isinstance(tasks, DAGTask):
            ...
            # TODO SNOW-1944797: we should make sure that none of the tasks is a finalizer
            #  before adding any of them as successor
            for t in tasks:
...
class DAGOperation:
    ...
    def drop(self, dag: Union[DAG, str]) -> None:
        ...
        with suppress(NotFoundError):
            root_task.suspend()
        try:
            # TODO SNOW-1944797: this won't return the Finalizer task, as it formally does not have a predecessor.
            #  We would need to run additional GET the root task and look into its task_relations field for Finalizer
