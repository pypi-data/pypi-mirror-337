...
from testbot.app.decorator import case
...
class TcDemo(TestCaseBase):
    ...
    def collect_resource(self, **kwargs):
        with self.step.start(headline="筛选设备") as step:
            self.logger.info(f"topology={self.pool.topology}")
            with step.start(headline="筛选PC设备") as step2:
                self.pc = self.pool.select_resource(resource_type="MacosDevice", count=1)[0]
                if self.pc:
            ...
            with step.start(headline="检查PC设备的IP地址") as step2:
                if self.pc.NetworkAtomModule.get_ip_address() == "0.0.0.0":
                    step2.passed(message="检查PC设备的IP地址成功")
                else:
                    step2.failed(message="检查PC设备的IP地址失败")

            with step.start(headline="筛选TV设备") as step2:
                self.tv = self.pool.select_resource(resource_type="TCLTVDevice", count=1)[0]
                if self.tv:
    ...
    def test(self, **kwargs):
        ...
        with self.step.start(headline="完成开机向导后，进入系统") as step:
            ...
            with step.start(headline="重新初始化grpc", message="") as step2:
                self.tv.CommSerialAtomModule.readline()
