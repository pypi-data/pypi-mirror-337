...
def moments(Y, Xp, Xa, Xc, consumer_idx, nne):
    ...
    ip = np.hstack([np.ones(p, dtype=bool), np.zeros(nne['dim'].loc['upper', 'p'] - p, dtype=bool)])
    ia = np.hstack([np.ones(a, dtype=bool), np.zeros(nne['dim'].loc['upper', 'a'] - a, dtype=bool)])
    ic = np.hstack([np.ones(c, dtype=bool), np.zeros(nne['dim'].loc['upper', 'c'] - c, dtype=bool)])
    ...
    for k, lam in enumerate(lamda):
        ...
        cf1, fl1 = reg_logit(lam, X_o, ys, None)               # simple logit
    ...
    mmt = mmt[~np.isnan(mmt)].astype(np.float32)
