...
class DocumentGenerator:
    ...
    def __init__(self, init_doc_path: str, output_dir: str = None):
        ...
        print(f"Loading initialization document from: {init_doc_path}")
        
        # Check if the file exists
        if not os.path.exists(init_doc_path):
            print(f"ERROR: Initialization file not found: {init_doc_path}")
            print(f"Current working directory: {os.getcwd()}")
            print(f"Absolute path: {os.path.abspath(init_doc_path)}")
        ...
        try:
            ...
            # Debug info - print first 100 chars to verify content
            print(f"Loaded initialization document with {len(self.init_content)} characters")
            print(f"Content preview: {self.init_content[:100]}...")
        except Exception as e:
            print(f"ERROR: Failed to read initialization file: {e}")
            raise
    ...
    def generate_all_documents(self) -> Dict[str, str]:
        ...
        # Generate the PRD with LLM enhancement first - we'll use this for task generation
        prd_path = self._generate_prd(docs_dir)
        generated_files["Product Requirements Document"] = prd_path
        
        # Read the generated PRD to use as input for task generation
        try:
            with open(prd_path, 'r', encoding='utf-8') as f:
                self.generated_prd_content = f.read()
            print(f"Read generated PRD from {prd_path} ({len(self.generated_prd_content)} chars)")
        except Exception as e:
            print(f"Warning: Failed to read generated PRD: {e}")
    ...
    def _generate_prd(self, output_dir: str = None) -> str:
        ...
        if use_llm:
            ...
            try:
                print(f"\n=== Generating PRD with {self.llm_provider.value} ===")
                ...
                print(f"Last 100 chars of prompt: {prompt[-100:] if len(prompt) > 100 else prompt}")
                ...
                print(f"Sending request to {self.llm_provider.value}...")
                response = self.llm_connector.request(request)
                prd_content = response.text.strip()
                
                print(f"Received response with {len(prd_content)} characters")
                print(f"First 100 chars of response: {prd_content[:100]}...")
    ...
    def _generate_cursorrules(self, cursor_dir: str) -> str:
        ...
        if use_llm:
            try:
                ...
                if len(cursorrules_content) > 500 and "be specific" not in cursorrules_content.lower() and "placeholder" not in cursorrules_content.lower():
                    ...
                    print("Warning: LLM generated a minimal cursorrules file. Falling back to enhanced template.")
        ...
        try:
            ...
            if any(keyword in self.init_content.lower() for keyword in db_keywords):
                specialized_rules.extend([
            ...
            if any(keyword in self.init_content.lower() for keyword in api_keywords):
                specialized_rules.extend([
    ...
    def _generate_action_items_with_llm(self) -> ActionPlan:
        ...
        """
        system_message = """
        You are an expert project planner. Based on the provided Product Requirements Document (PRD), 
        ...
        5. Include subtasks where appropriate to break down complex tasks
        ...
        9. Tasks should reflect the actual work needed to implement the requirements described in the PRD
        ...
        if hasattr(self, 'generated_prd_content') and self.generated_prd_content:
            prompt = f"""
            Below is the Product Requirements Document (PRD) for a software project. Please analyze it and create a comprehensive action plan:
            
            {self.generated_prd_content}
            """
            print("Using generated PRD as input for task generation")
        else:
            prompt = f"""
            ...
            {self.init_content}
            """
            print("Using initialization document as input for task generation")
