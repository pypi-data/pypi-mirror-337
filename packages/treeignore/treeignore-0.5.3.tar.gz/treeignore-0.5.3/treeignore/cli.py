...
def parse_args():
    parser = argparse.ArgumentParser(
        description="A tree-like directory listing that respects gitignore patterns"  # noqa: E501
    )
    ...
    parser.add_argument(
        ...
        "--show-perms",
        action="store_true",
        help="Print file permissions",
    )
    parser.add_argument(
        "-s",
        "--show-size",
        action="store_true",
        help="Print file sizes",
    )
    parser.add_argument(
        "-i",
        "--no-indent",
    ...
    parser.add_argument(
        "--noreport",
        action="store_true",
        help="Omit file/directory count at end",
    )
    ...
    parser.add_argument(
        "--no-ignore",
        action="store_true",
        help="Do not use gitignore filtering",
    )
    parser.add_argument(
        "--version",
        action="store_true",
...
def main():
    ...
    # If not in a git repo, just check for a .gitignore
    # in the current directory
    if not repo_root:
        print(
            f"Not in a git repository. Looking for .gitignore in {start_dir}"
        )  # noqa: E501
    ...
    def is_ignored(path, args):
        # If --no-ignore is specified, don't ignore anything except .git
        # internal folders
        if args.no_ignore:
            basename = os.path.basename(path)
            return basename in GIT_INTERNAL_IGNORES

        # If -a/--all is specified, don't ignore anything except .git
        # internal folders
        if args.all:
            basename = os.path.basename(path)
            return basename in GIT_INTERNAL_IGNORES
        ...
        # Check if this path should be always ignored
        ...
        if use_git_command and repo_root:
            ...
            # Convert to path relative to repo root for top-level .gitignore  # noqa: E501
            # patterns
            if repo_root:
            ...
            match1 = gitignore_spec.match_file(rel_path)
            match2 = is_dir and gitignore_spec.match_file(rel_path + "/")
...
def is_git_ignored(path, repo_root):
    ...
    try:
        ...
        # Return codes: 0=ignored, 1=not-ignored, 128=error
...
def load_all_gitignores(repo_root, current_dir):
    ...
    return (
        PathSpec.from_lines(GitWildMatchPattern, patterns)
        if patterns
        else None  # noqa: E501
    )  # noqa: E501
