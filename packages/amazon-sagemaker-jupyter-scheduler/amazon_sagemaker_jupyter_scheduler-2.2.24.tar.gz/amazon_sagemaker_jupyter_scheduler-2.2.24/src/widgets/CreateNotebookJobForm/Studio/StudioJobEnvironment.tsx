...
const StudioJobEnvironment: React.FunctionComponent<JobEnvironmentProps> = ({
  ...
  const [selectedKernel, setSelectedKernel] = useState<ParsedSpecName>({
    arnEnvironment: null,
    kernel: null,
    version: null,
  ...
  const handleImageSelection = (
    ...
    const imageVersionAlias = item.optionMetadata?.imageVersionAlias;
    ...
    setFormState({
      ...
      sm_image: item.value,
      sm_kernel: kernel ?? '',
      image_version_alias: imageVersionAlias ?? ''
    });
    setSelectedKernel({ arnEnvironment: item.value, kernel, version, imageVersionAlias  });
  };
