...
from ks3.responseResult import ResponseMetadata, ResponseResult, IterableResponseResult
...
class MultiPartUpload(object):
    ...
    def get_all_parts(self, max_parts=None, part_number_marker=None,
        ...
        if response.status == 200:
            ...
            return IterableResponseResult(self._parts, status=response.status, reason=response.reason,
    ...
    def complete_upload(self):
        ...
        if self.bucket.connection.enable_crc and len(self.part_crc_infos) > 0:
            ...
            logger.debug('upload key: {0}, bucket: {1}, check crc: [client crc: {2}, server crc: {3}]'
                         .format(self.key_name, self.bucket.name, parts_crc, resp.ChecksumCRC64ECMA))
            if not ks3.utils.check_crc(str(parts_crc), resp.ChecksumCRC64ECMA):
                raise KS3ClientError("Inconsistent CRC checksum, "
                                     "client_crc: %s, server_crc: %s" % (str(parts_crc), resp.ChecksumCRC64ECMA),
                                     resp.response_metadata.headers['x-kss-request-id'])
        return resp
    ...
    def cancel_upload(self):
        ...
        """
        return self.bucket.cancel_multipart_upload(self.key_name, self.id)
