...
class TestFeatureConceptAgent(unittest.TestCase):
    ...
    def test_process_concept(self, mock_openai):
        ...
        # Skip se os testes de OpenAI estiverem desabilitados
        if os.environ.get('SKIP_OPENAI_TESTS', 'false').lower() == 'true':
            self.skipTest("Testes de OpenAI estão desabilitados")
