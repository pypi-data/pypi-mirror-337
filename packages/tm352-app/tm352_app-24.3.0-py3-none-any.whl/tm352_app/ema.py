...
def test(filename: str) -> None:
    ...
    if result.returncode == 0:
        ...
        return
    if not os.path.isdir(os.path.join("test", "ema", "ema-service")):
        print(
            "[red bold]Error:[/red bold] No [cyan]test/ema/ema-service[/cyan] folder in the current folder. Required service not included in the submission"  # noqa: E501
        )
        return
    if not os.path.isdir(os.path.join("test", "ema", "ema-app")):
        print(
            "[red bold]Error:[/red bold] No [cyan]test/ema/ema-app[/cyan] folder in the current folder. Required application not included in the submission"  # noqa: E501
        )
        return
    if not os.path.isdir(os.path.join("test", "ema", "ema-app", "web")):
        print(
            "[red bold]Error:[/red bold] No [cyan]test/ema/ema-app/web[/cyan] folder in the current folder. Required web application not included in the submission"  # noqa: E501
        )
        return
    print(":hammer: Installing service dependencies")
    result = subprocess.run(  # noqa: S603
        ["npm", "ci", "--legacy-peer-deps"],  # noqa: S607
        cwd=os.path.join("test", "ema", "ema-service"),
        check=False,
    )
    if result.returncode != 0:
    ...
    result = subprocess.run(  # noqa: S603
        ["npm", "ci", "--legacy-peer-deps"],  # noqa: S607
        cwd=os.path.join("test", "ema", "ema-app"),
        check=False,
    )
    if result.returncode != 0:
    ...
    result = subprocess.run(  # noqa: S603
        ["npm", "ci", "--legacy-peer-deps"],  # noqa: S607
        cwd=os.path.join("test", "ema", "ema-app", "web"),
        check=False,
    )
    if result.returncode != 0:
    ...
    print("\n:hammer: Fixing paths\n")
    apibase_fixed = False
    if os.path.exists(os.path.join("test", "ema", "ema-app", "libraries", "BookingService.ts")):
        with open(os.path.join("test", "ema", "ema-app", "libraries", "BookingService.ts")) as in_f:
            lines = in_f.readlines()
        with open(
            os.path.join("test", "ema", "ema-app", "libraries", "BookingService.ts"),
            "w",
        ) as out_f:
            for line in lines:
...
No [cyan]apibase[/cyan] was found in the [cyan]test/ema/ema-app/libraries/PhotoService.ts[/cyan] file. The application may still work, but this cannot be guaranteed.
