...
class MessageMixin(PrettyReprMixin):
    ...
    def __init__(self, content=None, file=None, file_name = None, **kwargs):
        ...
        self.entities = {}
        self.file = file
        if self.file and not file_name:
            try:
                self.file_name = file.name
            except AttributeError:
                self.file_name = uuid4().hex
        else:
    ...
    def segmented(self, batch_size: int, with_format: bool = False):
        """
        Segments the message into smaller segments. Each segment
        is an altered version of the message itself, during which
        the generator yields the message object itself, but with
        the content property altered to the segment.
        When the generator is exhausted, the original content
        is restored to the message object.
        """
        ...
        segment = []
        clone = deepcopy(self)
        formatted_content = clone.content_with_format

        while formatted_content:
            next_word = formatted_content.pop(0)
            segment.append(next_word)
            if len(str().join(segment)) > batch_size:
                # We went too far, put the last word back
                formatted_content.insert(0, segment.pop())
                if with_format:
                    clone.content_with_format = segment
                else:
                    clone.content = segment
                yield clone
                segment.clear()
