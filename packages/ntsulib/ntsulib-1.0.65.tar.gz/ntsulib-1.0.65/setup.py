...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        # 1. 处理 PyInstaller Hook 文件（无论开发者还是用户均需执行）
        ...
        if not os.path.exists(hook_dest):
            print(f"⚠️ Hook 文件未正确安装到: {hook_dest}")
            try:
        ...
        # 2. 仅当 StubsFlag 不存在时（用户安装场景）处理存根文件
        print("StubsFlag: " + str(os.path.exists("StubsFlag")))
        if not os.path.exists("StubsFlag"):
            stubs_src = "./stubs/ntsulib"
            stubs_dest = os.path.join(self.install_lib, "ntsulib", "@types")
            if os.path.exists(stubs_src):
                try:
                    os.makedirs(stubs_dest, exist_ok=True)
                    for pyi_file in glob.glob(os.path.join(stubs_src, "**/*.pyi"), recursive=True):
                        rel_path = os.path.relpath(pyi_file, stubs_src)
                        dest_path = os.path.join(stubs_dest, rel_path)
                        os.makedirs(os.path.dirname(dest_path), exist_ok=True)
                        shutil.copy2(pyi_file, dest_path)
                        print(f"✅ 已安装存根文件: {dest_path}")
                    # 删除临时 stubs 目录
                    shutil.rmtree("stubs")
                    print("🗑️ 已删除临时 stubs 目录")
                except Exception as e:
                    print(f"❌ 存根文件处理失败: {e}")
            else:
                print("⚠️ 未找到存根文件目录: stubs/ntsulib")
        else:
            print("🛠️ 开发者模式：跳过存根文件安装（检测到 StubsFlag 文件夹）")
