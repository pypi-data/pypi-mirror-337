...
class Deeplake:
    ...
    def add(self, docs: List[str], metadata: List[Dict] = None):
        if not isinstance(docs, list) or not all(
            isinstance(text, str) for text in docs
        ):
        ...
        elif not isinstance(metadata, list) or not all(
            isinstance(meta, dict) for meta in metadata
        ):
    ...
    def similarity_search_with_score(
        ...
        results = self.vector_store.search(
            embedding_data=query, embedding_function=self.embedding_function
        )
