# adapted from original code written by Sebastian Rodrigue
import numpy as np
...
def get_area(lat_range: np.ndarray, cell_size: float) -> np.ndarray:
...
def compute_areas(y_res: float, y_size: int, origin_y: float) -> np.ndarray:
