    ...
    switch (K) {
      case 9:
        if (check_backward_warp(d_stride, D)) {
          kernel = backward_kernel_warp_primitive<scalar_t, d_stride,
    ...
    switch (K) {
      case 9:
        if (check_backward_warp(d_stride, D)) {
          ...
          kernel =
              backward_kernel<scalar_t, d_stride, stride_type, 1, 9, false>;
        }
        break;
      case 8:
        if (check_backward_warp(d_stride, D)) {
          ...
          kernel =
              backward_kernel<scalar_t, d_stride, stride_type, 1, 8, false>;
        }
        break;
      default:
        printf("K=%ld\n", K);
