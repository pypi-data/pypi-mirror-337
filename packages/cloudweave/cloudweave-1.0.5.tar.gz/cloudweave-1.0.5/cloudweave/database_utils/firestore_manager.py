...
class FirestoreManager(Singleton):
        ...
        with self._connection_lock:
            ...
            try:
                ...
                try:
                    ...
                    if credentials_dict:
                        gcp_options['credentials_object'] = credentials_dict
    ...
    def create_document(self, doc_id: Optional[str], collection_name: str, data: Dict[str, Any]) -> str:
        ...
        Args:
            doc_id: Optional document ID. If None, a UUID will be generated
        ...
        self.logger.info(f"Creating document in collection {full_collection_name}")
        ...
        if 'created_at' not in document_data:
            document_data['created_at'] = fs.SERVER_TIMESTAMP
        if 'updated_at' not in document_data:
            document_data['updated_at'] = fs.SERVER_TIMESTAMP
        
        try:
            if doc_id:
                # Use specified document ID
                self.logger.debug(f"Using provided document ID: {doc_id}")
                document_data['id'] = doc_id
                doc_ref = self._db.collection(full_collection_name).document(doc_id)
                doc_ref.set(document_data)
            else:
                # Generate new document ID
                doc_ref = self._db.collection(full_collection_name).document()
                doc_id = doc_ref.id
                self.logger.debug(f"Generated document ID: {doc_id}")
                document_data['id'] = doc_id
                doc_ref.set(document_data)
                
            self.logger.info(f"Successfully created document with ID {doc_id} in collection {full_collection_name}")
            return doc_id
                
        except Exception as e:
            self.logger.error(f"Error creating document in {full_collection_name}: {e}")
            raise
