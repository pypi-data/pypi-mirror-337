...
def download_stories(
    ...
    if interactive or (not urls and not file):
        rprint("[bold]Enter story URLs, one per line[/bold] (press Ctrl+D or Ctrl+Z on a new line to finish):")
        try:
            while True:
                line = Prompt.ask("URL")
                if line.strip():
...
def extract_story_urls(
    ...
    if interactive or (not urls and not file):
        rprint(
            "[bold]Enter URLs to extract from, one per line[/bold] (press Ctrl+D or Ctrl+Z on a new line to finish):")
        try:
            while True:
                line = Prompt.ask("URL")
                if line.strip():
    ...
    if download and all_extracted_urls:
        if Confirm.ask(f"Do you want to download all {len(all_extracted_urls)} extracted stories?"):
            ...
            download_stories(all_extracted_urls)
