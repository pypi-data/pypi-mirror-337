...
def mean_symmetric_scale(data: matrix_data, axis: Literal[0, 1, -1] = -1, is_verbose: bool = True) -> matrix_data:
    ...
    if axis == -1:
        ...
        ul.log(__name__).warn("The `axis` parameter supports only -1, 0, and 1")
        raise ValueError("The `axis` parameter supports only -1, 0, and 1")
...
class RandomWalk:
    ...
    def __init__(
        ...
        if not is_simple and self.is_ablation:
            ...
            ) = self._get_seed_cell_(init_status_no_weight, "ablation")
    ...
    def _get_seed_cell_cluster_weight_(self, seed_cell_index: collection) -> collection:
        ...
        seed_cell_cell_anno.insert(0, "rate", seed_cell_cell_anno["clusters"].map(seed_cell_clusters_rate_map))
        seed_cell_cluster_weight = seed_cell_cell_anno["rate"].values
        return mean_symmetric_scale(seed_cell_cluster_weight, is_verbose=False)

    def _get_seed_cell_weight_(self, seed_cell_index: collection, value: collection) -> collection:
        ...
        seed_cell_value = value[seed_cell_index]
        seed_cell_value_weight = sigmoid(mean_symmetric_scale(seed_cell_value, is_verbose=False))
        seed_weight_value *= seed_cell_value_weight * self._get_seed_cell_cluster_weight_(seed_cell_index)

        return seed_weight_value

    def _get_seed_cell_(self, init_data: AnnData, info: str = None) -> Tuple[collection, matrix_data, matrix_data, matrix_data, matrix_data]:
        """
        ...
        :return:
            ...
            4. Based on the init_data parameter, a reference seed cell weight is obtained for enrichment analysis assistance for each trait or disease, and each seed cell is assigned the same weight.
                Note that this only takes effect when `is_simple` is true.
            5. Reference seed cell weights for auxiliary enrichment analysis of each trait or disease based on the init_data parameter,
        ...
        for i in tqdm(self.trait_range):
            ...
            if trait_value_max > 0:
                ...
                _seed_cell_en_weight_ = self._get_seed_cell_weight_(seed_cell_en_index, init_trait_value)
                seed_cell_weight_en[:, i][seed_cell_en_index] = _seed_cell_en_weight_[::-1]
        ...
        return seed_cell_threshold, seed_cell_matrix, seed_cell_weight, seed_cell_matrix_en, seed_cell_weight_en

    @staticmethod
    def _scale_norm_(score: matrix_data, is_verbose: bool = False) -> matrix_data:
    ...
    def run_benchmark(self) -> None:
        ...
        cell_value = self._scale_norm_(self.random_seed_cell)
    ...
    def run_core(self) -> None:
        ...
        """
        if not self.is_simple:
            self.trs_adata.layers["seed_cell_weight"] = self.seed_cell_weight

        self.trs_adata.var["seed_cell_threshold"] = self.seed_cell_threshold
        self.trs_source = self._run_(self.seed_cell_weight, "run_core")

        self.trs_adata.layers[f"trs_source"] = to_sparse(self.trs_source)
    ...
    def _run_enrichment_(self, seed_cell_en_weight: matrix_data, label: str) -> None:
        ...
        if label == "run_en":
            if not self.is_run_core:
                ul.log(__name__).warn("Need to run the `run_core` method first in order to run this method.")
                self.run_core()

        elif label == "run_en_ablation_ncw":
            if not self.is_run_ablation_ncw:
                ul.log(__name__).warn("Need to run the `run_ablation_ncw` method first in order to run this method.")
            ...
            if not self.is_run_ablation_nsw:
                ul.log(__name__).warn("Need to run the `run_ablation_nsw` method first in order to run this method.")
            ...
            if not self.is_run_ablation_ncsw:
                ul.log(__name__).warn("Need to run the `run_ablation_ncsw` method first in order to run this method.")
                self.run_ablation_ncsw()

            _layer_label_ = "tre_ncsw"

        else:
            raise ValueError(f"{label} error. `run_en`, `run_en_ablation_ncw`, `run_en_ablation_nsw` or `run_en_ablation_ncsw`")
        ...
        for i in tqdm(self.trait_range):
            # Random walk
            cell_value = self._random_walk_(seed_cell_en_weight[:, i], weight=self.weight, gamma=self.gamma)
            cell_value = self._scale_norm_(cell_value)
            # separate
            cell_value_credible = np.array(trs_score[:, i]).flatten() - np.array(cell_value).flatten()
            trait_cell_enrichment[:, i][(cell_value_credible > self.credible_threshold) | (trs_score[:, i] > self.enrichment_threshold)] = 1
...
def overlap(regions: DataFrame, variants: DataFrame) -> DataFrame:
    ...
    if regions.shape[0] == 0 or variants.shape[0] == 0:
        ul.log(__name__).warn("Data is empty.")
...
def calculate_init_score_weight(
    ...
    if "trait_info" not in list(overlap_adata.uns.keys()):
        ul.log(__name__).error("The description of traits/diseases `trait_info` is not in `overlap_data.uns.keys()`")
    ...
    _init_trs_ncw_ = calculate_init_score(input_data, block_size=block_size)
    ...
    init_trs_adata = AnnData(to_sparse(_init_trs_weight_), obs=adata.obs, var=overlap_adata.uns["trait_info"])
