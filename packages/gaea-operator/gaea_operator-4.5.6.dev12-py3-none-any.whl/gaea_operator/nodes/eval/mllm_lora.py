...
from gaea_operator.config import MLLMLoRAConfig
...
def mllm_lora_eval(args):
    ...
    base_model_name = artifact.metadata["algorithmParameters"]["baseModelName"]
    ...
    # 3. 下载 base model
    base_model_uri = "/home/windmill/tmp/model"
    windmill_client.download_artifact(name=base_model_name, output_uri=base_model_uri)

    # 4. 生成评估配置文件
    MLLMLoRAConfig(windmill_client=windmill_client, tracker_client=tracker_client).write_eval_config(
    ...
    with run_deploy(DeployArguments(adapters=output_model_uri,
                                    verbose=False,
