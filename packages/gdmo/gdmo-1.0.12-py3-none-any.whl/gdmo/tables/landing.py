    ...
    def put_bronze(self):
        ...
        file_readers = {
            ...
            '.csv': lambda path: (
                ...
                .option("escape", '\"')  # Set escape character
                .load(path)
            ),
        ...
        try:
            ...
            # Check if the file is a CSV and needs preprocessing
        ...
        try: # Create a list of all columns in the stage table
          columns = [col for col in stage.columns]
        except Exception as e:
          raise Exception(f'Failed to create a column list: {e}. DF head: {stage.head()}')

        
        try:  #Always make sure the bronze table has DbxCreated, DbxUpdated, IsDeleted, and Sourcefile columns.
            stage = stage.withColumn('DbxCreated', F.lit(self._dbxcreated))\
                         .withColumn('DbxUpdated', F.lit(datetime.now()))\
                         .withColumn('IsDeleted',  F.lit(0))\
                         .withColumn('Sourcefile', F.lit(self._file_name))
        except Exception as e:
            raise Exception(f'Failed to add delta columns: {e}')



        try: # Reorder the columns to have delta columns at the start of the table.
            delta_columns     = ['DbxCreated', 'DbxUpdated', 'IsDeleted', 'Sourcefile']
        except Exception as e:
                    raise Exception(f'Failed to make delta column name list: {e}')

        try: # Select the columns in the desired order
        ...
        #If table does not exist yet, infer it
        if not table_exists:
        ...
        if self._distinct:
            #print("Applying distinct to the stage dataframe...")
        ...
        if self._loadtype =="overwrite":
            print("Overwriting the bronze table with the new data.")
            ...
            try:
                if self._fix_schema:
                    stage.write.format("delta") \
                        ...
                        .saveAsTable(self._bronze_table_path)
                else:
                    stage.write.format("delta") \
                        .mode("overwrite") \
                        .saveAsTable(self._bronze_table_path)
            ...
            print(f"Appending {self._bronze_table_path} with new data.")
            ...
            print("Merging the bronze table with the new data.")
