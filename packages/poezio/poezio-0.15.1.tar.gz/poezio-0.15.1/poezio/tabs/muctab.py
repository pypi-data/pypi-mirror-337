...
class MucTab(ChatTab):
    ...
    def handle_presence_unjoined(self, presence: Presence, own: bool = False) -> None:
        ...
        if typ == 'unavailable':
            return
        # Prevent wonky situation when joining a room with spotty connectivity
        # and which could lead to multiple presences of the same user if we are
        # unlucky.
        ...
        if own:
            status_codes = presence['muc']['status_codes']
            self.own_join(from_nick, user, status_codes)
    ...
    def on_user_nick_change(self, presence: Presence, user: User, from_nick: str) -> None:
        ...
        if user.nick == self.own_nick:
            ...
            self.change_ping_nick(user.nick, new_nick)
    ...
    def change_ping_nick(self, old_nick: str, new_nick: str) -> None:
        """
        Update the nick in the slix plugin.
        """
        old_jid = JID(self.general_jid)
        old_jid.resource = old_nick
        new_jid = JID(old_jid)
        new_jid.resource = new_nick
        self.core.xmpp.plugin['xep_0410'].update_nick(old_jid, new_jid)

    def enable_self_ping_event(self) -> None:
        """
        Enable the ping event.
        """
        timeout= config.get_by_tabname("self_ping_timeout", self.general_jid)
        interval = int(
            config.get_by_tabname("self_ping_interval", self.general_jid)
        )
        self.disable_self_ping_event()
        if interval <= 0:  # use 0 or some negative value to disable it
            return
        jid = JID(self.general_jid)
        jid.resource = self.own_nick
        self.core.xmpp.plugin['xep_0410'].enable_self_ping(
            jid,
            interval=interval,
            timeout=timeout,
        )
    ...
    def disable_self_ping_event(self) -> None:
        """Disable the ping event"""
        self.core.xmpp.plugin['xep_0410'].disable_self_ping(self.general_jid)

    def on_self_ping_result(self, status: PingStatus) -> None:
        """
        Handle the ping result.
        All possible values are through the PingStatus enum.
        """
        if status == PingStatus.DISCONNECTED:
            self.command_cycle("not in this room")
            self.core.refresh_window()
        elif status == PingStatus.TIMEOUT:
            if not self.lagged:
                self.lagged = True
                ...
                self._state = 'disconnected'
                self.core.refresh_window()
        elif status == PingStatus.JOINED:  # Re-send a self-ping in a few seconds
            self.reset_lag()
        else:
            # Do nothing if the status is "untried"
            pass
...
########################## COMMANDS ####################################
    ...
    def command_ignore(self, args: List[str]) -> None:
        ...
        if nick in self.ignores:
            self.core.information(f'{nick} is already ignored')
            return

        self.ignores.append(nick)
        if not user:
            self.core.information(f'{nick} is not in the room but is now ignored')
        else:
            self.core.information(f'{nick} is now ignored', 'info')
    ...
    def command_unignore(self, args: List[str]) -> None:
        ...
        try:
            ...
            self.core.information(f'{nick} is not ignored')
            return

        if not user:
            self.core.information(f'{nick} is not in the room but is now unignored')
        else:
            self.core.information(f'{nick} is now unignored')
