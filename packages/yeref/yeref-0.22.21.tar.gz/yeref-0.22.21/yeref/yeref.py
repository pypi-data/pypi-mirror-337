...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, cur_=1):
    ...
    try:
        ...
        print(f"========================")
        if len(POST_MEDIA) > 1 and POST_CHKBOX['POST_ISGALLERY'] and not POST_CHKBOX['POST_ISCOLLAPSE'] and not POST_ISPAY:
            POST_TEXT = '' if POST_TEXT == str_empty and POST_TYPE != 'text' else POST_TEXT
        ...
        if POST_CHKBOX['POST_ISCOLLAPSE'] and not message_id and not POST_ISPAY:
