...
def compute_amplitude_spectrum(signal, sampling_rate, lowcut=None, highcut=None, window_type=None):
    """
    ...
    n = len(signal)
    spectrum = np.abs(fft(signal))[:n // 2] / n  # нормализация

    # Умножение амплитуды на 2 (кроме нулевой частоты и частоты Найквиста)
    spectrum[1:-1] *= 2
