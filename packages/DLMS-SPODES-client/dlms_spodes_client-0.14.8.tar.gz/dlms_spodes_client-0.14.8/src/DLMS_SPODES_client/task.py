...
class InitType(ExTask):
    ...
    async def exchange(self, c: Client) -> result.Simple[collection.Collection]:
        col_id = collection.ID(
            man=(ldn := (await GetLDN().exchange(c)).value).get_manufacturer(),
...
class UpdateFirmware(ExTask):
    ...
    async def exchange(self, c: Client) -> result.Result:
        ...
        try:
            ...
            res.append(result.Error([exc.DLMSException(F"can't convert version to SemVer2.0: {e}")]))
            return res
        if semver < SemVer(0, 0, 25):
            c.set_error(Application.VERSION_ERROR, F"unsupport {semver}")
            res.append(result.Error([exc.VersionError(semver)]))
            return res
            ...
            return result.Error([exc.ITEApplication("Unknown metrology Version")])
        ...
        if (match := firm_id_pat.search(bytes(firm_id_value))) is None:
            ...
            return result.Error([exc.ITEApplication(F"not find Firmware ID in {firm_id_value}")])
        else:
        ...
        if (match := boot_ver_pat.search(bytes(boot_ver_value))) is None:
            ...
            return result.Error([exc.ITEApplication(F"not find Firmware ID in {boot_ver_value}")])
        else:
        ...
        if not (firms := get_firmware(manufacturer := c.objects.id.man)):
            ...
            return result.Error([exc.ITEApplication(F"not find firmwares for {manufacturer=}")])
                    ...
                    for i in tuple(indexes):
                        try:
                            is_readable = ass.is_readable(
                                ln=obj.logical_name,
                                index=i)
                            res.append(await ReadAttribute(
                                ln=obj.logical_name,
                                index=i
                            ).exchange(c))
                            if not is_readable:
...
@dataclass
class WriteParTranscripts(ExTask):
    ...
    async def exchange(self, c: Client) -> result.List:
        ...
        for par_value in self.par_values:
            par, value = par_value
            data = c.objects.par2obj(par).get_attr(par.i).copy()
