...
class IROri(nn.Module):
    ...
    def forward(self, x):
        kernel_theta = torch.atan2(self.grad_y + 1e-6, self.grad_x + 1e-6)/4  + self.gride
...
class IRGradOri(nn.Module):
    ...
    def forward(self, x):
        ...
        Weak_constraint = self.WC(Weak_constraint)
        Strong_constraint = self.SC(Strong_constraint)

        WS_constraint = torch.cat([Strong_constraint, Weak_constraint], dim=1)

        WS_constraint = self.WS(WS_constraint)

        out = WS_constraint + Strong_constraint
...
#         super().__init__()
#         self.kernel_size = kernel_size
#         self.grad = LearnableSobel(dim=dim)
#
#         # self.gride = nn.Parameter(self._create_position_grid(kernel_size), requires_grad=False)
#
#         self.register_buffer('gride', self._create_position_grid(kernel_size))
#         self.scale = nn.Parameter(torch.ones(1))
#         # self.magnitude_normal = nn.BatchNorm2d(dim)
#         # self.vector_normal = nn.BatchNorm2d(dim)
#         self.normal = nn.BatchNorm2d(dim)
#
#         self.learkrelu = nn.LeakyReLU(0.2, inplace=True)
#
#
#     def _create_position_grid(self, k):
#
...
#         magnitude = torch.sqrt(grad_x ** 2 + grad_y ** 2 + 1e-6)
#
#         return self.learkrelu(self.normal(vector * x * magnitude * self.scale))
