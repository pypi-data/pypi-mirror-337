...
class TransactionCache:
    ...
    def append_tx(cls, ds_name: str, tx):
        tx_group = cls.__transaction_cache.get(ds_name, None)
        if tx_group is None:
            tx_group = TransactionGroup()
            cls.__transaction_cache[ds_name] = tx_group
    ...
    def get_current_tx(cls, ds_name: str):
        tx_group = cls.get_cache_by_name(ds_name)
