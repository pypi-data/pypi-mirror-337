...
class SkillsAPIEndpoints:
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def list_skills(request: HttpRequest, character_id: int):
            ...
            requirements = {
                hullskills.hull.name: {
                    x.skill.type_id: SkillTierSchema.from_orm(x)
                    for x in hullskills.skill_checks.all()
                }
                for hullskills in ApprovedSkills.objects.select_related("hull").prefetch_related("skill_checks__skill")
            }
