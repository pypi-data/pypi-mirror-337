...
# Import the resource manager for consistent file access
...
def get_audio_path(filename):
    """Get the full path to an audio file using the resource manager."""
    # Use the resource manager to get the file path
    path = get_resource_path(filename)
    if path:
        return path
        
    # If resource manager failed, fall back to the old method
...
def play_audio(filename):
    ...
    Args:
        filename: Name of the audio file (e.g., 'greeting.mp3')
    """
    if SILENT_MODE:
        return
        
    # The AudioPlayer will handle all path resolution internally now
