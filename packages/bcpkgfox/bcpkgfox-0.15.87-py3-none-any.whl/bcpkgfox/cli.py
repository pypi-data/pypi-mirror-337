...
class cli:
    ...
    class exec_gen_:
        ...
        def preparations(self):
            self.current_dir = os.getcwd()
            self.target_file = os.path.join(self.current_dir, self.cli.file)
            ...
            try:
                ...
                try:
                    subprocess.run([
                        sys.executable,
                        "-c",
                        "import pyinstaller; print(f'PyInstaller found at: {pyinstaller.__file__}')"
                    ], check=True)
                except subprocess.CalledProcessError:
                    raise RuntimeError("PyInstaller installation failed! Check Python environment")
                ...
                command = [
                    ...
                    "--onefile",
                    "--clean",
                    "--distpath", os.path.abspath("dist"),  # Explicit output directory
                    "--workpath", os.path.abspath("build")  # Explicit build directory
                ]
                ...
                if "pynput" in self.cli.find_imports.main(return_=True): command.extend(["--hidden-import", "pynput.keyboard", "--hidden-import", "pynput.mouse"])
