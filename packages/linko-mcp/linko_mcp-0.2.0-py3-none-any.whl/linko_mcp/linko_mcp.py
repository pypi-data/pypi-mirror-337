...
def process_notes_response(notes):
    ...
    for note in notes:
        ...
        updated_at = note.get("updated_at", note.get("update_time", ""))
        
        # Extract associated data - handle both object and list formats
        # Handle knowledge/subject data in different formats
        knowledge_data = note.get("knowledge", [])
        knowledge_name = ""
        if isinstance(knowledge_data, dict):
            knowledge_name = knowledge_data.get("title", "")
        elif isinstance(knowledge_data, list) and knowledge_data:
            # Get the first knowledge title if it's a list
            if isinstance(knowledge_data[0], dict):
                knowledge_name = knowledge_data[0].get("title", "")
        
        # Handle resource data in different formats
        resource_data = note.get("resource", [])
        resource_name = ""
        if isinstance(resource_data, dict):
            resource_name = resource_data.get("title", "")
        elif isinstance(resource_data, list) and resource_data:
            # Get the first resource title if it's a list
            if isinstance(resource_data[0], dict):
                resource_name = resource_data[0].get("title", "")
        ...
        if len(content) > 2000:
            formatted_note["preview"] = content[:1997] + "..."
        else:
            formatted_note["preview"] = content
...
async def get_resources(
    ...
    - Filter by resource type (book, article, video, course, podcast, other)
    ...
    Args:
        ...
        resource_type: Type of resource to filter by (e.g., "book", "article", "video", "course", "podcast", "other")
...
async def get_subjects(
    ...
    """
    Get the distribution of the user's knowledge subjects from Linko.
    
    Core capabilities:
    - Get overview of your learning distribution across subjects
    - Search for a specific subject by name
    
    Usage examples:
    - "What subjects am I studying?" → get_subjects()
    - "Show me my knowledge distribution" → get_subjects()
    - "Tell me about my Psychology subject" → get_subjects(subject_name="Psychology")
    
    Args:
        subject_name: Optional name of a specific subject to search for
    
    Returns:
        A dictionary containing:
        - subjects: List of all subjects with their stats
