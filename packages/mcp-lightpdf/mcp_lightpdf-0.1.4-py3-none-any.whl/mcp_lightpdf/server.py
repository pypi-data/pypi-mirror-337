            ...
            inputSchema={
                ...
                "properties": {
                    "file_paths": {
                        ...
                        "description": "要转换的PDF文件路径(绝对路径)或网络URL列表"
                    },
                    "format": {
                        "type": "string",
...
async def batch_convert_documents(file_paths, format: str, output_dir: str = "") -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    ...
    async def process_single_file(file_index: int, file_path: str):
        """处理单个文件的转换"""
        try:
            # 获取信号量限制并发
...
async def convert_document(file_path: str, format: str, output_dir: str = "") -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    ...
    for dir_candidate in output_dir_candidates:
        if is_dir_writable(dir_candidate):
            output_dir = dir_candidate
            break
    
    if not output_dir:
        await handle_error("无法找到可写的输出目录")
