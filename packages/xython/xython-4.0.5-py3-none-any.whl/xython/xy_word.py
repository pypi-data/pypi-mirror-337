...
class xy_word:
	...
	def __init__(self, file_name=""):
		...
		"""
		self.xcolor = xy_color.xy_color()
		self.xre = xy_re.xy_re()

		self.xvar = xy_common.xy_common().xvar  # package안에서 공통적으로 사용되는 변수들
	...
	def check_font_style(self, input_list):
		...
		:return:
		"""
		check_bold = self.xvar["check_bold"]
		check_italic = self.xvar["check_italic"]
		check_underline = self.xvar["check_underline"]
		check_breakthrough = self.xvar["check_breakthrough"]
		check_alignment = self.xvar["check_alignment"]
		for one in input_list:
			if one in check_bold.keys():
				...
				self.font_dic["align"] = self.xvar["check_alignment"][one]
	...
	def check_letter_type_no(self, input_letter_type):
		...
		if type(input_letter_type) == type(123):
			...
			result = self.xvar["letter_type_vs_enum"][input_letter_type]
		return result
	...
	def delete_from_doc_start_to_nth_char_end(self, input_no):
		...
		if input_no > 0:
			self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no,
									 Extend=1)  # Extend = 0 은 이동을 시키는 것이다
		else:
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no, Extend=1)
	...
	def delete_from_doc_start_to_nth_line_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no, Extend=1)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no, Extend=1)
	...
	def delete_from_doc_start_to_nth_para_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no, Extend=1)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no, Extend=1)
		self.word_application.Selection.range.Text = ""
	...
	def delete_from_doc_start_to_nth_word_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no,
									 Extend=1)  # Extend = 0 은 이동을 시키는 것이다
		else:
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no, Extend=1)
	...
	def delete_from_range_end_to_nth_char_end(self, input_no):
		...
		:return:
		"""
		self.range.Range.Move(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no)
		self.delete_range()
	...
	def delete_text_option_all(self):
		...
		:return:
		"""
		self.xvar["text_option"] = {"scolor": "", "size": "", "underline": "", "italic": "", "bold": "",
	...
	def draw_line_color_for_table(self, table_obj, input_position, input_scolor="bla"):
		...
		for one_position in input_position:
			enum = self.xvar["table_line_position_vs_enum"][one_position]
	...
	def draw_line_style_for_table(self, table_obj, line_list, line_style):
		...
		for one_position in line_list:
			enum = self.xvar["table_line_position_vs_enum"][one_position]
			table_obj.Borders(enum).LineStyle = self.xvar["table_line_style_vs_enum"][line_style]
	...
	def draw_outline_for_selection_with_option(self, line_style=1, line_color="blu", line_width="+"):
		...
		self.selection.Borders.OutsideLineWidth = self.xvar["linewidth_vs_enum"][line_width]
		self.selection.Borders.OutsideColor = self.xvar["colorname_vs_24bit"][line_color]
	...
	def expand_range_to_nth_letter_type_end(self, input_range, letter_type, input_no, except_current=False):
		...
		letter_type_no = self.xvar["letter_type_vs_enum"][letter_type]
	...
	def expand_selection_by_line_base(self):
		...
		"""
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["line"])
	...
	def expand_selection_by_word_base(self):
		...
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["word"])
	...
	def find_nth_text_in_selection_with_colored(self, search_text, input_no, input_bg_scolor="blu"):
		...
		while self.selection.Find.Execute(search_text):
			if no == input_no - 1:
				...
				self.selection.Range.HighlightColorIndex = self.xvar["highlight_colorindex"][
	...
	def font_option_for_align(self, input_align="left"):
		...
		:return:
		"""
		self.xvar["text_option"]["align"] = input_align
	...
	def font_option_for_bold(self, input_bold=True):
		...
		:return:
		"""
		self.xvar["text_option"]["bold"] = input_bold
	...
	def font_option_for_color(self, input_color):
		...
		:return:
		"""
		self.xvar["text_option"]["scolor"] = input_color
	...
	def font_option_for_italic(self, input_italic=True):
		...
		:return:
		"""
		self.xvar["text_option"]["italic"] = input_italic
	...
	def font_option_for_name(self, input_name="Arial"):
		...
		:return:
		"""
		self.xvar["text_option"]["name"] = input_name
	...
	def font_option_for_size(self, input_size):
		...
		self.xvar["text_option"]["size"] = input_size

	def font_option_for_strikethrough(self, input_strikethrough=True):
		""""""

		self.xvar["text_option"]["strikethrough"] = input_strikethrough
	...
	def font_option_for_subscript(self, input_subscript):
		...
		:return:
		"""
		self.xvar["text_option"]["subscript"] = input_subscript
	...
	def font_option_for_superscript(self, input_superscript):
		...
		:return:
		"""
		self.xvar["text_option"]["superscript"] = input_superscript
	...
	def font_option_for_underline(self, input_underline=True):
		...
		:return:
		"""
		self.xvar["text_option"]["underline"] = input_underline
	...
	def get_start_table_index_in_selection(self):
		...
		if self.selection.Information(12) == False:
			...
			self.selection.MoveEnd(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=IngEnd - IngStart)
	...
	def get_xy_from_doc_start_to_nth_word_end(self, input_no):
		...
		self.doc.Range(0, 0).Select()
		self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no - 1)
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["word"])
		x = self.selection.Start
		y = self.selection.End
		return [x, y]
	...
	def make_field_format(self, field_name, format_style=""):
		...
		:return:
		"""
		self.xvar["field_type_vs_enum"] = {"today": "DATE", "createdate": "CREATEDATE", "filename": "FILENAME", "page": "PAGE", "total_page": "NUMPAGES",
										   "username": "USERNAME", "title": "TITLE", "now": "TIME", "subject": "SUBJECT"}
		if format_style == "":
			result = '"' + self.xvar["field_type_vs_enum"][field_name] + '"'
		else:
			result = '"' + self.xvar["field_type_vs_enum"][field_name] + '"' + f" \\@ {format_style}"
		return result
	...
	def new_shape(self, shape_name=None, pxy_list=None, size_whlist=[100, 100], transparency_0_1=None,
		...
		:return:
		"""
		shape_enum = self.xvar["shape_name_vs_enum"][shape_name]
	...
	def move_aaa(self):
		...
		:return:
		"""
		self.selection.MoveEndUntil(self.xvar["goto_vs_enum"]["line"])
	...
	def move_cursor_from_doc_start_to_nth_char_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no - 1)
		else:
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no - 1)
	...
	def move_cursor_from_doc_start_to_nth_char_start(self, input_no):
		...
		if input_no > 0:
			...
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no)
		...
		if input_no > 0:
			if letter_type_no in [3, 2, 1]:
				self.selection.MoveRight(Unit=letter_type_no,
										 Count=input_no)  # self.xvar["letter_type_vs_enum"]["char"]
			else:
				self.selection.MoveDown(Unit=letter_type_no, Count=input_no)
		else:
			if letter_type_no in [3, 2, 1]:
	...
	def move_cursor_from_doc_start_to_nth_line_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no - 1)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no - 1)
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_doc_start_to_nth_para_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no - 1)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no - 1)
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_doc_start_to_nth_word_end(self, input_no):
		...
		if input_no > 0:
			...
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no)
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_next_line_end(self):
		...
		self.selection.MoveDown(self.xvar["letter_type_vs_enum"]["line"])
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_next_line_start(self):
		...
		:return:
		"""
		self.selection.GoToNext(self.xvar["goto_vs_enum"]["line"])
		self.selection.Collapse(1)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_next_word_end(self):
		...
		"""
		self.selection.MoveRight(self.xvar["letter_type_vs_enum"]["word"])
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_nth_line_end(self, input_no):
		...
		"""
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no)
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_nth_para_end(self, input_no):
		...
		"""
		if input_no > 0:
			self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no)
		else:
			self.selection.MoveUp(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no)
		self.selection.Collapse(0)  # 0 : end, 1:start
	...
	def move_cursor_from_selection_end_to_nth_sentence_start(self, input_no):
		...
		"""
		if input_no > 0:
			self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["sentence"], Count=input_no)
		else:
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["sentence"], Count=input_no)
		self.selection.Collapse(1)  # 0 : end, 1:start
	...
	def move_selection_for_graphic(self, direction_i=2, count_i=1):
		...
		if type(direction_i) != type(123):
			direction_i = self.xvar["direction_vs_enum"][direction_i]
	...
	def move_selection_for_item(self, item_i, direction_i=2, count_i=1):
		...
		:return:
		"""
		item_enum = self.xvar["goto_vs_enum"][item_i]
		direction_enum = self.xvar["direction_vs_enum"][direction_i]
	...
	def move_selection_for_table(self, direction_i=2, count_i=1):
		...
		:return:
		"""
		item_enum = self.xvar["goto_vs_enum"]["table"]
		direction_enum = self.xvar["direction_vs_enum"][direction_i]
		self.selection.GoTo(item_enum, direction_enum, count_i)
	...
	def new_table_with_options(self, x_no, y_no, input_position="", input_scolor="bla", input_line_style="basic",
		...
		border_color = self.xcolor.change_scolor_to_rgbint(input_scolor)
		position_list = self.xvar["position_vs_enum"][input_position]

		for line_position in position_list:
			enum = self.xvar["table_line_position_vs_enum"][line_position]
			new_table_obj.Borders(enum).LineStyle = self.xvar["table_line_style_vs_enum"][input_line_style]
	...
	def paint_color_for_cell_in_table(self, table_obj, xy, color_index="red"):
		...
		:param color_index:
		"""
		table_obj.Cell(xy[0], xy[1]).Shading.BackgroundPatternColor = self.xvar["word"]["color_24bit"][color_index]
	...
	def paint_foreground_for_selection(self, input_color):
		...
		:param input_color: 색이름
		"""
		self.selection.Font.Shading.ForegroundPatternColor = self.xvar["colorname_vs_24bit"][input_color]
	...
	def paint_highlight_for_selection_by_1_10(self, input_bg_scolor="blu"):
		...
		color_no_list = list(self.xvar["colorname_vs_enum"].Value)
		self.selection.Range.HighlightColorIndex = self.xvar["highlight_colorindex"][
			input_bg_scolor]  # input_bg_scolor="blu"
	...
	def paint_highlight_from_char_no1_to_char_no2(self, input_no1, input_no2, input_bg_scolor="blu"):
		...
		my_range.HighlightColorIndex = self.xvar["highlight_colorindex"][input_bg_scolor]  # input_bg_scolor="blu"
	...
	def paint_shading_background_for_selection(self, input_color):
		...
		:param input_color: 색이름
		"""
		self.selection.Font.Shading.BackgroundPatternColor = self.xvar["word"]["color_24bit"][input_color]
	...
	def paint_shading_foreground_for_selection(self, input_color):
		...
		:param input_color: 색이름
		"""
		self.selection.Font.Shading.ForegroundPatternColor = self.xvar["word"]["color_24bit"][input_color]
	...
	def paint_xy_cell_in_table(self, table_obj, xy, color_index="red"):
		...
		:param color_index:
		"""
		table_obj.Cell(xy[0], xy[1]).Shading.BackgroundPatternColor = self.xvar["colorname_vs_24bit"][color_index]
	...
	def read_text_for_current_para(self, input_no=1):
		...
		self.selection.GoTo(What=self.xvar["goto_vs_enum"]["footnote"], Which=self.xvar["direction_vs_enum"]["first"],
							Count=input_no)
	...
	def replace_in_selection_with_color_size_bold(self, i_text, i_color="red", i_size=11, i_bold=False):
		...
		my_range.Font.Color = self.xvar["colorname_vs_24bit"][i_color]
		my_range.Select()
	...
	def select_current_line_at_selection_end(self):
		...
		"""
		self.selection.Collapse(0)  # 0 : end, 1:start
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["wdLine"])
	...
	def select_current_para_at_selection_end(self):
		...
		"""
		self.selection.Collapse(0)  # 0 : end, 1:start
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["wdParagraph"])
	...
	def select_current_sentence_at_selection_end(self):
		...
		"""
		self.selection.Collapse(0)  # 0 : end, 1:start
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["wdSentence"])
	...
	def select_from_doc_start_to_nth_word_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no + 1, Extend=1)
		else:
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=abs(input_no) + 1, Extend=1)
	...
	def select_from_range_end_to_nth_line_end(self, input_range="", input_no=1):
		...
		"""
		input_range = self.check_range(input_range)
		input_range.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no, Extend=1)
	...
	def select_from_range_end_to_nth_para_end(self, input_range="", input_no=1):
		...
		"""
		input_range = self.check_range(input_range)
		input_range.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["para"], Count=input_no, Extend=1)
	...
	def select_from_range_end_to_nth_word_end(self, input_range="", input_no=1):
		...
		input_range = self.check_range(input_range)
		input_range.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no, Extend=1)
	...
	def select_from_selection_end_to_nth_line_end(self, line_no):
		...
		:return:
		"""
		self.selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["wdLine"], Count=line_no)
		self.selection.Expand(self.xvar["letter_type_vs_enum"]["wdLine"])
	...
	def select_from_selection_end_to_one_line_end(self, input_no=1):
		...
		"""
		self.selection.Collapse(0)  # 0 : end, 1:start
		self.word_application.Selection.MoveDown(Unit=self.xvar["letter_type_vs_enum"]["line"], Count=input_no,
												 Extend=1)
	...
	def select_multi_char_in_para_no(self, para_no, y, length):
		...
		self.xvar["new_range"] = self.doc.Range(x, y).Select()
	...
	def select_nth_char_at_doc_start(self, input_no):
		...
		"""
		self.move_cursor_to_doc_start()
		self.selection.MoveRight(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no)  # wdCharacter 1
	...
	def select_nth_word_at_doc_start(self, input_no):
		...
		if input_no > 0:
			...
			self.selection.MoveLeft(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no - 1)
	...
	def set_font_align_for_selection(self, input_value):
		...
		:return:
		"""
		self.selection.ParagraphFormat.Alignment = self.xvar["check_alignment"][input_value]
	...
	def set_font_default(self, *input_list):
		...
		for one in input_list[1:]:
			if one in check_bold.keys():
				...
				self.font_dic_default["align"] = self.xvar["check_alignment"][one]
			elif type(one) == type(123) and one < 100:
	...
	def set_font_in_range_with_options(self, range_object="", input_list=[]):
		...
		if input_list:
			...
			if type(input_list) == type([]):
				...
				self.xvar["font"].update(input_list)

		range_object.Font.Size = self.xvar["font"]["size"]
		range_object.Font.Bold = self.xvar["font"]["bold"]
		range_object.Font.Italic = self.xvar["font"]["italic"]
		range_object.Font.Name = self.xvar["font"]["name"]

		range_object.Font.Strikethrough = self.xvar["font"]["strikethrough"]
		range_object.Font.Subscript = self.xvar["font"]["subscript"]
		range_object.Font.Superscript = self.xvar["font"]["superscript"]
		range_object.Font.Underline = self.xvar["font"]["underline"]
		rgbint = self.xcolor.change_scolor_to_rgbint(self.xvar["font"]["color"])
	...
	def set_font_with_all_options(self, scolor_i="", size_i="", bold_tf=False, italic_tf=False, underline_tf=False,
		...
		if align_lmr: self.selection.ParagraphFormat.Alignment = self.xvar["check_alignment"][align_lmr]
		if bakground_scolor:
	...
	def set_line_width_for_table(self, table_obj, inside_width="", outside_width=""):
		...
		:param outside_width:
		"""
		table_obj.Borders.InsideLineWidth = self.xvar["linewidth_vs_enum"][inside_width]
		table_obj.Borders.OutsideLineWidth = self.xvar["linewidth_vs_enum"][outside_width]
	...
	def set_range_from_doc_start_to_char_end(self, input_no):
		...
		self.range.Move(Unit=self.xvar["letter_type_vs_enum"]["char"], Count=input_no)
	...
	def set_range_from_doc_start_to_nth_word_end(self, input_no):
		...
		"""
		self.range = self.doc.Range(0, 0)
		self.range.Move(Unit=self.xvar["letter_type_vs_enum"]["word"], Count=input_no + 1)
		new_y = self.range.End
		self.range = self.doc.Range(0, new_y)
		return self.range
	...
	def setup_font(self, input_list):
		...
		for one in input_list:
			if type(one) == type(123):
				self.xvar["font"]["size"] = one
			elif one in ["진하게", "굵게", "찐하게", "bold"]:
				self.xvar["font"]["bold"] = True
			elif one in ["italic", "이태리", "이태리체", "기울기"]:
				self.xvar["font"]["italic"] = True
			elif one in ["strikethrough", "취소선", "통과선", "strike"]:
				self.xvar["font"]["strikethrough"] = True
			elif one in ["subscript", "하위첨자", "밑첨자"]:
				self.xvar["font"]["subscript"] = True
			elif one in ["superscript", "위첨자", "웃첨자"]:
				self.xvar["font"]["superscript"] = True
			elif one in ["underline", "밑줄"]:
				self.xvar["font"]["underline"] = True
			elif one in ["vertical", "수직", "가운데"]:
				self.xvar["font"]["align_v"] = 3
			elif one in ["horizental", "수평", "중간"]:
				self.xvar["font"]["align_h"] = 2
			elif one in self.xcolor.xvar["check_color_name"].keys():
				self.xvar["font"]["color"] = one
			else:
				self.xvar["font"]["name"] = one

		result = copy.deepcopy(self.xvar["font"])
		return result
	...
	def setup_font_default(self):
		...
		self.xvar["font"]["bold"] = False
		self.xvar["font"]["color"] = "bla"
		self.xvar["font"]["italic"] = False
		self.xvar["font"]["name"] = "Arial"
		self.xvar["font"]["size"] = 12
		self.xvar["font"]["strikethrough"] = False
		self.xvar["font"]["subscript"] = False
		self.xvar["font"]["superscript"] = False
		self.xvar["font"]["alpha"] = False  # tintandshade를 이해하기 쉽게 사용하는 목적
		self.xvar["font"]["underline"] = False
		self.xvar["font"]["align_v"] = 3  # middle =3, top = 1, bottom = 4, default=2
		self.xvar["font"]["align_h"] = 1  # None =1, center=2, left=1, default=1
		self.xvar["font"]["color"] = 1
	...
	def write_text_at_table_in_header_all(self, xy=[1, 1], input_value="", field_code="", field_format="", position="left"):
		self.xvar["position_vs_enum"] = {"left": 0, "middle": 1, "right": 2}
		for section in self.doc.Sections:
			header = section.Headers(1)
			header_range = header.Range
			for table in header_range.Tables:
				if input_value != "" and field_code == "":
					...
					aaa.ParagraphFormat.Alignment = self.xvar["position_vs_enum"][position]
	...
	def write_text_with_font_options(self, input_text):
		...
		if self.xvar["text_option"]["scolor"]:        my_range.Font.Color = self.xcolor.change_scolor_to_rgbint(
			self.xvar["text_option"]["scolor"])
		if self.xvar["text_option"]["size"]:        my_range.Font.Size = self.xvar["text_option"]["size"]
		if self.xvar["text_option"]["underline"]:    my_range.Font.Underline = self.xvar["text_option"]["underline"]
		if self.xvar["text_option"]["italic"]:        my_range.Font.Italic = self.xvar["text_option"]["italic"]
		if self.xvar["text_option"]["bold"]:        my_range.Font.Bold = self.xvar["text_option"]["bold"]
		if self.xvar["text_option"]["strikethrough"]: my_range.Font.StrikeThrough = self.xvar["text_option"][
			"strikethrough"]
		if self.xvar["text_option"]["name"]:        my_range.Font.Name = self.xvar["text_option"]["name"]
		if self.xvar["text_option"]["align"]:        my_range.ParagraphFormat.Alignment = self.xvar["text_option"][
			"align"]
		if self.xvar["text_option"]["subscript"]:    my_range.Font.Subscript = self.xvar["text_option"]["subscript"]
		if self.xvar["text_option"]["superscript"]: my_range.Font.Superscript = self.xvar["text_option"]["superscript"]
