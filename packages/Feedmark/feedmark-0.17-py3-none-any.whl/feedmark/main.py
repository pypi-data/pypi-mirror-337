...
from feedmark.loader import (
    read_document_from,
    read_refdex_from,
    convert_refdex_to_single_filename_refdex,
)
...
def main(args):
    ...
    argparser.add_argument("--version", action="version", version="%(prog)s 0.17")
    ...
    refdex = read_refdex_from(
        input_refdexes,
        input_refdex_filename_prefix=options.input_refdex_filename_prefix,
    )
            ...
            for section in document.sections:
                if section.title in refdex:
                    ...
                    if entry["anchor"] != section.anchor:
                        raise ValueError(
                            "Inconsistent anchors: {} in refex, {} in document".format(
                                entry["anchor"], section.anchor
                            )
                        )
    ...
    if refdex:
        for document in documents:
            document.rewrite_link_ref_defs(refdex)
    ...
    if options.output_json:
        ...
        output_json = {"documents": [d.to_json_data(**json_options) for d in documents]}
        try:
