...
def verify_vis_psf_files():
    ...
    if not os.path.isfile(f"{DATADIR}/pandora_vis_psf.fits"):
        ...
        logger.warning(f"VIS PSF downloaded to {DATADIR}/pandora_vis_psf.fits.")
...
def verify_nir_psf_files():
    ...
    if not os.path.isfile(f"{DATADIR}/pandora_nir_psf.fits"):
        ...
        logger.warning(f"NIR PSF downloaded to {DATADIR}/pandora_nir_psf.fits.")
...
def make_lowres_package_data():
    ...
    hdu = fits.HDUList(
        [
            ...
            fits.ImageHDU(psf.wavelength.value.astype(np.float32), name="WAVELENGTH"),
        ]
    )
    hdu[2].header["UNIT"] = psf.wavelength.unit.to_string()
    hdu.writeto(PACKAGEDIR + "/data/pandora_nir_psf_lowres.fits", overwrite=True)
    ...
    hdu = fits.HDUList(
        [
            primaryhdu,
            fits.CompImageHDU(
                psf.psf_flux[128:-128, 128:-128, 3:6, 3:6].transpose([2, 3, 1, 0]),
                name="PSF",
            ),
            fits.ImageHDU(psf.row[3:6, 3:6].value.astype(np.float32), name="ROW"),
            fits.ImageHDU(psf.column[3:6, 3:6].value.astype(np.float32), name="COLUMN"),
        ]
    )
    hdu[2].header["UNIT"] = psf.row.unit.to_string()
    hdu[3].header["UNIT"] = psf.column.unit.to_string()
    hdu.writeto(PACKAGEDIR + "/data/pandora_vis_psf_lowres.fits", overwrite=True)
