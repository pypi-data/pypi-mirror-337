classdef  SparsityInterfaceCommon < SwigRef
  ...
  methods
    ...
    %SUM Returns summation of all elements.
    %
    %  Sparsity = SUM(Sparsity x)
    %  DM = SUM(DM x)
    %  SX = SUM(SX x)
    %  MX = SUM(MX x)
    %  Sparsity = SUM(Sparsity x, int dim)
    %  Sparsity = SUM(Sparsity x, char dim)
    %  DM = SUM(DM x, int dim)
    %  DM = SUM(DM x, char dim)
    %  SX = SUM(SX x, int dim)
    %  SX = SUM(SX x, char dim)
    %  MX = SUM(MX x, int dim)
    %  MX = SUM(MX x, char dim)
    %
    %
    %Extra doc: https://github.com/casadi/casadi/wiki/L_2d6
    %
    %Doc source: 
    %https://github.com/casadi/casadi/blob/main/casadi/core/sparsity_interface.hpp#L596
    %
    %Implementation: 
    %https://github.com/casadi/casadi/blob/main/casadi/core/sparsity_interface.hpp#L596-L609
    %
    %
    %
