...
class MarkdownRenderer(Renderer):
    ...
    def render_link_ref_def(self, element: block.LinkRefDef) -> str:
        ...
        return f"[{element.label.text[1:-1]}]: {link_text}\n"
    ...
    def render_underscore_emphasis(self, element: inline.UnderscoreEmphasis) -> str:
        return f"_{self.render_children(element)}_"
    ...
    def render_link_ref(self, element: inline.LinkRef) -> str:
        link_text = self.render_children(element)
        if element.label:
            label = element.label
    ...
    def render_image_ref(self, element: inline.ImageRef) -> str:
        image_text = self.render_children(element)
        if element.label:
            label = element.label
            if label[0] == "[" and label[-1] == "]":
                label = label[1:-1]
            return f"![{image_text}][{label}]"
        else:
            return f"![{image_text}]"
