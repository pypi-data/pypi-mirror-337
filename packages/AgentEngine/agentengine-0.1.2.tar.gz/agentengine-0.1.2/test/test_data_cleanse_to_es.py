...
async def test_data_cleanse_to_es_integration():
    ...
    try:
        ...
        example_docs_dir = Path("example_docs")
        if not example_docs_dir.exists():
            print("Error: example_docs directory not found")
            return
        
        # Get all test files
        test_files = [
            str(f) for f in example_docs_dir.glob("**/*")
        ...
        async with httpx.AsyncClient(timeout=180.0, limits=limits) as client:  # 增加整体客户端超时和并发连接
            ...
            success = await test_single_file(client, data_cleanse_service_url, test_file, "test_new_index")
            ...
            success = await test_multiple_files(client, data_cleanse_service_url, test_files_subset, "test_multiple_files")
            ...
            success = await test_multiple_files(client, data_cleanse_service_url, larger_test_files, "test_larger_batch")
