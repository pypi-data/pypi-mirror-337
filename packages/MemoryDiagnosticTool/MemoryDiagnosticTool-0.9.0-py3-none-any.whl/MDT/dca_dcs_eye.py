...
clk_data_hash = re.compile("CHANNEL:\s*([0-9]),\s*PHY:\s*([0-9]),\s*PHYINIT:\s*BTFW:\s*\[DCSTDCDS\]\s*([0-9a-f]+)")
        ...
        for ch in chs:
            for phy in phys:
                for rw_i in ccc_rw_range:
                    ...
                    axes(ax[i])
                    title(f'CH{ch} PHY{phy} {rw}', fontsize = 12)
...
def analyze_log(datas):
    ...
    for line in datas:
        ...
        if(clk_data_hash_search):
            ...
            ##if channel == 1 and phy ==0 and reg =='0x49':
                # print(reg)
            ##    print(data)
