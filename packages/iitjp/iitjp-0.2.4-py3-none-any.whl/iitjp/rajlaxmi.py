...
def get_audio_path(filename):
    ...
    # Try all possible locations for the audio file
    possible_locations = [
        # First check in the DATA_DIR
        os.path.join(DATA_DIR, filename),
        
        # Check in the package directory itself
        os.path.join(os.path.dirname(os.path.abspath(__file__)), filename),
        
        # Check in the package data folder
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", filename),
        
        # Check in the package's parent data folder
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", filename),
        
        # Check in site-packages data folder
        os.path.join(PACKAGE_ROOT, "data", filename),
        
        # Check in site-packages itself
        os.path.join(PACKAGE_ROOT, filename),
        ...
        # Add specific site-packages paths
    ...
    # Try using pkg_resources if standard path resolution fails
    try:
        # Try to get the resource from the package
        import pkg_resources
        if pkg_resources.resource_exists("iitjp", f"data/{filename}"):
            return pkg_resources.resource_filename("iitjp", f"data/{filename}")
        elif pkg_resources.resource_exists("iitjp", filename):
            return pkg_resources.resource_filename("iitjp", filename)
    except Exception as e:
        print(f"[ℹ️ pkg_resources check failed: {e}]")
    
    # If pkg_resources fails, try another method for package data
    try:
        import importlib.resources as resources
        try:
            # Try with data subdirectory
            with resources.path("iitjp.data", filename) as path:
                return str(path)
        except (ImportError, ModuleNotFoundError):
            # Try directly in package
            with resources.path("iitjp", filename) as path:
                return str(path)
    except Exception as e:
        print(f"[ℹ️ importlib.resources check failed: {e}]")
    
    # Couldn't find the file
    print(f"[⚠️ Audio file '{filename}' not found in any expected location]")
    
    # As a last resort, look recursively in the package root and its parent directories
    for root_dir in [PACKAGE_ROOT, os.path.dirname(PACKAGE_ROOT), os.getcwd()]:
        for root, dirs, files in os.walk(root_dir):
            if filename in files:
                file_path = os.path.join(root, filename)
                print(f"[✓] Found audio file by searching: {file_path}")
                return file_path
