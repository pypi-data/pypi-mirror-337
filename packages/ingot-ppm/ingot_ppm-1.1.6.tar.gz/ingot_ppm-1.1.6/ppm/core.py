            ...
            async with self.session.get(f"{self.PYPI_JSON}/{package_name}/json") as response:
                if response.status == 200:
                    ...
                    if not data or not isinstance(data, dict):
                        print(f"获取{package_name}的信息失败：无效的响应数据")
                        return None
                    if 'releases' not in data:
                        print(f"获取{package_name}的信息失败：缺少版本信息")
                        return None
                    return data
                print(f"获取{package_name}的信息失败：HTTP状态码 {response.status}")
                return None
        except aiohttp.ClientError as e:
            print(f"获取包信息时发生错误: {e}")
            return None
        except json.JSONDecodeError as e:
            print(f"解析{package_name}的JSON响应时发生错误: {e}")
            return None
        except Exception as e:
            print(f"获取{package_name}信息时发生未知错误: {e}")
            return None
                ...
                if version:
                    ...
                    try:
                        ...
                        if not versions:
                            ...
                            return
                        target_version = versions[-1] if latest or not version else versions[0]
                    except KeyError:
