...
def save_modern_cursor_rules(rules_dir: str, rule_title: str, pattern: str, description: str, content: str, references: list = None) -> str:
    """
    Save a cursor rule file in the modern .mdc format.
    
    Args:
        rules_dir: Directory where to save the rule file
        rule_title: Title of the rule
        pattern: Glob pattern for files this rule applies to
        description: Semantic description of when the rule should be applied
        content: The main content of the rule file in markdown format
        references: Optional list of file paths to reference
    ...
    # Sanitize the rule title for use as filename
    # Replace spaces and other unsafe characters, limit length
    ...
    # Generate a safe filename from the rule title
    safe_filename = sanitize_filename(rule_title)
    file_path = os.path.join(rules_dir, f"{safe_filename}.mdc")
    
    # Construct the rule file content in .mdc format
    rule_content = [
        f"# {rule_title}",
        "",
        f'pattern: "{pattern}"',
    ...
    if references:
        ...
        for ref in references:
            rule_content.append(f"@file: {ref}")
