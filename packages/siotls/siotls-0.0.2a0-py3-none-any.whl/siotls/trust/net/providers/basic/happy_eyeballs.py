...
RFC6555_ENABLED = None

__all__ = ["RFC6555_ENABLED", "cache", "create_connection"]
...
class _RFC6555ConnectionManager:
    ...
    def _create_socket(self, family, socktype, proto, sockaddr):
        ...
        try:
            ...
            # this purpose.
            if self.timeout is socket._GLOBAL_DEFAULT_TIMEOUT:  # noqa: SLF001
    ...
    def _wait_for_connection(self, last_wait):
        ...
        select_timeout = (
                 self._get_select_time() if not last_wait
            else self._get_remaining_time() if self.timeout is not None
            else None
        )
    ...
    def _remove_all_errored_sockets(self):
        for sock in filter(self._is_socket_errored, self._sockets):
            self._selector.unregister(sock)
            self._sockets.remove(sock)
            _RFC6555ConnectionManager._close_socket(sock)
