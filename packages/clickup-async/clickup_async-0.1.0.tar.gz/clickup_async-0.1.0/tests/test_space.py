...
async def test_get_spaces(client, workspace):
    ...
    archived_spaces = await client.get_spaces(workspace.id, archived=True)
...
async def test_get_space(client, test_space):
    ...
    space_details = await client.get_space(test_space.id)
...
async def test_delete_space(client, workspace):
    ...
    try:
        ...
        space = await client.create_space(name=space_name, workspace_id=workspace.id)
    ...
    # Delete the space
    result = await client.delete_space(space.id)
    assert result is True

    # Verify the space is deleted
    with pytest.raises(ResourceNotFound):
        await client.get_space(space.id)
...
async def test_update_space_name(client, test_space):
    ...
    updated_space = await client.update_space(space_id=test_space.id, name=new_name)
