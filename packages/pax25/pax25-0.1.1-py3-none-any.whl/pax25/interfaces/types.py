...
class Interface(Generic[S]):
    ...
    def is_preferred_address(self, address: Address) -> bool:
        """
        Returns if this address is a preferred one. The station will search for an
        interface that considers an address preferred before trying more general
        channels.

        Note that this will be the first digipeater in a route, or the destination
        address if there are no digipeaters.
        """
        raise NotImplementedError  # pragma: no cover
...
class ConnectionSpec(TypedDict):
    """
    Specification of a UDP connected target.
    """
...
class UDPSettings(TypedDict, total=False):
    """
    Settings for the file input.
    """
    ...
    listening_port: int
    gateway: bool
    connections: Required[list[ConnectionSpec]]
...
class UDPInterfaceConfig(TypedDict):
    """
    Configuration for the UDP Interface.
    """
