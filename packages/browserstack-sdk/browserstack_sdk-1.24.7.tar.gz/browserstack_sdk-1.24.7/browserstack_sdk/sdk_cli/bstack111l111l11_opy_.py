...
class bstack111l11l111_opy_:
    timeout: int
    bstack111l11l11l_opy_: Union[None, Callable]
    bstack111l111l1l_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack111l111ll1_opy_=1, bstack111l11l11l_opy_=None, bstack111l111l1l_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack1l1l1l1_opy_ (u"ࠨࡩ࡯ࡸࡤࡰ࡮ࡪࠠ࡫ࡱࡥ࠾ࠥࠨ࿍") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        self.threads = [threading.Thread(target=self.worker, daemon=True) for _ in range(self.bstack111l111ll1_opy_)]
        ...
        while not self.bstack111l111lll_opy_.is_set():
            try:
                ...
                try:
                    ...
                    if callable(self.bstack111l11l11l_opy_):
                        self.bstack111l11l11l_opy_(e, job)
                finally:
                    self.queue.task_done()
            except queue.Empty:
                pass
            except Exception as e:
                if callable(self.bstack111l111l1l_opy_):
                    self.bstack111l111l1l_opy_(e)