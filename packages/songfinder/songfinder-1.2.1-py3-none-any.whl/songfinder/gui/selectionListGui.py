            ...
            for media in medias:
                ...
                if media and (extention not in mediaType):
                    anwser = tkMessageBox.askyesno(
                        "Type de fichier",
                        f'Le fichier "{mediaFile}.{extention}" ne semble pas être un fichier audio ou vidéo. '
        ...
        if images:
            for full_path in images:
                ...
                if full_path and (extention not in typeImage):
                    anwser = tkMessageBox.askyesno(
                        "Type de fichier",
                        f'Le fichier "{mediaFile}.{extention}" ne semble pas être un fichier image. '
                        "Voulez-vous continuer malgré tout ?",
                    )
        ...
        if element and self._searchFuntion and not element.exist() and element.nom:
            ...
            if searchResults and searchResults[0].etype == element.etype:
                tkMessageBox.showinfo(
                    ...
                    f'il va être échangé par "{searchResults[0].nom}".',
                )
