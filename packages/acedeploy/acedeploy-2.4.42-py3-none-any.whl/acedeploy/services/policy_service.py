...
def remove_database_name_from_view_statement(view_definition: str) -> str:
    ...
    """
    pattern = r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:SECURE\s+)?(?:RECURSIVE\s+)?VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\"\.)|(\w+\.))(?P<schema>(\"\w+\")|(\w+))\.(?P<name>(\"\w+\")|(\w+)\b)(?!\.)"
...
def add_string_to_object_ddl(object_type: PolicyAssignmentObjectTypes, object_statement: str, input_string: str) -> str:
    ...
    """
    if object_type == PolicyAssignmentObjectTypes.VIEW:
        regex_object=r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:SECURE\s+)?(?:RECURSIVE\s+)?VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\")|(\w+)\.)?(?P<schema>(\"\w+\")|(\w+)\.)?(?P<name>(\"\w+\")|(\w+)\b)(?!\.)"
    elif object_type == PolicyAssignmentObjectTypes.TABLE:
        regex_object=r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:TRANSIENT\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\")|(\w+)\.)?(?P<schema>(\"\w+\")|(\w+)\.)?(?P<name>(\"\w+\")|(\w+)\b)(?!\.)"
    elif object_type == PolicyAssignmentObjectTypes.DYNAMICTABLE:
        regex_object=r"\s*CREATE\s+(?:OR\s+REPLACE\s+)?DYNAMIC\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<database>(\"\w+\")|(\w+)\.)?(?P<schema>(\"\w+\")|(\w+)\.)?(?P<name>(\"\w+\")|(\w+)\b)(?!\.)"
    else:
