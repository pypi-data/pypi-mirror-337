...
class RedisObject:
    def __init__(self, host: str, port: int, password: str, db: int, key: str | int = None, suffix: str = None):
    ...
    async def __get_all_keys(self, offset: int = None, limit: int = None) -> list:
        ...
        cursor = 0
        while True:
            cursor, keys = await self._redis.scan(cursor, match=f"{self.__category__}{self._suffix}:*")
    ...
    async def load(self, key: str | int = None) -> bool:
        ...
        result = await self._redis.hgetall(name=f"{self.__category__}{self._suffix}:{self.key}")
    ...
    async def save(self, key: str | int = None, stream: bool = False, ttl_day: int = 0, ttl_hour: int = 0,
        ...
        hash_key = f"{self.__category__}{self._suffix}:{self.key}"
        ...
        if stream:
            await self._redis.xadd(f"{self.__category__}{self._suffix}:{STREAM_KEY}", {"event": STREAM_EVENT_SAVE, "hash_key": hash_key})
    ...
    async def delete(self, key: str | int = None) -> bool:
        ...
        result = await self._redis.delete(f"{self.__category__}{self._suffix}:{self.key}")
        if result:
            return True
    ...
    async def get_ttl(self, key: str | int = None) -> int:
        ...
        ttl = await self._redis.ttl(f"{self.__category__}{self._suffix}:{self.key}")
        if ttl != -2:
            return ttl
    ...
    async def get_stream_in_interval(self, start_timestamp: int, end_timestamp: int) -> list:
        stream_key = f"{self.__category__}{self._suffix}:{STREAM_KEY}"
