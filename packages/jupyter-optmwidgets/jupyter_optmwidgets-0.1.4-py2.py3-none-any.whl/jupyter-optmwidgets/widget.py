...
@register_line_magic
def create_mermaid(line):
    """Create a Mermaid diagram."""
    return None

class OptmWidget(widgets.DOMWidget):
    """A widget for displaying OPT Mentor content."""
    _view_name = widgets.Unicode('OptmView').tag(sync=True)
    _model_name = widgets.Unicode('OptmModel').tag(sync=True)
    _view_module = widgets.Unicode('jupyter-optmwidgets').tag(sync=True)
    _model_module = widgets.Unicode('jupyter-optmwidgets').tag(sync=True)
    _view_module_version = widgets.Unicode('^0.1.3').tag(sync=True)
    _model_module_version = widgets.Unicode('^0.1.3').tag(sync=True)
...
@register
class JSWidget(widgets.DOMWidget, widgets.ValueWidget):
