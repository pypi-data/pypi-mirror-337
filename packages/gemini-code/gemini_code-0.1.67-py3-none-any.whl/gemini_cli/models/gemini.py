...
RE-INTEGRATED QUOTA FALLBACK TO 1.5 PRO.
Includes Summarizer, Quality tools. Prompt v12. Syntax FIXED.
"""

import google.generativeai as genai
from google.generativeai.types import FunctionDeclaration, Tool
# --- ADDED Import for Quota Error ---
...
MAX_AGENT_ITERATIONS = 10 # Reduced back slightly
...
class GeminiModel:
    """Interface for Gemini models implementing an agentic loop with fallback."""

    # --- MODIFIED __init__ for Fallback Tracking ---
    def __init__(self, api_key: str, console: Console, model_name: str ="gemini-2.5-pro-exp-03-25"):
        ...
        self.initial_model_name = model_name # Store the originally requested model
        self.current_model_name = model_name # Track the currently active model
        ...
        try:
            # Use helper to initialize self.model with current_model_name
    ...
    # --- ADDED Helper to initialize model instance ---
    def _initialize_model_instance(self):
        ...
        # Check if model name is valid before attempting creation? Optional.
        try:
            ...
            logging.info(f"Model instance '{self.current_model_name}' created successfully.")
             ...
             logging.error(f"Failed to create model instance for '{self.current_model_name}': {init_err}", exc_info=True)
             # Re-raise the exception to be caught by the caller (__init__ or generate)
             raise init_err
    # --- End ADDED Helper ---
    ...
    # --- generate function RE-INTEGRATING Quota Fallback Logic ---
    def generate(self, prompt: str, conversation=None) -> str | None:
        logging.info(f"Agent Loop - Starting task: '{prompt[:100]}...' using model '{self.current_model_name}'")
        ...
        # Start history fresh for each user request in agent mode? Or accumulate?
        # Let's keep it fresh for now to simplify context management, but pass system prompt.
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                try:
                    # --- ADDED try/except ResourceExhausted ---
                    ...
                    if self.current_model_name == FALLBACK_MODEL:
                        logging.error(f"Quota also exceeded for fallback model '{FALLBACK_MODEL}'. Cannot proceed.")
                        self.console.print(f"[bold red]API quota exceeded for both primary ({self.initial_model_name}) and fallback ({FALLBACK_MODEL}) models. Please try again later.[/bold red]")
                        return f"Error: API quota exceeded for primary and fallback models."

                    # Switch to fallback model IF it's different from current
                    if self.current_model_name != FALLBACK_MODEL:
                        self.console.print(f"\n[bold yellow]Warning:[/bold yellow] Quota limit reached for [cyan]{self.current_model_name}[/cyan].")
                        self.console.print(f"Switching to fallback model [cyan]{FALLBACK_MODEL}[/cyan] for the rest of this session...")
                        self.current_model_name = FALLBACK_MODEL
                        try:
                            self._initialize_model_instance() # Re-create self.model with fallback name
                            self.console.print("[green]Fallback model initialized. Retrying last step...[/green]")
                            time.sleep(1)
                            # Add note to history?
                            # current_task_history.append({'role':'user', 'parts': [f"(System note: Switched to model {self.current_model_name} due to quota limit. Retrying last request.)"]})
                            continue # Retry the generate_content call at the start of the loop
                            ...
                            return f"Error: Quota exceeded for primary model, and failed to switch to fallback."
                    else:
                         # This case shouldn't be hit due to the check above, but safety first
                         logging.error("Already using fallback model, cannot fall back further.")
                         self.console.print(f"[bold red]API quota exceeded for model {self.current_model_name}. No further fallback available.[/bold red]")
                ...
                self.chat_history.append({'role': 'model', 'parts': [generated_text]}) # Use persistent history
                # self._manage_context_window() # Add context management if needed
                ...
                if not executed_tool_info:
                    logging.warning(f"No actionable tool calls found (Iter {iteration_count}). Asking LLM to clarify or finish.")
                    ...
                    self.chat_history.append({'role': 'user', 'parts': [clarification_request]}) # Use persistent history
                ...
                self.chat_history.append({'role': 'user', 'parts': [results_feedback]}) # Use persistent history
                # self._manage_context_window()
            ...
            if task_completed and final_summary:
                 ...
                 # Add final summary to history? Maybe not needed for user display.
                 ...
                 # ... (Unexpected exit logic remains the same) ...
    ...
    # --- _manage_context_window (optional context management) ---
    # def _manage_context_window(self): ...


    # --- _find_last_model_message (helper) ---
    ...
    # ... (_parse_and_execute_tool_calls remains the same robust version with Summarizer handling) ...
    def _parse_and_execute_tool_calls(self, text: str) -> list[tuple[str, dict]]:
        ...
        # ... (Same robust parsing logic as previous, including SummarizeCodeTool special init) ...
        executed_call_results = []
        matches = TOOL_CALL_PATTERN.finditer(text); any_matches = False
        for match in matches:
            any_matches = True; call_str = match.group(0); func_name = match.group(1); args_str = match.group(2).strip()
            logging.info(f"Found potential tool call: cli_tools.{func_name}({args_str[:100]}...)")
            args = {}; result_data = {"call_str": call_str}
            try:
                ...
                for kw_match in kw_arg_pattern.finditer(args_str):
                    ...
                    if kw_match.group(3) is not None: args[arg_name] = kw_match.group(3); logging.debug(f"P:'{arg_name}' triple-single")
                    elif kw_match.group(4) is not None: args[arg_name] = kw_match.group(4); logging.debug(f"P:'{arg_name}' triple-double")
                    elif kw_match.group(5) is not None: try: args[arg_name] = bytes(kw_match.group(5), "utf-8").decode("unicode_escape"); logging.debug(f"P:'{arg_name}' single") except Exception: args[arg_name] = kw_match.group(5)
                    elif kw_match.group(6) is not None: try: args[arg_name] = bytes(kw_match.group(6), "utf-8").decode("unicode_escape"); logging.debug(f"P:'{arg_name}' double") except Exception: args[arg_name] = kw_match.group(6)
                    elif kw_match.group(7) is not None:
                        literal_str = kw_match.group(7).strip()
                        try: args[arg_name] = ast.literal_eval(literal_str); logging.debug(f"P:'{arg_name}' literal")
                        except (ValueError, SyntaxError): logging.warning(f"Parse lit fail '{literal_str}' for '{arg_name}'. String fallback."); args[arg_name] = literal_str
                    else: logging.warning(f"Cannot parse value for '{arg_name}'. Skipping."); continue
                    last_pos = kw_match.end()
                if last_pos < len(args_str.rstrip(')')): logging.warning(f"Unparsed args remaining: '{args_str[last_pos:]}'")
            except Exception as parse_error: logging.error(f"Arg parse failed: {parse_error}", exc_info=True); result_data["result"] = f"Error: Arg parse failed - {parse_error}"; executed_call_results.append((func_name, result_data)); continue
            try:
                logging.info(f"Executing: func='{func_name}', args={ {k: (v[:50] + '...' if isinstance(v, str) and len(v)>50 else v) for k,v in args.items()} })")
                ...
                if not tool_instance: raise ValueError(f"Tool '{func_name}' unavailable/uninstantiable.")
                result = tool_instance.execute(**args); result_str = str(result) if result is not None else "(No output)"
                result_data["result"] = result_str; executed_call_results.append((func_name, result_data)); logging.info(f"Exec success: '{call_str}'. Result len: {len(result_str)}")
            except Exception as exec_error: logging.error(f"Exec failed: '{call_str}': {exec_error}", exc_info=True); result_data["result"] = f"Error during execution: {exec_error}"; executed_call_results.append((func_name, result_data))
        if not any_matches: logging.info("No parsable cli_tools calls found.")
        return executed_call_results
    ...
    def _create_system_prompt(self) -> str:
        ...
        system_prompt = f"""You are 'Gemini Code', an AI coding assistant operating in a CLI within the user's project directory. **Your ONLY way to interact is by generating Python code snippets that call functions from the `cli_tools` module.**
