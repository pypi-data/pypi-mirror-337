...
EMA_BASE = EmaParamBase instance, defined by model and data setup.
...
# ---------------------- Main module-global constant object
EMA_BASE = None
# = single package-global constant instance, created by initialize()
# to be used by modules ema_model, ema_group, and ema_respondent
# *** BUT: multiproc Pool workers see only EMA_BASE = None, not the initialized value.

def initialize(emf, effects, rv_class,
               restrict_attribute=False,
               restrict_threshold=False):   # ****** <--- copy EmaParamBase.initialize
    ...
    :return: an EmaParamBase instance = global EMA_BASE

    Result: global EMA_BASE instance with all properties initialized
    NOTE: xi parameter slices in order: all alpha, all beta, all eta
    """
    global EMA_BASE
    EMA_BASE = EmaParamBase.initialize(emf, effects, rv_class,
                                       restrict_attribute,
                                       restrict_threshold)
    return EMA_BASE
