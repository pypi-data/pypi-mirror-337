...
class xy_outlook:
	...
	def data_properties_names_for_mail(self):
		...
		result = self.xvar["all_properties_list"]
		return result
	...
	def filter_mails_in_input_folder_by_yyyymmdd(self, input_folder, input_yyyy_mm_dd):
		...
		folder_obj = self.check_any_folder(input_folder)
		dt_obj_to = self.xtime.change_anytime_to_dt_obj(input_yyyy_mm_dd)
	...
	def filter_mails_in_input_folder_from_today_to_nth_day_later(self, input_folder, input_day_no):
		...
		:return:
		"""
		dt_obj_to = self.xtime.get_dt_obj_for_today()
	...
	def filter_mails_in_input_mails_by_body_with_jf_sql(self, input_mails, jf_sql):
		...
		for index, one_mail in enumerate(mail_l1d):
			if self.xre.search_with_jf_sql(jf_sql, one_mail.Body):
	...
	def fliter_mails_in_input_folder_between_date1_and_date2(self, input_folder, start_date, end_date, sort_by=""):
		...
		:return:
		"""
		dt_obj_from = self.xtime.change_anytime_to_dt_obj(start_date)
		dt_obj_to = self.xtime.change_anytime_to_dt_obj(end_date)
	...
	def get_information_for_input_mail(self, input_mail):
		...
		abc = self.xutil.get_all_properties_for_object(input_mail)
		print(abc)
	...
	def get_properties_for_input_mail(self, input_mail):
		...
		result = self.xutil.get_all_properties_for_object(input_mail)
		return result
	...
	def print_mail_set_for_basic_datas(self, input_mails):
		...
		try:
			for one_mail in input_mails:
				...
				temp.append(self.xtime.change_dt_obj_to_text_time_with_format(one_mail.ReceivedTime))
	...
	def sample_001(self):
		...
		"""
		yyyy_mm_dd = self.xtime.get_yyyy_mm_dd_for_today()
	...
	def sort_mail_list_by_input_property(self, mail_list, input_property):
		...
		sorted_l2d = self.xutil.sort_list_2d_by_index(new_mail_list, 0)
	...
	def get_mail_obj_between_today_to_nth_day_before_for_input_folder(self, input_folder, input_no, sort_by=""):
		"""
		...
		:return:
		"""
		dt_to =self.xtime.get_dt_obj_for_ymd_list_as_23h_59m_59s()
		dt_from =self.xtime.get_dt_obj_for_ymd_list_as_0h_0m_0s()
		dt_from =self.xtime.shift_day_for_dt_obj(dt_from, -1* int(input_no))
		mails = self.get_sorted_mails_for_input_folder(input_folder, sort_by)
