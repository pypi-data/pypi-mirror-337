...
class OpenDicClient:
    ...
    def post(self, endpoint, data):
        ...
        response = requests.post(url, json=payload, headers={"Authorization": f"Bearer {self.oauth_token}", "Content-Type": "application/json"})
        print("URL:", url)
    ...
    def put(self, endpoint, data):
        ...
        payload = json.dumps(data)
        response = requests.put(url, json=payload, headers={"Authorization": f"Bearer {self.oauth_token}"})
        response.raise_for_status() # Raise an exception if the response is not successful
        return response.json()
