...
from . import SAXS_plotting
from .. import Global_Utilities as gu

# Main function definition.

def SAXS_Analysis_Main( ip ):

    print( "SAXS analysis has begun." )
    ...
    gu.print_files_read( len( data[1] ), 22 )
    ...
    if ip.extract_features:
        ...
        util.extract_SAXS_features( ip.output_directory, file_data, data )

    if ip.read_and_analyse_features:

        util.read_and_analyse_SAXS_features( ip, file_data )

    if ip.plot_data:

        SAXS_plotting.plot_data( ip, file_data, data )

    if ip.sandbox:

        util.sandbox( ip.directory, ip.output_directory, file_data, data )

    print( "SAXS analysis complete." )
