...
def _lcf_res(x, spec, ref_spec):
    return np.sum((spec - np.dot(ref_spec, x))**2)


def _lcf(x0, ref_spec, use_constr, spec):
    """
    Perform linear combination fitting for a single spectrum
    """
    rlt = []
    bounds = [(0, 1)] * x0.shape[0]  # Coefficients between 0 and 1
    if use_constr:
        constraint = {'type': 'eq', 'fun': lambda x: np.sum(x) - 1}  # Sum of coefficients = 1
    else:
        constraint = ()

    for ii in range(spec.shape[1]):
        result = minimize(_lcf_res, x0, args=(spec[:, ii], ref_spec), 
                        method='SLSQP', bounds=bounds, constraints=constraint)
        rlt.append([*result.x, result.fun])  
    return rlt


def lcf(spec, ref_spec, use_constr=True):
    """
    Multiprocessing linear-combination fitting
    Inputs:
        spec: flattened spectra data; 
            dim 0: normalized attenuation coeff (chi) as a function of energy
            dim 1: spatial position
        ref_spec: reference spectra;
            dim 0: normalized attenuation coeff (chi) as a function of energy
            dim 1: number of reference spectra
    Returns:
        rlt: ndarray; weight coeff for each ref_spec at each spatial position          
    """
    spec = spec.reshape(spec.shape[0], -1)
    bdi = chunking(spec.shape[1])
    x0 = np.ones((1, ref_spec.shape[1])).squeeze() / ref_spec.shape[1]  # Initial guess: equal parts of each reference
    with mp.Pool(N_CPU) as pool:
        rlt = pool.map(
            partial(_lcf, x0, ref_spec, use_constr), [spec[:, bdi[ii] : bdi[ii + 1]] for ii in range(N_CPU)]
        )
    pool.close()
    pool.join()
    
    return np.concatenate(rlt)
