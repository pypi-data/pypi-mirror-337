...
class Erode(MorphoPlugin):
    ...
    kernel_size:
        the amount of pixels eroded
    """

    def __init__(self): #PLUGIN DEFINITION
        MorphoPlugin.__init__(self)
        self.set_icon_name("Erode.png")
        self.set_image_name("Erode.png")
        self.set_name("Erode : Erode the selected objects")
            ...
            for t in dataset.get_times(objects):#For each time points in selected object
                ...
                if data is not None:
                    ...
                    for o in dataset.get_objects_at(objects,t):
                        printv('Erode the object ' + str(o.get_name()), 0)
                        bbox = self.dataset.get_regionprop("bbox", o)
                        databox = data[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]]
                        erode_box = np.uint8(binary_erosion(databox==o.id,np.ones([kernel_size,kernel_size,round(anisotropy*kernel_size)])))
                        databox[databox==o.id] = dataset.background
                        databox[erode_box == 1] = o.id
