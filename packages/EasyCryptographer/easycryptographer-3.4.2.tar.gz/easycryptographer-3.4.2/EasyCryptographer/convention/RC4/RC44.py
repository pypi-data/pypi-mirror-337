...
from cryptographer.RC4.RC4_method import RC4EncryptionMethod, RC4DecryptionMethod
...
def encryption():
    ...
    replace(encryption_text_after, ciphertext)


def decryption():
    try:
        RDM = RC4DecryptionMethod(decryption_text_need.get_content(), key_text_need.get_content())
        decrypted = RDM.decryption()
        replace(decryption_text_after, decrypted)
...
window = tk.Tk()
EasyAutoWindow(window, window_title="RC4Method", minimum_value_x=1312, minimum_value_y=876,
