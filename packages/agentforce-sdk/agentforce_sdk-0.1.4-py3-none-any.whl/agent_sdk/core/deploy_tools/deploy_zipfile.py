...
def undeploy(username, password, domain, bot_api_name='SampleBot'):
...
def deploy_multiple(username, password, domain, deploy_files, bot_api_name, agent_id):
    is_success = False
    for deploy_file in deploy_files:
        logger.info(f"Deploying file {deploy_file}")
        try:
            result = deploy_and_monitor(username, password, domain, deploy_file, True, bot_api_name)
...
def deploy_async(username, password, domain, zip_file_path, bot_api_name, agent_id):
    try:
        result = deploy_and_monitor(username, password, domain, zip_file_path, True, bot_api_name)
        status = result['deployResult']['status']
    except Exception as e:
        logger.error("Failed to deploy", e)
...
async def get_actions(username, password, domain):
    session_id, instance_url = login_to_salesforce(username, password, domain)
    headers = {
        'Authorization': f'Bearer {session_id}'
    }
...
def deploy_and_monitor(session=None, zip_path=None, agent_name=None, instance_url=None, session_id=None, check_auth_callback=None, username=None, password=None, domain=None, zip_file_path=None, bot_api_name='SampleBot', single_package=True):
    ...
    if session_id and instance_url:
        ...
        if deployment_result['deployResult']['success']:
            if not bot_version_id:
                bot_version_id = get_bot_version_id(session_id, instance_url, agent_name)
            activate_status = update_bot_version_status(session_id, instance_url, bot_version_id, 'ACTIVE')
...
def get_bot_username(username, password, domain):
...
if __name__ == "__main__":
    ...
    parser.add_argument('username', type=str, help='Salesforce username')
    parser.add_argument('password', type=str, help='Salesforce password')
    ...
    deployment_result = deploy_and_monitor(args.username, args.password, args.zip_file, args.single_package,
                                           args.bot_api_name)
