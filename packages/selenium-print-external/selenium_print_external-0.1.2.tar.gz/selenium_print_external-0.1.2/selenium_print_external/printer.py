...
class ExternalSeleniumPrinter:
    ...
    def __init__(self, driver: WebDriver):
        ...
        if not isinstance(driver, WebDriver):
            raise TypeError(
                "The provided driver must be an instance of selenium.webdriver.remote.webdriver.WebDriver or its subclass."
            )
        if not hasattr(driver, "execute_cdp_cmd"):
            raise AttributeError(
                "The provided driver does not appear to support execute_cdp_cmd, which is required for printing."
            )
