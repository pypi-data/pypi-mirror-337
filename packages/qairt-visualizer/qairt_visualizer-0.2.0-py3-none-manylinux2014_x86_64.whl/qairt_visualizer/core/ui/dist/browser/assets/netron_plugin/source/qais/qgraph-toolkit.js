...
import * as contextMenu from './qcontext-menu.js';
...
const searchControl = (data) => {
  ...
  if (olElement) {
    ...
    const liData = Array.from(liElements).map(li => {
      return { id: li.getAttribute('id'), value: li.innerHTML };
    });
...
const updateContextmenu = (data) => {
  contextMenu.init.updateContextMenu(data);
  contextMenu.init.removeContextMenu();
  contextMenu.init.createContextMenu(false);
}
...
iframeCommunicationService.addEventListener('update_context_menu', updateContextmenu);
