...
def group_heatmap(
    ...
    def _show_plot_(_label_: str, _layer_: str = None) -> None:
        try:
            ...
            ul.log(__name__).warn(f"Changing clustering parameters {e.args}")
            heatmap_annotation(
                adata_cluster.T,
                row_name=row_name,
                col_name=col_name,
...
def init_score_plot(
    ...
    try:
        ...
        ul.log(__name__).warn(f"{e}")
        ul.log(__name__).warn(f"Can try to improve the version of `seaborn`. `pip install --upgrade seaborn`")
...
def trs_plot(
    ...
    plot_columns: Tuple[str, str] = ("UMAP1", "UMAP2"),
    trait_cluster_map: dict = None,
    heatmap_width: float = 8.0,
    heatmap_height: float = 4.0,
    width: float = 8
) -> None:
