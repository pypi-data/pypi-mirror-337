...
class Alpaca:
    ...
    def inference(self, tokens, num_tokens=10, state_dict=None, detokenize=False, vocab=None, pad_token_id=0):
        if not state_dict:
        ...
        with torch.inference_mode():
            attention_mask = (tokens != pad_token_id).long().to(device)

            for _ in range(num_tokens):
                # The model takes both input tokens and target tokens (in autoregressive manner)
                output = transformer.forward(tokens, tokens, attention_mask=attention_mask)
                
                # Get the predicted token (next token in the sequence)
                predicted_token = output.argmax(dim=-1)[:, -1]

                # Append the predicted token to the sequence (this updates the full sequence context)
                tokens = torch.cat((tokens, predicted_token.unsqueeze(1)), dim=1)

                # Update attention mask for the new token
                attention_mask = torch.cat((attention_mask, (predicted_token != pad_token_id).long().unsqueeze(1)), dim=1)

            predicted_tokens = tokens.squeeze(0)
