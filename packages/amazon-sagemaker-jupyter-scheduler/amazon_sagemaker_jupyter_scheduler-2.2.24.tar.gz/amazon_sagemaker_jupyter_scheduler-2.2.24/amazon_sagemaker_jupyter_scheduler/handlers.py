        ...
        try:
            async with timeout(MAX_WAIT_TIME_FOR_API_CALL_SECS):
                if (
                    ...
                    self.log.info(f"[AdvancedEnvironmentsHandler] SageMaker Studio environment detected")
                    envs = await SageMakerStudioAdvancedEnvironments().get_advanced_environments(
                        self.log
                    )
                else:
                    self.log.info(f"[AdvancedEnvironmentsHandler] Vanilla JupyterLab environment detected")
                    envs = (
            ...
            # The block captures BotoCore errors raised in logging.py, async_with_metrics
            # ToDo: Need to clean the exception handling later.
            self.set_status(error.status_code)
            self.finish(
                json.dumps({"error_code": error.error_code, "message": str(error.error_message)})
            )
            ...
            self.log.exception(f"[AdvancedEnvironmentsHandler] Generic exception detected: {error}")
            self.set_status(500)
