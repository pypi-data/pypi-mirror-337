...
_OPERATORS = {
    ...
    "exists": lambda value, column: column.has(value) if bool(value) else ~column.has(value),
}
        ...
        for attr_name in annotations:
            try:
                ...
                if operator_key not in _OPERATORS:
                    raise ValueError(f"Unsupported operator '{operator_key}' in {attr_name}")
                column = getattr(model, column_name)
                filter_map[attr_name] = (column, operator_key)
            except AttributeError as exc:
                raise ValueError(f"Column '{column_name}' not found in model {model.__name__}") from exc
