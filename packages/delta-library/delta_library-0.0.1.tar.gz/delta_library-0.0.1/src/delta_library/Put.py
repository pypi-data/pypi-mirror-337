...
class Put:
    def __init__(self, stock_p, strike, dividend, interest_rate, dte=None, date=None, price=None, sigma=None, american_european="european", timezone="Europe/Amsterdam", date_format='%Y-%m-%d'):
        ...
        if american_european == "european":
            self.pricing_model = option_pricing.EurPut
        if american_european == "american":
            self.pricing_model = option_pricing.AmPut
        self.delta = None
        self.gamma = None
        self.theta = None
        self.vega = None
    ...
    def __str__(self):
        self.recalculate()
        return f"Put option with strike {self.strike} and price {self.price} and vol {self.sigma} at {self.date} ({self.dte} dte) \n Stock Price: {self.stock_p} \n Delta: {self.delta}\n Gamma: {self.gamma}\n Theta: {self.theta}\n Vega: {self.vega}\n Leverage: {self.leverage}\n Probability ITM: {self.probability_itm}"  
