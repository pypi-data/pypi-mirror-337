...
def get_gene_enrichment(
    ...
    if is_top and len(gene_list) < top_number:
        ul.log(__name__).warn(f"The number of parameters `top_number` ({top_number}) is greater than the number of genes ({len(gene_list)}).")
    ...
    for cluster in tqdm(cluster_list):
        ...
        if len(_cluster_gene_list_) == 0:
            ul.log(__name__).warn(f"The gene list for cluster {cluster} is empty.")
            continue
