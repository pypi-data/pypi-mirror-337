...
class JuliusCollectionHolder:   # TODO gc
    ...
    async def __update_collection(self, id: int, collection_id: str, scheme: Optional[str], force_reload: bool = False):
        ...
        if self.__is_local:
            ...
            res = await post_request_json(MONGO_LOAD(self.__dag_host_port), collection_and_scheme.json(), buffer=self.__logs_buffer, fail_info=f"{LOAD_COLLECTION_FAIL_MSG}: {collection_id}", operation_id=self.__operation_id)
