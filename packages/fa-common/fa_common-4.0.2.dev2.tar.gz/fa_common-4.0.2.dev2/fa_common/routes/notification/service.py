        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if workflow is not None:
                    ...
                    stat_msg = f"The workflow '{workflow.workflow_id}' submitted at '{start_date_string}' has finished with status: "
                    if workflow.status == JobStatus.FAILED:
                        stat_msg = stat_msg + f"❌ {workflow.status.value if workflow.status is not None else "None"}.<br><br>"
                    else:
                        stat_msg = stat_msg + f"✅ {workflow.status.value if workflow.status is not None else "None"}.<br><br>"
                    ...
                    <table>
                        ...
                        <tr>
                            <td>Status:</td>
                            <td>{workflow.status.value if workflow.status is not None else "None"}</td>
                        </tr>
                        <tr>
                            <td>Message:</td>
                ...
                logger.error(f"❌ Workflw with id '{workflow_id}' could not be retrieved during email sending. {e!s}")
