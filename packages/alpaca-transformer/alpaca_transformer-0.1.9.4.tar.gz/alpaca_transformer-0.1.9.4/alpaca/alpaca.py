...
class Alpaca:
    ...
    def inference(self, tokens, num_tokens=10, state_dict=None, detokenize=False, vocab=None):
        if not state_dict:
        ...
        # Move tokens to the same device as the transformer
        device = transformer.device  # Ensure the model device is used
        tokens = tokens.to(device)  # Move tokens to the same device
