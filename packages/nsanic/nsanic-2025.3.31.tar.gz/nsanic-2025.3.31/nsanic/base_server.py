...
class InitServer:
    ...
    def __init__(
        ...
        NLogger.init_conf(base_path=self.__conf.LOG_PATH, folder=self.__conf.SERVER_NAME.lower(), proc_split=1, proc_tab=proc_name, console=conf.DEBUG_MODE)
    ...
    async def __after_server_start(self, app, loop):
        ...
        for fun in self.__evt_start:
            (await fun(**{'app': app, 'loop': loop})) if asyncio.iscoroutinefunction(fun) else fun(**{'app': app, 'loop': loop})
        consts.GLOBAL_SRV_STATUS = True
