...
from ledger.api.api_helper.corporation_helper import CorporationProcess, CorporationWalletJournalEntry
...
class LedgerApiEndpoints:
    ...
    def __init__(self, api: NinjaAPI):
        ...
        @api.get(
            "{entity_type}/{entity_id}/ledger/date/{date}/view/{view}/new/",
            response={200: Any, 403: str, 404: str},
            tags=self.tags,
        )
        def get_ledger_new(request: WSGIRequest, entity_type: str, entity_id: int, date: str, view: str):
            try:
                date_obj = datetime.strptime(date, "%Y-%m-%d").date()
            except ValueError:
                return 403, "Invalid Date format. Use YYYY-MM-DD"
            ...
            # Fetch data
            output = ledger.generate_ledger_new()
            return output
