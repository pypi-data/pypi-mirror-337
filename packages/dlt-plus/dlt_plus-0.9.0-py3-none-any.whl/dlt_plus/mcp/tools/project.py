...
def add_source(source_name: str, source_type: str, ctx: Context = None) -> None:
    ...
    cli_helpers.add_source(run_context, source_name, source_type)
...
def add_destination(destination_name: str, destination_type: str, ctx: Context = None) -> None:
    ...
    if (run_context := ctx.fastmcp.project_context) is None:
        return "No dlt+ project is selected. Use `select_or_create_dlt_project()` first."
    cli_helpers.add_destination(run_context, destination_name, destination_type)
    # TODO we don't want to pass secrets to LLMs
...
def add_pipeline(
    ...
    if (run_context := ctx.fastmcp.project_context) is None:
        return "No dlt+ project is selected. Use `select_or_create_dlt_project()` first."
    cli_helpers.add_pipeline(run_context, pipeline_name, source_name, destination_name)
    # TODO we don't want to pass secrets to LLMs
...
def select_or_create_dlt_project(
    ...
    if project_context is None:
        ...
        project_context = cli_helpers.init_project(project_dir, name)
