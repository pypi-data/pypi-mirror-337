...
import logging
import threading
from Adlib.robos import *
from Adlib.importacao import checkEvent
...
class BotManager:
    ...
    def __init__(self, botTask, countdown=600):
        """
    ...
    def _managerTask(self):
        ...
        while not self._stopEvent.is_set():
            logging.info("FluxGuard está monitorando o robô")

            # Wait for the reset event or countdown expiration
    ...
    def startBot(self):
        ...
        with self._lock:
            ...
            if self._botThread is None or not self._botThread.is_alive():
                self._botThread = threading.Thread(target=self._mainTaskWrapper, daemon=True)
    ...
    def startManager(self):
        ...
        if self._managerThread is None or not self._managerThread.is_alive():
            self._managerThread = threading.Thread(target=self._managerTask, daemon=True)
    ...
    def stop(self):
        ...
        self._stopEvent.set()
        if self._botThread:
            self._botThread.join(timeout=5)
        if self._managerThread:
            self._managerThread.join(timeout=5)
