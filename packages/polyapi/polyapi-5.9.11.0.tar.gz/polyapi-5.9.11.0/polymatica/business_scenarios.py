...
from .common import \
    ...
    FORMAT_SETTINGS_KEYS, API_VERSION, ROOT_PARENT
...
class BusinessLogic:
    ...
    def get_scripts_description_list(self) -> List:
        ...
        def _rec_get_scripts_in_dir(curr_path: str = None, parent_id: str = self.ROOT_PARENT) -> list:
            results = self.exec_request.execute_request(
                params=urljoin(self.base_url, f"api/{self.API_VERSION}/script_folders?parent={parent_id}"),
                method='GET')
            folders: list = results.json()['results']

            results = self.exec_request.execute_request(
                params=urljoin(self.base_url, f"api/{self.API_VERSION}/scripts?parent={parent_id}"),
                method='GET')
    ...
    def run_scenario_impl(self, scenario_id: str, scenario_name: str):
        ...
        new_layer_data = self.execute_manager_command(command_name="user_layer", state="create_layer")
        layer_uuid = (self.h.parse_result(new_layer_data, 'layer') or dict()).get("uuid")
        if not layer_uuid:
        ...
        module_descs = self.h.parse_result(
    ...
    def set_olap_module_name(self, olap_module_id: str, new_module_name: str):
        ...
        commands = [self.manager_command.collect_command(
            module="manager",
            command_name="user_iface",
            state="save_settings",
            module_id=module_id,
    ...
    def create_sphere(self, cube_name: str, source_name: str, file_type: str, update_params: Dict = None,
        ...
        try:
            ...
            for param_info in param_types:
                check_f(param_info)
            cube_name = error_handler.checks(
    ...
    @timing
    def update_cube(self, cube_name: str, new_cube_name: str = None, update_params: Dict = None,
        ...
        try:
            param_types = [
                ["cube_name", cube_name, str],
                ["new_cube_name", new_cube_name, str, None],
            ...
            for param_info in param_types:
                check_f(param_info)
            cube_id, new_cube_name = error_handler.checks(
                self, self.func_name, cube_name, new_cube_name, cubes_list, current_update_params, user_interval,
