...
class Input_Parameters():
    ...
    def __init__( self ):
        ...
        self.peak_threshold_1 = 0

        self.peak_threshold_2 = 0
...
def read_parameters_from_numbers_file( filename, ip ):
    ...
    ip.derivative = bool( rows[12][1].value )

    ip.extract_features = bool( rows[13][1].value )

    ip.peak_threshold_1 = float( rows[14][1].value )

    ip.peak_threshold_2 = float( rows[15][1].value )

    ip.peak_spacing = float( rows[16][1].value )

    ip.peak_limit = float( rows[17][1].value )

    ip.read_and_analyse_features = bool( rows[18][1].value )

    ip.plot_data = bool( rows[19][1].value )

    ip.sandbox = bool( rows[20][1].value )

    ip.sample_mask = [int( i.value ) for i in rows[21][1:] if i.value != None]

    ip.feature_selection = [int( i.value ) for i in rows[22][1:] if i.value != None]
