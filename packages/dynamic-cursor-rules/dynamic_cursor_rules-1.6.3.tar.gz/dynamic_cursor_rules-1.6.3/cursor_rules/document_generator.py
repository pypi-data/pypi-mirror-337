...
class DocumentGenerator:
    ...
    def _generate_cursorrules(self, cursor_dir: str) -> List[str]:
        ...
        """
        # Import utils here to avoid any import issues
        from .utils import save_modern_cursor_rules
        # Import re and json here to ensure they're available
        import re
        import json
        ...
        if use_llm:
            try:
                ...
                1. A title that describes the rule category
                2. A pattern glob that specifies which files the rule applies to
                3. A description that explains when the rule should be applied
                4. The main rule content in markdown format (without prefixing every line with #)
                5. Optional file references using @file: syntax
                ...
                - references: (Optional) Array of example files to reference
                ...
                [
                  {
                    ...
                    "references": ["src/components/example/Button.tsx"]
                  },
                  {
                    "title": "Backend API Rules",
                    "pattern": "src/api/**/*.{ts,js}",
                    "description": "Guidelines for API implementation",
...
- Write tests for your code"""
    ...
    def _generate_action_items_with_llm(self) -> ActionPlan:
        ...
        def extract_json(text):
            """Extract JSON array from text content, even if surrounded by other content."""
        ...
        def attempt_json_repair(json_str):
            ...
            # Extract JSON array if surrounded by other content
            ...
            json_str = re.sub(r'([a-zA-Z0-9_]+):', quote_unquoted_keys, json_str)
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Initial JSON parsing failed: {e}. Attempting repair...")
                ...
                try:
                    ...
                    logger.warning(f"JSON repair attempt failed: {e2}")
                    # Log the content for debugging
                    logger.debug(f"Problematic JSON content: {json_content[:500]}...")
                    # Try one more time with a more aggressive approach - extract just the array structure
                    try:
                        # Look for array pattern and extract it
                        matches = re.search(r'\[\s*\{.+\}\s*\]', json_content, re.DOTALL)
                        if matches:
                            clean_json = matches.group(0)
                            phases_data = json.loads(clean_json)
                        else:
                            raise ValueError("Could not extract valid JSON array structure")
                    except Exception:
                        # If all parsing attempts fail, try the fallback generation
