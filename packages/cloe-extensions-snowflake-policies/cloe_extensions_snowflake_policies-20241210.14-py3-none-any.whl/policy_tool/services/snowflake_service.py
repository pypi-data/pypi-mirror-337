...
class SnowClientConfig(object):
    def __init__(
        ...
        sql_variables: Optional[str] = None,
    ):
        self.user = user
...
class SnowClient(object):
    """
    Objects that collects all operations and information of a Snowflake connection with the Snowflake Python Connector.
    """
    ...
    @staticmethod
    def _get_error_message(exception: Exception, statement: str) -> str:
        """
        Compose error message if the execution of a statement or query fails.
        """
        if hasattr(exception, "raw_msg"):
            message = exception.raw_msg.replace("\n", " ")
        else:
            message = str(
                exception
            )  # this makes sure that all kinds of errors can have a message, even if they do not have raw_msg attribute
        if hasattr(exception, "sfqid"):
            message = message + f"\nQuery ID: {exception.sfqid}"
    ...
    def execute_statement(self, statement: Union[str, List[str]]) -> None:
        ...
        try:
            for single_statement in statement_list:
                stripped_statement = single_statement.strip()  # remove whitespace from statement, as execute_string() might produce warnings if empty new lines are found after a semicolon
                log.debug(
    ...
    def execute_query(
        ...
        try:
            ...
            log.debug(
                f"FINISH execution [ '{query}' ]",
                operation=LogOperation.SQLCOMMAND,
                status=LogStatus.SUCCESS,
                db=self.database,
            )
        except Exception as err:
            raise RuntimeError(self._get_error_message(err, query)) from err
    ...
    def split_snowflake_list_representation(list_representation: str) -> list:
        ...
        list_from_sf_list_representation = [
            entry.strip() for entry in list_representation.split(",")
        ]

        return list_from_sf_list_representation
