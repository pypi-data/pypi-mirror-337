...
def str_splice(source: str,
    ...
    the returned list will contain *source* as its first element, and *None* as the remaining elements.

    Separators in *seps* can not be *None* or empty strings.
    If *source* starts with the separator, then the return list's first element will be an empty string.
    If *source* ends with the separator, then the return list's last element will be an empty string.

    :param source: the source string
    :param seps: the ordered list of separators
    :return: the ordered segments obtained, or *None* if *source* is not a string, or *seps* i.
    """
    # initialize the return variable
    ...
    if isinstance(source, str) and None not in seps and "" not in seps:
        ...
        result.extend([None] * (len(seps) - len(result)))
