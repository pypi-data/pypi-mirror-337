...
from paramkit.errors import ParamTypeError, ParamMissingError, ParamLengthExceedLimitError, ParamValueInvalidError
...
class P:
    ...
    def __init__(self,
                 ...
                 *,
                 must: bool = True,
                 typ: Any = None,
                 opts: Optional[Union[re.Pattern, str, list, tuple, set]] = None,
        ...
        :param must: Whether the parameter is mandatory
    ...
    def __check_bounds__(self):
        ...
        ) if any([self.ge, self.gt]) else (None, None)
        ...
        upper = min(
            ...
            key=lambda x: (x[0] if x[0] is not None else float('inf'), not x[1])
        ) if any([self.le, self.lt]) else (None, None)
    ...
    def __validate_value__(self):
        ...
        if not (
                (isinstance(self.opts, re.Pattern) and self.opts.fullmatch(str(self.value))) or
                (isinstance(self.opts, str) and re.fullmatch(self.opts, str(self.value))) or
                (isinstance(self.opts, (list, tuple)) and self.value in self.opts)
        ):
            raise ParamValueInvalidError(
                f'param `{self.name}` value is invalid, current: {self.value}, expected: {self.opts}')

    def __validate_limit__(self):
        ...
        if not (
                (self.ge is None or value >= self.ge) and
                (self.gt is None or value > self.gt) and
                (self.le is None or value <= self.le) and
    ...
    def validate(self):
        ...
        # Skip validation when not must and value is None
        if not self.must and self.value is None:
            return
