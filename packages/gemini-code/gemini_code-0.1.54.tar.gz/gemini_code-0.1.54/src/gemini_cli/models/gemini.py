...
HANDLES QUOTA FALLBACK TO 1.5 PRO.
Prompt v11.1. Syntax FIXED.
"""

import google.generativeai as genai
from google.generativeai.types import FunctionDeclaration, Tool
from google.api_core.exceptions import ResourceExhausted # <--- Import Quota Exception
import re
import json
import logging
import ast
...
FALLBACK_MODEL = "gemini-1.5-pro-latest" # Define fallback model
...
class GeminiModel:
        ...
        try:
            ...
            try:
                ...
                if 'Test OK' not in text_content: logging.warning("Connectivity test response unexpected.")
                 ...
                 logging.warning(f"Quota likely exceeded for initial model '{self.current_model_name}' even during connectivity test.")
                 # Optionally trigger fallback here too if needed, but maybe wait for generate call
    ...
    def _initialize_model_instance(self):
        ...
        logging.info(f"Model instance '{self.current_model_name}' created.")


    # ... (get_available_models remains the same) ...
    def get_available_models(self): return list_available_models(self.api_key)

    # --- generate function with Quota Fallback Logic ---
    def generate(self, prompt: str, conversation=None) -> str | None:
        logging.info(f"Agent Loop - Starting task for prompt: '{prompt[:100]}...' using model '{self.current_model_name}'")
        ...
        orientation_context = ""
        ls_result = "(ls tool not found or failed)"
        try:
            with self.console.status("🧭 Getting directory listing...", spinner="earth"):
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                llm_response = None
                try:
                    # Use status for LLM call
                    with self.console.status(f"[yellow]Iter {iteration_count}: Assistant planning/coding...", spinner="dots"):
                ...
                # --- Fallback Logic ---
                    ...
                    full_log.append(f"System Alert: Quota exceeded for {self.current_model_name}.")
                    ...
                    if self.current_model_name == FALLBACK_MODEL:
                        ...
                        self.console.print(f"[bold red]API quota exceeded for both primary and fallback models ({FALLBACK_MODEL}). Please try again later.[/bold red]")
                    ...
                    try:
                        ...
                        # Add notification to history so LLM knows about the switch? Optional.
                        # current_task_history.append({'role':'user', 'parts': [f"(System note: Switched to model {self.current_model_name} due to quota limit.)"]})
                        logging.info(f"Successfully switched to fallback model: {self.current_model_name}")
                        self.console.print("[green]Fallback model initialized. Retrying last step...[/green]")
                        time.sleep(1) # Give user time to see message
                        # Retry the generate_content call immediately within the loop
                        continue # Skip rest of the current loop iteration and retry generate_content
                ...
                except Exception as generation_error: # Catch other generation errors
                     logging.error(f"Error during LLM generation: {generation_error}", exc_info=True)
                     # Handle potential blockages mentioned in _extract_text_from_response
                     error_text = self._extract_text_from_response(llm_response) if llm_response else f"Generation Error: {generation_error}"
                     if "(Response blocked" in error_text or "(Prompt blocked" in error_text or "(Response generation incomplete" in error_text:
                          self.console.print(f"[bold red]{error_text}[/bold red]")
                          return f"LLM Error: {error_text}" # Stop the process if blocked
                ...
                tool_calls_executed = self._parse_and_execute_tool_calls(generated_text) # This function now shows status
            ...
            final_summary = ""
            # Use a status for the final summary generation
            with self.console.status("[yellow]Assistant preparing final summary...", spinner="dots"):
                if task_completed:
                    ...
                    if completion_text_only and len(completion_text_only) > 10:
                         ...
                         # Use generate_content with history + request
    ...
    # ... (_parse_and_execute_tool_calls remains the same robust version with console status) ...
    def _parse_and_execute_tool_calls(self, text: str) -> list[dict]:
        ...
        for match in matches:
            ...
            try:
                ...
                elif func_name == 'ls' and 'path' in args: status_message = f" Listing directory [cyan]{args.get('path', '.')}[/cyan]..." # Use get for optional path
                elif func_name == 'grep' and 'pattern' in args: status_message = f"🔍 Searching for pattern '{args['pattern'][:20]}...'..."
                elif func_name == 'glob' and 'pattern' in args: status_message = f" Finding files matching [cyan]{args['pattern']}[/cyan]..."
                ...
                result_str = "(Execution failed before completion)"
    ...
    # ... (_extract_text_from_response remains the same robust version) ...
...
"""
    ...
    def _create_tool_definitions(self) -> list[Tool]:
        # ... (Same TRULY FIXED syntax block from previous correction) ...
