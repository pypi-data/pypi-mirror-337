...
class NumpyArray(ArrayLike):
    ...
    def non_null_items(self) -> Iterable[Tuple[IndexKey, RealNumber]]:
        if self.ndim() == 1:
            yield from self._iterate_row_non_null_items(self._array, 0)
        else:
            for row_index, row in enumerate(self._array):
                yield from self._iterate_row_non_null_items(row, row_index)

    def _iterate_row_non_null_items(self, row, row_index: int) -> Iterable[Tuple[IndexKey, RealNumber]]:
        for index, value in enumerate(row):
            if value != 0:
                yield (row_index, index), value
