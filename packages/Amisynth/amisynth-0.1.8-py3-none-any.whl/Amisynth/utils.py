...
class ContextAmisynth:
    ...
    def set_context(self, ctx):
        ...
        if isinstance(ctx, commands.Context):
            ...
            self.emoji = str(ctx.emoji) 



            guild = self.bot.get_guild(ctx.guild_id) if ctx.guild_id else None
            member = guild.get_member(ctx.user_id) if guild else None
            self.user_roles = [role.id for role in member.roles if role.name != "@everyone"] if member else []
