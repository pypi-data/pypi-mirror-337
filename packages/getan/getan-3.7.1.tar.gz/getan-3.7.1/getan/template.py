# -*- coding: utf-8 -*-
#
# (c) 2014, 2017, 2018, 2020 Intevation GmbH <https://intevation.de>
# Authors:
...
def decimal_hours_filter(delta):
    days = delta.days
    seconds = days * 3600 * 24
    s = seconds + delta.seconds

    return(decimal_hours(s))
...
def duration(entries):
    """Return the sum of the durations of the given entries."""
    total = timedelta(0)
...
def sum_duration_filter(projects):
    """Return the sum of the durations of the given projects."""
    total = timedelta(0)
    for project in projects:
        total += project.get_total_duration()
    return total
...
def render(template, database=None, year=None, week=None, project=None,
    ...
    env.filters["human_time"] = human_time_filter
    env.filters["date_format"] = date_format
    env.filters["decimal_hours"] = decimal_hours_filter
    env.filters["duration"] = duration
    env.filters["sum_duration"] = sum_duration_filter
