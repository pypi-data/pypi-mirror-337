...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, cur_=1):
    ...
    try:
        ...
        if POST_CHKBOX['POST_ISCOLLAPSE'] and not message_id and not POST_CHKBOX['POST_ISPAY']:
            reply_markup.row(types.InlineKeyboardButton(text="ᴿᵁᴺ", callback_data=f'pst_{ENT_TID}_{POST_TID}_0_0_0_run'))
        else:
        ...
        for button in sorted_buttons:
            try:
                ...
                if button['knd'] == 'button_in':
                    cb_ = f"pst_{ENT_TID}_{POST_TID}_{button['bid']}_0_0_btn"
                    ...
                    cb_ = f"pst_{ENT_TID}_{POST_TID}_{button['bid']}_0_0_{button['knd']}"
                    counter = '⁰' if button['bid'] not in counters else await upper_register(counters[button['bid']])
                    btn = types.InlineKeyboardButton(text=f"{counter} {button['lbl']}", callback_data=cb_)
                    rows[row_index].append(btn)
                ...
                if v[0]:
                    if len(tmp) > 0 and tmp[-1] is None:
                        ...
                        if 'ᴵ' in v[0]:
                            ...
                            counter = '⁰' if btn_ix not in counters else await upper_register(counters[btn_ix])
                            cb_ = f"pst_{ent_id}_{post_id}_{btn_ix}_0_0_btn"
