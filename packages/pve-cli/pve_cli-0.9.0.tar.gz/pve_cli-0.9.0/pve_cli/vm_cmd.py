...
def migrate(
    ...
    if 'do-not-migrate' in vm.get('tags', []) and not force:
        raise PVECLIError(f'Migration of vm VM {vm["name"]} ({vm["vmid"]}) is forbidden because of "do-not-migrate" tag. Use --force to migrate anyways.')

    migrate_vms(api=proxmox_api, dest_node=dest_node, vmid_list=[vm['vmid']], parallel_migrations=1, with_local_disks=with_local_disks)
