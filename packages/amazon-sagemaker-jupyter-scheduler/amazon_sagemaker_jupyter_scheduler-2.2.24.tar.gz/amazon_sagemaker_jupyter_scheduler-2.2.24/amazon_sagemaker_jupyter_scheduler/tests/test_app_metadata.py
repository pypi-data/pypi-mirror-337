...
MOCK_RESOURCE_METADATA_CN = """
{
    "AppType": "KernelGateway",
    "DomainId": "d-xxxxxxxxxxxx",
    "UserProfileName": "profile-name",
    "ResourceArn": "arn:aws-cn:sagemaker:us-east-2:account-id:app/d-xxxxxxxxxxxx/profile-name/KernelGateway/datascience--1-0-ml-t3-medium",
    "ResourceName": "datascience--1-0-ml",
    "AppImageVersion":""
}
"""
...
class TestAppMetadataUtils:
    ...
    def test_get_partition(self, mock_file_contents):
        result = get_partition()
        assert result == "aws"

    @patch("builtins.open", new_callable=mock_open, read_data=MOCK_RESOURCE_METADATA_CN)
    def test_get_partition_roundtable_region(self, mock_file_contents):
        result = get_partition()
