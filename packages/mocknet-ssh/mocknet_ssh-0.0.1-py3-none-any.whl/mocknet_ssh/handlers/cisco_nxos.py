...
COMMANDDIR = "mocknet_ssh.handlers.command_output.cisco_nxos"
OS = "cisco_nxos"


class CiscoNXOSHandler(asyncssh.SSHServerSession):
    ...
    def connection_made(self, chan: asyncssh.SSHServerChannel) -> None:
        self._chan = chan
        logger.info("📡 Cisco NX-OS SSH session opened")

    def shell_requested(self) -> bool:
        return True

    def session_started(self) -> None:
        logger.info("🚪 Cisco NX-OS session started")
        if self._chan:
            self._chan.write("Welcome to Cisco NX-OS\n")
    ...
    def _handle_command(self, cmd: str) -> None:
        ...
        if cmd in ["conf t", "configure terminal"]:
            self.mode = "config"
    ...
    def _prompt(self) -> str:
        return {"exec": "switch> ", "config": "switch(config)# "}.get(
            self.mode, "switch> "
        )
...
def main() -> None:
    ...
    asyncio.run(run_ssh_server(users, handler_factory=CiscoNXOSHandler))
