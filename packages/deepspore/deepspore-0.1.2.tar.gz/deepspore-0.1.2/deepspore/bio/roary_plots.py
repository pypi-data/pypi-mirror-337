...
class Options():
    def __init__(self,
                 tree='RawDatas/core_gene_alignment.aln.treefile',
                 spreadsheet='RawDatas/gene_presence_absence.csv',
                 format='png',
                 outputpath='results'):
        self.tree = tree
        self.spreadsheet = spreadsheet
        self.format = format
        self.skipped_columns = 14
        self.labels = 'test'
        self.outputpath = outputpath


def roaryPlots(tree, spreadsheet, format, outputpath):
    ''''
    如下传参即可
    '''
    options = Options(tree, spreadsheet, format, outputpath)
    ...
    sns.despine(left=True,
                bottom=True)
    plt.savefig(outputpath+'/'+'pangenome_frequency.%s' %
                options.format, dpi=600)
    plt.clf()
    ...
    with sns.axes_style('whitegrid'):
        ...
        plt.tight_layout()
        plt.savefig(outputpath+'/'+'pangenome_matrix.%s' %
                    options.format, dpi=600)
        plt.clf()
    ...
    a = plt.pie([core, softcore, shell, cloud],
                ...
                autopct=my_autopct)
    plt.savefig(outputpath+'/'+'pangenome_pie.%s' % options.format, dpi=600)
    plt.clf()
...
#     sns.despine(left=True,
#                 bottom=True)
#     plt.savefig(outputpath+'/'+'pangenome_frequency.%s' % options.format, dpi=600)
#     plt.clf()
#
#     # Sort the matrix according to tip labels in the tree
#     roary_sorted = roary_sorted[[x.name for x in t.get_terminals()]]
...
#                        do_show=False,
#                        )
#         plt.savefig(outputpath+'/'+'pangenome_matrix.%s' % options.format, dpi=600)
#         plt.clf()
#
#     # Plot the pangenome pie chart
#     plt.figure(figsize=(10, 10))
