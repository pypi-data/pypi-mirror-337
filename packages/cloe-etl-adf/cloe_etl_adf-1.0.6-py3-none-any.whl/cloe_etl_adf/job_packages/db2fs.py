...
def transform_db2fs_to_factory_object(
    ...
    parameters: dict[str, str] = {
        ...
        "fileNameTenantName": job.data_source_info.tenant.name
        if job.data_source_info.tenant is not None
        else "",
    ...
    if job.data_source_info.base_obj.content == "delta":
        parameters["sequenceColumnName"] = (
            job.base_obj.sequence_column_name
            if job.base_obj.sequence_column_name is not None
            else ""
        )
    ...
    type_properties = factory_objects.ExecutePipelineTypeProperties(
        ...
        pipeline={
            "referenceName": f"job_db2fs_{job.data_source_info.base_obj.content}_{job.source_connection.name}_{job.sink_connection.name}",
