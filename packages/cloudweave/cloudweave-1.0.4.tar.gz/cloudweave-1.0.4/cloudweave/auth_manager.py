...
from cloudweave.autoscale_manager import AutoscaleManager, CloudProviderType
...
class CloudWeaveAuthManager(Singleton):
    ...
    def get_autoscale_manager(self,
                            cloud_provider: Union[str, CloudProviderType],
                            instance_id: Optional[str] = None,
                            logger_options: Dict[str, Any] = None,
                            **kwargs) -> AutoscaleManager:
        """
        Get or create an autoscale manager instance.
        ...
        if not instance_id:
            if isinstance(cloud_provider, str):
                provider_str = cloud_provider
            else:
                provider_str = cloud_provider.value
            instance_id = f"autoscale-{provider_str}-{uuid.uuid4()}"
        
        # Check if we already have this instance
        if not hasattr(self, '_autoscale_instances'):
            self._autoscale_instances = {}
            
        if instance_id in self._autoscale_instances:
            self.logger.info(f"Returning existing autoscale manager instance: {instance_id}")
            return self._autoscale_instances[instance_id]
        ...
        # Create new autoscale manager instance
        self.logger.info(f"Creating new autoscale manager instance: {instance_id}")
        autoscale_manager = AutoscaleManager(
        ...
        # Store reference
        self._autoscale_instances[instance_id] = autoscale_manager
        
        return autoscale_manager

    def list_autoscale_manager_instances(self) -> Dict[str, 'AutoscaleManager']:
        """
        List all autoscale manager instances.
        
        Returns:
            Dictionary of instance IDs to autoscale manager instances
        ...
        if not hasattr(self, '_autoscale_instances'):
            self._autoscale_instances = {}
        return self._autoscale_instances
