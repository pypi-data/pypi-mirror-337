...
class LLMSearch:
    ...
    async def __call__(self, user_query: str, max_urls=1,
        ...
        yield StreamMessage(type="parameters",
                            content=f"Search parameters: {parameters.max_urls} urls, {parameters.max_iterations}iters, {parameters.num_gen_search_queries} queries per step, {parameters.delay_between_searches} delay.")
        ...
        Path("results.json").write_text(plan.model_dump_json(indent=2))
...
async def main(user_query):
    ...
    search_tool = LLMSearch(LLM('dsollama'), llm_threads=4, search_threads=8,search_parallel=False)
...
if __name__ == "__main__":
    asyncio.run(main("find top 5 best casual pebble watch ."))