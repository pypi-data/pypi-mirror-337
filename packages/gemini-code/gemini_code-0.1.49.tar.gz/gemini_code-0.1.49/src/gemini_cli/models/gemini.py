...
Includes Test Runner tool. Prompt v11. Syntax FIXED.
"""
...
class GeminiModel:
    ...
    def __init__(self, api_key, model_name="gemini-2.5-pro-exp-03-25"):
        ...
        # Use the V11 prompt for forced orientation loop
        ...
        try:
            ...
            # Chat history managed manually within generate()
    ...
    def generate(self, prompt: str, conversation=None) -> str | None:
        ...
        try:
            ...
            # Simulate the structure expected by _parse_and_execute_tool_calls if needed,
            # or call the tool instance directly for simplicity here.
            ls_tool = get_tool("ls")
            if ls_tool:
                ...
                logging.warning("Could not find 'ls' tool for mandatory orientation.")
                orientation_context = "Note: Could not retrieve directory listing.\n"
            ...
            # Decide if we should proceed without orientation or return error
            # return f"Failed initial step: Could not list directory - {orient_error}" # Option to fail fast

        # === Step 2: Initialize History and Start Agent Loop ===
        # History starts with System Prompt, Orientation, and User Request
        current_task_history = [
            {'role': 'user', 'parts': [self.system_instruction]}, # System prompt context
            # Provide Orientation + User Prompt together in the first user turn
        ...
        # Log for context building during the loop, separate from direct LLM history
        full_log = [f"System (Orientation):\n{orientation_context}", f"User: {original_user_prompt}"]

        iteration_count = 0
        task_completed = False
        last_llm_response_text = "" # Store last raw text for final check
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                last_llm_response_text = generated_text # Store this response
                logging.info(f"LLM suggested action/code (Iter {iteration_count}):\n---\n{generated_text}\n---")

                # Add LLM's thought/code to history
                ...
                if TASK_COMPLETE_PATTERN.search(generated_text):
                    logging.info("Task completion signal detected.")
                    task_completed = True
                    break
                ...
                if not tool_calls_executed:
                    ...
                    task_completed = True # Assume completion if no action proposed
                    break
                ...
                # Add results + next instruction to history
                current_task_history.append({'role': 'user', 'parts': [results_feedback]})
                full_log.append(f"System (Tool Results - Iter {iteration_count}):\n{results_feedback.splitlines()[0]}...") # Log start of feedback

                if any_errors:
                     logging.warning("Errors occurred during tool execution. Loop will continue for potential correction.")
            ...
            final_summary = ""
            if task_completed:
                # If the last LLM response *was* the completion signal and potentially had text before it
                # We might want to use that text directly if it looks like a summary.
                # Otherwise, explicitly ask for one.
                completion_text_only = TASK_COMPLETE_PATTERN.sub('', last_llm_response_text).strip()
                if completion_text_only and len(completion_text_only) > 10: # Arbitrary length check
                     ...
                     # Use generate_content with the *entire history* built during the loop
    ...
    def _extract_text_from_response(self, response):
        try:
            ...
            if response.candidates:
                 # Handle case where response might be blocked
                 if response.candidates[0].finish_reason.name != "STOP":
                     logging.warning(f"Response generation stopped due to: {response.candidates[0].finish_reason.name}")
                     # Consider how to handle safety/other blocks - maybe return safety feedback?
                 ...
                 # Process parts if response is valid
                 for part in response.candidates[0].content.parts:
                     if hasattr(part, 'text'): all_text.append(part.text)
                 return "".join(all_text)
            # Fallback for non-candidate responses (less likely with generate_content)
