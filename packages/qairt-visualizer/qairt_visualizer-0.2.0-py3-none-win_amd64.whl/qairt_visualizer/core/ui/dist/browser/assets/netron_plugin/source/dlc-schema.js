...
$root.dlc.v4.TensorData = class TensorData {
    ...
    static decode(reader, position) {
        ...
        $.data_position = reader.table(position, 10, $root.dlc.v4.DataPosition.decode); // QAIRT Netron: Add data_position metadata describing the tensor's position in model.params.bin
        return $;
    }
};

// QAIRT Netron: Add DataPosition describing the file offset and size of the tensor data in model.params.bin
$root.dlc.v4.DataPosition = class DataPosition {

  static decode(reader, position) {
      const $ = new $root.dlc.v4.DataPosition();
      $.offset = reader.int64_(position, 4, 0);
