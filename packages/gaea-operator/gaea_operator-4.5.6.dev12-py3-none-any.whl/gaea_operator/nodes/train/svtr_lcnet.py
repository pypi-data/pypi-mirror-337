...
from gaea_operator.config import SVTRLCNetConfig, KEY_NETWORK_ARCHITECTURE, KEY_EARLY_STOPPING
...
def ppyoloe_plus_train(args):
    ...
    # 3. 下载预训练模型
    pretrain = {"英文": "en_PP-OCRv4_rec_train",
                "中文": "ch_PP-OCRv4_rec_train"}
    ...
    metric_names = [LOSS_METRIC_NAME, PRECISION_METRIC_NAME, RECALL_METRIC_NAME, ACCURACY_METRIC_NAME]
    trainer.track_model_score(metric_names=metric_names)
    trainer.track_train_log(output_uri=args.output_uri)
    trainer.launch()
    trainer.paddleocr_export(model_dir=args.output_model_uri)
