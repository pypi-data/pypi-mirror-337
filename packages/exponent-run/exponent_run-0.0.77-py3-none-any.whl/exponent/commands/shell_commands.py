...
from exponent.utils.colors import adjust_color_for_contrast, blend_colors_srgb
...
class Theme:
    ...
    def __init__(self, term_fg: Color, term_bg: Color):
        ...
        self.block_header = adjust_color_for_contrast(term_bg, block_target, 1.2)
        self.block_body = adjust_color_for_contrast(term_bg, block_target, 1.1)
        self.block_footer = adjust_color_for_contrast(term_bg, block_target, 1.05)
        self.footer_text = blend_colors_srgb(term_fg, self.block_footer, 0.4)
        self.dimmed_text = blend_colors_srgb(term_fg, term_bg, 0.5)
...
class BaseView:
    ...
    def _render_block_header(self, text: str) -> list[str]:
        return [
            bg_color_seq(self.theme.block_header),
            erase_line_seq(),
            bold_seq(),
    ...
    def _render_block_footer(
        ...
        seqs: list[Any] = [
            bg_color_seq(self.theme.block_footer),
            erase_line_seq(),
            italic_seq(),
        ]
        ...
        seqs.append([fg_color_seq(self.theme.footer_text), text, reset_attrs_seq()])
    ...
    def _render_natural_edit_diff(self, write: dict[str, Any]) -> list[Any]:
        ...
        highlighted_diff = highlight_code(
            diff, "udiff", self.theme, line_numbers=False, padding=(0, 1)
        )

        return [
            self._render_block_header("Diff"),
            bg_color_seq(self.theme.block_body),
            erase_line_seq(),
            "\n",
            highlighted_diff,
            "\n",
    ...
    def _render_code_block_content(self, content: str, lang: str) -> str:
        return highlight_code(
            content.strip(), lang, self.theme, line_numbers=False, padding=(1, 1)
        )
    ...
    def _render_code_block_output(self, content: str) -> list[Any]:
        ...
        output = [
            [bg_color_seq(self.theme.block_body), erase_line_seq(), line, "\n"]
            for line in lines
        ]
    ...
    def _render_file_write_block_content(self, content: str, lang: str) -> str:
        return highlight_code(
            content.strip(), lang, self.theme, line_numbers=False, padding=(0, 1)
        )
    ...
    def _render_command_block_content(self, data: dict[str, Any]) -> list[Any]:
        ...
        lines = pad_left(content, " ").split("\n")
        seqs: list[Any] = [bg_color_seq(self.theme.block_body)]
...
class LiveView(BaseView):
    ...
    def _handle_thinking_event(self, event: dict[str, Any]) -> None:
        ...
        if self.thinking_start_time is not None:
            ...
            render(
                [
                    f"\r{fg_color_seq(self.theme.dimmed_text)} Exponent thought for {duration_str}\x1b[0m\n\n"
                ]
            )
    ...
    def _handle_command_chunk_event(self, event: dict[str, Any]) -> None:
        ...
        # because it's changing not incrementally and breaks the layout
...
class CharBuffer(Buffer):
    ...
    def render_new_chars(self, message: str) -> list[Any]:
        ...
        available_width = self.term_width - 2
        seqs = []
...
class LineBuffer(Buffer):
    ...
    def render_new_lines(
        ...
        seqs.append(bg_color_seq(theme.block_body))
...
def highlight_code(
    ...
    return (
        result.replace("\x1b[0m", f"\x1b[0m{bg_color_seq(theme.block_body)}")
        + "\x1b[49m"
    )
...
def get_term_colors() -> tuple[Color, Color]:
    fg = Color("#fff")
    bg = Color("#000")

    if not POSIX_TERMINAL or not sys.stdin.isatty():
        return (fg, bg)  # Not supported on Windows or when stdin is not a TTY
