...
class GeminiModel:
    ...
    def generate(self, prompt, conversation=None):
        ...
        try:
            ...
            # For simple prompts, don't use chat history
            if not conversation or prompt.startswith('gemini '):
                print("Using direct generation without history")
                content_response = model_with_tools.generate_content(prompt)
            else:
                # Convert our conversation format to Gemini format for history
                gemini_messages = self._format_conversation_for_gemini(conversation)
                
                # Create a proper chat for history
                try:
                    chat = model_with_tools.start_chat(history=gemini_messages)
