    ...
    def _monitor_upstream(self) -> None:
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if self.downloader.download_file(DISTRIBUTION_FILE, os.path.join(self.data_dir, DISTRIBUTION_FILE), retries=1):
                    with open(os.path.join(self.data_dir, DISTRIBUTION_FILE), "r") as f:
                        distribution_content = f.read()
                else:
                    logger.error("Failed to download distribution file, attempting to delete")
