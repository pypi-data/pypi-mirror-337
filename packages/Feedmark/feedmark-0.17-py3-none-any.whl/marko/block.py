...
class LinkRefDefs:
    """Dictionary-like object that manages link reference definitions in the document."""

    def __init__(self) -> None:
        self._store: dict[str, tuple[str, str]] = {}
        self._unnormalized_labels: dict[str, set[str]] = {}

    def __repr__(self) -> str:
        return "LinkRefDefs({})".format(repr(self._store))
    ...
    def add(self, label: str, dest: str, title: str) -> None:
        ...
        if normalized_label not in self._store:
            self._store[normalized_label] = (dest, title)
        self._unnormalized_labels.setdefault(normalized_label, set()).add(label)

    def extend(self, link_ref_defs) -> None:
        for name, (dest, title) in link_ref_defs.items():
            self.add(name, dest, title)

    def get(self, label: str) -> tuple[str, str] | None:
        normalized_label = self._normalize_label(label)
        return self._store.get(normalized_label)
    ...
    def unnormalized_labels_for(self, label: str) -> set[str]:
        return self._unnormalized_labels.get(label) or set()
