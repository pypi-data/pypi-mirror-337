...
def compute_theta_phi_biomarker_sk(
    ...
    resp_affected = [
        sum(stage_likelihoods_posteriors[p] >= curr_order) if diseased[p] else 0
        for p in participants
    ]

    resp_nonaffected = [
        sum(stage_likelihoods_posteriors[p] < curr_order) if diseased[p] else 1
        for p in participants
    ]

    # Weighted average
    theta_mean = np.sum(resp_affected*measurements)/np.sum(resp_affected)
    phi_mean = np.sum(resp_nonaffected*measurements)/np.sum(resp_nonaffected)

    # Weighted STD
    theta_std = np.sqrt(np.sum(resp_affected*(measurements - theta_mean)**2) / np.sum(resp_affected))
    phi_std = np.sqrt(np.sum(resp_nonaffected*(measurements - phi_mean)**2) / np.sum(resp_nonaffected))
    return theta_mean, theta_std, phi_mean, phi_std
...
def update_theta_phi_estimates(
    ...
    for biomarker, (
        ...
        if algorithm == 'em': 
            theta_mean, theta_std, phi_mean, phi_std = compute_theta_phi_biomarker_sk(
            participants,
            measurements,
            diseased,
