...
def save_modern_cursor_rules(rules_dir: str, rule_title: str, pattern: str, description: str, content: str, references: list = None) -> str:
    ...
    filename = rule_title.lower().replace(" ", "_").replace("/", "_").replace("\\", "_")
    # Ensure it has the .mdc extension
    ...
    # Create the content in proper .mdc format
    lines = [
        f"# {rule_title}",
        "",
        f"pattern: \"{pattern}\"",
    ...
    # Note: Don't prefix every line with #, just include the content directly
    lines.append(content)
    
    # Add file references if provided
    if references and isinstance(references, list):
        lines.append("")  # Add a blank line before references
