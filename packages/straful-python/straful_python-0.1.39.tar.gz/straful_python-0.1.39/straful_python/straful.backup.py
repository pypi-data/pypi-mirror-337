...
In case the service has been recently started please wait 5 minutes for it to become fully functional."
    ...
    def submit_job(
        ...
        if circuit is not None and not isinstance(circuit, QuantumCircuit):
            print("The 'circuit' argument must be an instance of QuantumCircuit.")
            return
        if (
            circuits is not None
            and not isinstance(circuits, list)
            or not all(isinstance(circ, QuantumCircuit) for circ in circuits)
        ):
            print("The 'circuits' argument must be a list of QuantumCircuit instances.")
            return
        if shots is None:
