...
def get_stories(workspace_id: int, options: dict = None) -> str:
    ...
    Args:
        ...
        options: 可选参数字典，支持以下字段：
            ...
            - name: 标题
            等等...
    Returns:  <str>  # 需求所有字段数据的 json 格式，返回需求链接给用户，链接要可点击，链接的格式 https://www.tapd.cn/{workspace_id}/prong/stories/view/{id}
    """
    story_condition = {
...
def create_story(workspace_id: int, name: str, options: dict = None) -> dict:
    ...
    created_story = client.create_or_update_story(new_story)
    return {
        "url": f'https://www.tapd.cn/{workspace_id}/prong/stories/view/{created_story["data"]["Story"]["id"]}', # 返回给用户时，链接要可点击
...
def get_bug_custom_fields(workspace_id: int) -> str:
    """获取 TAPD 缺陷自定义字段配置
    TAPD API 文档：https://open.tapd.cn/document/api-doc/API%E6%96%87%E6%A1%A3/api_reference/bug/get_bug_custom_fields_settings.html
    Args:
        workspace_id: 项目ID（必填）
    Returns:  <str>  # 需求所有自定义字段配置数据的 json 格式
    """
...
def update_bug(workspace_id: int, options: dict = None) -> str:
    ...
    return json.dumps(created_bug, indent=2, ensure_ascii=False)
...
def create_bug(workspace_id: int, title: str, options: dict = None) -> dict:
    ...
    return {
        "url": f'https://www.tapd.cn/{workspace_id}/bugtrace/bugs/view/{ret["data"]["Bug"]["id"]}', # 返回给用户时，链接要可点击
...
def get_workflows_all_transitions(workspace_id: int, options: dict = None) -> dict:
    ...
    TAPD API 文档：https://open.tapd.cn/document/api-doc/API%E6%96%87%E6%A1%A3/api_reference/workflow/get_workflow_all_transitions.html
    Args:
        workspace_id: 项目ID（必填）
        options:
...
def get_workflows_status_map(workspace_id: int, options: dict = None) -> dict:
    ...
    TAPD API 文档：https://open.tapd.cn/document/api-doc/API%E6%96%87%E6%A1%A3/api_reference/workflow/get_workflow_status_map.html
    Args:
        workspace_id: 项目ID（必填）
        options:
            - system: 系统名。取 bug （缺陷的）或者 story（需求的）（必填）
...
def get_workflows_last_steps(workspace_id: int, options: dict = None) -> dict:
    ...
    TAPD API 文档：https://open.tapd.cn/document/api-doc/API%E6%96%87%E6%A1%A3/api_reference/workflow/get_workflow_last_steps.html
    Args:
        workspace_id: 项目ID（必填）
        options:
            - system: 系统名。取 bug （缺陷的）或者 story（需求的）（必填）
...
def get_stories_custom_fields_settings(workspace_id: int) -> dict:
    ...
    TAPD API 文档：https://open.tapd.cn/document/api-doc/API%E6%96%87%E6%A1%A3/api_reference/story/get_story_custom_fields_settings.html
    Args:
        workspace_id: 项目ID（必填）
...
@mcp.tool()
def get_iterations(workspace_id: int) -> dict:
