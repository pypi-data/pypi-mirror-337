...
from briton.proto import BritonStub, InferenceAnswerPart, LookaheadDecodingConfig, XGrammarConfig
...
def _start_briton(config_pbtxt_path, tp_count, briton_env):
    # TODO(mahmoud): remove this once the experiments are over
    if os.getenv("BRITON_EXPERIMENTAL") == "1":
        briton_path = shutil.which("Briton")
        custom_trt_path = os.path.join(os.path.dirname(briton_path), "libcustomtrt.so")
        print(f"Using custom TRT: {custom_trt_path}", flush=True)
        briton_env["LD_PRELOAD"] = custom_trt_path
