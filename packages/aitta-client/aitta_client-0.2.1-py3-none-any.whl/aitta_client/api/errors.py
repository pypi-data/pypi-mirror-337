...
class APIErrorResponse(APIError):
    ...
    def __init__(self, error_data: data_structures.APIError) -> None:
        ...
        super().__init__(f"The API responded with error '{self.error_type}': {self.error_description}")
...
def handle_error_responses(response: requests.Response, content_type: str = "application/hal+json") -> None:
    ...
    if not response.ok:
        try:
            ...
            if response.status_code == 500:
                raise APIError("The API encountered an internal error and did not provide a response.")
            raise MalformedAPIResponse("The API server responded with an error response, but it could not be parsed.") from e

    if "content-type" not in response.headers:
        raise MalformedAPIResponse("The API server response did not provide a content type header.")
    elif response.headers["content-type"] != content_type:
        raise MalformedAPIResponse(f"The API server response did not have the expected content type. Expected '{content_type}' but got '{response.headers['content-type']}'.")
